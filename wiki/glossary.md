...
| SIGReg | Sketched-Isotropic-Gaussian Regularizer | Anti-collapse term: project embeddings onto `M` random unit directions and penalise each 1-D projection's Epps–Pulley distance from `N(0,1)`; by Cramér–Wold this matches the joint to `N(0, I)`. Introduced in [[wiki/entities/lejepa.md]] (Balestriero & LeCun 2025), where the `N(0,I)` target is *derived* as the probe-risk minimiser; applied to a world model in [[wiki/entities/lewm.md]] |
