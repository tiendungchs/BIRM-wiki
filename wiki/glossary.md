...
| SCAN | Simplified version of the CommAI Navigation tasks | Compositional instruction-to-action benchmark (`walk twice` → `WALK WALK`); the `add jump` split trains on one occurrence of a verb and tests its compositional uses, the `length` split tests extrapolation past the training output length ([[wiki/entities/mlc.md]]) |
