...
**Coverage note.** Three sources ingested. Hassabis et al. 2017 (a survey) built the skeleton, so most pages state a mechanism and its role in the framing without a quantitative result. Geirhos et al. 2020 added the first page carrying an evaluation constraint rather than a mechanism — and its consequence (the intended solution is not identifiable from data alone) now qualifies every other page. Schmidgall et al. 2023 added the *update-rule* layer — the two plasticity pages, plus the wiki's first entity page — and is the first source carrying quantitative comparisons *between* biologically-motivated methods rather than between biology and engineering. Known holes are tracked in [[wiki/architectural-gaps.md]] and [[wiki/priority-tasks.md]].
