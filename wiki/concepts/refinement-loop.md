...
- **The rejector is not independent of the proposer** (`G68`). Every loop here scores candidates with the generator's own likelihood or with a check built from the same pairs the generator fit, so the wiki still holds no rejector whose correctness criterion comes from outside the system that produced the candidate.
...
- **[[wiki/entities/universal-transformer.md]]** — the loop compressed into one differentiable forward pass and stripped of its verifier: a shared operator rewrites the same candidate representation `T` times, with a per-position sigmoid deciding when to stop instead of a check against the task's demonstration pairs — which is why it trains end-to-end and why nothing in it can tell a revision that moved the state from a revision that improved it.
- **[[wiki/entities/varc.md]]** — the from-scratch column's best score with the loop removed: after test-time training, inference is a single feedforward pass per view with no propose-and-check anywhere, and it beats TRM by ~10 points and HRM by ~14 on ARC-AGI-1 — so what the recurrent systems were buying is not recursion but a way to install 2D structure, which a canvas and a scale augmentation buy more cheaply.
