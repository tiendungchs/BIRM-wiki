...
- **[[wiki/entities/model-free-episodic-control.md]]** — the behavioural form of this page's result, as an argument rather than an analysis: a scrub jay recovering a cache is better off recalling the exact hiding spot than sampling a distribution over likely ones (Clayton & Dickinson 1998), so where the reward structure is a set of particulars the optimal transport out of the fast store is zero and the instance store is asymptotically correct.
