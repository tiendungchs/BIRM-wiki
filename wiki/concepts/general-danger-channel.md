...
- **[[wiki/concepts/intrinsic-motivation-typology.md]]** — the sign problem generalised past active inference: *every* novelty cell in the computational typology (rarity, prediction error, expectation violation) rewards the unfamiliar, and the response this page measures to an unfamiliar food or object is the threat channel — so `T358` is not a quarrel with one objective but with a whole family of intrinsic rewards.
- **[[wiki/concepts/information-sampling-vs-search.md]]** — the opposite sign on the same stimulus class, and a third shape for `T358`: human curiosity is an inverted U over *confidence*, peaking at intermediate knowledge rather than at maximal unfamiliarity, which is neither this page's annealing threat penalty nor the monotone novelty bonus it disputes (Gottlieb & Oudeyer 2018).
- **[[wiki/entities/lateral-habenula.md]]** — the second collapse point in the same series: this page's parabrachial population multiplexes pain, malaise, itch, satiety and novelty before the midbrain hears any of it, and the habenula performs a second, wider convergence one stage later (basal ganglia, hypothalamus, pallidum, septum, cortex, retina) before inverting the result onto every aminergic nucleus — two funnels in a row, which sharpens `T357` from *where the address survives* to *how many times it is destroyed*.
- **[[wiki/concepts/learning-progress.md]]** — the third arm of `T358` with a fitted coefficient: neither a novelty bonus nor an annealing novelty penalty predicts approach-then-abandonment of a permanently unpredictable activity, and a derivative-of-competence term predicts both halves, with the divergence appearing at ~trial 80 (Ten et al. 2021).
