...
- **[[wiki/concepts/inductive-bias.md]]** — the same multi-environment signal spent differently: IRM charges the environments against a penalty and returns a representation directly, while bias learning charges them against a hypothesis-space search and returns a *space* a per-task learner must still search — both need several environments, and Baxter's `O(k + W/n)` says how many, which the IRM theorem's linear-general-position condition does not.
