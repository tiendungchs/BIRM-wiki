...
| Human learning-progress estimates ([[wiki/concepts/learning-progress.md]], Ten et al. 2021) | **CPM**, not LPM | Measured — and it settles which of the two. The fitted signal is `\|Δ PC\|` over the agent's own *achievement* record with no predictor of the environment anywhere, so it is the competence family; the knowledge-family LPM (derivative of prediction error) is still unmeasured in humans. The fit also refutes this page's framing of the families as alternatives: `w_PC` and `w_LP` are **uncorrelated** (`r = −0.077`/`0.062`) co-determinants of one utility |
