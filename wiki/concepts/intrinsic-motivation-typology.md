...
| Human learning-progress estimates ([[wiki/concepts/learning-progress.md]], Ten et al. 2021) | **CPM**, not LPM | Measured — and it settles which of the two. The fitted signal is `\|Δ PC\|` over the agent's own *achievement* record with no predictor of the environment anywhere, so it is the competence family. The fit also refutes this page's framing of the families as alternatives: `w_PC` and `w_LP` are **uncorrelated** (`r = −0.077`/`0.062`) co-determinants of one utility |
| Infant attention allocation ([[wiki/concepts/realised-information-gain.md]], Poli et al. 2020) | **IGM**, not LPM | Measured — and the knowledge-family cell it fills is not the one the family's own literature predicted. The fitted quantity is `D_KL(p^j ‖ p^{j−1})`, an information gain over a Dirichlet posterior (IGM), *not* a derivative of prediction error over a window (LPM); it beats surprise (`e^{\|β\|} 7.02 vs 2.44`) and environmental entropy (`1.27`) on the look-away hazard in 43 8-month-olds. Two of this page's cells appear in one regression with **opposite signs** — the DSM/NM-side surprise term raises disengagement while the gain term lowers it — which the typology's one-reward-per-agent format cannot state |
| Empowerment `C(A_t → S_{t+1})` ([[wiki/concepts/empowerment.md]], Salge et al. 2013) | **no cell** | Task-independent *control* capacity, model-free and goal-free — a competence-family quantity computed without episodes, and in a deterministic world just `log` of the `n`-step reachable-set size. The typology's clearest missing row, now filled in with a page rather than a name |
...
- **The typology has no cell for control capacity.** Empowerment, mutual information between actions and future states, and every "keep your options open" objective are competence-like but need no goals and no episodes. Either the family is broader than the paper's architecture or these belong in a fourth family. **The candidate is now on the wiki with its failure modes attached** ([[wiki/concepts/empowerment.md]], Salge et al. 2013): it is permutation-invariant, computable from local dynamics, and drives a pendulum to upright with no value function — but its landscape goes *flat* whenever sensing or acting is unbounded relative to the other, its state ranking can invert with the action-power constraint `P`, and it will not seek a goal state that is poorly empowered. So the fourth family exists and is not free: it trades the missing-region-partition problem of the progress models for a missing-resolution problem of its own.
...
- **[[wiki/concepts/empowerment.md]]** — the fourth-family candidate this page's open problems name: `𝔈 = max_{p(a)} I(S_{t+1}; A_t)` is competence-like (it scores what the agent can bring about) yet needs no goals, no episodes, no achievement measure and no predictor, and it passes the permutation test cleanly — while failing a criterion this page never states, since it is *not* invariant to re-binning the same channels and its landscape flattens entirely whenever sensing or acting is unbounded (Salge et al. 2013).
- **[[wiki/concepts/expected-value-of-control.md]]** — the missing subtrahend for every family listed here: each row specifies a bonus *added* to the reward, none prices the computation that earns it, and that page attaches the disutility to the controller's intensity rather than to the outcome — so an intrinsic reward and its cost finally sit in one expression.
- **[[wiki/concepts/eigenoption-discovery.md]]** — an intrinsic reward this typology's three families have no cell for: `rᵉ(s,s′) = eᵀ(φ(s′) − φ(s))` is a linear functional of the *representation's* change, with no uncertainty, prediction error or learning progress (not knowledge-based), no goal or achievement measure (not competence-based) and no body statistic (not morphological) — a fourth, representational family whose rewards are generated wholesale by one eigendecomposition (Machado et al. 2017).
- **[[wiki/concepts/hindsight-goal-relabelling.md]]** — a competence-based scheme with this page's reward slot left empty: goals are self-set and scored by achievement exactly as that family specifies, but there is no competence-progress term at all — the motivational work is done entirely by the *goal sampler* (draw from the agent's own realised future, never from the global history), which is a design the typology's reward-formula format cannot express.
- **[[wiki/concepts/learning-progress.md]]** — this page's LPM/CPM cells fitted to 382 humans, with the one result the typology's reward-formula format cannot express: an *externally supplied objective* did not add a reward term or scale the intrinsic ones — it flipped the sign of the competence coefficient (`w_PC` `+0.255` → `−0.232`) and left the progress coefficient untouched (`p = 0.799`), so extrinsic instruction enters this space as a write to one weight rather than as a competing motivation (Ten et al. 2021).
- **[[wiki/concepts/effort-based-decision-making.md]]** — the sign check this page's reward-formula format never runs: every family here adds a bonus to `r` for novelty, progress, information or competence and so hard-codes demand as *attractive*, while a two-sided elicitation (reverse cognitive effort discounting — start from equal offers and let the discount run in either direction) finds **83 of 85** humans refusing to pay anything for a harder `N`-back. Demand is a cost by default and a bonus only in the boredom regime, which is a state rather than a trait — so the typology's families describe the minority condition and price none of the computation that earns the bonus (Westbrook & Braver 2015).
- **[[wiki/concepts/default-policy-regularization.md]]** — the subtrahend in a form these families could actually use: a divergence between the running policy and a distilled default is computable from quantities a multi-task agent already holds, so the cost of the computation that earns a novelty or progress bonus can be written into the same objective instead of being omitted. It also relocates the cost's argument — distance from habit rather than task demand — which predicts that the novelty bonus and the effort cost are correlated by construction, since novel states are exactly where the default is wrong.
- **[[wiki/concepts/learned-industriousness.md]]** — the origin account for a coefficient every family here hard-codes: if demand's sign and magnitude in the objective are *conditioned* by a history in which exertion paid, then a bonus for difficulty is a learned state of the agent rather than a design choice, and two agents with different reward histories should price the same term differently. No family here has a write path to its own bonus coefficient (`G128`).
- **[[wiki/concepts/explore-exploit-division-of-labour.md]]** — the structural alternative to this page's mixing coefficient: instead of adding an epistemic bonus to `r` with a tuned weight, put a hard *temporal* boundary on the extrinsic term — a protected, externally subsidised phase scored on nothing, then a phase where the return is live — which is the same trade expressed as a schedule rather than as an `α` and is unbuilt anywhere in the wiki (Gopnik 2020).
- **[[wiki/concepts/self-boosting-knowledge-acquisition.md]]** — the row this page's three families have no cell for, and the composition rule its open problems ask for: a reward computed from the *structure of the agent's store* (`Gaps(K) = αK(N − K)`, frontier edges from held to unheld items) rises over the first half of a large domain where every formula here decays, and its account of Flow is a **product** of a knowledge term and a competence term rather than a weighted sum of two families presented as alternatives (Murayama 2022, `G129`).
- **[[wiki/entities/feudal-networks.md]]** — a competence-style intrinsic reward whose goal space is discovered rather than enumerated: the goal is a direction in a learned latent space, so the goal distribution is a continuous manifold instead of a list of target states, achievement is graded (cosine over the last `c` steps) instead of binary, and the goal-setter is trained by the *extrinsic* return — so what the agent pays itself for is chosen by what pays (Vezhnevets et al. 2017).
- **[[wiki/entities/diayn.md]]** — the second reward this typology's three families have no cell for: `log q_φ(z|s) − log p(z)` scores the agent's own *identifiability from its trace*, with no predictor and no error (not knowledge-based), no goal set and no achievement measured (not competence-based) and no body statistic (not morphological), while passing the permutation test cleanly.
- **[[wiki/entities/go-explore.md]]** — a measured dissociation this page's reward-formula format cannot express: `W = 1/√(C_seen+1)` is the UM count bonus exactly, and with the state representation held fixed it yields 12,240 on Montezuma's Revenge as an additive reward and 1,700,000 as a *selection weight over an external archive* — so a curiosity formula's value is a property of the formula and its consumer jointly, which is the same lesson [[wiki/concepts/replay-prioritisation.md]] reports with the roles reversed (Ecoffet et al. 2021).
- **[[wiki/concepts/realised-information-gain.md]]** — the knowledge-family cell measured in humans at 8 months, and two corrections to how this page frames the space. (i) The regionalisation this page calls load-bearing is not solved there either — it is *supplied* by the experiment, one Dirichlet posterior reset at every sequence boundary, which is exactly the incremental `{ℛ_n}` handed over by fiat. (ii) The protection against the stochastic trap comes from the **prior's arithmetic** rather than from the reward's form: the `1/(j−1+K)` decay in the predictive distribution shrinks posterior jitter under noise, so an IGM-shaped reward can avoid the trap this page attributes only to progress-based cells — cheaply, and only while the environment is stationary (Poli et al. 2020).
