...
- **A symbol set is not a reward-composable model.** Effect-equivalence classes are defined against the transition structure alone, so a new goal must be re-planned over the symbols rather than evaluated by composing the learned model with the new reward (`G28`).
...
- **[[wiki/concepts/discrete-relaxation-gradients.md]]** — the training mechanism every system in this page's table presupposes and none of them names: DeepSym's and LatPlan's discrete bottlenecks need a gradient through a non-differentiable sample, and the categorical relaxation is what would let those two-valued bottlenecks become `k`-way alphabets without an `O(k)` marginalisation per step.
- **[[wiki/concepts/independent-causal-mechanisms.md]]** — the same relativity of granularity reached from a graph rather than from a body: there a unit is whatever can be separately intervened upon, here whatever the effect repertoire can separately produce, so both make the vertex set a function of an agent's action set rather than of the world.
- **[[wiki/concepts/empowerment.md]]** — an affordance detected as a change in channel capacity rather than as a learned category: in the box-pushing grids empowerment rises near the box only when it is both pushable *and* perceivable, is unchanged by perceiving a box that cannot be moved, and is flat when a movable box cannot be sensed — so "what I can do here" falls out of one scalar with no effect clusters, no symbols and no supervision (Klyubin et al. 2005, via Salge et al. 2013).
