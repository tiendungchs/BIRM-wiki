...
- **[[wiki/entities/lisa.md]]** — the same behavioural variable predicted from the rival code: STAR-2 makes **tensor rank = relational complexity**, so how many relations must be integrated for one inference is read straight off the representation's order, where a synchrony code gets the same prediction from how many bindings fit in a phase set (Knowlton, Hummel & Holyoak 2012). Rank and phase count are two different physical quantities generating one measured difficulty curve, and no experiment in the wiki separates them.
...
- **[[wiki/concepts/relational-bottleneck.md]]** — the same role/filler factorisation with the compound refused entirely: TPR keeps both constituents by forming an outer product and pays `dim V_F · dim V_R`, while a relational bottleneck keeps roles and fillers in separate pathways and lets only their inner products cross, so nothing is combined, nothing must be undone, and what is given up is every operation that needs the binding to be an object (Webb et al. 2024).
- **[[wiki/concepts/activity-slots.md]]** — this page's one-hot-`Ψ_R` limit, measured on a trained network: a structured activity slot is a filler bound to a local role code, and the linearity a tensor-product binding guarantees shows up as **slot algebra** (substitution in one slot is a fixed vector offset independent of the other slots) — with the condition under which it *fails* named, namely velocity summed into the memory units rather than into the synapses or a separate population ([[wiki/empirical-tensions.md]] T317; Whittington et al. 2025).
