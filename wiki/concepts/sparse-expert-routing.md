...
> **Provenance.** Fedus, Dean & Zoph 2022, *A Review of Sparse Expert Models in Deep Learning* (`raw/fedus-2022-sparse-expert-models-review.md`). A survey by the Switch Transformer authors, so its empirical rows are citations to others' work (and to their own); the wiki takes it as the **anchor for routing as an architectural primitive** and as the source for the routing taxonomy, the load-balance problem, the upstream/downstream dissociation, and the expert-specialisation evidence. Individual systems have their own entity pages — the originating one is [[wiki/entities/sparsely-gated-moe.md]] (Shazeer et al. 2017, `raw/shazeer-2017-sparsely-gated-mixture-of-experts.md`), which supplies the gate as first written, the balance-loss ablation, and the capacity-vs-data measurement, and [[wiki/entities/switch-transformer.md]] (Fedus, Zoph & Shazeer 2022, `raw/fedus-2022-switch-transformers.md`), which supplies the `k = 1` head-to-head, the stability interventions priced individually, the distillation ceiling and the negative results.
...
| **Module bids from its own state** ([[wiki/entities/rims.md]], Goyal et al. 2019) | Each module computes a query from **its own recurrent hidden state** with private weights `W^q_k`, attends over the input set with a **null row appended**, and is activated if it puts *less* mass on the null row than its competitors | Context-dependence is structural, not hoped for — the same input routes differently by history. No shared router matrix, hence no rich-get-richer path and **no balance loss reported necessary**. Abstention is first-class: the null row is a learned reserve price, not a ranking | Each module needs private query parameters and a persistent state, so it is not a drop-in for a stateless feed-forward expert; and the reserve-price test is discarded by then taking a fixed top-`k_A` |

**The class the survey's taxonomy cannot express, and why it matters.** All four rows above compute the routing score from the input, by a router the modules do not own — which is what makes `T296` (a fixed hash competes) a live threat to the whole literature. The fifth row moves the query to the consumer: `Q = h_{t,k} W^q_k` rather than `h(x) = W_r x`. That is the difference between a dispatcher assigning work and a set of processes bidding for it, and it is the only arrangement here in which "the same token must go to different experts in different contexts" — `T296`'s discriminating condition — holds by construction rather than by hope. It is also the arrangement biological attention is described in ([[wiki/concepts/attention.md]], biased competition), and it has never been run at mixture-of-experts scale.
...
The naive failure is a rich-get-richer collapse: a slightly-favoured expert receives more tokens, trains faster, becomes more favoured. The remedies partition exactly as the taxonomy does — **penalise imbalance** (auxiliary loss, Shazeer 2017), **constrain it away** (BASE/S-BASE assignment), **make it structurally impossible** (expert-choice routing, hashing), or **grow out of it** (dense-to-sparse annealing). The originating system's own ablation is the only place the penalty is priced: **no balance loss at all costs 4.2 perplexity and leaves the busiest expert at 17.8× the mean load, while every non-zero coefficient across a 100× range recovers full quality** (Shazeer et al. 2017; table on [[wiki/entities/sparsely-gated-moe.md]]). Two losses are used and they are not the same object — `Importance` constrains the batch sum of gate *values* and is differentiable directly; `Load` constrains the *count* of inputs routed, which is an integer, and is made differentiable by integrating the router's injected noise analytically. Either alone fixes quality; only the load term controls the tail that decides whether a device runs out of memory. **So the noise in "noisy top-`k`" is a differentiability device for a batch statistic, not an exploration device** — a distinct manoeuvre from relaxing the sample ([[wiki/concepts/discrete-relaxation-gradients.md]]).

A wiki reading: load balance is not a systems detail imported from distributed training, it is the [[wiki/concepts/representational-collapse.md]] problem stated over a discrete selection variable, and the four remedies are the same four families that appear wherever a discrete latent must not collapse.
...
| **Upstream, capacity × data** | The interaction, measured once at the origin and never revisited: the same architecture *saturates* past ~1B MoE parameters on an 829M-word corpus and keeps paying to **68B parameters / 65,536 experts (99.994% sparsity)** on a 100B-word one — then **degrades at 131,072 experts**, since each expert's batch is `kbd/n` and past some `n` no expert is estimable. Sparse capacity is indexed to a token budget, and there is a data-dependent ceiling on expert *count* that binds below the router (Shazeer et al. 2017) |
...
| **Downstream, reasoning-heavy** | **Worse at matched pre-training perplexity.** A 1.6T/2048-expert Switch model with the FLOPs of a 2B dense model fine-tunes poorly on SuperGLUE; Artetxe et al. find worse fine-tuning on HellaSwag, PIQA, Winogrande. **At matched *FLOPs* the sign flips**: Switch-Base beats T5-Base 79.5/75.1 on SuperGLUE, 73.3/66.6 on Winogrande, 54.0/51.8 on ANLI-R3, and Switch-XXL takes ANLI to 65.7 against a prior best of 49.4 — so this row is a statement about what the *extra parameters* bought, not about sparsity costing reasoning at fixed compute ([[wiki/entities/switch-transformer.md]]) |
...
Sparse models diverge more often, and more often at scale (GShard at 1T with bfloat16; Switch-XXL; ST-MoE multilingual; LIMoE multimodal). **The primary source refines what "at scale" means, and the refinement matters:** Switch-C at 1.6T parameters and 2048 experts shows *no instability at all*, while Switch-XXL at 395B with ~10× the FLOPs per token is sporadically unstable — so divergence tracks FLOPs per token rather than parameter count or expert count, and the sparse axis is the safe one to scale ([[wiki/entities/switch-transformer.md]], Fedus, Zoph & Shazeer 2022). The catalogue of fixes is informative because every one of them targets the **router**, not the experts:
...
- **No relational expert has ever been observed.** Every reported specialisation is lexical, perceptual or modality-level. Whether this is a limit of the routing granularity, of the interpretability method (which cannot see contextual specialisation), or of the objective, is unseparated. **Changing the routed unit from a token to a recurrent module does not fix it** ([[wiki/entities/rims.md]]): the specialisations that emerge there are temporal phase (receiving vs dormant in a copy task) and image region, still not relational — and they **decay over training**, a possibility no mixture-of-experts specialisation table has ever checked for, since all of them are read at convergence.
- **No sparse expert model has a state, so none can abstain.** An expert is a stateless function evaluated on whatever it is handed; there is no representable "nothing here for me". [[wiki/entities/rims.md]] shows the two properties arrive together — a module with a persistent recurrent state can compare the input against a null option and decline, and a module that declines costs nothing because its state simply persists. What that buys is untested at scale, and what it costs is that experts can no longer be identical stateless blocks the hardware constraint above assumes.
...
- **There is a hardware lower bound on expert size that no proposal prices.** An expert's compute-to-I/O ratio equals its hidden-layer width, and it must exceed the cluster's compute-to-bandwidth ratio — "thousands to one" — so experts cannot be small. Every fine-grained module proposal in this wiki (typed primitives, per-relation experts) sits on the wrong side of that inequality, which is a reason deployed experts stay coarse that is independent of anything about learning (Shazeer et al. 2017).
...
- **[[wiki/concepts/error-consistency.md]]** — the audit any diversity-assuming design should run and none does: chance-discounted per-trial agreement `κ` between experts on held-out items is a label-free measure of how many *effectively independent* members a population has, and on the one population ever measured (sixteen ImageNet CNNs spanning four families and 18–121 layers) it came back .711–.793, i.e. close to `n` copies of one decision rule.
...
- **[[wiki/entities/neural-module-networks.md]]** — the counterexample to two of this page's open problems, from outside the mixture-of-experts literature: modules that specialise *relationally* (`combine[and]` intersects two attention maps, `re-attend[above]` shifts one upward) and that are heterogeneous by construction (five distinct computations, five type signatures) — obtained by changing the routed unit from a token to a sub-question and by replacing the learned router with an unlearned dependency parser (Andreas et al. 2016).
...
- **[[wiki/concepts/discrete-relaxation-gradients.md]]** — the entry missing from this page's taxonomy: ST-Gumbel-Softmax makes the top-`k` argmax itself differentiable, so gradient reaches `W_r` through *which* experts were chosen and not only through the `p_i` scaling — the dead end Shazeer et al. 2017 accepted in exchange for escaping the bandit formulation.
- **[[wiki/entities/switch-transformer.md]]** — the primary source under most of this page's design rules, and the three things only it carries: the `k = 1` head-to-head where top-1 loses marginally at capacity factor 2.0 and wins at 1.0 (the regime that actually binds), the ~30% distillation ceiling that is flat from 82% to 99% compression, and two negative results — rerouting overflow tokens to their second-choice expert buys nothing, and sampling the router softmax is 0.10 nats *worse* than a hard argmax.
- **[[wiki/entities/sparsely-gated-moe.md]]** — the originating system: this page's primitive as first written (noisy top-`k` with a learned noise scale, between LSTM layers, applied convolutionally over positions), plus the three measurements only the primary source reports — the balance-loss ablation, the capacity-saturates-without-data result, and the expert-count ceiling at 131,072.
- **[[wiki/concepts/cellular-scaling-rules.md]]** — the comparative-biology case for this page's axis, stated as an exponent rather than an efficiency: adding units at fixed per-input fan-in is `mass ∼ N^1.0` scaling, a dense layer is `N^2.0`, and the clade that scaled units at constant wiring-per-unit (primates, `N^1.056`) holds seven times the neurons of the clade that grew wiring-per-unit (rodents, `N^1.550`) at the same brain mass — so routing sparsity is a claim about how the cost curve bends, not about savings at a chosen size **(brainstorm)**.
- **[[wiki/concepts/connectivity-scaling-bottleneck.md]]** — the biological version of this page's hardware constraint, with an exponent attached: the crossing point serving a growing set of modules scales sub-isometrically (corpus callosum `∼ cortical surface^0.88`), and the measured response is more local clustering and more divergence between duplicate halves — which predicts that per-expert cross-device bandwidth, not parameter count, is what caps expert count **(brainstorm)**.
- **[[wiki/concepts/network-communication-models.md]]** — where this page's router sits in the taxonomy of communication policies, and it is the expensive corner: computing `softmax(W_r x)` over all `E` experts before selecting `k` is a *centralised* protocol paying maximal **informational cost** (the knowledge of global structure a forwarding decision consumes), the one cost dimension no efficiency table here reports; the decentralised alternative is greedy navigation over an embedding — `O(degree)` per hop, no gating network, and 70–100% of shortest-path efficiency in three species' connectomes.
- **[[wiki/concepts/adaptive-computation-time.md]]** — the complementary axis of conditional computation, and the one this page's hardware constraint does not bind: a router varies *which* parameters run at fixed depth, a halting unit varies *how many times* the same parameters run — which delivers the heterogeneous-depth experts listed above as unattempted without making any expert small, since the extra depth costs transitions rather than parameters (Graves 2016).
- **[[wiki/entities/rims.md]]** — the routing class this page's taxonomy has no row for, and the one that makes context-dependence structural: the top-`k` is taken over scores each module computes *from its own recurrent state* with private query weights, against an appended null row that acts as a reserve price — so the same input routes differently by history, no shared router matrix exists to collapse, no balance loss is needed, and "no module applies here" is representable (Goyal et al. 2019).
- **[[wiki/entities/lateral-frontoparietal-network.md]]** — the biological case where the gate may be **wired rather than computed**: inside the human relational integrator, visuospatial and semantic integration peaks are offset along a dorsoventral axis, and the proposed cause is that dorsal and ventral cells differ in which first-order store projects to them. That is expert specialisation with no router at all — input locality doing the gating — and it is the alternative this page's learned-router taxonomy has no row for (`T338`).
- **[[wiki/concepts/width-versus-parameter-count.md]]** — why this architecture's shape is the right one on the accuracy side as well as the compute side: growing unit count at fixed per-token fan-in is exactly the move that wins a controlled width-versus-weights sweep, and routing is what turns the absent edges into saved FLOPs, which static random sparsity on dense hardware cannot do (Golubeva et al. 2021).

- **[[wiki/entities/ligo.md]]** — the other way to get a large transformer without paying full price for it, on the orthogonal axis: routing buys capacity at fixed FLOPs *per token at inference*, LiGO buys the same final dense model at ~45% fewer FLOPs *during training* by initialising it from a smaller trained one — the two levers are compatible and have never been combined.
- **[[wiki/entities/subiculum.md]]** — the biological dual of this page's scheme: machine routing partitions a module by *input*, learned at run time, whereas the hippocampal output stage is partitioned by **output destination**, fixed developmentally, with each partition's afferent set following from the partition rather than selecting it — and with contiguous discrete tiling making a coordinate a usable proxy for expert identity (Kinman et al. 2026).
- **[[wiki/entities/continuous-modern-hopfield-network.md]]** — the same softmax-sharpening pathology at a different address: an attention head that commits to the single-pattern regime has `diag(p) − ppᵀ ≈ 0`, so its own sharpness zeroes the gradient reaching its keys, which is the continuous analogue of a router collapsing onto one expert — and `1/√d_k` and the router z-loss are the same patch applied to the same failure (Ramsauer et al. 2020).
