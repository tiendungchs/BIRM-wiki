...
**A trained transformer already does part of this, and the control proves it is not free.** The hash-and-lookup hypothesis in [[wiki/concepts/memorisation-vs-generalisation.md]] is the machine restatement of the mossy-fibre argument, made for the same reason: an athlete name arrives at MLP2 of Pythia 2.8B as a *linear sum* of its two token embeddings, and basis-aligned lookup on a sum is provably leaky — a unit whose input weights pool all baseball players and that fires on both Michael Jordan and Tim Duncan must fire on Tim Jordan or Michael Duncan. Nanda et al. 2023 measure the breaking directly, by testing whether `MLP(A) + MLP(B) = MLP(C) + MLP(D)` for name-swapped pairs:

| Quantity | Fraction of the way to fully broken linear structure |
|---|---|
| MLP outputs, layers 2–6 (trained) | **60–70%** |
| Same layers with **weights and biases randomly shuffled** | **20–40%** |
| Residual stream after layer 2 → after layer 6 | 30% → 50% |

The shuffled-weight control is what makes this a finding: an MLP is nonlinear and would break linear structure by accident, and the trained one breaks it two to three times harder than chance. **Gradient descent discovers write-side randomization without being asked for it.**

**But the learned hash is not content-blind, and that is where the analogy fails** ([[wiki/empirical-tensions.md]] T286). Mossy-fibre randomization is a wiring property — it cannot know which patterns matter. The learned version does: **known** names produce a higher MLP output norm than unknown ones, already at MLP1, and a linear sport probe rises *smoothly* through the supposed hashing layers rather than staying at chance until a lookup stage. So what optimisation produced is a hash that has already begun to answer the question, which is exactly what a few steps of gradient descent applied to a pure hash would produce — and it means "hash on write" understates the prescription. The write-side transform in a trained system is a **content-dependent** separator, and the wiki has no account of what sets its content-dependence.
...
| **Theta phase** (4–8 Hz) | Alternates the whole system between separation-favouring and completion-favouring settings **a few times per second** | The one controller that never *sets* the knob: entorhinal drive dominates on one phase (encode) and CA3 recurrence on the other (retrieve), with LTP and LTD assigned to opposite half-cycles. Because it runs far faster than task control, it removes the need for a mode decision entirely — and the two half-cycles then double as the minus/plus phases of an error-driven learning rule ([[wiki/concepts/encoding-retrieval-alternation.md]]; Hasselmo et al. 2002 via O'Reilly et al. 2011) |
...
**The oscillatory row is a fourth architecture and the only one that refuses the question.** Rows one to three all pick a value; the theta row says the conflict is unresolvable *as a setting* and should be resolved *as a schedule*. O'Reilly & McClelland 1994's parameter analysis is why: Hebbian potentiation on the recurrent collaterals deepens basins (helps completion) and destroys separation unless heterosynaptic depression runs alongside it, and stacking feedforward layers (EC → DG → CA3) compounds separation but does **not** compound completion. So the two operations do not merely trade off along one axis — they want *opposite signs on the same plasticity rule*, which a single static bias cannot deliver and an alternation can.
...
**The de-aliasing row has a measured counterexample, and it is geometric rather than sensory.** In a hairpin maze of ten identical, identically-oriented arms, CA3 and CA1 place fields **repeat** across every arm run in the same direction (CA3 median `r = 0.72`, n = 111; CA1 `r = 0.70`, n = 47), in step with a grid code that has fragmented into the same repeating submap and re-zeroes at each turn (Derdikman et al. 2009, [[wiki/empirical-tensions.md]] `T347`). So the separator does not separate here: structurally distinct positions in one continuous environment receive the same code. Two readings, and the wiki cannot yet choose. Either the transfer curve is doing exactly what it should — the arms are near-identical inputs and reuse is the correct call, in which case "de-aliasing" is a *demand* the curve can rationally decline — or compartment identity is carried by cells the recording did not sample (23 place cells were excluded for firing only in the end arms, i.e. some cells do code arm identity). Either way the row above should be read as a job the fast store performs *by degree on the inputs it is given*, not as a guarantee that a stored key is unique over an environment.
...
- **Feedback inhibition alone is sufficient for the endpoint.** A spiking recurrent network with soft winner-take-all and a purely local timing-based Hebbian rule — no task, no error signal — reaches the same orthogonalized representation. That is the wiki's cheapest mechanistic account of separation-by-competition, and it needs neither sparse random projection nor a neuromodulatory controller ([[wiki/concepts/inhibitory-control-of-coding.md]]), and it is also the reason orthogonality looks optional: on the *same* symbol stream six trained architectures reach full next-observation accuracy with their aliased states still strongly correlated, so orthogonality appears only where something imposes it — soft winner-take-all, feedback inhibition, or an explicit correlation penalty in the loss ([[wiki/empirical-tensions.md]] T53). What biology buys by paying for the extra property is untested.
...
- **What sets the knob?** The controllers are named, none is characterised as a function. See gap G38. **And it is not settled that a controller is needed**: sweeping the bias exhaustively in a working agent gives a wide tolerant plateau rather than a narrow optimum, so on a stationary observation distribution a frozen threshold suffices ([[wiki/entities/hami.md]], [[wiki/empirical-tensions.md]] T138). **One instance now has both ends identified** (de Sousa et al. 2026): for the overlap between two episodes encoded days apart, the actuator is NDNF⁺ neurogliaform interneurons in the stratum lacunosum moleculare and the controller is a vmPFC→MEC projection whose activity tracks contextual similarity × elapsed time, bidirectionally moving dCA1 ensemble overlap between ~12% and ~25% with no weight change. Still not a *function* — what similarity metric vmPFC computes, over what representation, is unmeasured — but it is the first controller→actuator→feature chain the wiki holds.
...
- **Newborn vs. mature granule cells.** Loss/gain-of-function says newborn cells enable separation; a computational account says immature cells are too broadly tuned to separate and instead *integrate* patterns; a reactivation study suggests mature cells may be functionally retired. Unresolved, and the two readings prescribe **opposite growth policies** for a fast store: under the first, neurogenesis is capacity added where separation demand is high — grow the sparse layer when discrimination fails; under the second it is a temporary *integration* bias, the opposite sign, with the separating population being the young one and the mature population doing something else. What would settle it is parametric manipulation of environmental similarity with mature and immature populations distinguished in the same animal.
...
- **[[wiki/concepts/path-integration.md]]** — the other route to de-aliasing, measured failing alongside this one in the same tissue: when geometry fragments the grid into repeating per-compartment submaps, CA3 and CA1 repeat with it, so neither the structural code nor the separator distinguishes identical subspaces (Derdikman et al. 2009, `T347`).
...
- **[[wiki/concepts/schema-assimilation.md]]** — proposes a criterion for this page's knob that is not input similarity: rats learning new goals in an already-schematised arena code them with the *same* cells and similar patterns as the old goals, and the two diverge only over subsequent days, so schema membership selects assimilate-then-differentiate while schema absence selects separate-at-write — the two policies gap G38 needs an arbiter between (Preston & Eichenbaum 2013).
- **[[wiki/entities/hippocampal-prefrontal-channel.md]]** — what a hub write triggers: a one-line input to a high-degree node runs completion over the stored pattern, so the channel's bandwidth requirement is set by attractor structure rather than by memory size.
- **[[wiki/concepts/successor-representation.md]]** — completion with a predictive component: the recurrent read-out on a novel input returns not the stored pattern but its discounted future.
- **[[wiki/entities/hami.md]]** — this page's knob reduced to one cosine threshold per factor and then *swept*: 43.54% of the threshold space reaches ≥90% accuracy with a contiguous high plateau (context ∈ [0.20, 0.99], event ∈ [0.45, 0.95]), the two factors of a conjunctive code tolerate different biases, and over-separation fails *through capacity* (symbol explosion exhausts the store) rather than through confusion — the first joint statement of G38 and G42 (Poursiami et al. 2025).
- **[[wiki/entities/hisd.md]]** — the same trade one level up, in symbols rather than patterns: an over-separating terminal alphabet (44 noisy Minecraft skills) makes near-duplicate behaviour strings distinct, so a deterministic grammar reuses nothing and emits one private parse tree per episode, and the authors' own remedy — a probabilistic grammar marginalising over noisy parses — is completion applied to structure (Harvey et al. 2026).
- **[[wiki/concepts/vector-symbolic-binding.md]]** — the trade appearing inside a single vector: superposition needs its components to interfere little while graded type similarity needs them to interfere usefully, and the contextualization weight `κ_c` is a continuous, per-query dial between the two; unbinding also requires an attractor cleanup step to snap a noisy decode onto a stored token.
- **[[wiki/entities/arc-vsa-solver.md]]** — the dial measured on rule induction rather than on memory: a condition learned over near-orthogonal colour codes gives strict rules (100% on Sort-of-ARC), the identical learner over overlapping shape codes gives abstraction *and* over-generalisation (89%), with the length-scale that sets the overlap fixed by hand (G38).
- **[[wiki/entities/spiking-hippocampal-cam.md]]** — the transfer curve pinned at the separation extreme by assumption: its cue is one-hot because the dentate gyrus is taken to sparsify maximally, so there is no bias left to set (G38) — and the sparsifier that would produce the code is not modelled, which moves the hard half of addressing outside the store and onto its caller.
- **[[wiki/concepts/random-feedback-addressing.md]]** — the allocate-vs-reuse bias as a *connection-formation prior* over a shared hub rather than a threshold or a neuromodulator: a concentration parameter κ on which sensory units may converge on the same control unit runs eight regions from fully separate (κ = 0, each gets an idiosyncratic private shadow of the same control signal) to fully merged (κ ≥ 0.2, a write to one completes across all), and the over-completion failure mode is named and unusual — a percept with no cause rather than over-generalisation (G38; Park & Serences 2025).
- **[[wiki/concepts/encoding-retrieval-alternation.md]]** — the knob resolved in *time* rather than in parameters: a 4–8 Hz oscillation runs the circuit at the separation-favouring setting on one half-cycle and the completion-favouring setting on the other, which is the only candidate controller for G38 that needs neither an error signal, a neuromodulator nor a learned gate — and the resulting minus/plus phase pair turns the store into an error-driven learner.
- **[[wiki/concepts/complementary-learning-systems.md]]** — a limit on what separation alone can buy: however non-overlapping the DG/CA3 code is, reading it back through a *dense* entorhinal population re-imports the interference, so the circuit needs a sparse invertible CA1 codec between the store and the cortex (O'Reilly et al. 2011) — a third position on T33 alongside "relay" and "recombiner".
- **[[wiki/entities/cn-dpm.md]]** — the allocate-vs-reuse bias as an explicit likelihood ratio against a Dirichlet-process `α` rather than as a similarity threshold, with the cost of misplacing it measured for the first time: on Split-MNIST a 200-nat change in `log α` moves accuracy 54.04 → 93.23 and the component count 3.2 → 5.0, and `α` is hand-set so that exactly the known number of components appears (gap G38).
- **[[wiki/entities/ch-hnn.md]]** — the separation/completion bias as a *learned function of input similarity* rather than a fixed sparsity level: an upstream network is trained so that the cosine overlap between two inputs' binary masks equals the perceptual similarity of the inputs, with a separate sparsity term (`ρ ≈ 1/n`) setting the density — overlap for similar episodes, disjoint units for dissimilar ones, decided per sample at run time (gap G38).
- **[[wiki/concepts/memorisation-vs-generalisation.md]]** — the machine measurement of this page's write-side randomization, arrived at independently and for the same reason (a summed-token input makes basis-aligned lookup leak onto name recombinations): trained MLPs break linear structure 60–70% of the way where shuffled weights reach 20–40%, so hashing is discoverable by gradient descent — but known names get a larger transform than unknown ones, so unlike mossy fibres the learned separator is content-dependent (T286).
- **[[wiki/concepts/multi-token-embedding.md]]** — the place in a language model where hash-on-write is forced rather than optional: the input to fact lookup is a *sum* of name-token embeddings, so `Michael Jordan` and `Michael Smith` arrive half-identical, and any basis-aligned lookup on the unscattered sum would fire on recombinations that were never stored.
- **[[wiki/concepts/convergent-circuit-motifs.md]]** — the phylogenetic argument for this page's mechanism: dentate→CA3, Kenyon cells→mushroom-body output and cephalopod amacrine→vertical-lobe output are the same expansion→compression shape reached from three independent anatomies, which is evidence the motif is forced by the problem rather than a vertebrate contingency.
- **[[wiki/entities/neural-episodic-control.md]]** — the same bias set on the *read* side rather than the write side: with keys written unconditionally, all separation/completion control collapses into the retrieval kernel, and `1/(‖h−h_i‖²+δ)` is chosen over a Gaussian precisely because a Gaussian exponentially suppresses everything but the nearest key when all 50 neighbours are far — completion by heavy tail, never swept (G38; Pritzel et al. 2017).
- **[[wiki/entities/resonator-network.md]]** — completion used as a subroutine of *search* rather than of retrieval: each factor's codebook projection is a completion step over a deliberately noisy superposed guess, and separation between codebook atoms is what makes a superposition of `D^{F−1}` combinations decodable at all — so the two operations run every iteration rather than at different stages.
- **[[wiki/concepts/tensor-product-representation.md]]** — the same trade pushed back to the design step, before any code is chosen: under positional roles, superposing `ab` and `cd` is indistinguishable from superposing `ad` and `cb`, while a right-neighbour contextual role set keeps them apart — so choosing the *role decomposition* already fixes where a superposition sits on this axis, independently of sparsity or dimension.
- **[[wiki/concepts/retrieval-mediated-learning.md]]** — proposes a trigger for this page's knob that costs no extra machinery: write-time integration runs wherever the store completes anyway, so completion selects which overlapping episodes get merged into a composite and the separation/integration budget follows the completion threshold rather than a policy (G38).
- **[[wiki/concepts/hippocampal-long-axis.md]]** — an anatomical candidate for the knob this page says nothing sets (G38): place-field width grows ~tenfold along the long axis, and wider fields overlap more, so position on the axis is a completion-favouring gradient that needs no cholinergic mode bit — untested, and it predicts the bias should differ between dorsal and ventral recordings of the same animal.
- **[[wiki/entities/entorhinal-cortex.md]]** — types the input the separator receives: the dentate/CA3 front end is fed only by reelin⁺ layer II cells whose local network has no recurrent excitation at all, and single axons deliver the same vector to dentate gyrus and CA3 together (Witter et al. 2017).
- **[[wiki/entities/cerebellum.md]]** — the circuit this page's Marr example lives in, now described as a whole: the Golgi-cell set point is the runtime sparsity controller, the parallel-fibre expansion is the separator, and what the separated code is *for* is stated — addressing a learned controller, with the climbing fibre supplying the error that shapes the Purkinje cell's read-out.
...
- **[[wiki/entities/bb-model.md]]** — completion doing the whole retrieval job in an implemented multi-region model: current injected into a single perirhinal identity cell recovers place, boundary-vector, object-vector and boundary-identity populations, and every amnesia the model simulates is a statement about which populations remain reachable once one input is cut.
- **[[wiki/entities/dense-associative-memory.md]]** — the transfer curve made a single scalar applied at the read-out instead of at the code: raising the energy exponent `n` sharpens each stored pattern's contribution, so small `n` is the completion end (several memories blend into one decision) and large `n` the separation end (> 8000 of 10,000 MNIST test images have no second memory within 0.9 of the winner) — with no change to sparsity, dimensionality or the write rule (Krotov & Hopfield 2016).
- **[[wiki/entities/two-body-dense-associative-memory.md]]** — a re-partition of the completing store itself: CA3 is proposed to hold *both* the feature neurons and the memory neurons of a bipartite associative memory, with place cells as the memory neurons (a place field being an aggregation of grid and sensory input) and the protocol-silent pyramidal cells as the unaccounted population. Offered as a hypothesis with no measurement, and hedged by the authors on the grounds that hippocampal retrieval circuitry cannot presently be separated from its other functions (Krotov & Hopfield 2021).
- **[[wiki/entities/continuous-modern-hopfield-network.md]]** — separation given a number and a bound: `Δ_i = x_iᵀx_i − max_{j≠i} x_iᵀx_j` (half the squared nearest-neighbour distance for equal-norm patterns), with every retrieval-error bound of the form `exp(−βΔ_i)`, so "how far apart must two memories be" has a closed-form answer — and the intermediate regime, where a read returns the mean of a *cluster* of similar patterns, is completion-over-a-neighbourhood as a stable fixed point rather than as a failure (Ramsauer et al. 2020).
- **[[wiki/concepts/key-value-memory.md]]** — what these two operations are *for*, under the address/content split: they are error correction on the **key**, cleaning a noisy query onto the right address before any content is read — and the review explicitly declines between the recurrent entorhinal-hippocampal loop and dentate-separation/CA3-completion as the circuit that does it (`T49`).
