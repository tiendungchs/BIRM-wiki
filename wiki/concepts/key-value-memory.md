...
- **[[wiki/concepts/associative-memory.md]]** — the operation this page's split is a decision *within*: the four decisions (code, kernel, separation operator, write rule) are independent, so `K = V` is a constraint on the first and every autoassociative capacity number is a number for the constrained case — and this page's linear-layer identity is that page's fifth instance of the elimination principle.
