...
- **[[wiki/concepts/inductive-bias.md]]** — names and separately quantifies the two biases Theorem 1 says are jointly required: the model-side bias is the hypothesis space `H` (priced by its covering number) and the data-side bias is the environment distribution `Q` (priced by the number of tasks sampled from it), so the impossibility result becomes a budget rather than a dead end (Baxter 2000).
- **[[wiki/concepts/o-information.md]]** — what the total-correlation penalties of FactorVAE and β-TCVAE are half of: `C` is the collective-constraints term of the decomposition `Ω = C − B`, so penalising it leaves shared randomness `B` untouched and a low-`C` code can still be either redundancy- or synergy-dominated — a distinction none of the six objectives or nine metrics here can make.
- **[[wiki/concepts/label-free-model-selection.md]]** — the instrument this page's central negative result demands: since every disentanglement metric needs ground-truth factors, the >12,000-model sweep's "no unsupervised selector" finding is the opening measurement there — and the cheap test it proposes is to compute effective rank on these same codes and see whether a capacity statistic carries any structural signal.
