...
- **Is the map amodal at all?** The one design that matches relational demand across domains finds the hippocampus only in the spatial version, and the mentalising network carrying the social one (Kumaran & Maguire 2005, [[wiki/empirical-tensions.md]] T45). Every "beyond space" row above is an unmatched contrast; until one of them is re-run against a domain-matched control, the generality of the map is asserted, not measured.
