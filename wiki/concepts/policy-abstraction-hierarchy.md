...
## The machine mainstream's stack, and what it splits on

> **Companion.** Kawaharazuka, Oh, Yamada, Posner & Zhu 2025 (`raw/kawaharazuka-2025-vision-language-action-robotics.md`), a survey of Vision-Language-Action models. Everything in this section is reported there and attributed to its own source.

Hierarchical control is now the standard design for robot policies — the survey's "most recent generation" — which gives this page a large, independently motivated sample to check the biology's constraints against. The interface forms actually built:

| Interface passed down | System | What the level below receives |
|---|---|---|
| A subtask sentence | Atomic Skill, LMM-3DP (frozen VLM as the planner), Hi Robot (planner trained) | Cleaner language, and the same image |
| A "language motion" — an intermediate action phrase | RT-H, LoHoVLA, `π₀.₅` — high and low level are **one network**, switched by prompt | Same, plus the whole observation |
| An unnamed latent vector | FiS-VLA, OpenHelix, DP-VLA | An unconstrained code, and the same observation |
| Attention over the upper level's tokens | GR00T N1 (diffusion transformer cross-attending a vision-language backbone) | The upper level's full token set |

**Two observations, both load-bearing for this page.**

1. **The split is on *rate and body-independence*, not on policy order.** The upper level runs at a low frequency over language and images; the lower level emits chunks at up to 50 Hz by flow matching. A `Π : c → π_i` that names which rule is live is not what any of these systems computes — the upper level names a *subgoal*, which is a temporal abstraction ([[wiki/concepts/temporal-abstraction-options.md]]), or a level of [[wiki/concepts/cross-embodiment-transfer.md]]'s body-independence ladder. So the machine field's "hierarchy" and this page's are different axes with the same name, and no reported system stacks them.
2. **Gap G59 holds without exception across the sample.** In every row the lower level sees the observation its manager saw. The medial-wall result above says the biology withholds it — pre-SMA gets the tactic and the resolved action and *not* the cue position — and the survey names the corresponding machine failure independently: reliability improves when the low-level policy receives "cleaner and more concise language inputs", i.e. the benefit is attributed to narrowing the *instruction* channel while the visual channel stays wide open. Nobody has run the biology's version, which is the same experiment with the image withheld from the low level.

**A third design point the biology has no analogue of: severing the learning signal between levels.** The survey's recommendation to practitioners is **gradient insulation** — block gradients from the (randomly initialised) action head from reaching the vision-language backbone, or freeze the backbone entirely (GR00T N1.5) — because the transient from an untrained lower level degrades the upper level's pretrained representations. **(brainstorm)** Stated in this page's terms, that is the admission that the machine stack has *no per-level credit assignment at all*: unable to attribute error to a level, the field's answer is to switch the upper level's learning off. The biology's parallel-search account needs exactly the missing quantity — a per-level signal that attenuates a level whose hypotheses go unrewarded — and this is the sharpest evidence yet that nobody has it (gap **G65**, and the open problem "the credit signal per level is not named" below).
...
- **[[wiki/entities/basal-ganglia.md]]** — the internal wiring of a single one of this page's cortico-striatal loops (opponent D₁/D₂ spiny-projection populations, one broadcast dopamine scalar with opposite signs on excitability *and* on plasticity), plus a second specialisation axis orthogonal to the rostro-caudal one: the dorsomedial/associative loop dominates early acquisition and the dorsolateral/sensorimotor loop takes over once the program is automated, so loops differ by *stage* as well as by policy order. Nakahara et al. 2001 turn that axis into a runnable claim: the two loops learn the *same* sequence in parallel and in **different coordinate systems** — anterior striatum ↔ dorsolateral prefrontal cortex in visual coordinates, fast but working-memory-dependent; posterior striatum ↔ supplementary motor area in motor coordinates, slower to learn but faster and more reliable to execute — with control migrating from the first to the second, and muscimol dissociating early from late production by striatal position (Miyachi et al. 1997, 2002; via Helie et al. 2013). So the second axis is not a hierarchy of abstraction at all but **two learners at different rates over different representations of one task**, which is the [[wiki/concepts/complementary-learning-systems.md]] split applied to policies rather than to memories.
- **[[wiki/concepts/cross-embodiment-transfer.md]]** — the axis the machine mainstream's two-level policies actually split on: the upper level is slow and body-independent (language, subgoal, latent), the lower level is fast and body-specific, which is neither this page's policy order nor temporal extent — and every such system violates gap G59 by handing the lower level the same observation (Kawaharazuka et al. 2025).
- **[[wiki/concepts/temporal-abstraction-options.md]]** — the other axis of frontal hierarchy, and the pair looks exhaustive: this page's orders abstract over *rules* (a rule whose output names a rule), options abstract over *time* (an action whose execution names a policy), and both are claimed for the same rostro-caudal machinery with the same cortico-striatal loop — so a builder must decide whether one stack serves both roles or two are needed (Botvinick, Niv & Barto 2009).
- **[[wiki/concepts/sparse-expert-routing.md]]** — the engineering opposite of this page's result, and the comparison is decision-relevant: a top-`k` router selects before computing, where frontal cortex engages every order of rule from the first trial and *attenuates* the unrewarded one over learning — run-all-and-prune against select-then-run, with the machine literature paying for its choice in load-balance losses and routing instabilities the biological policy cannot have.
- **[[wiki/concepts/abstraction.md]]** — the census that files policy order as family O, orthogonal to the format family: a rule's distance from an action and a code's invariance to content are separate axes, and this page measures the first with no measurement of the second on the same system.
- **[[wiki/entities/lateral-frontal-pole.md]]** — the rung above this page's ladder and the challenge to reading control off it: the frontal pole is assigned *temporal* control (integrating past information into future action) above contextual control and feature control, but the ordering reverses when hierarchy is scored by efferent-minus-afferent influence instead of by activation abstractness, which puts mid-dorsolateral prefrontal cortex at the apex (`T332`) — so this page's rostro-caudal gradient may be a gradient of representation with no claim of command attached.
- **[[wiki/concepts/affective-opponency.md]]** — a control dimension none of these loops represents: *vigour*, set by the average rate of reward and controllable punishment acting as an opportunity cost for the passage of time, so how fast a selected policy is executed is computed separately from which policy is selected (Niv et al. 2007 via Boureau & Dayan 2010).
- **[[wiki/concepts/expected-value-of-control.md]]** — the direct denial of this page's ordering, on the medial wall (`T364`): the expected-value-of-control criterion contains no level argument and predicts engagement wherever a process cannot run on prespecified parameters, irrespective of abstractness or temporal extent — while the dorsal-anterior-cingulate gradient it reviews (motor conflicts caudally, strategy conflicts rostrally; block- versus trial-level incentives) is this page's ordering appearing anyway, in studies that confounded abstraction with difficulty.
- **[[wiki/concepts/effort-based-decision-making.md]]** — a proposed determinant of which level of this hierarchy actually runs, and it is a priced quantity rather than a task property: an agent's elicited sensitivity to cognitive effort is hypothesised to predict reliance on model-based over model-free control and on proactive over reactive control, both of which are the computationally expensive member of their pair. If that holds, occupancy of the higher-order bands is set by a per-agent cost parameter measurable *before* the task, which no arbitration scheme in the wiki reads (Westbrook & Braver 2015).
- **[[wiki/concepts/default-policy-regularization.md]]** — what a levelled control cost would require: pricing a controlled policy against the *nearest* habit rather than a global one needs one distilled default per band of this hierarchy, which would make the rostro-caudal ordering a ladder of increasingly abstract defaults and give each level its own automatisation dynamic. With a single default the account is level-blind, like the expected-value-of-control criterion it otherwise disagrees with.
- **[[wiki/entities/feudal-networks.md]]** — the machine hierarchy that meets this page's non-bypassability prescription halfway and by a different route: `π_t = SoftMax(U_t w_t)` with a **bias-free** `φ` means the goal embedding can never be constant, so the lower level cannot algebraically factor the manager out of its policy — non-bypassability by multiplicative binding rather than by withholding the argument. The withholding itself is not done: the Worker's recurrent net still receives the shared perceptual embedding, so `G59` holds in the row that matters (Vezhnevets et al. 2017).
