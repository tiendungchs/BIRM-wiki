...
| **`θ` is set by hand** | The threshold *is* the separation/completion bias — raise it and the store insists on a new entry, lower it and it completes to a stored one — and it is a design constant here, which is `G38` in one parameter |
...
For [[wiki/concepts/latent-graph-discovery.md]] the relevant operation is **node identification under aliasing and noise**: deciding that the current observation is (or is not) an already-known graph position. This page also supplies the wiki's cheapest answer to `G42`: `M·P(match)` bounds the population error from `n`, `a`, `s`, `θ` and `M` alone, so "how full is the store" is computable with no training and no measurement, and the degradation is a rising false-positive rate rather than a cliff. This page prices that operation exactly — the decision is a thresholded overlap over ~25 bits, the false-merge rate is computable, and the cost does not grow with the number of known positions until the union limit. It supplies nothing about where the codes come from (hardness sources 1–2 and 4–6 untouched) and everything about source 3, aliasing: two distinct positions with distinct SDRs will not be confused, at a rate you can name in advance.
...
- **[[wiki/entities/early-visual-system.md]]** — where the natural-scene sparsification this page's decorrelation argument leans on (Vinje & Gallant 2000) is scored rather than cited: the context that produces it accounts for the majority of a V1 cell's response variance, so sparsity under natural input is a property of the surrounding population and not of the classical receptive field.
- **[[wiki/concepts/random-feedback-addressing.md]]** — the same random projection used in the opposite direction: expansion into a wide random layer buys capacity and separability going up, and the identical wiring read transposed buys *selectivity* coming down, with the balanced-weight condition `ΣW = 0` playing the role sparsity plays here (Park & Serences 2025).
- **[[wiki/entities/ch-hnn.md]]** — code density as a trained quantity instead of an architectural constant: the gating network's loss carries an explicit `β·|‖R‖₁ − ρc|` term alongside the similarity-matching term, so sparsity is balanced against similarity fidelity rather than set by the designer.
- **[[wiki/concepts/convergent-circuit-motifs.md]]** — this page supplies the substrate-free half of a two-sided argument: expansion recoding is recommended by Cover's theorem and Kanerva's derivation with no biology in them, *and* independently re-invented across four phyla, and the agreement of the two is what breaks the wiring-cost confound that convergence alone cannot.
- **[[wiki/concepts/fast-weight-programming.md]]** — this page's principle re-derived from linear algebra to raise a memory bound: an outer-product store holds at most `d_dot` non-interfering associations, so DPFP expands the key into products of rectified pairs (`φ_{iν}(k) = r([k;−k])_i · r([k;−k])_{i+ν}`, `d_dot = 2 d_key ν`) whose supports are near-disjoint — expansion plus rectification bought for orthogonality rather than for discriminability, deterministic and parameter-free (Schlag et al. 2021).
- **[[wiki/concepts/tensor-product-representation.md]]** — the conjunction operator taken over Boolean rather than real scalars: superposition by logical OR (`1+1=1`) instead of arithmetic addition is the same construction with a different scalar field, and Smolensky flags the trade directly — thresholded retrieval from a lightly loaded Boolean store can be *exact*, where real-valued superposition buys graded similarity and pays interference.
- **[[wiki/entities/saycam-baby-vision.md]]** — a selectivity measurement on a self-supervised trunk: the class-selectivity index is low throughout and rises only toward the output, so categories demonstrable by linear read-out have no unit-level correlate — the distributed side of this page's trade-off, measured rather than assumed.
- **[[wiki/concepts/width-versus-parameter-count.md]]** — the same word applied to the other object, kept apart deliberately: sparsity here is in the **activity** (`a/n` held at 0.5–3%, priced in recognition error), there it is in the **connectivity** (1–10% of weights present, priced in test accuracy at a fixed weight budget) — the two optimal bands are numerically similar and nothing in the wiki relates them.
