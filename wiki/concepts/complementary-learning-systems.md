...
- **[[wiki/concepts/arbitrary-sensorimotor-mapping.md]]** — the fast/slow dissociation on material with no internal regularity: hippocampal ablation blocks acquisition of *new* arbitrary cue→action mappings while pre-lesion mappings are performed normally with the same cues and movements, so the cortical residue cannot be a compressed structure — and the acquisition rate the fast store must support is measured (~3 trials/cue to substantial learning, 24 new mappings/day) (Wise & Murray 2000).
