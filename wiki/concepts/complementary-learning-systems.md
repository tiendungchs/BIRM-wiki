...
- **[[wiki/concepts/manifold-constrained-learning.md]]** — proposes a boundary between fast and slow that is geometric rather than quantitative: within-span re-association of existing population patterns is learned in one session (adaptation), while changing the span itself is not learned at all on that timescale and is the posited job of multi-day skill learning — a fast/slow split diagnosable from the population geometry before any training is run (Sadtler et al. 2014).
