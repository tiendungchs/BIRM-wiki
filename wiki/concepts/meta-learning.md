...
**Emergence conditions.** Recurrence suffices — no particular gating mechanism is required — *provided* the inputs carry the previous action and reward and the dynamics maintain information over the task horizon. The reward therefore enters twice — as the outer loop's teaching signal and as an observation to the inner one — which is the only half of `G57` anyone has built; what is still missing is a system in which the two roles can be lesioned apart. That is the scope statement the optimization equation above lacks: it says which circuits should and should not show the effect.
...
- **Weights vs. activity for the fast level** — CLS and meta-RL disagree, and they may be complementary (episodic fast-M for bindings, recurrent fast-M for policies). Either way the split delivered here is a *rate* difference — outer loop at 0.00005, inner learner orders of magnitude faster — and not a factorization of the variables, which is `G1`.
...
- **[[wiki/entities/default-mode-network.md]]** — the nearest biological demand for this page's two-timescale structure outside task learning: the internal/external mode-assignment gain is developmentally trained, has a critical window, and stays plastic in adults, so it is a control parameter learned on a slower schedule than the policy it gates — which the outer loop here supplies in kind but not in shape, since it optimises the whole fast learner rather than one gain.
- **[[wiki/concepts/environment-invariance.md]]** — the same multi-environment signal spent differently: this page reads it as a task distribution for an outer loop, that one converts it directly into a penalty, so it needs no inner-loop update and cannot use adaptation speed as its causal signal.
- **[[wiki/entities/scan.md]]** — where the learning-to-learn proposal was first stated as the fix for a measured compositional failure ("expose a network to many learning environments regulated by similar rules"), five years before [[wiki/entities/mlc.md]] ran it on these very splits.
- **[[wiki/concepts/structural-learning.md]]** — the outer loop with its mean deleted: the task parameter is sampled at expectation zero so `E[T]` is the identity and nothing can be installed in the average mapping, yet transfer is large — which separates what a sampler's mean installs from what its *support* installs, and yields a one-line design rule (sample the family's parameter symmetrically about the identity) plus a measurement of the boundedness limit in the same experiment, since the same parameter exposure inside a looser family learns slower than no exposure at all (Braun et al. 2009).
- **[[wiki/concepts/inductive-bias.md]]** — the sample-complexity statement of what this page's outer loop buys: `p(T)` is an environment distribution `Q`, the product is a hypothesis space rather than an initialisation, and generalisation to a *novel* task is guaranteed once the number of **tasks** (not just examples) passes a capacity-controlled threshold — `m = O(k + W/n)` examples per task, `n = O(W)` tasks (Baxter 2000).
- **[[wiki/concepts/curriculum-learning.md]]** — the same task sequence read for *order* rather than for average: this page samples `p(T)` i.i.d. and asks what the mean installs, a curriculum anneals `Q_λ` from low entropy to `p(T)` and claims the order alone selects a different local minimum at fixed architecture and fixed data — a payoff no meta-learning objective in the wiki scores (Bengio et al. 2009).
- **[[wiki/entities/asymmetric-self-play.md]]** — an inner loop arriving as a by-product of episode structure rather than as an objective: five proposed goals per episode beats one, and the authors attribute it to the recurrent policy internalising the environment constants (object properties, dynamics) that are fixed within an episode — so early goals are probes that pay off on later ones, and a goal curriculum and a fast-adaptation task turn out to be the same schedule (OpenAI et al. 2021).
