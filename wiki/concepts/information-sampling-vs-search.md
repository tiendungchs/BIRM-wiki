...
| Eight-month-old infants watching cue–target sequences look away when the environment is **either** too predictable or too unpredictable (quadratic in negative entropy), but the effect is the *weakest* of three simultaneous terms — `e^{\|β\|} = 1.27` against `2.44` for surprise and `7.02` for realised information gain ([[wiki/concepts/realised-information-gain.md]], Poli et al. 2020) | the intermediate-complexity preference this page treats as the headline developmental effect is real, uninstructed and present at 8 months — and it is a third-order term. The strong term is the progress-like one this page endorses on robot evidence, now with a human coefficient at the earliest age it has been measured |
...
- **[[wiki/concepts/explore-exploit-division-of-labour.md]]** — a *temporal* answer to this page's open search problem instead of a normative one: run broad search with the instrumental objective switched off for a protected, externally subsidised period and switch it on later, rather than pricing a query against a goal it does not have — and it supplies the measurements this page's curiosity section never reaches, preschoolers out-inferring adults on low-prior causal hypotheses and escaping the learning trap that adults' early commitment seals (Gopnik 2020).
- **[[wiki/concepts/self-boosting-knowledge-acquisition.md]]** — the missing size variable for this page's complexity objection: the hypothesis space is not merely uncounted but *grows with what the agent has learned* (`Gaps(K) = αK(N − K)`), so search is not a transient regime that sampling eventually replaces — and this page's observation that memory makes search specific, which is how lifelong interests compound within a domain, is that claim stated without the derivation (Murayama 2022).
- **[[wiki/concepts/sign-tracking-and-goal-tracking.md]]** — the primary source behind this page's sign-tracking evidence, and it separates the two readings one step further: approach to the predictor and accuracy of the prediction are dissociated within a single paradigm — matched acquisition curves, dopamine signature and dopamine-dependence only on the approach side — so "engaging with the cue" is not a degraded form of "consulting the cue" but a separately taught product (`T360`, `T404`).
- **[[wiki/concepts/realised-information-gain.md]]** — this page's developmental ladder given its bottom rung and its terms ranked: at 8 months, with no instruction possible, a belief-update magnitude dominates the decision to keep looking (`e^{|β|} = 7.02`), a surprise term carries the opposite sign (`2.44`), and the intermediate-complexity inverted U this page leads with is the weakest of the three (`1.27`) — so the page's endorsement of learning-progress heuristics over novelty heuristics is supported, while its framing of the Goldilocks effect as the central infant finding is not (Poli et al. 2020).
