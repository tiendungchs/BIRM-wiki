...
**Equation (1) assumes the hierarchy is a chain, and cortex's top level is not in it.** `y^{S_i}` couples only to `S_{i−1}` and `S_{i+1}`; every layer is both a consumer and a supplier for its immediate neighbours, and a top-level revision reaches the sensory layer only after `L−1` relays. Satpute et al. 2026 (`raw/satpute-2026-dmn-hierarchical-generative-model.md`, via [[wiki/entities/default-mode-network.md]]) argue the highest cortical system does the opposite on both sides: it **receives selectively** — proportionally more from high-level association cortex than from sensory areas — and **projects diffusely to many levels and laminar types at once**. Three consequences for this page:

| | Chain (equation 1) | Broadcast apex |
|---|---|---|
| Cost of a global model revision | `L−1` relays, during which intermediate levels compute on a repudiated prior | one act |
| Where the compression is enforced | at every hop, uniformly | asymmetrically — on the *upward* path only; the downward path is deliberately wide |
| Control variable available | precision gates `α, β, γ` set the **strength** of the update, per level, independently | additionally a **scope**: how many levels a revision is permitted to reach, gated by whether the immediate past still predicts the input |

The scope variable is the one this page has no expression for. `γ → 0, β ≪ 0` decouples a level from sensation — a *depth-wise local* operation — but nothing in the formalism says "revise all levels now, sourcing the replacement from stored priors rather than from the input", which is what the source assigns to the default mode network at event boundaries ([[wiki/concepts/event-segmentation.md]]). Logged as [[wiki/empirical-tensions.md]] T259; the architecture is treated at [[wiki/concepts/broadcast-hierarchy.md]].
...
| **Nudging** ([[wiki/concepts/equilibrium-propagation.md]]) | Add `βC(h_L, y)` to the energy: `F = E_PCN + βC` | `C` is free. The update is `(1/β)·∂F/∂θ` at the nudged state, and → `∇_θC` as `β → 0` |
...
**One error unit or two, and a spike count that does not fall.** Both claims above are contested by the cortical cell-class literature (Keller & Mrsic-Flogel 2018, [[wiki/concepts/prediction-error-neurons.md]]):

- **`ξ` is signed, and cortex may not be able to carry a signed residual on one population.** L2/3 principal cells have baseline rates too low for a rate *decrease* to encode "less input than predicted", so the error computation may need two oppositely wired circuits — bottom-up excitation vs top-down inhibition for the positive error, top-down excitation vs bottom-up inhibition for the negative one — with opposite effects on the receiving state units. [[wiki/empirical-tensions.md]] **T341**. A machine network using one signed unit is making an unstated assumption about baseline activity.
- **The efficiency justification does not survive counting.** For every bottom-up spike cancelled there must be a top-down spike carrying the prediction that cancelled it, so total spikes are roughly unchanged and predictive coding is *not* cheaper per bit of transmitted information. The architecture's actual payoff is that the state estimate is updated by top-down as well as bottom-up input, and can therefore be run with `γ → 0` as a simulation — which is the same switch this page derives below, arrived at as the *reason for the architecture* rather than as a mode.
- **The strict hierarchy of equation (1) is an assumption about cortex, not a finding.** Any two areas with a learned transformation between their coordinate systems can exchange predictions and errors in *both* directions, whether or not either is above the other; a variant with predictions ascending and errors descending is a live alternative. [[wiki/empirical-tensions.md]] **T259**, **T341**.
...
**The direct test, and it half-passes.** Bastos et al. 2015 correlate the Granger-influence asymmetry of 28 macaque visual area pairs against those pathways' anatomical SLN, per frequency, and read the direction assignment off the resulting correlation spectrum (`raw/bastos-2015-feedforward-feedback-frequency-channels.md`, [[wiki/concepts/inter-areal-synchrony.md]]). Feedback is carried at **beta, 14–18 Hz** and feedforward at **gamma, 60–80 Hz** — the derivation's prediction, measured on the anatomy rather than inferred from a cognitive contrast. But feedforward is *also* carried at **theta, ~4 Hz**, well below the feedback band, and nothing in the integration argument predicts a slow forward channel. The ordering claim (`error faster than the estimate that absorbs it`) therefore holds between beta and gamma and fails at theta; what survives unqualified is the weaker claim that direction is carried by fixed, separate channels. The authors' escape is that theta feedforward is microsaccadic — a resampling of the retina at 4 Hz, i.e. a new input volley rather than a cortical message — and their conditional-Granger analysis finds the theta influence to be network-mediated where gamma and beta are direct. Logged as [[wiki/empirical-tensions.md]] **T379**.
...
- **[[wiki/concepts/counterstream-organization.md]]** — removes one of the anatomical arguments for this framework's up/down asymmetry: quantitative dual-tracer tracing shows topographic precision, divergence and axonal bifurcation index the supragranular/infragranular compartment rather than the direction of the pathway, so "feedforward is topographic and driving, feedback is diffuse and modulatory" holds only for the long-range pathways that originate exclusively from layer 3B and layer 6, and not for the 75% of corticocortical neurons that are short-to-medium range.
...
- **[[wiki/entities/spiking-tem.md]]** — an explicit predictive population in a trained circuit: MEC III is trained to predict MEC II one step ahead, and its spatial tuning is only measurable against the *future* position (gridness low at the current position, significantly higher one step ahead), which is what a prediction-carrying layer should look like in recordings.
...
- **[[wiki/concepts/inter-areal-synchrony.md]]** — where the derived frequency asymmetry becomes a measurement protocol, and where it is tested: band-limited coupling types an inter-module link, the high/low power ratio of two populations says which one is integrating the other's residual, and correlating per-frequency Granger asymmetry against anatomical SLN confirms beta-feedback / gamma-feedforward while adding a theta feedforward channel the integration argument does not predict (T379).
...
- **[[wiki/concepts/equilibrium-propagation.md]]** — the rule that this page's energy rescues: `E_PCN` puts the free equilibrium at the unique global minimum reached in one forward pass, collapsing the relaxation cost that had confined the method to MNIST, and mod-PGD's inhibitory buffer is this page's `Σ` knob rediscovered as a projection rule.
- **[[wiki/concepts/language-of-thought.md]]** — the rival this page's compose-by-relaxation proposal is aimed at: mutual constraint satisfaction gives composition with no shared symbolic format and no binding operation, at the price that a low-energy configuration is consistent rather than true.
- **[[wiki/concepts/violation-of-expectation.md]]** — the behavioural read-out of this page's central quantity: surprise as prediction error at one level of an abstraction hierarchy, used to rank two futures of one past, with a video model's error trace standing in for infant looking time.
- **[[wiki/entities/ms-ssm.md]]** — the same slow-above/fast-below timescale hierarchy from the opposite direction: this page *derives* it from the coupling (a higher attractor prescribes the manifold the lower states flow over, so it must change slowly), while MS-SSM *installs* it as a prior on the eigenvalue distribution — coarse bands initialised near `|λ| = 1`, fine bands near 0 — and prices it at 5.5 points of ListOps against an unstratified initialisation; its stated motivation is `(tentative)` evidence for hierarchical predictive coding across cortical timescales in human language processing (Caucheteux et al. 2023, cited secondhand).
- **[[wiki/entities/conceptor.md]]** — the architecture's shape without its mathematics: a three-layer hierarchy passing *hypotheses* down (a conceptor is a prior on which subspace the layer below should occupy) and progressively de-noised signal up, with a trust scalar on each link setting the mixing rate — built from linear algebra on state correlation matrices, with no generative model, no residual and no free energy, and beating a parameter-matched linear filter at SNR = 0.5 (Jaeger 2014).
- **[[wiki/concepts/default-self-model.md]]** — the framework applied to the self as generated object: positive illusions as an unattenuated prior in a default social-cognition generative model, with a specific region proposed as the correction term.
- **[[wiki/concepts/broadcast-hierarchy.md]]** — the challenge to equation (1)'s coupling assumption: an apex that receives selectively from high-level areas and projects diffusely to many levels and laminar types at once makes a global model revision cost one hop instead of `L`, and adds an update *scope* variable this page's precision gates cannot express.
- **[[wiki/entities/global-neuronal-workspace.md]]** — the review's own functional reading of a broadcast: workspace neurons hold a compressed high-level model, top-down signals are its predictions and bottom-up signals amplify or contradict them — with the local/global oddball dissociation as evidence that only the *global* prediction error requires a conscious model (the auditory mismatch negativity survives coma, sleep and inattention; the P3 to a global sequence violation does not).
- **[[wiki/entities/integrated-world-modeling-theory.md]]** — the folding construction: a disentangled variational autoencoder folded at its bottleneck so encoder level `k` aligns with decoder level `k` *is* a predictive-coding hierarchy, which makes this page a wiring pattern over a standard architecture rather than a separate model class — with the hedge that descending slow-band input may gate ascending errors rather than explain them away.
- **[[wiki/concepts/loopy-belief-propagation.md]]** — what happens *between* hierarchies where this page describes what happens within one: several folded hierarchies wired at their bottlenecks form a graph with cycles, and iterating messages across it is a turbo-code rather than a deeper stack of the same residual.
- **[[wiki/entities/ventral-visual-stream.md]]** — the rival account of the same anatomy, and the cleanest statement of what would separate them: the feedforward null covers foveal, uncluttered, unoccluded, uncued input under 200 ms and explicitly cedes rivalry, clutter, search and cross-fixation memory to feedback, so the dispute is over where in that space the first pass stops sufficing ([[wiki/empirical-tensions.md]] T278).
- **[[wiki/concepts/neuromodulatory-metaparameters.md]]** — a use for the residual that is neither a weight update nor a precision term: in the two-modulator conceptual model, a predictive module's mismatch signal triggers a noradrenaline-like transient that raises gain and plasticity network-wide, so prediction error is consumed as a *reconfiguration trigger* — the earliest available detector of a contingency change, firing before the value estimate has moved (Mei et al. 2025).
- **[[wiki/entities/cpc.md]]** — the machine method that takes this page's name and drops its mechanism: no residual is ever computed in observation space, the prediction is scored by discriminating the true future latent from random ones, and the paper cites Rao & Ballard and Friston as motivation only.
...
- **[[wiki/entities/a24b-m2-v1-projection.md]]** — turns the descending term from a postulate into a measured projection, and prices it: the prediction path into mouse V1 has roughly twice the cell count of the thalamic input and contacts >73% of neurons in layers 1, 2/3 and 6 while touching no layer-4 cell, so `top-down` cannot be realised as a scalar gain or a low-rank modulation; and its amplitude tracks the *predicted change* given the current image rather than the action alone, which is the state-dependence the equations imply and most implementations drop.
- **[[wiki/entities/visual-predictive-coder.md]]** — the derivation this page never gives for *why* the generative model's hidden state should be position: expand the next-observation posterior over latent coordinates and the three factors are encoder, transition operator and decoder, so an implicit spatial frame is the sufficient statistic of the history for the future rather than a stipulated content of the model (Gornet & Thomson 2024).
- **[[wiki/concepts/homeostatic-need-signal.md]]** — the interoceptive half of this page's scheme, measured rather than postulated: a need population carries a set-point deviation from which a *learned* prediction of relief is subtracted, so the trace falls at a food-predicting cue before any nutrient and is retracted when the cue is disconfirmed — and it is the reduction of that interoceptive error, not its level, that conditions preference for exteroceptive cues.
- **[[wiki/concepts/apical-amplification.md]]** — a physical delivery mechanism for the descending term with its sign reversed: feedback arriving on the apical tuft multiplies the receiving cell's feedforward transfer function (threshold halved, gain raised) instead of being subtracted from it, which needs no learned inhibitory converter and makes a well-predicted input the *loudest* thing on the ascending wire (`T374`, Larkum 2013).
- **[[wiki/entities/dendritic-integration-theory.md]]** — states the cost this framework must pay at the cell: the layer-5 pyramid intrinsically amplifies a match between the descending and ascending streams, so a predictive-coding reading of the same compartments has to buy the sign flip with an added learned inhibitory circuit — treated there as a constraint neurobiology imposes rather than a free modelling choice (`T374`).
- **[[wiki/entities/retrosplenial-cortex.md]]** — a named cortical node for the descending term, with the direction of influence shown to be learned: mouse primary visual neurons track stimulus properties before learning and become more strongly driven by top-down retrosplenial modulation after it, and the region's lesion deficit appears exactly when a learned prediction is violated by distal cues misaligned against local structure (Alexander et al. 2023).
- **[[wiki/concepts/reference-frame-transformation.md]]** — a generative model obtained without training a decoder: reading a frame-conversion interface top-down renders the store's contents into sensory populations on the *same* weights used for inference, with the bottom-up/top-down gain pair as the precision term deciding which direction currently wins ([[wiki/entities/bb-model.md]]).
- **[[wiki/concepts/realised-information-gain.md]]** — the two-policy corollary of this page's framework, measured: faced with input inconsistent with the model, a prediction-error minimiser can update or disregard, and 8-month-olds take the second option monotonically in surprise (`e^{|β|} = 2.44` on the look-away hazard) while a belief-update term in the same regression pulls the other way (`7.02`). So **disengagement is a predicted behaviour of a prediction-error-minimising system**, not a failure of curiosity — and the precision that would decide between the two policies is exactly what neither source measures (Poli et al. 2020).
