...
- **[[wiki/concepts/cortical-state-bistability.md]]** — accumulation with no evidence to accumulate: under a constant dichoptic input, prefrontal low-frequency burst amplitude ramps linearly and the number of recruited sites jumps non-linearly before an internally generated switch, and piecemeal perception is the sub-threshold arm this page's stopping rule has no state for.
- **[[wiki/entities/global-neuronal-workspace.md]]** — where the decision threshold comes from on this account: the criterion is the bifurcation point of a recurrent prefrontal cascade, and response bias is the distance of the *pre-stimulus* state from it — both measurable before the stimulus arrives.
- **[[wiki/concepts/metacognitive-efficiency.md]]** — the type 1 / type 2 split applied to this page's mechanism: the same accumulated decision variable that crosses the bound is the raw material a confidence rating reads, so `meta-d′ < d′` measures evidence present in the accumulator and lost before the read-out, and `meta-d′ > d′` implies post-decisional accumulation the commit did not use.
- **[[wiki/concepts/adaptive-computation-time.md]]** — the same stopping problem with the enumeration requirement dropped: instead of thresholding a normalised posterior over `N` named alternatives, a sigmoidal halting unit accumulates its own probability mass until it passes `1−ε`, trained end-to-end against the downstream loss plus a time penalty — so it applies where the alternatives do not exist yet, and it forfeits every optimality guarantee this page has in exchange (Graves 2016, gap G107).
- **[[wiki/concepts/selective-prediction.md]]** — the static form of the same rule: a bound on a decision variable *is* a threshold on a confidence rate, so this page's collapsing bounds are the time-varying `θ` that the selective-classification formalism has no version of; and conversely the accumulated evidence at the moment of commitment is the natural `κ_f` for a system that decides when to stop rather than what to answer.
- **[[wiki/entities/universal-transformer.md]]** — this page's accumulate-to-threshold shape running inside a representation rather than over a decision variable: halting mass accrues per sequence position until a hand-set threshold, the crossing step is down-weighted by the remainder so the commitment is an interpolation rather than a choice, and there is no error-rate guarantee attached to the threshold — while the ponder time it produces does track a counted quantity (1/2/3 supporting facts → 2.3/3.1/3.8 steps).
- **[[wiki/concepts/expected-value-of-control.md]]** — names this page's parameters as control *outputs* with a price: thresholds and response biases governing the speed–accuracy tradeoff are what a control signal sets, so post-error slowing is a re-specification of control intensity paid for out of `Cost(signal)` rather than a change in the evidence.
- **[[wiki/concepts/numerosity.md]]** — an independent derivation of the same psychophysical law from the code rather than from the decision: numerical distance and size effects fall out of the overlap of logarithmically scaled tuning curves, so Weber behaviour needs no threshold, no accumulator and no multistable operating point — which makes the two pages' accounts of one signature separable by whether the limit moves when the decision process is changed (Nieder 2016).
