...
- **[[wiki/concepts/relational-reinterpretation.md]]** — the case against reading one recursive operator as the whole discontinuity: agrammatic aphasics with no mental-state vocabulary pass causal-reasoning and theory-of-mind batteries while frontal degeneration destroys higher-order relational integration with language intact, and decades of ape symbol training bought word-order sensitivity and no gain on any higher-order relational task.
- **[[wiki/entities/recursive-npi.md]]** — the machine instantiation of the finite-elements-plus-one-recursive-operation claim, and evidence for the separability hypothesis from the engineering side: the recursive operator is installed by editing traces alone, leaving every other component of the architecture untouched, and is then testable on its own.
- **[[wiki/concepts/numerosity.md]]** — the shared side of this page's number row, described down to the circuit: approximate Weber-limited magnitude is a peak-tuned, log-scaled labelled-line code in parieto-frontal cortex, present in monkeys never trained on number. The unshared side gains nothing — the open-ended integer list is called a *preadaptation* that symbolic mathematics "co-opts", with no mechanism for the conversion — so the boundary this page draws now separates a fully described code from an entirely undescribed operator (Nieder 2016).
