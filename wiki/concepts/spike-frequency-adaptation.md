...
- **Nothing sets the adaptation time constants** ([[wiki/architectural-gaps.md]] G78). `τ_θ` (or the four DEXAT parameters) fixes memory span, interference, and gradient flow, and is chosen by grid search; the review proposes Bayesian or gradient-based optimisation as future work. A store whose capacity is a hand-tuned hyperparameter cannot meet a task whose span is discovered at run time. **The review's own proposal has since been built** (Wang et al. 2024, [[wiki/concepts/neuronal-parameter-heterogeneity.md]]): `τ`, `C`, `u_th`, `u_re` and a self-inhibition gain are learned *per neuron* by gradient descent on a held-out split, in a bi-level program alternating with the weight loop, and the measured product is exactly what this bullet asks for — accuracy becomes insensitive to the `τ` and `v_th` **initialization**, so a badly chosen constant is repaired rather than inherited. What it does not supply is the run-time half: `α` is frozen at deployment, so a span discovered mid-episode is still out of reach.
...
- **[[wiki/concepts/synaptic-plasticity.md]]** — one of the three biological causes of SFA is presynaptic vesicle depletion, so short-term depression and spike-frequency adaptation are the same functional variable measured at different ends of the synapse; which end it sits on decides whether the adaptation is private to a cell or specific to one incoming axon. Second contact, and the more interesting one: the same state variable — per-neuron, reset by a spike, recovering over tens of milliseconds — is what suppresses all but the first spike of a train from contributing to spike-timing plasticity (`τ_pre ≈ 34 ms`, `τ_post ≈ 75 ms`; Froemke & Dan 2002, [[wiki/empirical-tensions.md]] T329), so adaptation appears twice in the same cell with the same shape: once gating *firing*, once gating *writing*.
...
- **[[wiki/concepts/spike-train-error-metrics.md]]** — `G78` in a second variable: there the exponential filter constant `τ_q` on the *error* signal sets the finest learnable output latency (`τ_q = 0` → ~7 ms floor, `τ_q = 10 ms` → ~3 ms), exactly as `τ_a` here sets memory span, and in both cases the constant is found by parameter sweep rather than set by anything in the model.
- **[[wiki/concepts/neuronal-parameter-heterogeneity.md]]** — this page's design fork taken on the other branch and its `G78` answered halfway: self-inhibition subtracts the unit's last output from its *input current* rather than raising its threshold, and the coupling gain `γ^k` joins `τ^k`, `C^k`, `u_th^k`, `u_re^k` in a per-neuron vector learned by bi-level gradient descent — which is the first mechanism in the wiki that sets an adaptation constant by anything other than a sweep, and whose measured benefit is insensitivity to the initialization rather than raw accuracy.
- **[[wiki/concepts/learnable-synaptic-delays.md]]** — the first temporal parameter in the wiki that is *not* swept: a synaptic delay is set by exact gradient on the task loss, which is `G78`'s counter-example — and its own defect is `G78` relocated, since the maximum delay `d_max` bounds the representable lag exactly as `τ_a` bounds memory span, and nothing estimates it from data. The two variables are also complements rather than rivals: adaptation says how long a unit remembers its own firing, a delay says when another unit's spike arrives.
- **[[wiki/concepts/intrinsic-timescale-measurement.md]]** — the measurement `G78`'s per-neuron adaptation constants are usually justified by, and which does not actually license them: the cortical timescale hierarchy is fitted at the population level and never resolves single neurons, so it constrains an area's horizon without saying that any unit owns one ([[wiki/empirical-tensions.md]] T323).
