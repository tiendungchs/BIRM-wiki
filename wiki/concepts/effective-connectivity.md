...
- **[[wiki/concepts/network-control-theory.md]]** — what this page's estimate is *for* once it exists: controllability, driver sets and minimum control energy are all computed on a coupling matrix, and the review that states the control programme also states the matching warning — the `A` recovered by systems identification from regional time series need not be the anatomical connection matrix, which is this page's model-inversion argument reached from the intervention side (Bassett & Sporns 2017).
