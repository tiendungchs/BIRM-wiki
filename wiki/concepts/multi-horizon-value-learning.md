...
- **[[wiki/concepts/affective-opponency.md]]** — where this page's machinery absorbs a withdrawn biological claim: average-reward reinforcement learning antagonizes the phasic error with the long-run rate, which was the formal motivation for assigning that rate to a neuromodulator, and Boureau & Dayan concede the antagonism "could be realized in many other ways" — a bank of discount factors being one — so the retraction costs the chemistry and leaves the algebra intact (T134).
- **[[wiki/concepts/expected-value-of-control.md]]** — the term this page's machinery has no counterpart for: the control criterion recurses through future control decisions at a single `γ`, compressing a control-expensive state's price and a distant reward's payoff onto one horizon — so the mixture-of-horizons construction that recovers non-exponential discounting on the value side has nothing corresponding on the cost side.
- **[[wiki/concepts/conditioned-reinforcement.md]]** — the construction that could carry two irreconcilable kernels at once: `d(t) = ∫₀¹ w(γ)γᵗ dγ` recovers human hyperbolic discounting and pigeon reciprocal summation `Σ 1/t_i` as two settings of `w` over the same torso, which is the cheapest resolution of a disagreement that otherwise looks like a species difference. Nobody has fitted `w` to schedule-choice data.
