...
- **[[wiki/concepts/reference-frame-transformation.md]]** — the interface this code sits on the allocentric side of: each sublayer of the transform bank is a copy of the boundary/object-vector population tuned to one heading, so the identity-invariance that makes this a reusable code is also what forces a separate identity stream to be bound in alongside it.
- **[[wiki/entities/bb-model.md]]** — the model that predicted object-vector cells before they were measured, and the reason it predicted them: they are the cheapest way to code objects given that a boundary-vector transform bank already exists, so an object code was obtained by reusing an interface rather than by adding a mechanism.
