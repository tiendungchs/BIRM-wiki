...
- **Context length is the hard capacity bound on the fast level.** A Transformer compresses only a few kilobytes at a time; algorithmic reasoning and long-term memory need long contexts, which is exactly where these models are known not to generalise. An in-context inner loop cannot be the whole answer to fast **M** while its store is a fixed byte budget with quadratic cost ([[wiki/concepts/working-memory.md]], gap G14 — nothing consolidates what the context learned into the parameters).
