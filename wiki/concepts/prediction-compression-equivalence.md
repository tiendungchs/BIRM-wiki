...
- **The rate does not decompose.** No procedure attributes a model's saved bits to memorised marginals vs. in-context adaptation vs. recovered structure. Without that decomposition the instrument measures the sum of what the wiki wants to distinguish. **One split is available and it is exact**: name a relevance variable `Y` and the rate separates as `I(X;X̂) = I(X̂;Y) + I(X̂;X|Y)`, relevant bits plus nuisance bits ([[wiki/concepts/information-bottleneck.md]], Tishby & Zaslavsky 2015). It costs a `Y` this page's instrument deliberately does without, and it does not touch the memorised/in-context/structural split.
- **Adjusted rate has no fair split for a learner** — and this is the row `G35` is built on: parameters-in-part-one puts Chinchilla at 14008% against gzip's 32.3%, so no neural model in the wiki has ever paid for itself. Parameters-in-part-one is the strict reading and kills every neural model; script-in-part-one (prequential) requires training during decoding. Neither corresponds to the genome-bottleneck accounting the wiki actually cares about.
...
- **[[wiki/entities/arc-agi-3.md]]** — the equivalence used as an *authoring* gate rather than as a solver or an analysis: two environments count as insufficiently distinct if one program solves both while being ≥50% shorter than the two independent solution programs concatenated, which makes joint-versus-separate description length a computable novelty test run before a task is admitted to a benchmark.
- **[[wiki/concepts/nameability.md]]** — the same move one representational level up: a cheap, label-free, solver-independent index of how much structure a task holds, measured as the length of a natural-language description of its rule rather than as bits per position — and correlated at `r = −0.50` with human accuracy on ARC.
- **[[wiki/concepts/benchmark-contamination.md]]** — the equivalence used as a forensic instrument: `p(n-gram | prompt) = Π p_M(xᵢ | x_<ᵢ)` makes perplexity and exact-match `n`-gram recall two readings of one quantity, so a compression advantage on a specific text is evidence the text was in the corpus — and, conversely, low loss on a benchmark item is uninterpretable until that is ruled out.
- **[[wiki/concepts/spike-encoding-schemes.md]]** — the equivalence stated as an engineering objective: the GAGamma encoding scheme maximises information subject to minimum spike density given a prior on the signal, which turns the choice of neural code into a rate–distortion problem rather than a design preference (Auge et al. 2021).
- **[[wiki/concepts/broadcast-hierarchy.md]]** — the wiring that implements this identity in cortex: convergent input onto fewer, slower, broader-receptive-field units *is* the compressor, hierarchical level is a compression ratio rather than an index, and the source adds a constraint — the compression is deliberately lossy, targeting approximations sufficient for action rather than reconstructions.
- **[[wiki/entities/default-mode-network.md]]** — the anatomy that implements the equivalence at network scale: fewer, less dense, hypomyelinated units with convergent input and long integration windows recode high-dimensional input into the low-dimensional priors this network broadcasts — lossily, into approximations sufficient for action rather than reconstructions.
- **[[wiki/concepts/memorisation-vs-generalisation.md]]** — the incompressible limit of this equivalence, and its interpretability consequence: a macrofeature is exactly a description that shortens the code for many examples at once, so "no task-relevant macrofeatures" is "no compression available", and a network solving such a task is a lookup table whose shortest description is the data.
- **[[wiki/entities/mae.md]]** — the case where the identity holds cleanly and is still uninformative: the target is fixed, so the objective genuinely is a code length on held-out pixels (unlike a JEPA's moving target) — and across the mask-sampler ablation lower loss accompanies *worse* representations, so "how much structure was found" and "how well was it compressed" come apart within one method.
- **[[wiki/concepts/information-bottleneck.md]]** — the relevance-conditioned version of this page's instrument: the same rate splits exactly into `I(X̂;Y) + I(X̂;X|Y)`, so a model can be charged for its nuisance bits alone — and the finite-sample bound there runs the argument this page cannot, that compression is *necessary* for generalization because the sample complexity is charged against `K ≈ 2^{I(X̂;X)}` and not against `|X|`.
- **[[wiki/concepts/independent-causal-mechanisms.md]]** — the criterion this page's descent cannot express: ICM asks for a description length **additive over modules** (parts that do not help compress each other, `G26`), and nothing analogous to next-token loss computes that, which is the concrete missing measurement.
- **[[wiki/concepts/prediction-error-neurons.md]]** — the counting argument that breaks the equivalence in neural hardware: every bottom-up spike cancelled by a prediction is paid for by a top-down spike carrying it, so predicting does not by itself reduce transmitted spikes and compression must be argued from the code rather than inherited from the predictor.
- **[[wiki/concepts/predictive-coding-free-energy.md]]** — the earliest biological instance of this page's identity being adopted as a design objective rather than measured after the fact: Rao & Ballard 1999 read their cost `E` (squared residuals plus priors on activity and weights) explicitly as **minimum description length**, so cortical predictive coding was proposed from the start as a code-length minimiser, and the residual it descends is the number of bits the level above failed to save.
- **[[wiki/entities/visual-predictive-coder.md]]** — the same equivalence written as a marginalisation rather than a code length: the latent that minimises next-frame error is the sufficient statistic of the observation history, and in a spatially-embedded world that statistic is position — so the map is what compression of a trajectory *is*.
- **[[wiki/concepts/optimal-hierarchy-criterion.md]]** — the same two-part code applied to *behaviour* rather than to a symbol stream: the option library is part one, the residual root-level choices are part two, and the hierarchy minimising the total is provably the one that discovers new policies in the fewest trials — the identity used as a design criterion instead of as an instrument (Solway et al. 2014).
