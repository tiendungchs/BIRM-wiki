...
| Cost is general | dACC also tracks physical effort and revises expected reward downward for it | `Cost` is not specific to cognitive control — **contested** (`T397`): dopamine antagonism moves rats' physical-effort preference and leaves cognitive-effort preference intact, and the metabolic constraint that prices physical action has no cognitive counterpart, so co-activation may reflect a recruited system rather than a shared cost variable |
...
- **What is the form of `Cost`?** Asserted monotonic in intensity, never measured. Without it the theory predicts the *direction* of every effect and the magnitude of none. **An instrument now exists and a functional form still does not** ([[wiki/concepts/effort-based-decision-making.md]], Westbrook & Braver 2015): cognitive effort discounting titrates paired (load, reward) offers to indifference and returns `SV = indifference amount / offered amount` per demand level, with delay designed out (all loads equal duration) and risk designed out (pay not contingent on performance). What that buys is a measured, per-agent discount *curve* in reward units; what it does not buy is a fitted kernel, and the source declines to commit to hyperbolic, exponential or labour/leisure. **The labour/leisure alternative this page flags has since been tested and survives** — income-compensated wage changes shift mental-effort allocation exactly as a concave utility surface predicts (Kool & Botvinick 2014/2018), but the surface is the *net* utility, not `Cost`, and its central consequence is one this page's formulation cannot express: the marginal cost of effort is **baseline-dependent**, varying with how much effort is already invested and how much income already received, where `Cost(signal)` is a function of the signal alone. The cost term itself is still contested between convex and hyperbolic fits (`T398`), possibly unidentifiably so. It also bounds where `Cost` may be read: cost elicited by choice *before* engagement is unambiguously an input to the specification decision, where every during-task index (arousal, performance) is compatible with being an output of it.
- **What does estimating EVC itself cost?** The theory's own regress: a cost-benefit analysis of control is itself a controlled process. Untreated here, and answered two ways elsewhere — by refusing to run the computation (the cost as an evolutionary **heuristic**, Kool & Botvinick 2013), or by making the cost a gradient term paid during learning rather than an allocation decision made at choice ([[wiki/concepts/default-policy-regularization.md]]).
...
- **[[wiki/concepts/learning-progress.md]]** — a two-benefit utility with this page's subtrahend missing: `U = w_PC·PC + w_LP·LP` prices competence and its derivative and nothing else, so the uninstructed group's preference for the easiest activity is confounded with plain effort avoidance — which this page would score as a lower `Cost(signal)` rather than as a competence preference (Ten et al. 2021).
- **[[wiki/concepts/token-reinforcement.md]]** — a dataset where this page's amortised cost term is the wrong shape: with unit price (responses per reinforcer) held constant and the cost of reaching the redemption opportunity raised, response rate fell anyway (Bullock & Hackenberg 2006), so effort enters behaviour per redemption episode rather than per unit of expected return.
- **[[wiki/concepts/default-policy-regularization.md]]** — the direct disagreement about `Cost`'s argument: this page prices control **intensity** and declares identity free, where a divergence-from-default penalty prices the signal's **content**, so two equally intense control signals cost differently according to how far each departs from the agent's distilled habit. It also supplies a non-regressive answer to the "what does estimating EVC cost?" problem, and makes habitisation a consequence of cross-task compatibility rather than of practice.
- **[[wiki/concepts/effort-based-decision-making.md]]** — the measurement of this page's free function, and the challenge to its scope: cognitive effort discounting titrates paired (load, reward) offers to indifference and returns `Cost` as a per-agent curve in reward units with delay and risk designed out of the procedure, turning "asserted monotonic in intensity" into an elicited quantity that differs across agents *at matched performance*. Two things it takes back. The cost may not be one term (`T397`), and the dorsal anterior cingulate reading of it is weaker than this page's evidence table suggests — the correlation between dorsal anterior cingulate recruitment and self-reported desire to avoid a task **vanishes once errors and response times are partialled out**, leaving conflict monitoring as the sufficient account on that dataset.
- **[[wiki/concepts/learned-industriousness.md]]** — the write port this page's `Cost(signal)` does not have: the aversiveness of effort is a conditioned quantity, lowered by a history in which reinforcement was contingent on high performance, and the conditioning attaches to the response-produced *sensation* so the reduction transfers across tasks, dimensions and modalities. It also contradicts three of this page's structural commitments at once — the cost is vector-valued over performance dimensions rather than scalar in intensity, it is keyed to context cues (which agent is asking), and its transfer breadth is set by the breadth of a verbal category label (`G128`). Its cross-modal transfer result also bears on `T397` and points the opposite way from the dopamine dissociation.
