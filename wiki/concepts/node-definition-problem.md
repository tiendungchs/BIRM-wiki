...
- **The one criterion in the wiki that is neither spatial nor statistical: separate intervenability.** Schölkopf et al. 2021 define a unit as a constituent that "in principle permits separate interventions" — objects are the paradigm case, and object-centric representation learning is thereby a special case of causal factorization rather than a rival to it ([[wiki/concepts/independent-causal-mechanisms.md]]). Three things follow that this page's connectomics framing cannot state. (i) The vertex set is decided by the *intervention repertoire*, not by the observation: "which variables can be extracted and their granularity depends on which distribution shifts, explicit interventions, and other supervision signals are available". (ii) It is testable without labels — clamp and cluster the co-response ([[wiki/concepts/counterfactual-probing.md]]) — which is more than any parcellation criterion above offers. (iii) It is exactly what an undirected functional connectome *cannot* supply, since the criterion is defined by response to intervention and fMRI edges are undirected by instrument. **(brainstorm)** So the two literatures on this page disagree about what a node is for: connectomics picks units to make edge *estimates* stable, causal representation learning picks them to make edges *manipulable*, and only the second criterion survives being asked a counterfactual.
...
- **[[wiki/concepts/independent-causal-mechanisms.md]]** — supplies the wiki's only non-spatial, non-statistical criterion for the vertex set: a unit is whatever can be intervened on alone, which makes granularity a function of the available intervention repertoire and concedes this page's problem from the causality side ("no general conditions under which coarse-grained variables admit causal models with well-defined interventions").
- **[[wiki/entities/rims.md]]** — the vertex set declared as a hyperparameter (`k_T` modules) with the contents left entirely to training, and the one probe of what the vertices came to mean is a warning for this page: the module-to-object correspondence required an imposed spatial mask to be visible at all and dissolved as the model got better, so a learned vertex set can have no stable referent even when the units are enumerated by construction (Goyal et al. 2019).
