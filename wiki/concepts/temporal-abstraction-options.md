...
The paper's own conclusion is that this is HRL's central open problem, and it is the wiki's gap G33. Five families in the source, none settled, plus two added by later ingests:
...
| **Graph partitioning** | Build the state-transition graph explicitly, run a partitioning/clustering algorithm, take **access points between partitions** as subgoals | The state graph itself — i.e. this route consumes the output of [[wiki/concepts/latent-graph-discovery.md]] and returns a subgoal set. **Built, by George et al. 2021** — see below |
...
| **Program compression (added by Ellis et al. 2021)** | Solve the tasks, then compress the *solved plans* — refactor each into semantically equivalent forms and adopt the fragment that most shortens library + corpus. On 107 block-tower copy tasks this yields parametric options the paper names as such (arches, staircases, bridges) ([[wiki/entities/dreamcoder.md]]) | Nothing the other families need: no reward signal, no bottleneck statistic, no novelty term, no termination condition — only a corpus of solutions and a description-length objective, which is also its limit (options are compressions of what was already solved, so nothing is discovered *ahead* of competence) |

**The graph-partitioning family is now built end to end** ([[wiki/entities/cscg.md]], George et al. 2021). InfoMap run on a clone-structured HMM's learned transition matrix, in a 4×4 grid of aliased rooms joined by corridors, returns communities that respect room boundaries (a room may split into two or three communities; **no community straddles rooms**), and a second pass over the community graph recovers the four hyper-rooms — a three-level hierarchy from random-walk experience with no reward, no bottleneck statistic supplied and no level count specified in advance. Planning top-down through it gives paths **25% shorter** than flat Dijkstra on the same graph; surrogate communities of the same size that ignore room boundaries give paths **35% longer**, which isolates the benefit in the partition rather than in hierarchy per se.

Three things the built version settles that the family's statement left open:

| Question | Answer from the built version |
|---|---|
| Does partitioning work when the state graph is *estimated* rather than given? | Yes, and it is the only route that does — the partition is run on **latent** clone states. The control matters more than the result: an SR computed over the *observations* of the same maze yields no recoverable communities under multidimensional scaling or community detection, because aliased observations have no modularity to find. So this family needs a de-aliaser upstream, not merely a transition estimate |
| What supplies the level count? | Nothing in the model — the modeller re-runs the partitioner on its own output. Two passes give three levels; a third pass is neither triggered nor forbidden |
| Is re-partitioning scheduled? | No. The failure mode predicted below (a shortcut found after options were compiled is invisible until the partition is redone) is confirmed by omission: partitioning is a one-shot offline step in every experiment |
...
**The families are commensurable, and the yardstick exists** ([[wiki/concepts/optimal-hierarchy-criterion.md]], Solway et al. 2014). An option inventory is a *model* of the behaviours the agent will have to produce, scored by its Bayesian model evidence — provably the same quantity as the shortest code for those behaviours and the fewest trial-and-error attempts to discover a randomly drawn task or subtask. That makes "which family is right" a question with an answer for the first time, and it retrospectively licenses the graph-partitioning row: the maximum-evidence partition lands on topological bottlenecks, so community detection is an approximation to a provable optimum rather than an intuition. Two caveats it carries into this table — the criterion's data are the *already-solved* optimal policies, so it scores a decomposition rather than discovering one; and evaluating it is cheap while maximising it is a search over all partitions, run here as binary integer programming under a genetic algorithm with up to 1000 restarts.
...
- **[[wiki/concepts/incentive-salience.md]]** — a pursuit weight on an option that no framework here has: incentive salience is claimed to attach to *action* representations as well as to cues, so an option can be executed for its own sake at an intensity unrelated to its termination value — a persistence mechanism and a failure mode that an option defined by `(I, π, β)` alone cannot express.
...
- **[[wiki/entities/simple-cycle-reservoir.md]]** — the crudest temporal hierarchy with a universality guarantee attached: `λ` alone sets an exponential memory horizon, and a Multi-Cycle Reservoir with *different* `λ` per block read out by a linear mixture `h(Σ a_i x^(i))` is a bank of horizons — defined in that paper (Def. 7) and never exploited, since all its theorems use identical blocks.
- **[[wiki/entities/ltc.md]]** — the continuous extreme of the same question: an option is a temporally extended commitment with a discrete termination condition, while a liquid time constant is an integration window that varies smoothly with the input and never terminates — so the two bracket "over what interval does this computation hold", and the empirical split in LTC's own results (largest margin on irregular traffic signals, a loss to LSTM on sequential MNIST) says an adaptive window pays exactly when the informative timescale of the input changes and is inert otherwise.
- **[[wiki/entities/cfc.md]]** — a termination condition made smooth and time-typed: the CfC gate `σ(−f(x,I;θ_f)·t)` takes *elapsed time multiplied by a learned rate* as its argument and convexly interpolates between the `t → −∞` and `t → +∞` endpoints of the underlying flow, so "how long this computation holds" is a learned scalar against a real duration rather than a discrete decision — and feeding a raw timestamp in is what lets it handle irregular sampling where ODE-RNNs fail for every solver.
- **[[wiki/entities/s4.md]]** — a multiscale temporal decomposition that is discovered rather than imposed: first-layer kernels cover a few steps of local context while last-layer kernels span all 16,384 positions at varying frequencies, with no hierarchy in the architecture — and the timescale of the entire stack is rescalable *after* training by one scalar `Δ` (the implied continuous sampling resolution), which holds 96.30% of 98.32% raw-speech accuracy at half the input frequency with no gradient step (Gu et al. 2022).
- **[[wiki/entities/ms-ssm.md]]** — a timescale hierarchy with neither a termination condition nor a variable-length commitment, and the fully *parallel* alternative to temporal chunking: `S+2` dyadic bands (filter support `2^s·K`, eigenvalues stratified into disjoint intervals so coarse bands get effective memory `O(1/δ)` near the unit circle) all run at once and are re-weighted at every position by `E_t = Linear_E(x_t)` — where an option commits to one grain until a boundary fires, this holds every grain simultaneously, and it is what wins the wiki's one nested-bracket evaluation task (ListOps 63.04 vs S4's 59.60, Karami et al. 2025).
- **[[wiki/concepts/autonomous-pattern-generation.md]]** — a timescale set by what is *re-injected* rather than by how fast state decays: a bank of periodic drivers at different periods, broadcast with per-unit gains `μ_i`, gives a hierarchy of temporal grains that is a signal rather than a parameter — hence gateable and phase-shiftable at run time, unlike `λ`, `τ` or `Δ` — with no termination condition and no commitment (G67).
- **[[wiki/entities/arc-agi.md]]** — the wiki's only behavioural record of a bottleneck decomposition being used: human action traces on one ARC task converge on a few intermediate states, and those states are the task's *objects*, so the subgoal partition is by object rather than by graph connectivity — and choosing the objects is itself unsolved (G75).
- **[[wiki/entities/baba-is-ai.md]]** — a hand-written option set (`break{rule}`, `make{rule}`, `goto{object}`) whose preconditions are checked by the *solver* rather than by the environment, which is exactly where its errors enter: plans reference tiles and objects that are not in the scene, and assert blocked paths that are clear.
- **[[wiki/entities/fluxx.md]]** — option interruption with the option's *parameters* rather than its policy changed mid-flight: the turn is a macro-action (draw `n`, play `m`, discard to limits) whose counts can be rewritten during its own execution, reconciled monotonically as floors already satisfied rather than as debts.
- **[[wiki/entities/nomic.md]]** — the other branch of the option-interruption fork: its Rule 107 voids any rule-change claiming to apply before the moment it was adopted, so a partially executed macro-step is never re-parameterised at all — the re-entry problem is removed rather than reconciled monotonically as in [[wiki/entities/fluxx.md]].
- **[[wiki/concepts/replay-prioritisation.md]]** — the untested scale claim: the normative replay priority `Gain × Need` is asserted to carry over unchanged when the unit of backup is an option rather than a single transition, which would make offline replay a search over this page's objects instead of over states (Mattar & Daw 2018).
...
- **[[wiki/entities/cscg.md]]** — the graph-partitioning family's first working instance, and the demonstration that this family's real prerequisite is a *de-aliaser* and not just a transition estimate: community detection on a learned clone graph recovers rooms and hyper-rooms, while the same detection on an SR over the raw aliased observations recovers nothing (George et al. 2021).
- **[[wiki/concepts/intrinsic-motivation-typology.md]]** — an option-discovery scheme arriving from the motivation side: the competence-based family runs goal-reaching *episodes* on an abstract clock `t_g` (goal set → act → terminate on success or timeout), which is this page's option, and scores each one by achievement, competence progress or incompetence — so the option set is generated by what the agent is currently getting better at rather than by the environment's reward or its transition graph.
- **[[wiki/concepts/empowerment.md]]** — macro-actions generated from reachability rather than from reward or bottlenecks: *impoverished empowerment* keeps only the `n`-step action sequences contributing most channel capacity and re-expands from their endpoints, building long sequences from a skeleton, with a small amount of noise acting as the term that keeps the retained endpoints spread across the state space instead of clustered (Anthony et al. 2011, via Salge et al. 2013).
- **[[wiki/concepts/expected-value-of-control.md]]** — the rival account of the tissue the hierarchical-reinforcement-learning reading claims: where Holroyd & Yeung make dorsal anterior cingulate cortex an option selector for superordinate temporally extended actions, the expected-value-of-control model makes it a level-blind specifier of control identity and intensity (`T364`) — and it prices an option differently, as a control signal paid for continuously by its intensity rather than by termination-condition bookkeeping.
- **[[wiki/concepts/optimal-hierarchy-criterion.md]]** — the objective this page's seven discovery families were groping for: an option inventory is a *model*, scored by the Bayesian model evidence of the behaviours the agent must produce, which is provably the same inventory as the shortest code for those behaviours and the fewest trials to discover them — and the corner-option agent's slower-than-flat learning is that score read in trials rather than in bits (Solway et al. 2014).
