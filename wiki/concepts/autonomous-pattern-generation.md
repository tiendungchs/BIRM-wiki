...
| 2 | **Multi-pattern storage and switching.** No account of how one substrate holds many trajectories and selects between them at a commanded time — the [[wiki/concepts/temporal-abstraction-options.md]] termination question, unanswered here |
| 3 | **Conditional continuation.** Generating "the next phrase given the previous one" needs cross-cycle state; nothing in the contractive family provides it |
| 4 | **How many clocks, at what periods.** A bank of drivers at different frequencies is the crudest possible temporal hierarchy and nothing sets its size — [[wiki/architectural-gaps.md]] G79 |
...
- **[[wiki/concepts/temporal-abstraction-options.md]]** — a temporal hierarchy built from *drivers* rather than from decay constants or termination conditions: `k` clocks at `k` periods with per-unit gains `μ_i` is a bank of timescales that is a signal rather than a parameter, hence gateable at run time (G79).
