...
- **[[wiki/entities/affordance-active-inference-model.md]]** — open problems 1 and 3 answered by nesting rather than by a clock: a slow Lotka–Volterra generator sets the *rate* of a fast one, so the phase-breaking variable is the higher level's own state, and because the whole stack sits in a *generative model* the sequence is a belief that a precise enough prediction error can interrupt, not an output that cannot be stopped.
...
- **[[wiki/entities/spiking-neural-networks.md]]** — the same architecture arrived at by accident and not recognised: most "deep" spiking hierarchies are a fixed preprocessing stack, one plastic layer and a linear classifier, which is this page's reservoir-plus-readout shape reported as depth (Tavanaei et al. 2019). Also the substrate the liquid state machine was defined on, and the place where an adaptive-threshold module added to a reservoir (LSNN) closes the gap to LSTM on sequential MNIST and TIMIT.
- **[[wiki/concepts/spike-encoding-schemes.md]]** — the encoder-side echo of the reservoir result: random connections between Gaussian receptive fields and input neurons learned faster and to higher accuracy than structured one-field-per-neuron wiring (iris), so expansion recoding pays before the network as well as inside it.
- **[[wiki/concepts/circuit-size-separation.md]]** — the expressiveness result the liquid state machine was built on top of, and the gap it fills: Maass 1997 proves what small type-B spiking networks *can* compute while every construction is hand-built, so the reservoir-plus-readout move is the same author converting a hand-built existence proof into something trainable.
- **[[wiki/entities/liquid-state-machine.md]]** — the original of this page's architecture, run in the other direction: an input-driven spiking liquid whose transient state is read by memoryless readouts, where this page's liquid is clock-driven and emits. Its separation property is the formal version of this page's basis argument, and its theorem 1 converse states the shared ceiling exactly — an LSM approximates a filter *iff* that filter has fading memory, so period-locking here and the 750 ms horizon there are two faces of one class restriction.
- **[[wiki/entities/sigma-pi-reservoir.md]]** — the same reservoir-plus-ridge-readout skeleton pointed at prediction rather than generation, and the same class restriction: the buffer is a permutation ring and the added Pi-neuron stage enriches *what function of the recent window* is computed without extending how far back the window reaches, so higher-order features buy nothing against the fading-memory ceiling this page runs into.
- **[[wiki/concepts/output-null-subspace.md]]** — the third answer to open problem 2, and the cheapest: `k` outputs from one unchanged recurrent net and one unchanged read-out, selected by the **initial condition** the state is parked at rather than by `k` read-outs, a clock phase or an inserted filter — and because the generated object is a transient rather than an orbit, this page's period-locking boundary does not apply to it; the cost is that a starting point has no algebra, where a conceptor has `∧ ∨ ¬` `(tentative)`.
