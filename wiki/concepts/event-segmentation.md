...
| **Learned halting (compute allocation as segmentation)** | Attach a sigmoidal halting unit to a recurrent predictor and train it against `L + τ·(steps taken)`; a boundary is wherever the network buys extra computation. On raw character streams it pauses at spaces and pauses *longer* at commas and full stops ([[wiki/concepts/adaptive-computation-time.md]], Graves 2016) | The only detector here with **no hand-set threshold** — the criterion is trained by the downstream loss — and the only signal that stays flat on irreducible noise (random ID digits), where prediction error and predictive entropy both spike. Costs: a hand-set price `τ` on computation instead (`G107`), and the boundary is a by-product rather than a variable anything can be indexed by |
...
**One detector is not on the list because it is measured rather than proposed, and it segments *space*.** The entorhinal-hippocampal map of a hairpin maze breaks into a repeating per-arm submap that resets at each turning point, with the adjacent-position population correlation collapsing exactly at maximum curvature (`r = 0.44` vs a Monte-Carlo mean of `0.62`), and fields inside an arm anchored to the *preceding* turn (Derdikman et al. 2009, [[wiki/concepts/path-integration.md]], `T347`). Two things make it worth this page's attention. It carries the **negative control every row above lacks**: running the identical zig-zag trajectory in the same box with the internal walls removed produces no fragmentation at all, so the detector is not firing on trajectory statistics, on turning, on occlusion of distal cues or on the reward schedule — only on physical partition. And it is a boundary that *is* read out: unlike the dynamical-prior row, the reset is visible as a discontinuity in a downstream-readable code, so something can be indexed by it. What it does not supply is the detector's own mechanism — "geometry" is a description of the stimulus, not of the computation, and the paper's own candidate (border cells firing at the moment realignment should occur) is a suggestion with no test.
...
- **No learned boundary detector.** Between hand-set thresholds and backpropagation-through-time gates there is nothing local, learned, and derived from the objective. The threshold in question is the allocate-vs-reuse bias (`G38`): a boundary declares that the current situation is a new event rather than a continuation of a stored one, and no detector here computes it from a runtime signal.
...
- **A boundary says *when* to load a schema and never *which*.** Section 2 makes event schemata the wiki's missing edge type; nothing on this page retrieves one from a library, which is `G37` at the point where segmentation hands over.
...
- **[[wiki/entities/state-space-composition.md]]** — the consumer that presupposes this page's operation: a state space assembled from blocks needs its own extent decided before "which blocks apply" is answerable, and the model has no segmenter — while the tissue it models starts a fresh frame at every geometric partition.
- **[[wiki/concepts/path-integration.md]]** — segmentation observed doing work on a structural code: compartment boundaries re-zero the integrator, so each segment is dead-reckoned from its own origin and the environment is covered by a mosaic of frames rather than one chart — with the walls-removed control ruling out trajectory statistics as the trigger (Derdikman et al. 2009).
...
- **[[wiki/concepts/ignition.md]]** — a candidate boundary generator with a timestamp: if serial computation advances only when a global commit fires, then computational step boundaries and perceived event boundaries could be one signal read at two scales. That generator fires on **onset and not on offset** — a commit is a coalition winning the bus, and nothing wins when the incumbent merely fades — so it emits one boundary per stimulus where an input-change detector emits two ([[wiki/empirical-tensions.md]] T271, Naccache et al. 2025).
- **[[wiki/entities/cn-dpm.md]]** — segmentation with no segmenter and no boundary variable at all: novelty is decided per sample by a Chinese-restaurant likelihood ratio, unexplained samples accumulate in a buffer, and the only event that resembles a boundary is the buffer filling — so granularity is set jointly by `α` and the buffer capacity `M`, and the scheme still works when tasks cross-fade instead of switching (Fuzzy Split-MNIST, 93.22 against 93.23).
- **[[wiki/concepts/discrete-relaxation-gradients.md]]** — would replace this page's hand-set boundary thresholds with a learned one: a boundary indicator is a Bernoulli latent, and relaxing it lets the downstream task's loss train the detector directly, which is exactly the missing piece G27 charges the detector families with **(brainstorm)**.
- **[[wiki/concepts/adaptive-computation-time.md]]** — the first detector in this list whose threshold is *trained* rather than hand-set, and the only signal that separates structure from noise: a halting unit optimised for `L + τ·ponder` pauses at spaces, commas and full stops in raw character streams and stays flat on random ID digits, because compute is bought where the current input will move *future* predictions, not where the current prediction is hard — which is exactly what prediction error and predictive entropy cannot distinguish (Graves 2016).
- **[[wiki/concepts/schema-assimilation.md]]** *(also)* — the only operation in the wiki that *removes* stored structure: rapid-eye-movement sleep is proposed to disband existing schemas so new ones can be built, with the tonal/atonal melody dissociation as its evidence — the counterpart to slow-wave sleep's gist-extraction, and the piece a boundary-detection account of event models has no way to produce (Gilboa & Marlatte 2017).
- **[[wiki/entities/posterior-cingulate-cortex.md]]** — the same boundary signal read out as a policy variable: dorsal posterior cingulate activity correlates with change points in context and predicts switch decisions in bandit and foraging tasks, which is this page's segmenter used to decide *whether to abandon the current option* rather than to index a memory.
