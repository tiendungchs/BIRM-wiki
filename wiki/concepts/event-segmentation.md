...
- **[[wiki/concepts/ignition.md]]** — a candidate boundary generator with a timestamp: if serial computation advances only when a global commit fires, then computational step boundaries and perceived event boundaries could be one signal read at two scales.
