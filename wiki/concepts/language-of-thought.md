...
- **[[wiki/entities/dreamcoder.md]]** (Ellis et al. 2021) abstracts subcomponents shared across previously solved tasks into new library entries — heuristic hierarchical Bayesian inference of *the prior itself*, and the only concrete proposal the wiki holds for gap G26 (a *learned* prior over programs). Its claim that compression over solved tasks yields good primitives conflicts head-on with the wiki's only measurement of a length prior's effect on primitiveness ([[wiki/empirical-tensions.md]] T155). **The change-of-basis is now measured, not asserted:** from `map`/`fold` plus arithmetic and 60 numerical physical laws, it builds inner products, vector sums and norms, then an inverse-square-law schema that covers Newton's law and Coulomb's — 93% of the corpus after 8 cycles, in a language whose expressivity never changed. It also supplies the *machine* form of this page's installed/acquired dispute, and answers it in the direction neither side expected: learning from a minimal basis is possible (it reinvents `fold`, then `unfold`, then everything else from a 1959 Lisp subset plus the Y-combinator) and costs about a CPU-year, which the authors read as an argument for installing *richer* generic resources rather than poorer ones.
...
- **[[wiki/concepts/causal-model-building.md]]** — the consumer of this page's operational definition of a counterfactual (condition the trace → intervene → re-execute), which is what forces a causal model to be a *sampler with an inspectable trace* rather than a distribution over outcomes.
- **[[wiki/concepts/simulation-based-planning.md]]** — the same program queried three ways: a rollout is a forward sample, an inference about the past is a conditioned one, and a counterfactual is a re-execution of a modified trace, which is what makes recursive nesting of inverse planning well-defined rather than merely suggestive.
- **[[wiki/entities/mlc.md]]** — the systematicity half of the Fodor & Pylyshyn challenge met by a standard neural network (100% exact match on a compositional instruction task the same architecture scores 0% on without meta-training) and the productivity half conceded (100% error on length-extrapolation splits) — plus the finding the LoT position does not predict: a perfectly systematic gold-grammar oracle is a *worse* model of human responses than a neural one that also reproduces human biases.
- **[[wiki/entities/conceptarc.md]]** — a price on the format claim, via LARC (Acquaviva et al. 2022, second-hand): a human's natural-language rendering of an ARC concept, given without the demonstrations, lets a *second human* solve the task ~88% of the time and the best program-synthesis receiver ~12%, so the deficit is in the receiver rather than in what language can carry.
- **[[wiki/concepts/discrete-infinity.md]]** — a third position in this page's innate/acquired dispute (T8): the composer is innate but is *one* type-blind operator, domain-general and plausibly not selected for language, which neither of T8's existing positions allows — and it dissolves the equivocation problem without requiring a shared symbolic format.
- **[[wiki/concepts/relational-reinterpretation.md]]** — supplies the criterion T8 is declared undecidable for want of: four separately-testable properties (concatenative composition, explicit type/token, tokened structural relations, classical inferential systematicity) that separate productive composition from associative combination behaviourally — plus a graded reading of the format claim itself, since on this account honeybees have a proto-language of thought and "every species gets the syntax it deserves".
- **[[wiki/concepts/shared-intentionality.md]]** — makes T8's installed-vs-acquired dilemma empirically addressable for one construction: `attitude(agent, p)` is exactly the embedding a language of thought supplies for free, and the developmental record has it *acquired from the ambient language* and dated — frozen formulae (`I think`, `I bet`) at ~2;0, productive with third-person subjects, modals and tenses by ~3;6 — with the formal use preceding the concept.
- **[[wiki/entities/dreamcoder.md]]** — the running machine instance of "the effective language of thought changes with learning": expressivity fixed, the set of *short* thoughts rewritten every cycle by compressing refactorings of the solved corpus, with the physics run as a worked change of basis.
- **[[wiki/entities/scan.md]]** — the empirical test of Fodor & Pylyshyn's systematicity challenge on modern sequence models, and the result that complicates its dichotomy: withholding a verb's composed uses drops accuracy to 1.2%, but titrating 8/16/32 composed examples back in yields 38.3/77.8/88.4%, so the network is an example-hungry generaliser rather than a system without recombinable expressions (Lake & Baroni 2018).
- **[[wiki/concepts/tensor-product-representation.md]]** — the original technical reply to Fodor & Pylyshyn, written as the paper's stated motivation: recursively structured expressions with constituent structure and a systematic syntax, realised as vector algebra where `car`/`cdr`/`cons` are linear maps — offered explicitly as *not* an implementation of serial symbol manipulation, since the processing of the resulting pattern is meant to be parallel soft-constraint satisfaction over its micro-features.
- **[[wiki/concepts/reasoning.md]]** — filed there as definition R4: the same operation with the answer taken as a posterior rather than a point, which makes it the only notation that prices uncertainty over outputs instead of over candidates.
