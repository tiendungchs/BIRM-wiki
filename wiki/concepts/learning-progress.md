...
- **The supply is exhausted by the learning it drives** (`G129`). LP falls to zero on a mastered activity and on an unlearnable one alike, so the drive it models has no term that *grows* with what the learner already holds — the difficulty ladder is a fixed four-rung set and nothing in the estimator reads the structure of the agent's own store to generate a new rung. This page is the row's cleanest instance precisely because its decay is principled rather than incidental.
