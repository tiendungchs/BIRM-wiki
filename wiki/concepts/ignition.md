...
This is the biology `G91` is measured against, and the distance is the whole gap: the commit here is discrete, dateable and exclusive, while every broadcast in a wiki architecture is a continuous weighted mixture with no threshold and no gate that learned what deserves to cross it.
...
- **[[wiki/concepts/apical-amplification.md]]** — the same commit logic one scale down: a voltage-dependent conductance makes feedback able to amplify only what is already active, here Ca²⁺ inside one cell rather than NMDA across a network, with a burst as the all-or-none report; and the ~0.5 s dendritic disinhibition a cell needs before it can associate is a cellular candidate for the several-hundred-millisecond build-up this page measures (Larkum 2013).
- **[[wiki/entities/dendritic-integration-theory.md]]** — the cellular precondition this page's threshold does not include: attentional amplification and global broadcast are "impotent if the pyramidal cells are decoupled", so a network-level commit presupposes a per-cell switch with its own controller, and the two accounts locate the all-or-none character in different places — a population cascade here, a regenerative Ca²⁺ event inside one cell there.
