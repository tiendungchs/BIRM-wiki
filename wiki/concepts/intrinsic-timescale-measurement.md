...
| **Every value is a direct-fit point estimate** | The fit is the construction [[wiki/concepts/timescale-estimator-bias.md]] audits: negatively biased, bias growing with `τ` and with trial shortness, not subtractable, and trial-duration-dependent across the six pooled datasets. The bias has never been priced for these recordings, so the spread, the ratios and the `τ`–`B` correlation rest on an unvalidated magnitude (`G114`). Also untested: whether two timescales beat one here — on macaque V4 a Bayes factor chooses **two** |
...
- **[[wiki/concepts/timescale-estimator-bias.md]]** — the audit of this page's estimator: a direct exponential fit to a *sample* autocorrelation is negatively biased by finite sample size, by an amount that grows with the true `τ` and with the shortness of the trial, so the `50–350 ms` values here are systematically short, the `7×` spread is a lower bound, and numerical comparison across the six datasets' different foreperiod lengths is unlicensed — while the rank orderings survive, being monotone in `τ` (`G114`).
- **[[wiki/concepts/stationary-surrogate-null.md]]** — the same finite-sample bias on the other statistic: an autocorrelated signal's naive estimator misreports `τ` there and a covariance's variability here, and both are corrected the same way — by comparing against a generative process whose true answer is fixed by construction (Laumann et al. 2017).
