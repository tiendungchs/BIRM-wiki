...
**The band is a default state, and the antagonism is slow** (Lundqvist et al. 2016, `raw/lundqvist-2016-gamma-beta-bursts-underlie-working-memory.md`, the primary recording behind the 2018 result above). Beta is what the tissue does when nothing is being encoded or read: 61% of 321 prefrontal sites stayed in beta for the entire trial and their spiking carried **no** stimulus information; at the 39% that did carry it, beta was suppressed at stimulus onset and again before the read. Three properties a machine gate should copy or reject deliberately:

| Property | Measurement | Design consequence |
|---|---|---|
| Beta bursts are **not** an inhibition of spiking *per se* | Spike rate inside beta bursts is indistinguishable from outside (`p = 0.98`), while gamma bursts raise it (`p < 10⁻¹²`) | The channel suppresses the *coding* regime, not activity — a gate on which dynamical state the population is in, not a gain on its output |
| The antagonism is a **regime** statistic, not a cycle-by-cycle one | 112/126 gamma-modulated sites show a significant negative beta–gamma correlation across the trial (`r = −0.41`), but no beta sub-band's power predicts any gamma sub-band's power within the same 50 ms bin on the same trial | Two slow occupancy processes, not one mutually-inhibiting pair at the timescale of a burst. A machine gate implemented as an instantaneous multiplicative mask would be the wrong model of it |
| Beta is the **resting** condition of the channel, not a signal that must be sent | It is present throughout the trial and is *interrupted*, briefly, by encoding and decoding | The default is "do not express" and the controller emits *permissions*, which inverts every gate in the wiki — all of which default open and emit suppressions |

Compare [[wiki/concepts/cortical-state-bistability.md]]: the same hold/release shape in the same lobe, with the release band an order of magnitude slower (1–9 Hz) and the *held* content beta-coherent rather than beta-silenced. Both cannot be one mechanism as stated; what they share is a content-free occupancy variable that decides expression without carrying anything.

### The same trick addressed by place instead of by time

Beta selects *when* an item may be expressed at the sites already holding it. Alpha (8–13 Hz, thalamic and layer-V generators) selects *where* processing is forbidden, and the two dissociate on their driving variable (Roux & Uhlhaas 2014, `raw/roux-2014-wm-oscillations-alpha-gamma-theta-gamma.md`).

| | Prefrontal beta (20–35 Hz) | Posterior/premotor alpha (8–13 Hz) |
|---|---|---|
| Addressed by | the site holding the content | the region that must not process |
| Scales with | whether the held item is still needed | the number of **distractors**, not of targets |
| Lateralisation | — | to the task-**irrelevant** hemifield (Sauseng et al.) |
| Behavioural sign | falls before a read | 10–14 Hz delay power correlates *positively* with reaction time |
| Causal test | not run | 10 Hz repetitive transcranial magnetic stimulation over the task-irrelevant hemisphere **raises** working-memory capacity (Sauseng et al.) |

**Why the causal row matters beyond the band.** It is the wiki's one demonstration that *adding* inhibition to a population holding nothing improves a downstream capacity — an inhibitory channel paying for itself in span rather than in selectivity, which none of the four families above is tested for. **(brainstorm)** The machine form is a suppression mask over *inputs the controller has ruled out*, driven by the count of distractors rather than by the contents of the store: a second, disjoint budget alongside the store's own ([[wiki/concepts/working-memory.md]]).

**The unresolved half.** The same literature reads alpha delay activity as maintenance of content rather than as its suppression, with load effects reported in both directions; the inhibition reading rests mainly on source localisations placing the alpha *outside* the task's own network. The distinction is measurable and rarely measured — a suppressor should track distractors, a store should track targets.
...
- **The one inhibitory channel with a named *learning* job is not resolved to a family.** The replay filter needs a population that is cue-responsive during exploration but keeps *stable* ripple recruitment, and its authors nominate cholecystokinin-expressing cells — i.e. this page's *Id2*-*Sncg* subfamily, which is measured here as suppressed or weakly entrained during ripples (Liao et al. 2024, [[wiki/entities/inhibitory-replay-filter.md]]). Weak entrainment is compatible with "stable" but is not the same claim, and the two measurements are in different subfields (CA3/CA2 versus CA1), so the assignment is a hypothesis this page's fingerprinting method could settle and has not been asked to.
- **The erase channel described here is addressed, and no machine store has one** (`G49`): beta bursts drop information at the sites holding an item that is no longer needed, which is erase-by-relevance rather than by age or by eviction order, and it is delivered by the controller rather than by a fixed policy.
...
- **[[wiki/entities/inhibitory-replay-filter.md]]** — a fifth job for an inhibitory channel that this page's four do not cover: not shaping a feature of the online code but *carrying a learned memory of which representations were unpredictable*, written by symmetric spike-timing LTP at `I→E` synapses and read out during ripples. It also inherits this page's open problem — it needs a cue-responsive, ripple-stable population and treats inhibition as one pool, which the four-family fingerprint could resolve.
...
- **[[wiki/concepts/vectorized-instructive-signals.md]]** — the same NDNF⁺ neurogliaform family holding a second write-gate, in cortex rather than CA1: driving layer-1 NDNF⁺ cells suppresses the apical-tuft magnitude residual that carries a layer-5 pyramidal neuron's own teaching signal, abolishing reward and error information and blocking learning — so "which inhibitory family decides what gets written" has two measured instances with one cell class (Francioni et al. 2026).
...
- **[[wiki/concepts/working-memory.md]]** — where this page's control channels acquire a schedule: prefrontal beta bursts suppress gamma and informative spiking at exactly the sites that carry an item, fall before that item is read and rise once it is no longer needed, making inhibition the timer of expression rather than a gain on the code (Lundqvist et al. 2018) — and where the alpha channel's payoff is priced: a suppressor driven by the distractor count rather than the target count, whose stimulation *raises* the store's span, which makes protection a second capacity budget alongside storage (Roux & Uhlhaas 2014).
...
- **[[wiki/concepts/random-feedback-addressing.md]]** — suppression obtained without an addressed suppressive signal: top-down drive is purely excitatory and feature-addressed, and the sensory ring's own long-range inhibition converts it into a rightward shift of the *unattended* stimulus' response function — so enhance-the-relevant and suppress-the-rest are one control signal plus one local circuit property, and the model correspondingly has no way to express "stop attending to X" (Park & Serences 2025).
- **[[wiki/concepts/inter-areal-synchrony.md]]** — the page that licenses this one's band claims, and the reason an inhibitory rhythm can be read as an address at all: a band identifies whether a correlation is direct drive, a relayed clock, or a content format, so "alpha over the irrelevant hemifield" is a claim about a channel type and not only about power.
- **[[wiki/concepts/cortical-state-bistability.md]]** — the same lobe's hold/release variable with the roles reassigned: beta is the default regime in both accounts, but there the incumbent content is beta-*coherent* and released by a content-free 1–9 Hz burst, while here beta is what silences coding and the interruption is an item-specific gamma burst — so whether the release signal carries content is an open fork, and only the content-free version needs a separate mechanism to decide what takes over.
- **[[wiki/entities/entorhinal-cortex.md]]** — the same interneuron families doing a second, different job: in medial entorhinal layer II the family choice is by *target principal-cell class* rather than by coding feature — parvalbumin⁺ closes the reelin⁺ (→ dentate/CA3) network, 5HT3a/cholecystokinin closes the calbindin⁺ (→ CA1) network, and the two never contact each other — so the inhibitory pool is what keeps two parallel output channels isolated, which a single global normalisation layer cannot express (Witter et al. 2017).
- **[[wiki/concepts/prediction-error-neurons.md]]** — where an interneuron class becomes the computation rather than a modulator of it: the descending prediction is glutamatergic and its subtraction is performed by a local learned inhibitory circuit, so *which* interneuron subset receives the top-down projection decides whether a cell computes a positive or a negative prediction error.
