...
Ranking by SLN% yields a **better-constrained** hierarchy, and it disagrees with the binary one: frontal eye field drops from level 8 to level 4, alongside V3/V3A.

**How much better-constrained, measured** (Markov et al. 2014, `raw/markov-2014-anatomy-of-cortical-hierarchy.md`; 26 macaques, 339 pathways, 11 injected areas — full treatment on [[wiki/concepts/counterstream-organization.md]]). Fitting hierarchical levels by maximum likelihood from `μ = Φ(Xβ)` over the incidence matrix of the injected graph, with a **beta-binomial** likelihood because neuron counts are overdispersed:

| Claim | Status after the quantitative fit |
|---|---|
| SLN% yields *one* hierarchy | **No.** 95% confidence intervals on the fitted levels are **±1–2 levels**, and the solution is identified only up to an affine transform (V1 pinned at level 1 by fiat) |
| Levels are discrete | **No** — the SLN distribution above 0.1 is approximately uniform with no clustering: positive evidence for a continuum |
| The frontal eye field drops | **Partly** — it *splits*: 8L falls to the level of **V4** (and receives direct V1 and V2 input), while 8m stays high and clusters with the dorsal stream |
| SLN travels as a ruler between injection sites | **Only in the ventral stream.** Probit-SLN correlations across injection pairs: mean **0.73** for V1/V2/V4/TEO/TEpd/MT/DP, ~0 for 8m, STPc, 7A; adjacent 8L and 8m correlate at ~0. Prefrontal areas had to be excluded from the distance fit. Recorded as [[wiki/empirical-tensions.md]] `T378` |
| SLN is a primitive quantity | **No** — it is generated by two distance constants. Supragranular cells project further in the feedforward direction, infragranular cells further in the feedback direction (`P < 0.01` and `P < 0.001` respectively), so the laminar ratio is their ratio at a given separation and hierarchical distance *is* physical distance seen through two kernels |

**And the same paper removes the anatomical argument that licenses the feedforward/feedback asymmetry.** Dual side-by-side injections show that projection-zone extent, overlap and double-labelling are 2–12× larger for infragranular than for supragranular parent cells — *within the same pathway*, and supragranular **feedback** is as point-to-point as supragranular feedforward. Topographic precision indexes the **compartment**, not the direction; feedback reads as diffuse only because infragranular cells dominate it. The clean FF-topographic / FB-diffuse dichotomy survives only for the long-range pathways, which originate exclusively from layer 3B and layer 6 — and 75% of corticocortical neurons are short-to-medium range. See [[wiki/concepts/counterstream-organization.md]].
...
**The laminar half of that table now has an intracellular test, and it comes out in favour of a real L2/3-vs-L5/6 division of labour.** Whole-cell recording in mouse V1 under closed-loop visuomotor coupling (Jordan & Keller 2020, [[wiki/concepts/prediction-error-neurons.md]]) finds that superficial cells integrate visual-flow speed and locomotion speed with **opposing** signs (17/22 cells in an opposing-sign quadrant; corr-vs-corr R = −0.65), while cells at 480–750 µm integrate both **positively** (2/12). The dissociation is not one of input strength: visual-flow responses (2.0 ± 1.9 vs 1.3 ± 2.8 mV) and locomotion-onset depolarizations (2.8 ± 2.2 vs 3.5 ± 2.2 mV) are statistically indistinguishable between the tiers. What is confined to L2/3 is the *opposition* — i.e. the subtraction — so on this measurement the deep tier holds a positively weighted sum of self-motion and sensory flow, which is what both columns of the table above want it to hold (a prediction under Bastos, an internal representation under Keller) and is not what either assigns to L2/3. One caveat the authors state: only 2 of 14 deep recordings were below 600 µm, so this is a claim about L5 with L6 barely sampled, which is exactly the depth-vs-class confound `T371` names.
...
- **[[wiki/concepts/counterstream-organization.md]]** — the quantitative retrograde-tracing audit of this page's SLN% rule and of its feedforward/feedback typing: the rule gives an optimized hierarchy with ±1–2 level confidence intervals over the ventral stream only, SLN itself is generated by two direction-specific distance constants rather than being primitive, and topographic precision turns out to index the supragranular/infragranular compartment rather than the direction of the pathway.
...
- **[[wiki/concepts/prediction-error-neurons.md]]** — the rival cell-class assignment on the same lamination, built from mouse closed-loop physiology instead of from the equations: it splits this page's single L2/3 error population in two by sign and demotes L2/3 interneurons from *encoding a variable* to *relaying the drive a comparator subtracts*, so the two schemes predict opposite effects of silencing the same cells — and the one laminar claim the two schemes share is now measured intracellularly: the *opposition* between top-down and bottom-up drive, i.e. the subtraction itself, is present in L2/3 and absent in L5/6 even though both tiers receive the two inputs at comparable strength (Jordan & Keller 2020).
...
- **[[wiki/entities/dorsal-visual-stream.md]]** — the substrate of a proposal that makes reportability a property of the *receiver*: the two dorsal pathways supporting conscious perception target granular cortex with a layer IV, the pathway supporting non-conscious visually guided action targets agranular cortex without one, so a stream may become reportable exactly when it is re-sorted by an input stage before use — untested, and offered by its authors as a coincidence worth pursuing (Kravitz et al. 2011).
