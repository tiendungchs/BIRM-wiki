...
| **Split towers + learned head** ([[wiki/entities/learningmatch.md]]) | `MLP([f(q), f(d), (q−d)², (q+d)/2])` — encoders factorised, *score* not | not run on LIMIT | The encodings still precompute; the head runs `O(n)` times per query but is a small MLP (20 µs), not a full model call |

**Two more exits, from the symbolic side of the same problem** ([[wiki/entities/macfac.md]], Forbus, Gentner & Law 1995). There the index is a count vector over the predicate vocabulary, and the scaling worry is the mirror image of this page's: not too few dimensions to separate the sets, but a vocabulary of 10⁵–10⁶ predicates and a substrate with limited bandwidth. The two proposed remedies are neither of the moves in the table above:

| Route | Construction | Property |
|---|---|---|
| **Semantic compression** | predicates form a specialisation lattice; pick any partition of it; each component counts its predicate *and everything beneath it in the partition* | dimension reduction by **quotient of the representation language** — items differing only below the cut become indistinguishable at stage 1, and the cut is an explicit abstraction level rather than an implicit information loss |
| **Factorisation** | partition the vocabulary into tightly interrelated subsets, one vector each, scores accumulated across modules | caps any single module's width; requires cross-module synchronisation |

Neither is run. What they contribute here is that the reduction is *chosen and interpretable*: a random or learned projection to `d` dimensions gives no account of which distinctions it discarded, and a lattice partition names them exactly. **(brainstorm)** Running several partitions of the same lattice in parallel is a coarse-to-fine index whose levels are all cheap, which is the retrieval analogue of the multi-resolution allocate-vs-reuse proposal in [[wiki/architectural-gaps.md]] G38.

**The fifth row is the middle of this table and it is unpriced.** The rank-`d` bound is a statement about the *score*, not about the encoders: it applies the moment the compatibility is a bilinear form in two independent encodings, and it stops applying as soon as anything non-bilinear sits on top. Nothing forces the un-factorised part to be the expensive part. Green et al. 2025 keep shared-weight encoders — cacheable per item — and replace the dot product with a 5-layer head, and their depth ablations say **that head is where the accuracy lives**: sweeping encoder depth "did not have a significant impact", sweeping head depth did, up to a knee at 5 layers. Two things follow for a memory architecture:

- It is a **cheaper exit from the bound than a cross-encoder**, on the axis that matters: the per-candidate cost drops from a forward pass over the raw pair to a forward pass over two cached vectors. Multi-vector late interaction (row 2) is the same move with the head constrained to MaxSim; this is the unconstrained version, and it has no theory either.
- The head there needs the pair's **raw mean** as an extra input, not just its difference — because the target similarity is *position-dependent over the manifold*. Every fixed score form in this wiki (inner product, `ℓ₂`, cosine, order-embedding hinge) is translation-invariant in the embedding, so none of them can express a similarity whose scale varies with *where* the pair sits. That is a different expressivity limit from the rank bound, it is not fixed by raising `d`, and it is what [[wiki/empirical-tensions.md]] T173 is about.
...
- **[[wiki/entities/dinov2.md]]** — the training-side term that buys the quality this page bounds, with its price and its scope measured: a Kozachenko–Leonenko nearest-neighbour entropy penalty at weight 0.1 is worth **+8.3 mAP** on instance retrieval, ~0 on segmentation and 0.5 on an ImageNet linear probe — so spreading an embedding is specifically a purchase of inner-product-ranking quality, and the objective term that buys it is invisible to the benchmark encoders are ranked on.
...
- **[[wiki/entities/learningmatch.md]]** — the escape route between the two ends of this page's table: keep the factorised encoders (so the index still precomputes) and make only the *score* un-factorised, with ablations saying the learned head is where the accuracy lives and the encoders are nearly free.
- **[[wiki/concepts/representational-collapse.md]]** — the read-out that entropy-shaped anti-collapse terms actually buy (+8.3 mAP from KoLeo, and nothing on segmentation), plus a structural ceiling on the joint-embedding row that is not about collapse at all.
- **[[wiki/entities/simple-cycle-reservoir.md]]** — the same currency in the temporal domain: both convert a structural demand into a required dimension (here `n' ≈ (N+1)·n·k`, `k > nm`, as the price of constraining the recurrent coupling to a ring and the input weights to signs), and both find dimension monotonically good — jointly the counterweight to T75's tipping-point result.
- **[[wiki/entities/transformer.md]]** — the earliest measurement of this page's bound, in the architecture that made the read ubiquitous: with `d_v` and total compute held fixed, narrowing the *scoring* dimension alone costs 0.7 BLEU (`d_k` 64 → 32 → 16 gives 25.8 → 25.4 → 25.1), and the authors conclude that "a more sophisticated compatibility function than dot product may be beneficial" — i.e. the factorised bilinear form this page's theorem prices was already suspected from ablation in 2017, and nothing replaced it (Vaswani et al. 2017).
- **[[wiki/concepts/vector-symbolic-binding.md]]** — the escape route absent from this page's table: leave the score bilinear and re-encode the *items* until the structural property is a surface feature, which raises what a fixed-`d` dot product can express and pays in dot-product variance rather than in dimension (99/100 correct orderings at `n`=4096 against 94/100 at 2048); the probe-side re-weighting also changes what similarity means without touching the index (T173).
- **[[wiki/concepts/analogical-mapping.md]]** — the same escape route applied to correspondence rather than ranking: the comparator stays a normalised binary dot product and the code is augmented with the roles an element fills, after which `argmax` over that score reproduces mappings that `O(n⁴)` constraint-satisfaction networks were built to compute.
- **[[wiki/entities/macfac.md]]** — the extreme of the compile-into-a-fixed-vector trade: the stored code keeps only functor *counts*, so every structural distinction collapses at the comparator, and the second stage exists purely to restore what the compression threw away — which prices the rank-`d` ceiling as a two-stage design decision rather than a defect. Approached from the other side, its index dimension is the size of the predicate vocabulary, and its two proposed reductions — quotient by a specialisation lattice, factorisation into per-subset vectors — are exits from the width problem that *name* the distinctions they discard, which no projection in this page's table does.
- **[[wiki/concepts/memory-read-and-erase.md]]** — the rival locus for the same ceiling: if the bound is in the selection rather than in the carrier, enlarging a store buys nothing, which is the assumption every key-value design makes and none tests.
- **[[wiki/entities/mlc.md]]** — a third point on the encoding/scoring axis: each stored example is encoded *in the context of the query* (query copied and concatenated to each of `m` study examples, one shared encoder, an index embedding per origin) but independently of every other example, which costs `m·O((q+s)²)` instead of `O(S²)` and structurally forbids any comparison between two stored items before the decoder's cross-attention sees them.
- **[[wiki/entities/spiking-hippocampal-cam.md]]** — a store that sits outside this page's bound by refusing to rank: its `content → {cues}` read is a Boolean OR over stored content rows, so the addressable answer sets are exactly the unions of those rows — no margin, no dimension spent, no `(1+1/γ)^d` ceiling. The price is precisely what the margin was buying, since "best match" is inexpressible; jointly the two pages name an axis (rank-free retrieval trades selectivity for unbounded addressability) that neither had alone.
- **[[wiki/entities/sigma-pi-reservoir.md]]** — the wiki's other dimensioning bound, over a different quantity: `D = O((pR/ε)²)` bounds how accurately a fixed-width superposition's inner product reproduces a polynomial kernel, where this page bounds `d` by which *subsets* a similarity store can return — and the gap between them is the decode step, since that scheme never unbinds anything, which is why it runs at `D = 28` where a decodable HRR needs thousands.
- **[[wiki/concepts/memorisation-vs-generalisation.md]]** — the same bound argued combinatorially rather than by rank, and the mechanism proposed to escape it: a basis-aligned lookup unit whose input weights pool the entities of one class must fire on recombinations of their parts (fire on Michael Jordan and Tim Duncan ⇒ fire on Tim Jordan or Michael Duncan), and hashing the entity representation before the lookup is what removes the constraint.
- **[[wiki/concepts/multi-token-embedding.md]]** — a superposition with no capacity statement at all: every attribute of every known entity is claimed to be a linear subspace of one residual stream, packed cheaply because facts about different entities are never co-active on the same token, and nobody has asked how many attributes fit before the projections stop separating.
- **[[wiki/entities/neural-episodic-control.md]]** — this bound applied to a *policy* rather than to a corpus: the agent's action-values are a kernel-weighted top-50 read over a learned embedding, so how many distinct value estimates it can express is limited by how many retrieval sets the embedding dimension can address — a constraint no episodic-control paper prices, and one that tightens as the store grows toward its `5×10⁵` entries per action (Pritzel et al. 2017).
- **[[wiki/concepts/fast-weight-programming.md]]** — the same dimension bounding the other quantity: where this page caps how many *retrieval sets* a `d`-dimensional similarity read can address, an outer-product store caps how many *items* it can hold at `d_dot`, since retrieval `Wφ(q)` returns a mixture over every non-orthogonal key — and the measured breakdown of linear attention sits exactly at that bound (Schlag et al. 2021). Both are pre-training geometry; a builder is bound by the smaller.
- **[[wiki/concepts/tensor-product-representation.md]]** — a limit of a different type from this page's rank-`d` bound: the annihilator condition makes certain *sets of structures* linearly dependent at every dimension and for every choice of encoding vectors, so it is neither statistical nor payable in width, and it fires exactly on fully crossed stimulus grids of the kind the wiki's relational benchmarks are built from (`G106`). It also supplies the capacity axis this page collapses — how many bindings can be *generated* per step is hardware and sharp, how many *maintained* is `~√N` and graded.
- **[[wiki/concepts/projection-distortion.md]]** — the other bound on the same bottleneck, in a different currency: sphere packing limits how many distinct retrieval sets a `d`-dimensional store can be asked for, Johnson–Lindenstrauss limits how faithfully pairwise distances survive the reduction (≥ 1,842 dimensions for 20% fidelity at 10,000 points), and any low-dimensional code whose downstream reads are metric must clear both.
- **[[wiki/entities/dense-associative-memory.md]]** — the item-count bound this page is the complement of: sharpening the energy exponent takes storage to `α_n N^{n−1}` patterns, but says nothing about how many distinct *retrieval sets* the store can be addressed for, which stays capped by the embedding dimension and the score margin. A store can be exponentially deep and still unaskable.
- **[[wiki/entities/continuous-modern-hopfield-network.md]]** — the item-capacity bound on the same object this page prices by retrieval sets, with the margins playing identical roles: `N ≥ √p c^{(d−1)/4}` counts how many patterns a `d`-dimensional softmax store holds given a separation `Δ_i`, where this page's `C(n,k) ≤ (1+1/γ)^d` counts how many top-`k` sets the same `d` can address given a margin `γ` — and in both the exponent is destroyed by the robustness requirement rather than by the combinatorics (Ramsauer et al. 2020).
- **[[wiki/entities/three-factor-key-value-memory.md]]** — the item-count bound with the input dimension decoupled from it: slots are free hardware, so a store's capacity stops being capped by the pattern size and becomes a design choice, leaving this page's query-set bound `C(n,k) ≤ (1+1/γ)^d` as the constraint that still binds — the embedding dimension of the key does not grow when slots are added (Tyulmankov et al. 2021).
- **[[wiki/concepts/key-value-memory.md]]** — the bound that survives the address/content split: keys optimised for discriminability are still scored by a factorised inner product, so this page's limit on askable query sets binds the address space however it was learned — and the key-value framing says the constraint on memory performance is retrieval rather than storage, which is this page's thesis stated behaviourally.
- **[[wiki/concepts/associative-memory.md]]** — the item-count ledger this page is the complement of, assembled in one table with its accounting units made explicit: `0.14N`, `α_n N^{n−1}`, `min(N_f^{n−1}, N_h)`, `√p c^{(d−1)/4}`, `K^max ∼ N³` and `C ≈ 1.0N` all count items, and this page's `(1+1/γ)^d` counts the questions — a store can be exponentially deep and still unaskable.
