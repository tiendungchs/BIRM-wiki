...
> **Secondary reading.** Kirsanov 2026, *Each Neuron Gets Its Own Gradient* (`raw/kirsanov-2026-vectorized-dendritic-gradients.md`) — a working neuroscientist's commentary on the same paper, written the week it appeared. It supplies three things the primary does not state in these terms: what hypothesis class the result eliminates, why the BCI design forecloses the depth question, and a concrete extension that would reopen it.
...
**Behavioural result.** Learning happens by *asymmetric regulation*: `P+` neurons hold their activity while `P−` neurons are downregulated over days, with `P₀` (matched for day-1 activity) falling between them. Read as a *strategy* rather than a statistic, this is **learning by sparsification**: the animal solves the task by withdrawing drive rather than adding it, which is the metabolically cheap direction (Kirsanov 2026, tentative). **(brainstorm)** If suppression is the default learning move under an energy constraint, a built credit rule should be asymmetric in the same direction — cheap low-threshold depression against expensive high-threshold potentiation — which is the opposite of the symmetric `±Δw` most local rules assume ([[wiki/concepts/synaptic-plasticity.md]]).
...
## Four findings that change what the wiki should build
...
**4. It eliminates one hypothesis class, and exactly one.** The defensible pre-result position was that *all* cortical credit is a broadcast third factor multiplying local coactivity — dopamine supplies the magnitude, and all the specificity comes from which cells happen to be eligible. That is now dead as a **complete** account (Kirsanov 2026): `P+` and `P−` cells are intermingled, anatomically indistinguishable and simultaneously active — hence equally eligible — yet receive oppositely-signed dendritic instruction, and no product of a broadcast scalar with an unsigned eligibility trace can produce opposite signs in two co-active cells. What survives is the **hybrid**: a scalar source whose *delivery* is spatially addressed. That is not a weaker version of the eliminated class, it is a different architecture — the address has to be computed somewhere — and distinguishing the two is the open problem of where the signal comes from, below. It is also why [[wiki/concepts/reward-prediction-error.md]] is relocated rather than refuted.
...
- **The task has no hidden layer, so the hard half of credit assignment was never on the table.** The `P+`/`P−` cells *are* the output: their activity is the controlled variable, so each cell's contribution to the objective is direct, observable, and known to the experimenter by construction. Structural credit assignment is only hard when a unit's contribution is **indirect** — routed through downstream units whose own credit is also unknown. The design's central virtue (a reward function stipulated *over the recorded cells*) is precisely what forecloses that case (Kirsanov 2026). The result therefore certifies that cortex **delivers** addressed credit; it says nothing about whether cortex **computes** addresses for units that do not touch the output.
- **Local or propagated?** Whether each layer computes its own error or receives one from the layer above — the difference between a deep credit-assignment scheme and a stack of shallow ones — is the depth question the row above leaves open, and it decides whether this mechanism reaches depth at all. **(brainstorm, following Kirsanov 2026)** The paradigm extends to test it: let the decoder read a population *downstream* of the imaged cells, so the imaged cells contribute only through an unrecorded relay. A residual that still flips sign with each imaged cell's **indirect** causal role would be the first in vivo evidence of credit propagating across a hop; a residual that collapses onto the population mean would show the tuft is a delivery channel with a one-hop reach and that depth is paid for somewhere else. No other entry in the wiki's credit-assignment catalogue ([[wiki/concepts/biologically-plausible-credit-assignment.md]]) has an in vivo test this concrete.
...
- **[[wiki/concepts/synaptic-plasticity.md]]** — the write rule this signal would steer, and a sign mismatch worth noting: the animal learned almost entirely by *suppression*, while the local rules on that page are symmetric in potentiation and depression, so an energy-constrained learner may want a depression-biased kernel.
...
- **[[wiki/entities/ventral-tegmental-area.md]]** — the anatomical intermediate on this page's axis, measured rather than inferred: a handful of instructive channels addressed end-to-end by dedicated, non-collateralizing wires, so coarse vectorization is bought with development instead of with a backward pass — and the input nucleus, not the signal's content, is what selects the channel.
