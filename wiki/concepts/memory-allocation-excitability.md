...
The premises are individually supported — SWRs replay recent experience, and disrupting SWRs causes strong memory deficits — but the middle link (excitability → SWR participation) is stated as *likely*, not shown. The *downstream* link is now supported at the population level and by a different variable: awake ripples at the reward site replay the trial just completed, and the following sleep's ripples revisit those same trials rather than a frequency-matched sample of experience, so something written during waking does bias the offline sampling distribution — but the decoded unit is one **trajectory**, not the tag-glued cluster of episodes this page's mechanism would predict (`raw/talk-nd-memory-gating.txt`, **(tentative)**; [[wiki/concepts/offline-replay.md]]). For [[wiki/concepts/offline-replay.md]] this is a candidate answer to that page's open arbitration problem: a **cheap, content-blind prior on what gets replayed**, requiring no evaluation of generalisability, no reward and no value-of-information computation. Replay priority ∝ excitability tag is one multiplication.
...
- **Whether allocation may carry information at all is contested by a working store.** BTSP models its plateau gate as an *input-independent* Poisson process at `f_q ≈ 0.005`, and credits exactly that independence with a flat recall curve across 10,000 sequentially learned items, while the postsynaptically-gated rule that recruits the same cells repeatedly degrades early traces badly ([[wiki/entities/btsp-cam.md]]). If allocate-to-link is right, temporal linkage is free and interference is the price; if BTSP is, non-interference is free and every cross-episode edge needs a separate mechanism ([[wiki/empirical-tensions.md]] T73). The direct test — plateau incidence conditioned on a cell's recent excitability tag — has not been run. **The independence premise is weaker than that framing assumes**: the primary BTSP source models plateau probability as raised by long-range entorhinal drive and lowered by local feedback inhibition, so the gate is input-*sensitive* at the population level even where it is uncoordinated across cells (Milstein et al. 2021, [[wiki/concepts/synaptic-plasticity.md]], `T391`). That is a third allocation variable beside the excitability tag and the weight — a closed-loop write *rate* — and it biases allocation by location and behavioural relevance without needing any per-cell tag at all.

---

## Connections

- **[[wiki/entities/inhibitory-replay-filter.md]]** — the allocation question answered from the other side, by construction: pairing optogenetic stimulation with a sensory cue entrains a stable *non-spatial* selectivity onto arbitrary hippocampal pyramidal cells, so which cell carries a feature is set by rapid plasticity over an already-available input mix rather than by developmental wiring — and the same pairing decides whether that cell is later recruited to or suppressed from ripples.
- **[[wiki/concepts/language-of-thought.md]]** — the same primitive stated symbolically: `mem` makes a function's random choice once per distinct argument and caches it, which is allocation-on-demand keyed by an identity token rather than by excitability, and it is what lets a program mint a new individual with persistent properties mid-execution (G69). Neither literature has been read against the other — the symbolic version has no capacity limit and no competition, which is the whole content of this page.
...
- **[[wiki/concepts/arbitrary-sensorimotor-mapping.md]]** — the same allocation question three orders of magnitude faster: ~30% of striatal cells distinguish novel from familiar mappings *immediately after cue onset*, which is a within-trial trigger for spending a write, against this page's hours-long excitability window (Wise & Murray 2000).
- **[[wiki/concepts/spike-frequency-adaptation.md]]** — the same K⁺ conductance seen from the other side: CREB tags a cell by *reducing* its spike-frequency adaptation, so allocation and adaptation are opposite manipulations of one knob — and a network that uses adaptation as a working-memory store is spending the variable this page's allocation mechanism needs free.
- **[[wiki/entities/bb-model.md]]** — allocation by fixed random projection rather than by excitability: a pool of uncommitted "reservoir" place cells receives sparse random grid input (3%) and mutual inhibition against the committed population, so *which* cells will represent unvisited space is decided before the space is visited and experience only supplies the bindings.
- **[[wiki/entities/three-factor-key-value-memory.md]]** — the machine statement of what allocation is *worth*, which the biological literature describes mechanistically and never prices: sequenced least-recently-used selection of the writing unit gives `C ≈ 1.0N` against `C ≈ 0.16N` for independent stochastic selection at matched hardware, so the coordination signal this page's tag-and-capture story does not supply is a 6× difference (`T391`; Tyulmankov et al. 2021).
