...
- **[[wiki/entities/lewm.md]]** — the same method at the cheapest point the wiki has measured: cross-entropy-method MPC on `‖ẑ_H − z_g‖²` over a **single 192-d embedding per frame**, a full plan in under a second and ~48× faster than the same planner on a frozen-foundation-encoder world model, which says the planner's bill is set by the model's token count rather than by whether it predicts pixels or features.
...
- **[[wiki/concepts/counterfactual-probing.md]]** — the same forward model queried for *structure* instead of for value: a rollout scored by what co-varies with an injected perturbation rather than by where it ends up, which is a use of imagination this page's search framing has no slot for.
- **[[wiki/entities/othellogpt.md]]** — the cheapest existing demonstration that an edit to an internal state propagates to action: adding a probe direction installs a counterfactual board — including boards unreachable by legal play — and the model's move distribution follows it (0.10 errors against a 2.72 null). One-step and imposed from outside rather than searched, so it is a working *write port* for a planner, not a planner.
- **[[wiki/concepts/violation-of-expectation.md]]** — the negative result that bounds what a passively-trained latent predictor is good for as a rollout engine: the properties a planner most needs from physics (solidity, collision) are exactly the ones at chance, while object permanence and continuity are near-ceiling.
- **[[wiki/concepts/external-verification.md]]** — the wiki's nearest candidate for an internal verifier (reject a plan whose rolled-out outcome misses) and the reason it fails the independence condition: the acceptance test is computed by the same learned model it is checking, which is the learned-reward-model row of that page's failure table.
