...
| **Colouring / bounded activation** | Activate outward from a point, do not cross boundaries; test whether an "infinity point" got activated | Much better — it explains why most of a curve can be ignored when the activation *leaks out* of a corridor and reaches infinity, which is the cheap case humans exhibit. **But it predicts computation time rising with figure size, and human in/out judgement is close to scale-independent** (a slight RT increase with size, under investigation at the time). The paper leaves this unresolved — **and it is now resolved**: run the activation at four scales and let local clearance between target and distractor pick the level, and enlarging the figure moves grouping up a scale, cancelling ([[wiki/concepts/incremental-grouping.md]], Mollard et al. 2026) |
...
- **The base/routine boundary is asserted, not measured.** Ullman argues that collinear arrangements of tokens are found by a *tracing routine* rather than made explicit by early grouping (against Marr 1976), while conceding grouping processes do exist in the base representation. Which grouping lives where is left open — the question `T151` asks in machine form. Mollard, Bohte & Roelfsema 2026 ([[wiki/concepts/incremental-grouping.md]]) draw the line explicitly and put it in the *base*: collinearity-and-connectedness is computed everywhere in parallel by a frozen feedforward population, and the routine that follows only *traverses* what that population permits — but the criterion is a supervised label the designers wrote, so the boundary has been placed by fiat rather than measured, and the authors' attempt to learn both jointly failed at more than one scale.
...
- **Eye movements are excluded rather than modelled.** The tracing and indexing results are all obtained under presentations too brief for saccades, so the account is of *internal* shifts. The overt case is now in the wiki ([[wiki/concepts/active-vision.md]], Ballard et al. 1997) and it does not merely extend this page — it **contradicts the incremental representation on the point that matters**. Ullman's routines write structure that later routines read; the block-copying data say almost nothing is written: what survives a saccade is the one or two values the current step binds, colour is not retained across model fixations that have already occurred (+50 ms when it is changed before pickup, against +129 ms after), and subjects prefer the *zero-carry* strategy at a cost of 1.0–1.5 s per block. So the second store exists, and it is two or three registers rather than a partial parse of the scene.
...
- **No architecture has an incremental representation** — a goal-dependent, spatially addressed store of partial parses that survives to the next query on the same input. [[wiki/entities/multiscale-tracing-network.md]] is the closest: a spatially addressed, cue-dependent tag written onto the base representation in the base representation's own units. It fails the clause that matters — it is a *fixed point*, not a store, so it does not survive a change of cue and nothing reads it but the output unit.
- **Two of the five operations are now implemented and learned; the assembly mechanism is not.** Bounded activation and boundary tracing turn out to be the *same* operation at two scales, trained from binary reward alone and generalising 4× beyond its curriculum ([[wiki/entities/multiscale-tracing-network.md]]) — which removes the objection that the operations are too expensive to learn and leaves the composition problem untouched, since exactly one routine exists there and nothing selects it.
- **The operation set has never been implemented as a differentiable or learnable module bank**, so "is this set sufficient?" has never been tested on any benchmark. The cheapest test available: score the five operations against the seven BlindTest tasks and against ARC's grid operations, and report which grid predicates are *not* expressible (`T215`, `G17`). The seven tasks are now mapped onto four of the five operations ([[wiki/entities/blindtest.md]]) and the mapping leaves *shift of processing focus* unmeasured, since no BlindTest task varies where the focus must start.
...
- **Indexing is stimulus-driven and the deictic account says pointers are goal-driven** (`T387`). The odd-man-out rule fixes where a routine starts by local surprise, with no goal term in it; Ballard et al. 1997 measure the *same* fixated location yielding different bound values at different task steps. Nothing decides whether these are two theories or two stages, and the two-stage reading has an unrun cost: a task-relevant item that is not an odd-man-out would need a serial scan to be bound.
...
- **[[wiki/concepts/active-vision.md]]** — the overt half of this page, and the correction to its most speculative claim: routines scheduled as *actions* with a measured price (1.3 fixations per block, 3.0 s per cycle, 22% controller occupancy, a look/remember exchange rate that shifts when saccade amplitude rises to 70°), and an incremental representation that turns out to be two or three bound registers rather than a retained partial parse — the world is re-read instead of remembered.
...
- **[[wiki/concepts/visual-indices.md]]** — what a *mark* is placed on, specified independently and measured: 4–5 stimulus-assigned pointers that survive the processing focus moving away and that hold no description of what they point at, which is the tag this page's relational operations need and which Pylyshyn argues cannot be placed on an internal representation at all. The two disagree on query-relativity — an index is assigned before any question is asked — and agree that the answer is obtained by routing rather than by tuning (Pylyshyn 2001, `T387`).
- **[[wiki/concepts/incremental-grouping.md]]** — two of this page's five elemental operations built, trained and priced, and the resolution of its one unresolved empirical failure: bounded activation predicts grouping time rising with figure size, human judgement is near scale-independent, and the missing term is a scale hierarchy whose level is selected by clearance between target and distractor, so approaching a display lengthens the curve and moves grouping up a scale, cancelling (Mollard, Bohte & Roelfsema 2026).
- **[[wiki/entities/blindtest.md]]** — this page's operation set scored, one task per operation, by authors who never cite it: tracing and marking floored (39–57% on path counting, shape counting and row/column counting), local predicates over separated items at ceiling, and the floor lifting only when the clutter is removed — the Minsky–Papert profile for an order-limited reader, plus the finding that the read-out channel this page derives from conjunction search is where the information is lost (`T388`).
- **[[wiki/entities/varc.md]]** — the null result this page has to answer: 60.4% on ARC-AGI-1 with no routine, no assembly, no elemental operations and no incremental representation — per-pixel classification end to end — while its layer-wise attention maps show the operation *sequence* (neighbourhood of a core, then rays along eight directions) compiled into network depth rather than run over time, which is what a routine looks like when the task set is small enough to amortise one per task token.
- **[[wiki/concepts/perception-reasoning-dissociation.md]]** — this page's query-relativity requirement turned into a measurement result: a perception stage that describes each image *once*, in a generic object vocabulary fixed before the rule is known, is a parse in the wrong tense, and it recovers 12 of ~80 missing points on Mini-ARC — the wiki's cleanest quantitative evidence that a query-independent parse is not what a routine replaces (Wang et al. 2026).
- **[[wiki/entities/multiscale-tracing-network.md]]** — the first working implementation of two of this page's five elemental operations (bounded activation, boundary tracing), learned by reinforcement rather than hand-built, and the resolution of its one unresolved empirical failure — the multiscale hierarchy is why grouping time does not scale with figure size. What it does not supply is assembly: there is exactly one routine, and it is not composed.
