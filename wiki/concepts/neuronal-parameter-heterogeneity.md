...
- **[[wiki/concepts/structured-flows-on-manifolds.md]]** — the same conclusion reached from connectivity rather than from cell parameters, and with a mechanism: uniformity is an equivariance, an equivariant system's reduced flow is degenerate, and breaking the symmetry — in the limit, with a *single* heterogeneous long-range connection — is what gives the low-dimensional dynamics any structure at all.
- **[[wiki/concepts/timescale-hierarchy.md]]** — the cheapest possible version of this page's thesis: functional heterogeneity from **one** global scalar times a structurally measured depth coordinate, with nothing learned per unit — and the ablation that makes heterogeneity load-bearing rather than decorative (remove the gradient and the timescale spread collapses, with no relation to hierarchy surviving). This page's Laplacian smoother over neighbouring units is a weak, learned form of the same spatial gradient (Chaudhuri et al. 2015).
- **[[wiki/concepts/intrinsic-timescale-measurement.md]]** — the conflation this page's validation may be resting on: HIFI's learned per-neuron `τ` matches recorded single-neuron distributions, while the areal timescale that motivates heterogeneous time constants is explicitly *not* a single-neuron property and is a statistic of the whole recurrent circuit — two different quantities the wiki has been treating as one ([[wiki/empirical-tensions.md]] T323).
- **[[wiki/concepts/prediction-error-neurons.md]]** — the biological argument that per-unit constants are functional rather than manufacturing scatter: in mouse V1 layer 2/3, six intrinsic properties of the kind this page makes learnable (resting potential, input resistance, `V_m` variance, baseline rate, membrane time constant, spike threshold) jointly predict 77% of the variance in a cell's visuomotor mismatch response, with no single property dominating — so the constants index which sign of prediction error the cell computes (Jordan & Keller 2020).
