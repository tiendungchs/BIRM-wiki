...
| **A 0% elicitation rate is compatible with strict necessity** | Frontopolar cortex is this page's floor, and it is also the one site where a **0.33 cc** lesion selectively abolishes analogical reasoning while sparing a stimulus- and format-matched perceptual control (Urbanski et al. 2016, [[wiki/entities/lateral-frontoparietal-network.md]]). Stimulation-null and lesion-critical in the same tissue: the two instruments answer different questions, and only the second licenses "load-bearing". An architecture whose abstract layer resists activation edits has not thereby been shown to be doing nothing — the discriminating test is removal, not injection |

---

## Connections

- **[[wiki/entities/hippocampal-prefrontal-channel.md]]** — the perturbation design this page's logic demands and nobody has run on an inter-regional coupling: detect a named network pattern in real time and perturb only during it, which is the only way to distinguish a *communication mode* from the anatomical link it rides on (Shin & Jadhav 2016).
- **[[wiki/entities/lateral-frontoparietal-network.md]]** — the case that separates *steerable* from *necessary* in one tissue: frontopolar cortex sits at this page's 0% elicitation floor, and a third of a cubic centimetre of it removed abolishes analogy specifically, with severity graded by how many of the site's afferent tracts were cut — so a null under focal excitation reports on the coding scheme and says nothing about whether the component is required.
...
- **[[wiki/entities/integrated-world-modeling-theory.md]]** — the one escape route offered for the prefrontal nulls: if frontal cortex is required to *bootstrap* a coherent world model and dispensable for running it afterwards, every adult stimulation study is blind to its contribution by construction, and the dispute becomes a developmental prediction rather than a synchronic one.
- **[[wiki/concepts/cortical-state-bistability.md]]** — a perturbation this page's instrument has not run: the prefrontal null is obtained by stimulating *sites*, whereas the switch-gating variable measured there is a *rhythm*, so driving 1–9 Hz bursts and scoring switch rate is a different causal test with a different predicted sign.
...
- **[[wiki/entities/dendritic-integration-theory.md]]** — a per-unit reason a focal perturbation does or does not spread: activity grows only through cells whose apical and basal compartments are coupled, so a perturbational-complexity measure reads the gate rather than the connectivity, and the same instrument means different things under the two interpretations.
- **[[wiki/concepts/attractor-identification.md]]** — the same injected current read for a different property: this page asks whether a focal edit changes reportable content, criterion `C2` there asks whether the state *flows back* to its manifold afterwards, and criterion `C3`'s localization test is a perturbation whose effect is read after the drive stops — persisting along the attractor set if the region generates the dynamics, relaxing if it only reads them out.
