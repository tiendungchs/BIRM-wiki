...
- **And the gap has now been measured, as a rank deficit.** The uniform `σ_{m-1}` optimum is full-rank; the covariance of a trained SimCLR's embedding has a block of singular values at **zero** (Jing et al. 2022, [[wiki/entities/directclr.md]]). Two causes, both outside this page's decomposition: an augmentation whose variance exceeds the data's along a direction kills that direction (`Σ̂₁ ⊀ Σ̂₀`), and an over-parametrised trunk drives the product of aligned weight matrices low-rank on its own. So `L_uniform` is not merely unminimised — whole directions are absent from the embedding it is computed on, which is a specific and checkable reason the monitor below might be reading a healthy design rather than a failure ([[wiki/empirical-tensions.md]] T314).
...
- **[[wiki/entities/directclr.md]]** — the spectrum this page's theorem predicts and nobody had plotted: the loss converges to a full-rank uniform target and the trained embedding is rank-deficient, so "the minimum is not in the feasible set" has a measured shape and two named causes.
...
- **[[wiki/entities/cpc.md]]** — the loss this page decomposes, and its primary source: CPC derives `I ≥ log N − L_N` and reads the objective as InfoMax, which is exactly the justification this page's Theorem 1 replaces with alignment plus a vMF-kernel entropy estimate.
- **[[wiki/concepts/label-free-model-selection.md]]** — where this page's two-scalar monitor sits among the alternatives: it is the only label-free selector validated against a dense read-out, and the only one that returns a pair rather than an ordering, which is what a checkpoint-selection rule needs (`G108`).
