...
- **The knowledge-boundedness limit recurses.** A discovered rule is bounded by the task distribution it was discovered on, exactly as a meta-learned initialization is — and a rule that fails outside `p(T)` is worse than backpropagation, which at least fails predictably. This is `G18` in its own words, and the restriction bullet below is why it stays open: the hand-imposed search restrictions are what buys the generalisation, so nothing yet shows a rule surviving outside them.
...
- **The candidate basis is closed under addition, and biology's is not.** Every search here composes terms that are functions of the *current* pre/post state; none carries a variable set by the unit's own previous updates. The one measured nonlinearity of that kind — first-spike suppression, a per-neuron efficacy zeroed at each spike and recovering over 34–75 ms, multiplying the pairwise kernel (Froemke & Dan 2002, [[wiki/empirical-tensions.md]] T329) — is therefore outside the span of every pool the wiki records, and would have to be added as a *state variable* rather than as a term. **(brainstorm)** That is the cheapest concrete extension available to this page's method: meta-learn the recovery constants of a write-refractory variable, not just the coefficients of the update.
...
- **[[wiki/concepts/synaptic-plasticity.md]]** — supplies the rules whose hand-set coefficients this page hands to an outer loop; without a local rule to parameterize there is nothing to meta-optimize. It also supplies the one term-shape the search spaces here cannot express: a per-neuron efficacy carrying the history of the unit's own updates (T329), which is a state variable rather than a basis function.
...
- **[[wiki/concepts/neuroscience-ai-transfer.md]]** — the transfer channel used in a third mode: neither importing a mechanism nor exporting an algorithm, but taking a biological mechanism and handing its free parameters to an optimizer.
