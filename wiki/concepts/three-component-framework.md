...
- **[[wiki/concepts/dendritic-computation.md]]** — an architecture-slot proposal whose parameter is derived rather than tuned: the segment nonlinearity is the architectural commitment, and its threshold `θ ≈ 9–20` falls out of minimising detection error over a wide sweep of the other parameters — then matches the measured NMDA spike threshold of 8–20.
- **[[wiki/concepts/canonical-cortical-microcircuit.md]]** — the mirror image of most entries here: the architecture slot is specified in unusual detail (laminar excitatory graph, patch geometry with a 2:1 spacing rule, two inhibitory classes by axon geometry) while the objective and the learning rule are entirely absent, so it is a template awaiting the other two components rather than a competing framework (Douglas & Martin 2004).
- **[[wiki/concepts/representational-collapse.md]]** — what makes the objective slot hard to fill rather than merely empty: the cheapest solution to any predictability objective is degenerate, so a candidate for `g` needs a component this framework does not name.
- **[[wiki/entities/simple-cycle-reservoir.md]]** — the wiki's strongest evidence that the three slots are not equally load-bearing: architecture reduced to one scalar `λ` on a ring, learning rule reduced to ridge regression, objective to squared error, and the triple is *proved* universal over all time-invariant fading-memory filters — so within a stated function class the architecture slot is the cheapest question, not the heaviest, and it can be filled by construction rather than by search.
- **[[wiki/entities/s4.md]]** — an ablation whose decisive variable falls outside all three slots: architecture (a linear state-space layer), learning rule (Adam over all `5N` parameters) and objective (cross-entropy) are held fixed while only the *initial value* of the state matrix `A` varies, every arm reaches 100% training accuracy, and validation accuracies separate by more than 15 points — so the result is carried by a prior on a parameter's starting point, which the framework has no slot for (Gu et al. 2022).
- **[[wiki/entities/ms-ssm.md]]** — the second result to land outside all three slots, and the one that says what shape the missing slot has: architecture, learning rule and objective are held fixed while only the *distribution* the initial eigenvalues of `A` are drawn from changes (scale-stratified vs Mamba's), and ListOps moves 57.49 → 63.04 — so the fourth slot [[wiki/entities/s4.md]] opened is not "an initialisation" but "a **measure** the initialisation is drawn from", which is a strictly richer object and still has no name in the framework (Karami et al. 2025).
- **[[wiki/entities/lru.md]]** — the fourth slot priced down to three numbers: [[wiki/entities/s4.md]] put >15 points of validation accuracy into an *initialisation* outside all three slots and identified the occupant as HiPPO, a projection of history onto an orthogonal polynomial basis; LRU matches the six-task Long Range Arena average including Path-X with eigenvalues drawn uniformly from a ring, so the occupant is a **measure over the unit disc** specified by `r_min`, `r_max` and `θ_max` — still in none of the three slots, but a far weaker thing to have found there ([[wiki/empirical-tensions.md]] T177).
- **[[wiki/concepts/information-bottleneck.md]]** — a hidden argument in the objective slot: a compression objective is undefined until a *relevance variable* is named, so two systems logged here as having the same objective can have different information budgets, and the slot needs a second field the specification does not currently have.
- **[[wiki/concepts/inductive-bias.md]]** — makes this page's central term a measurable object: the architecture/objective/learning-rule slots are three constructions of one hypothesis space `H`, the genome bottleneck is a budget on `log C(ε, H)`, and `n = O(W)` states how many task draws an evolutionary outer loop would need to install a bias of that size (Baxter 2000).
- **[[wiki/concepts/apical-amplification.md]]** — an architecture-slot entry with the other two slots conspicuously empty: the coincidence nonlinearity and the compartment topology are specified to the millisecond, while nothing says what objective makes an apical synapse come to carry the right prediction or what rule writes it.
