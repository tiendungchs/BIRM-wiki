...
- **[[wiki/concepts/independent-causal-mechanisms.md]]** — the principle this procedure measures: step 5's co-response score is a direct test of the *influence* clause (does intervening here move that?), and the page states in structural-equation vocabulary what the omitted step 1 costs — a counterfactual needs the noise variables *fixed*, which is precisely what separates a structural causal model from a causal graph.
