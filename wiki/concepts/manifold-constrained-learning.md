...
> - An unnamed Nature Neuroscience study, January, relayed in a video transcript (`raw/talk-nd-brain-learning-limits.txt`) — macaques, ~90 M1 units, cursor BCI. Asks whether an existing neural trajectory can be run **backwards**. Everything sourced to it is marked **(tentative)**: no paper was read, and no authors, sample sizes or statistics are available.
...
## The third constraint: temporal order, even inside the manifold

A third BCI (Brain–Computer Interface) result, reported in a talk rather than read from the paper — Nature Neuroscience, January, authors not named in the source, so everything here is **(tentative)** (`raw/talk-nd-brain-learning-limits.txt`). Same paradigm, same area: macaques, ~90 M1 (Primary Motor cortex) units, cursor driven by a linear projection of the 90-D activity to 2-D. What changes is the axis being probed: not *which subspace* the activity must occupy but *which direction it may flow in*.

| Step | Manipulation | Result |
|---|---|---|
| Calibration | Monkey watches the cursor move on its own; the projection where neural trajectories look most like the cursor's paths is taken as the **movement-intention view** | Under this mapping the animals control the cursor immediately and smoothly |
| Diagnosis | In the intention view, leftward and rightward trajectories overlap and appear to be one path traversed in both directions. A second projection — the **separation-maximising view** — is found | In that view the two trajectories are entirely distinct and curve in different directions: left and right are **not** the same neural pattern time-reversed |
| Test 1 | Switch the interface to the separation-maximising view | Cursor now moves in curved paths; the animals do **not** straighten them, despite the visual feedback and despite straightening being the reflexive correction for a reaching error |
| Test 2 (motivation control) | Require the cursor to stay inside a narrow corridor between targets — achievable only by emitting one of the existing trajectories **in reverse temporal order** | Consistent failure across sessions, with reward available and the corridor visible |

The transcript's own caveat is the one that matters: Test 1 alone is confounded by the animals still being rewarded on curved paths, so "no attempt" is not distinguishable from "no ability". Test 2 exists to remove that, and is the load-bearing condition.

**What this adds to the two constraints above.** Sadtler et al. constrain the *set* of reachable patterns (the span); Golub et al. constrain the *repertoire* within that set (re-labelling only). Both are constraints on a static set of points. Time-reversal is not: the reversed trajectory visits the same points, so it violates neither. It is forbidden anyway. The constraint is therefore on the **flow field**, not on the reachable set — the population's dynamics are directional, and volitional control does not get to choose the direction. The river metaphor the talk uses is precise in exactly this respect: the channel bounds where the water goes, but the slope is a separate fact and it fixes which way.

**(brainstorm) This is the first constraint on this page that a feedforward model cannot exhibit.** A frozen encoder has a span and a repertoire; it has no arrow of time. Whatever implements the reversal ban has to be recurrent — asymmetric recurrence being the obvious candidate ([[wiki/concepts/attractor-dynamics.md]] separates `J^S`, which fixes the reachable set, from `J^A`, which supplies the direction of travel and does not enter the stationary distribution). That split predicts the result: a within-session learner can re-weight what `J^S` already spans but cannot flip the sign of `J^A`. If it holds, the testable version for a model is that fine-tuning a recurrent net to emit a learned sequence backwards should be strictly harder than fine-tuning it to emit an equally novel forward sequence over the same state set — with sequence-set novelty held fixed, which is the control the biological experiment achieves by construction.

**Also a methodological point worth reusing.** The two projections are two views of the *same* recording, and they disagree about whether left and right are the same pattern. Any wiki claim of the form "these two conditions use the same population code", when it rests on a low-dimensional projection, is underdetermined in exactly this way — a second projection may separate them ([[wiki/concepts/population-geometry.md]]).
...
- **Is the reversal ban dynamical or decoder-specific? (tentative)** The talk's failed condition required a time-reversed trajectory *under a particular projection*; nothing rules out that some other readout of the same reversed activity would have succeeded, or that the corridor task was simply harder for reasons unrelated to direction. A direction-matched control — same corridor difficulty, forward-order solution — is not reported in the transcript and may or may not be in the paper.
- **The reversal result has no manifold measurement attached (tentative).** It is not stated whether the reversed trajectory lies inside the fitted intrinsic manifold. If it does, the flow constraint is genuinely a third and independent constraint; if the reversed path leaves the manifold, the result collapses into Sadtler et al.'s. Reading the paper settles this and nothing else does.
...
- **[[wiki/concepts/attractor-dynamics.md]]** — the mechanism most likely to *be* the constraint: recurrence that stabilises a manifold is also what forbids leaving it, which makes this page the behavioural cost of G47's built-in topology. The `J^S`/`J^A` split there also supplies the candidate mechanism for the temporal constraint: the symmetric part fixes the reachable set, the antisymmetric part fixes the direction of travel, and only the first is what fast learning re-weights (talk-nd-brain-learning-limits, **(tentative)**).
...
- **[[wiki/concepts/offline-replay.md]]** — the direct counter-case to the temporal constraint: hippocampal sequences replay forward *and* reverse, so directionality of population flow is not a universal fact about cortex-like circuits but an area- and regime-specific one — logged as [[wiki/empirical-tensions.md]] T80.
- **[[wiki/concepts/temporal-coding.md]]** — the constraint stated at the level this page reaches from above: what a population can emit is bounded not only in state space but in *order*, so a model whose sequence generator can be run in either direction for free is claiming a freedom the measured system does not have (talk-nd-brain-learning-limits, **(tentative)**).
- **[[wiki/entities/neo-neural-theorizer.md]]** — the same constraint installed as a loss rather than discovered in a cortex: a decode–encode consistency term forces every intermediate program state back onto the manifold of valid latents, and removing it collapses primitive recovery from 1.000 to 0.002 — so staying inside the manifold is not only what a population *can* do, it is what makes an operation reusable as a building block.
- **[[wiki/entities/s4.md]]** — the geometry chosen rather than discovered, and the choice measured: the latent trajectory lives on whatever manifold the state matrix `A` imposes, HiPPO's being the coefficient space of an orthogonal-polynomial expansion of the input history, and replacing it with a random (or random diagonal, or random normal-plus-low-rank) matrix costs over 15 points of validation accuracy while costing *nothing* in fit — every initialisation reaches 100% training accuracy (Gu et al. 2022).
- **[[wiki/concepts/structured-flows-on-manifolds.md]]** — a candidate account of why the intrinsic manifold is the boundary of what is learnable within a session: if the manifold is fixed by the coupling's spectral structure at the network's operating point rather than by the task, then within-manifold perturbations only re-route an existing flow while outside-manifold ones would require the enslaved directions to become free.
- **[[wiki/entities/lru.md]]** — the chosen geometry made cheap: [[wiki/entities/s4.md]] confines the state trajectory to the coefficient space of a polynomial expansion of history and loses 15 points of generalisation if that choice is randomised, while LRU confines it to the span of `N` complex exponentials whose rates and frequencies are *drawn from a hand-specified product measure over the unit disc* — and the drawn version generalises as well as the derived one on five of six Long Range Arena tasks, so what the constraint has to supply is a distribution rather than a construction.
- **[[wiki/concepts/output-null-subspace.md]]** — the same decomposition with the roles reversed: the BCI decoder *is* `W_out`, its 2-D control space is the potent subspace and everything else the animal's units do is null, so this page's perturbations move the potent directions while the intrinsic manifold stays put — and the untested question it raises is whether the decoder's kernel is a *usable* workspace, i.e. whether an animal can be trained to compute in it and commit by rotating out of it `(tentative)`.
- **[[wiki/concepts/structural-learning.md]]** — the write half of this page's read: here the low-dimensional subspace is a pre-existing property of the population that bounds what is learnable, there it is *installed* by a zero-mean parameter sweep and bounds where the learner searches — same constraint, opposite direction of causation, and neither page has the other's operation (Braun et al. 2009).
- **[[wiki/concepts/projection-distortion.md]]** — the standard this page is the wiki's only entry to meet: a fitted low-dimensional object earns evidential status by *predicting the outcome of a manipulation it never saw*, which is unavailable to any embedding defended by fidelity statistics, since those are satisfied equally by an embedding fitted to an arbitrary shape.
- **[[wiki/concepts/topological-latent-decoding.md]]** — measures the force behind this page's constraint: net flux off the manifold significantly exceeds flux along it, in radial and tangential components, so the off-manifold patterns this page finds unlearnable are the ones the dynamics actively undo.
