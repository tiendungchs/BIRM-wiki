...
- **[[wiki/entities/waterbirds.md]]** — what the interpolating regime costs an *objective*: once training loss vanishes, the training loss is uninformative about every group at once, so any criterion defined on it (worst-group, reweighted, group-adjusted) is a no-op — leaving the regime is the precondition for the objective to select anything at all.
...
- **[[wiki/entities/dense-associative-memory.md]]** — this page's spectrum installed as an architectural dial rather than read off a task: the energy exponent `n` moves a hidden unit from a macrofeature shared across 3–5 classes (small `n`) to a whole-class template (> 40% of memories voting for exactly one class at `n = 30`), and both ends hit the same MNIST accuracy — so task performance cannot say which of the two readings a trained layer admits, which is this page's claim restated as a control parameter.
