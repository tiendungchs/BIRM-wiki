...
- **[[wiki/entities/inhibitory-replay-filter.md]]** — the ratio doing content selection rather than stabilisation, and left unlearned: replay dynamics are `σ((W − αW^I)ŷ)` with `α > 1` asserted (`1.5`) on the grounds that inhibition predominates during ripples, so how aggressively the offline sample is pruned is a free excitation/inhibition parameter that no mechanism in the model sets.
...
- **[[wiki/entities/global-neuronal-workspace.md]]** — the condition under which ignition exists at all: balanced amplification requires feedforward excitation offset by local inhibition, so the same knob that sets speed–accuracy also sets whether a cascade takes off, dies, or runs away.
