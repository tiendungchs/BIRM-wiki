...
- **[[wiki/entities/retrosplenial-cortex.md]]** — what happens at the top of the stack when the evidence accumulates: two reviews of the same association cortex fourteen years apart converge on *fewer* functions and then on none, because each additional recording adds another variable the region encodes — so a module-level function label for associative cortex is a provisional compression of a tuning inventory (`T383`).
