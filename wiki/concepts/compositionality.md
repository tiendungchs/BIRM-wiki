...
- **The read-out of a compositional code is dense** (`G97`). Every operator above — tensor products, circular convolution, vector addition — writes into a shared vector space that a linear decoder reads whole, so the separation the binding buys is spent again at the read, which is why clean-up memories are needed and why capacity statements are about the decoder rather than the code.
...
- **[[wiki/entities/resonator-network.md]]** — the read side of productivity, and the evidence that a fixed-width composition is invertible *in practice* rather than only in principle: a 500-dimensional vector holds one of 250 million possible 1–3-object scenes and every factor of every object is recoverable, which is what separates a compositional code from a merely compressive one — at the price of a search with no convergence guarantee.
- **[[wiki/concepts/tensor-product-representation.md]]** — concatenative composition realised in a vector space rather than asserted against it: constituents stay separately addressable inside the compound (an exact partial inner product reads either factor without destroying it), `cons`/`car`/`cdr` are linear maps on the role factor alone, and recursion costs dimensions rather than fidelity — the opposite corner from the functional compositionality of a convolutional binder.
- **[[wiki/entities/esbn.md]]** — one facet isolated and driven to ceiling architecturally: the entity vocabulary is swapped wholesale between train and test while the rule vocabulary is held fixed, so the withheld-entity sweep measures recombination on the filler axis alone.
- **[[wiki/concepts/independent-causal-mechanisms.md]]** — the neighbouring decomposition principle in a different currency: this page asks how parts combine into a whole, ICM asks whether the parts can be *changed independently*, and a system can satisfy either without the other.
...
- **[[wiki/concepts/structural-learning.md]]** — the non-compositional route to covering a family: one low-dimensional structure plus a continuous coordinate instead of a library of recombinable parts, which buys interpolation *and* extrapolation along the coordinate and buys nothing at all off it — the trade this page's parts-and-relations route makes in the opposite direction (Braun et al. 2009).
- **[[wiki/concepts/curriculum-learning.md]]** — this page supplies the wiki's one non-oracular easiness measure: where the target relation composes, hop `k` is learnable only after hop `k−1` holds, so the difficulty ordering is fixed by the task's algebra rather than by a quantity the learner is still trying to estimate (Bayes-correct side, margin, generative factors).
- **[[wiki/entities/recursive-npi.md]]** — the differentiable-programming row of this page's table, now held first-hand and with the productivity question settled by construction: a tail call in the *training trace* makes composition depth a program parameter rather than a data parameter, and the same finite call turns generalization into a coverage proof.
- **[[wiki/concepts/visual-routines.md]]** — compositionality asserted of *procedures* rather than of representations: a fixed set of elemental operations is reused across an unbounded set of spatial relations, which is what keeps the set small, and the price is that shared hardware cannot run at two locations at once — so sequencing is a consequence of composition here, not an implementation detail.
