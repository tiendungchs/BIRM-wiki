...
- **[[wiki/concepts/hippocampal-long-axis.md]]** — the same wiring-cost argument in a subcortical structure, plus a case the two scalars would have to score jointly: the hippocampal long axis is a smooth gradient in extrinsic connectivity and represented scale *and* carries sharply bordered gene-expression domains and an abrupt association-fibre seam, so smoothness and waviness are high at once rather than trading off.
- **[[wiki/entities/subiculum.md]]** — a subcortical case where the type annotation *is* the position: excitatory subtypes tile the structure as discrete abutting patches across three axes, so coordinates substitute for molecular labels (the review's "Rosetta stone"), which is the strong form of this page's claim that layout carries information (Kinman et al. 2026).
