...
**Machine status — intuitive physics, and it is the strongest challenge on this page.** Garrido et al. 2025 (`raw/garrido-2025-intuitive-physics-v-jepa.md`) run the violation-of-expectation protocol on **V-JEPA**, a video model trained only to predict masked spatiotemporal blocks *in its own representation space* on natural internet video — no object variable, no simulator, no hand-coded abstraction, no benchmark data. Against a null of `n = 20` randomly-initialised networks of the same architecture, it reaches **98%** pairwise accuracy on IntPhys, 66% on GRASP, 62% on InfLevel-lab, and matches or beats naive humans on every IntPhys property. Object permanence `g = 9.0`, continuity `g = 11.0`, shape constancy `g = 8.1`. The result survives every ablation the authors run: three masking objectives, three corpora, **128 hours of unique video** at fixed compute, and a **115M-parameter** encoder. Pixel-space prediction (VideoMAEv2) and multimodal LLMs (Qwen2-VL-7B, Gemini 1.5 pro) sit at the untrained null. See [[wiki/concepts/violation-of-expectation.md]].

The authors state the consequence directly: *"core knowledge is not necessary for deep learning systems to understand intuitive physics"*. Three qualifications hold that down to what it actually shows.

1. **It is a competence claim, not an origin claim.** Nothing here shows the *human* system is learned; it shows a sufficient learned route exists, which is what T12 has always lacked on the B side.
2. **The coverage is partial and the partition is principled.** Properties intrinsic to one object (permanence, continuity, shape) are large effects; object-against-world properties (support, gravity, inertia) are moderate and fail on InfLevel, where the disambiguating event precedes the model's 3–4 s window; **object–object interaction — solidity and collision — is at chance**, as is colour constancy. The *contact* principle, one of the object system's three, is the one that did not emerge.
3. **The failure profile is stable under scaling, retraining and objective change** — which is precisely the invariance this page's own signature-limit criterion uses to certify an *installed* prior ([[wiki/empirical-tensions.md]] **T161**). A learned model with no prior reproduced the diagnostic.
...
**A second battery, in 3-D physics rather than gridworld, and the perception column nobody else reports** (Shu et al. 2021, [[wiki/entities/agent-benchmark.md]]). AGENT scores the same core system over goal preferences, action efficiency, unobserved constraints and cost-reward trade-offs, with 13 trial types each authored to defeat one heuristic. On ground-truth 3-D state the ordering repeats — inverse planning .96, a graph-encoder ToMnet .90, single human .91 — and holds under leave-one-type-out (.94 vs .65) and leave-one-scenario-out (.94 vs .71). Two findings this adds. (i) **Aggregate parity is not profile parity**: the neural model's per-type accuracy correlates **0.06** with the human per-type accuracy, against 0.55 for the inverse planner, and it scores **.05 — far below chance — on the type authored to catch "the straightest path is least surprising"**, predicting agent trajectories *through solid walls*. (ii) **Behind a trained derenderer instead of given state, the same two models score .65 and .51 (chance)** — so the agent system's machine status is "demonstrated where the object system is solved by fiat", and the two cannot be scored apart. That is also this section's mechanism claim made concrete: the failures of an agent-system model are object-system failures.
...
**And the criterion now has a machine counterexample, which is the sharpest thing this page has to absorb.** V-JEPA has no installed prior of any kind, and it satisfies both tests. Its intuitive-physics failure profile — intrinsic object properties large, object-against-world properties moderate, **object–object interaction at chance** — is *constant* across a 115M→ViT-H scaling range, across three pretraining objectives, across three corpora and across a 100× change in unique-data volume; and its errors correlate with human errors in exactly the condition (violation behind an occluder) where humans degrade. Constancy-under-perturbation therefore does not discriminate an installed prior from a learned regularity whose *bottleneck* is architectural — here a 3–4 s memory window and a framerate, which no amount of scale or data moves because neither lever touches them. **The repair, if there is one:** a signature limit certifies installation only if the quantitative constant survives perturbation of the *resource* it could otherwise be attributed to. Sweep the context window and the framerate; if the collision failure moves, it was a capacity limit, not a prior ([[wiki/empirical-tensions.md]] T161, [[wiki/concepts/violation-of-expectation.md]]).
...
- **Is installing these priors a benefit or a liability, and what decides?** The wiki's use of core knowledge is as the term that equalises `P` in a human/machine comparison ([[wiki/concepts/skill-acquisition-efficiency.md]]). The opposite reading has now been argued in print: for an agent to be evaluated in *arbitrary* worlds, any world-specific commitment — objectness, action-on-contact, and classical physics explicitly named — is a bet that can be wrong, and a wrong prior is worse than no prior, so a general reasoner should install none (Pfister & Jud 2025, [[wiki/concepts/problem-framing.md]], [[wiki/empirical-tensions.md]] T211). The general form is an unquantified trade-off: more regularity assumptions ⇒ a smaller covered subset of worlds and a more efficient algorithm, and a higher chance of a mismatch with the world actually met. Nothing calibrates it, and nothing supplies a withdrawal mechanism for an assumption that is failing — which is gap G23's entry test asked over *worlds* rather than over entities.
- **Nothing selects which system applies.** Signature limits individuate the systems and the entry conditions gate them one at a time; what is missing is the arbitration when several apply or none obviously does, which is `G37` at the level of an installed prior library rather than a learned one.
- **What computes the entry condition? — *now answered for the object system, and it is learned*.** "Moves cohesively ⇒ object system" presupposes a cohesion detector upstream of the system it gates, and nothing on this page specifies it. [[wiki/entities/spelkenet.md]] supplies one: a 7B autoregressive flow model trained on 7k hours of unlabeled internet video, poked with a synthetic one-pixel motion token, returns the set of pixels that consistently move with the poke across sampled outcomes — beating a *supervised* segmenter on a benchmark built to Spelke's own criteria (Average Recall 0.541 vs SAM2's 0.482) with no segmentation label at any point. Three consequences. (i) The detector is neither installed nor stipulated, which weakens the innateness reading of the object system's entry test specifically — cohesion is recoverable from passive video by a model with no object variable in it. (ii) The same probe returns the **support relation** (poking the base of a stack returns everything it supports) and a **rigidity** readout (`p_motion` uniform for rigid bodies, poke-localised for deformable ones), i.e. the contact principle and the object/substance distinction come out of one operation rather than three modules. (iii) What it does *not* supply is the gating: the detector says which pixels cohere, not which downstream system should receive them, so gap G23's router is still missing one level up.

---

## Connections

- **[[wiki/concepts/problem-framing.md]]** — the antagonist on what these priors are worth: a benchmark over arbitrary worlds must withhold them, because outside the world they were selected for an installed prior is a mis-specified constraint rather than a head start (T211).
...
- **[[wiki/entities/agent-benchmark.md]]** — the agent system probed where it cannot be isolated from the object system: 3-D scenes with gravity, jumping and occlusion, in which the neural baseline's agent-reasoning errors are physics violations, and in which removing ground-truth state takes every model to .51–.65.
...
- **[[wiki/entities/spelkenet.md]]** — the object system's entry condition built and measured: cohesion under applied force, computed by poking a video world model rather than by a stipulated principle, which closes this page's last open problem for one system and shows the detector can be learned from passive observation alone (Venkatesh et al. 2025).
- **[[wiki/concepts/counterfactual-probing.md]]** — the method behind that detector, stated generally: intervene on a generative model's input and cluster the response distribution, which is this page's "grouped by response to force" definition turned into a runnable operation over any domain whose model can be addressed at a point.
...
- **[[wiki/entities/lewm.md]]** — an entry condition that nobody installed and nobody probed for: a 15M latent world model's surprise rises significantly for objects that teleport (`p < 0.01`, three environments) and not significantly for objects that change colour, so a plain predictability objective produces a violation detector aligned with physical continuity and blind to appearance (Maes et al. 2026).
- **[[wiki/concepts/violation-of-expectation.md]]** — this page's own developmental instrument, exported to machines and turned back on it: a video model with no object variable passes the object-system probes (98% on IntPhys, ≥ naive humans on every property) while failing solidity and collision, so the protocol certifies the competence and is silent about whether it was installed (Garrido et al. 2025).
- **[[wiki/entities/ilp-arc-synthesizer.md]]** — objectness supplied as three authored types, and the observation the priors do not settle: one grid admits several equally valid object parses (a rectangle *is* lines *is* points), which that system resolves by keeping all of them until a transformation explains one. **(brainstorm)** it also suggests an unassigned role for this page's principles — as *rejectors* of proposals that violate cohesion or solidity, before any outcome is observed.
- **[[wiki/entities/autotom.md]]** — the agent core system supplied as a *menu* rather than as a model: goal, belief, observation and interactive state are four authored variable types with authored factorisations, and which of them are instantiated for a given episode is decided by search — the closest thing the wiki has to a prior with the entry test G23 asks for, though the test is inference confidence rather than domain membership.
- **[[wiki/entities/corethink-compositional-reasoner.md]]** — a competing operationalization of the priors ARC declares: 22 transformation families mined by hand from the ARC training sets and *called* a core-knowledge set, so the same term names a benchmark-derived inventory here and a developmental one on this page — and the two need not have any items in common.
- **[[wiki/entities/arc-vsa-solver.md]]** — the wiki's clearest case of a prior installed as an *algebra*: geometry and number are consequences of the Spatial Semantic Pointer encoding (binding = translation, origin = identity, magnitude = similarity), objectness is a separate searched parse, goal-directedness is absent — and because an encoding is always on, the entry-test problem (G23) is sidestepped rather than answered.
- **[[wiki/entities/mlc.md]]** — a third route for installing a prior, alongside this page's evolved modules and the data-driven route: sample the bias into the *task distribution* at a calibrated frequency (80% algebraic outputs, 20% one-to-one/argument-flipped heuristics, matching measured human accuracy), which reproduces human inductive biases without writing any of them into the architecture — including mutual exclusivity in the direction opposite to the neural-network default — and inherits a price now measured: the sampled prior arrives at the right marginal rate and the wrong conditional one, applied absolutely (98.0% vs human 68.2%) in contexts the sampler never varied ([[wiki/empirical-tensions.md]] T202).
- **[[wiki/concepts/test-time-training.md]]** — a second way to install this page's content in a machine, weaker than an enumerated operation set and empirically more scalable: supply the priors as an *invariance group* (rotations, reflections, colour permutations, transposes) used to augment the demonstration pairs, so the system is told what must not change the answer rather than what operations exist (Chollet et al. 2024).
- **[[wiki/entities/arc-agi-2.md]]** — the prior-equalization claim measured instead of asserted: across 407 participants, no self-reported background variable (programming, mathematics, puzzle aptitude, industry) predicts performance, which is what "requires only Core Knowledge" should imply — while the benchmark's in-context-symbol-definition axis is the first ARC demand that recombining the four declared priors plainly does not cover.
- **[[wiki/entities/arc-agi-3.md]]** — the declared prior set edited between benchmark generations, which is a change in what a score can be credited to: number and counting are dropped, basic physics is added, and goal-directedness is sharpened into *agentness* (objects recognised as acting with intent) — plus the wiki's strictest cultural-symbol exclusion, naming "green means go" as a specific ban.
- **[[wiki/entities/raven.md]]** — evidence that solving items which *use* a prior does not mean the prior is installed: an ARC solver correct on tasks built around the geometry-and-topology notions *top/bottom* and *boundary* scores 29% and 8% on hand-authored variations of those same two concepts (Odouard & Mitchell 2022).
- **[[wiki/entities/conceptarc.md]]** — the most concrete inventory the wiki holds of what the objectness, number and geometry priors look like as *separately scorable* competences (16 named concept groups), with the measurement that a solver can hold one and not its neighbour: the first-place ARC-Kaggle program ranges 0.23 (*Copy*) to 0.77 (*Extend To Boundary*) across them.
- **[[wiki/entities/poe-arc-solver.md]]** — the priors installed as an *invariance group* with a formal statement of what that buys (`P(s|p) = P(φ(s)|φ(p))` for D₈ symmetries, colour permutations and example re-orderings), and the failure mode that follows: an invariance constrains without enumerating, but it is wrong exactly on the tasks where colour or orientation carries the meaning, and nothing detects that per task.
- **[[wiki/concepts/nameability.md]]** — the limit of this page's inventory read as a *hypothesis space*: humans solving ARC describe the rules with words no core system yields ("tail", "tetris", "vessel"), so the four priors bound what the tasks require and not what solvers reach for. The same behavioural data supplies the cheapest audit of whether a machine holds the objectness prior at all — whether its *errors* respect it, which human errors do and DSL-search errors do not.
- **[[wiki/concepts/rule-level-evaluation.md]]** — the sharpest negative measurement for the machine side of this page: on a corpus authored around objectness, number and geometry, frontier model rules operate on pixels, ordered colour integers, bounding boxes and 4-/8-connectivity, and three independently-trained systems miss largely the *same* tasks (pooling adds +9 points), which points at a shared missing prior rather than three training accidents.
- **[[wiki/entities/bib.md]]** — the agent-system battery of the paragraph above, paged primarily: five infant expectations rendered one-for-one as gridworld stimuli, including the rational/irrational contrast where the correct response is *no expectation* — a scorable form of calibrated ignorance no other machine battery in the wiki asks for.
- **[[wiki/concepts/certification-instruments.md]]** — signature limits and the resource-stress probe are instrument `I2` there, and the inventory takes back the failure mode that bounds them (`F8`): a model with no prior exhibits an invariant failure profile too, so constancy-under-perturbation is necessary and not sufficient.
- **[[wiki/concepts/human-baseline.md]]** — signature limits are the human-side analogue of profile matching — certify a prior by an invariant *failure* profile rather than a success rate, which is what a per-item human profile would supply for the machine comparison.
- **[[wiki/entities/math-perturb.md]]** — the entry-test argument transposed from an installed prior to a *learned* skill, and priced: a solution technique retrieved without an applicability gate costs 13–16 points at the 2025 frontier when one clause edits away the property it exploited, with the model silently reverting the modified condition to the original rather than declining the technique.
- **[[wiki/entities/integrated-world-modeling-theory.md]]** — the Kantian version of this page’s list (space, time, cause as preconditions for an experienceable world) together with the Helmholtzian counter-ordering that selfhood and agency come *first* and the categories are constructed from them — a developmental sequence claim that this page’s infant evidence could in principle adjudicate.
- **[[wiki/concepts/discrete-infinity.md]]** — the shared/unique subtraction run on this page's primitives: approximate magnitude with scalar variability and small-`N` tracking are present in animals, the open-ended integer list is not, so number is the one core system observable both with and without the generative operator attached — and the chimpanzee's thousands-of-trials-per-numeral against the child's single successor induction is the price of lacking it.
- **[[wiki/concepts/emergent-modularity.md]]** — the same subtraction run over a wider trait list and reaching this page's verdict on number independently: approximate magnitude, small-`N` tracking, addition and subtraction are all reconstructed in the great-ape last common ancestor and are already **modality-independent** there (monkeys match heard call counts to seen face counts), so the content-invariant code predates the species by ~14 Myr.
- **[[wiki/concepts/motivation-representation-synergy.md]]** — the motivational half of this page's social-agent system: perspective taking is present in great apes and in several non-ape species when tested competitively, so the agent primitive is not the human-specific part; what is human-specific is a disposition to deploy it cooperatively, maturing on a cross-culturally invariant schedule (pointing to inform ~12 months, belief–desire psychology ~4 years).
- **[[wiki/concepts/shared-intentionality.md]]** — the wiki's page for this one's unresolved seventh-system candidate: shared intentionality has the profile a core system is supposed to have (fixed ~9-month onset, no known environmental accelerator, autism as a selective deficit, nine joint-attention tasks emerging in developmental synchrony) and still no signature limit, so it sharpens the individuation problem above rather than resolving it.
- **[[wiki/concepts/developmental-heterochrony.md]]** — bears on this page's open question about what sets developmental tuning: the postnatal expression trajectory is a mammalian invariant in *shape* (human–mouse median `r = 0.83`) and evolves only in *timing*, so a competence can appear on a lineage with no new content and no new mechanism — only a shifted window.
- **[[wiki/concepts/relational-bottleneck.md]]** — declared orthogonal by its own authors, and composable rather than competing: the bottleneck accounts for domain-general relational induction and takes no position on innate object representations, while object-centric visual processing has already been stacked in front of it to supply the objects it compares (Webb et al. 2024).
- **[[wiki/concepts/inductive-bias.md]]** — the price list for what this page treats as given: a core system is a hypothesis space `H` selected over evolutionary time rather than estimated from an environment sample, which is why no task count `n` appears in the runtime cost of instantiating one — the sampling was paid for on a different timescale, and the genome bottleneck is a budget on `log C(ε, H)`.
- **[[wiki/concepts/numerosity.md]]** — the cellular realisation of this page's number system, and it converts two of the table's rows into measurements: modality-invariance becomes 11% of lateral-prefrontal cells tuned to the same count in vision *and* audition, and the format/precision split becomes a logarithmically compressed axis (the format, fixing tuning width by scale invariance) with the width itself set by flank-tuned lateral inhibition (the precision). It also weakens the strongest deflation of the installed reading — number-tuned cells are present in monkeys never trained to judge number (Nieder 2016).
- **[[wiki/concepts/active-vision.md]]** — the same small integer reached from the action side and with a cost function behind it: 2–3 simultaneously bound deictic pointers against the object system's 3–4 trackable entities, where the bound here is derived from `(MV)^s` search scaling rather than observed — and the measured human preference is to stay *below* it, at a cost of 1.0–1.5 s per block (Ballard et al. 1997).
- **[[wiki/concepts/visual-indices.md]]** — a candidate mechanism for the object system's small integer and its entry conditions: 3–4 trackable entities become a pool of 4–5 stimulus-assigned sticky pointers, cohesion and continuity become the assignment and maintenance rules (targets survive deletion/accretion along a contour and are lost to shrink-and-regrow at matched times), and infants' numerical sensitivity becomes individuation running before any property is encoded — with Chiang & Wynn 2000 (things seen poured or disassembled are not tracked) as the case where prior knowledge appears to decide what counts as an individual (Pylyshyn 2001).
- **[[wiki/entities/varc.md]]** — the weakest form in which the wiki has seen these priors installed and still enough for 60%: objectness and geometry enter only as an invariance group over an image plane (integer rescaling, translation, flips, rotations, colour permutations) plus a canvas, with no operation set, no object and no parse — and the one prior that has to be bought with data rather than architecture is *scale*.
- **[[wiki/concepts/explore-exploit-division-of-labour.md]]** — re-reads this page's nativism as a life-history strategy rather than a claim about content: rich domain-specific structure available before experience is the *precocial* program (galliforms, competent at hatching, brief juvenile period), which wins in constrained predictable environments and is dominated by extended exploration as environment dimensionality and variability rise — so "how much is built in" becomes a question about the environment the designer expects rather than about which core systems are real (Gopnik 2020).
