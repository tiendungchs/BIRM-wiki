...
- **[[wiki/concepts/cortical-state-bistability.md]]** — what the baseline is made of on one cortical patch: the same 1–9 Hz / 20–40 Hz alternation that gates perceptual update free-runs in resting state at a matched, gamma-distributed rate (1.2 s vs 1.54 s), so the idle mode here is an oscillator that was never started rather than a distinct network configuration.
- **[[wiki/entities/posterior-cingulate-cortex.md]]** — the vascular reading of this page's selectively vulnerable, 40%-above-mean region: it is among the most densely vascularised cortex in the brain and dual-supplied, so the high tonic metabolic rate and the near-absence of focal ischaemic damage are two faces of one arrangement — and the deactivation this page defines a zero for is, in the dorsal subregion, *required*, since inactivating it reduces learning.
