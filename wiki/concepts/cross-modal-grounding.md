...
- **[[wiki/concepts/representational-collapse.md]]** — supplies the derivation of the contrastive column (NCE as a substitute for intractable model samples) and the complicit instrument: `argmax`-scored caption discriminations return 100% to a fully collapsed model.
- **[[wiki/entities/raven.md]]** — the same species of benchmark-side defect outside vision-language: RAVEN's distractors are the correct answer with one attribute perturbed, so attribute-wise majority vote over the candidate set alone scores >90% without ever consulting the matrix — a sibling of this page's `argmax` tie-break and missing-negative-image channels.
- **[[wiki/concepts/rule-level-evaluation.md]]** — the inverse of this page's nouns-bind/relations-fail pattern, measured on ARC-format grids: frontier vision-language models state the intended *relational* rule on up to 40% of tasks and then mis-read the percept it applies to (visual-error rate 49–77%, grid dimensions the commonest failure), so rendering the task as an image costs 40–60 accuracy points relative to serialising it as text.
- **[[wiki/concepts/loopy-belief-propagation.md]]** — the non-contrastive alternative to fusion: treat each modality as a noisy channel carrying one shared latent word and decode by iterated extrinsic message passing, which yields per-channel noise models (hence modality-specific precision) where a contrastive objective yields a single shared rate bound.
- **[[wiki/entities/integrated-world-modeling-theory.md]]** — the architectural claim behind that alternative: modalities couple by *sharing* bottlenecks rather than by a fusion head, so a stream can be added or removed without retraining the others, and the payoff claimed is inferential synergy — a joint estimate strictly better than either stream reaches alone.
- **[[wiki/concepts/convergent-circuit-motifs.md]]** — a biological implementation of this page's binding criterion as a *coherence gate*: cells reported to fire maximally when visual and vibrational evidence agree in space and time, i.e. conjunction detection rather than a learned alignment objective `(tentative)`.
- **[[wiki/concepts/controlled-semantic-cognition.md]]** — the architectural question this page's objective-level analysis assumes away: whether the modalities meet in *one* transmodal store whose learned metric is the concept space, or connect pairwise with no shared address (`T331`). The clinical evidence for the store is a single lesion site producing impairment across every modality and category, and the graded-hub mechanism predicts this page's category-tilted failures from wiring distance alone, with no objective encoding them.
- **[[wiki/concepts/information-bottleneck.md]]** — the principle this page's rate–distortion statement is a generalisation of: a multimodal objective is a bottleneck whose relevance variable is the *other* modality, so the weaker channel caps `I(X̂;Y)` directly, and text randomization is lowering the rate on purpose rather than regularising.
- **[[wiki/entities/saycam-baby-vision.md]]** — the strict no-language baseline for this page's claims: SAYCam records audio and the study discards it, so whatever visual categories emerge there are what grounding gains must be priced *against*.
- **[[wiki/entities/blindtest.md]]** — localises the late-fusion cost this page's taxonomy leaves abstract: visual features are extracted before the question is seen, and a linear probe recovers ≥99.47% of what the full model then fails to report, both before and after the projection layer, so the defect sits in the encoder→decoder interface rather than in either component (`T388`).
- **[[wiki/entities/vdm-i2i.md]]** — this page's "a video model downgrades to an image model" concern with the caption channel *deliberately removed*: the text embedding is held neutral and uninformative, and what transfers is spatial layout rather than described content — while the paired VLM control (image adds nothing, image-only collapses to 0.00 on Sudoku) is the noun-bias failure at its limit.
