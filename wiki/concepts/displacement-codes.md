...
- **[[wiki/entities/visual-predictive-coder.md]]** — the same subtraction run on learned place-like blobs instead of on a grid module, which removes the assumption that the format must be periodic for the difference to mean anything; it is the agent-to-agent case, so the object-to-object case this page predicts is still unrecorded and now also unbuilt.
- **[[wiki/entities/cerebellum.md]]** — the competing consumer of the same efference copy: there a single subcortical forward model with no map turns the layer-5 motor output into a *predicted sensory consequence*, while here the identical signal is differenced against a location code to yield a *spatial relation* — so the two disagree about what the branch carries rather than about whether it exists (`T344`).
- **[[wiki/concepts/reference-frame-transformation.md]]** — the same operation with one end outside the store: differencing two object-anchored frames and converting a stored frame to a viewpoint are both "apply a group element to a code", and both are only defined once the two spaces are commensurate — which the banked implementation obtains by construction and the frame-to-frame version has to establish.
