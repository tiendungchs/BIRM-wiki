...
| **Quantiser over candidate trajectories** ([[wiki/entities/gcq.md]]) | The update is not run at all: the encoder emits `s_{1:n}` and is matched to `argmin_i ‖s_{1:n} − (e_i ⊕ a_{1:n−1})‖` over `K` bump-initialised trajectories | Hand-designed (toroidal CANN, frozen); only the encoder/decoder are trained | Path-consistency enforced *on a perception module* by a commitment loss rather than by a recurrence; `n` frames compress to one index per module; prediction error cannot compound because the latent is a pure function of the index and the action sum |
