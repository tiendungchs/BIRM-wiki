...
- **[[wiki/concepts/explore-exploit-division-of-labour.md]]** — the same word ("annealing") running on a different variable in the opposite direction: Bengio's `λ` raises the entropy of the *data* distribution over training, while the developmental schedule lowers the temperature of the *hypothesis search* over the lifespan — orthogonal levers that the continuation-method vocabulary conflates — and the developmental version violates weight-monotonicity outright, since adolescence re-heats the social domain while the physical one cools (Gopnik 2020).
- **[[wiki/concepts/self-boosting-knowledge-acquisition.md]]** — a third self-paced generator alongside learning progress and the 70–80% set-point, ordered by *structure* rather than by difficulty or by rate: take next the unheld item with the most edges into the store, which needs no teacher, no difficulty label and no competence estimate — and which predicts that a large domain's ordering keeps producing candidates where a progress-based one runs out (Murayama 2022, `G129`).
- **[[wiki/entities/asymmetric-self-play.md]]** — the automatic, human-free closed loop this page's open problems ask for, and the most damaging control in the wiki for its definition: four hand-designed curricula annealed by Automatic Domain Randomization (distance, rotation tolerance, pick-up and stack probabilities) acquire **no** pick-up or stacking skill, while an adversarial proposer whose entropy tracks the solver's failures solves all four holdouts and generalises zero-shot to table setting and tangram — so Bengio's two monotone conditions again select the family that does not pay (OpenAI et al. 2021).
- **[[wiki/concepts/realised-information-gain.md]]** — the self-paced `λ` at the earliest age it has been measured, and a warning about its form: the ordering signal driving 8-month-olds' engagement is a *retrospective* belief-update magnitude, `D_KL(p^j ‖ p^{j−1})`, not any forward estimate of an item's learnability — so the schedule it generates can only discover an easier item by sampling it, and it cannot satisfy this page's monotone-reweighting condition because nothing in it looks ahead. It also fails the entropy condition in the other direction: the gate is per-region and resets at every task boundary (Poli et al. 2020).
