...
- **[[wiki/concepts/broadcast-hierarchy.md]]** — a structural account of *why* the slow streams sit where they do: hypomyelination, low neurite density and convergent input give apex regions long integration windows, so timescale is set by conduction physics and receptive-field width rather than by a decay constant — and within one network the anterior (delta/theta-dominant) and posterior (alpha-dominant) complexes differ on exactly those anatomical variables.
- **[[wiki/concepts/transthalamic-context-routing.md]]** — a timescale split enforced by the *route* rather than by the unit or the receptive field: the direct cortico-cortical edge passes fast, tuned, time-locked signals and the parallel transthalamic edge passes slow, untuned, high-rate ones, with the open question being whether the split is a labelled line or a low-pass property of the thalamocortical synapse (Neske & Cardin 2025).
- **[[wiki/entities/lru.md]]** — the axis this page's banks do not have: every timescale split here (and in [[wiki/entities/ms-ssm.md]], [[wiki/entities/simple-cycle-reservoir.md]] Def. 7, [[wiki/entities/s4.md]]'s `Δ`) partitions eigenvalue **magnitude**, i.e. decay rate, while LRU shows the **phase** of the same eigenvalue independently sets *what* the channel stores — a kernel that oscillates many times over the window is a local average of an oscillation pattern rather than a global summary — and a uniform phase draw at `L = 16k` produces a model that trains but generalises at chance until the range is cut to `[0, π/10]`, which is then worth 94.2% on Path-X (Orvieto et al. 2023).
- **[[wiki/concepts/intrinsic-timescale-measurement.md]]** — the same question with a single-area instrument, and a disagreement about how many timescales an area has: that page fits *one* decay constant per area to its own spiking noise and gets a clean monotone hierarchy, this page fits an authored coalition alphabet to whole-brain signals and gets six concurrent streams — the only trace of multiplicity in the single-area measurement is the fit's offset `B`, which reports power beyond the probe window without resolving it.
- **[[wiki/concepts/timescale-hierarchy.md]]** — a circuit mechanism for the spread this page measures, and one that does not deliver the shared codebook: one scalar gradient of recurrent gain on a weighted connectome produces a *continuum* of localised timescales (fast eigenvectors at sensory areas, slow at frontal), so the timescale spread is explained while the shared 126-state alphabet and the asynchrony are not (Chaudhuri et al. 2015).
- **[[wiki/concepts/stationary-surrogate-null.md]]** — the null this page ran on its spatial claim but not on its temporal one: phase-randomised surrogates preserve spectra and cover the alphabet test, while transition matrices, occupancies and lifetimes are scored only against occupancy- and lifetime-preserving shuffles, which do not absorb the sampling variability that reproduces whole state sequences in stationary data (Laumann et al. 2017).
- **[[wiki/concepts/timescale-estimator-bias.md]]** — the same question (how many timescales does a signal have?) answered by a test rather than by an authored alphabet: a Bayes factor over one- versus two-timescale Ornstein–Uhlenbeck mixtures, `B₂₁(ε) = CDF_{M₂}(ε)/CDF_{M₁}(ε)`, which chooses **two** on macaque V4 spiking — agreeing with this page that one constant per area is too few, while reaching it from the signal's own autocorrelation with no state codebook assumed.
