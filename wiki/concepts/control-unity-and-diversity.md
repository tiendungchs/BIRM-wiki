...
### The cheapest version of those graph predictors, measured task-free at two ages

Eng et al. 2025 (`raw/eng-2025-pfc-connectivity-executive-function.md`) reduce the graph story to **one scalar** — the mean off-diagonal of a prefrontal module's own idle-state correlation matrix — and show it predicts control performance in both a 4–5-year-old and an 18–22-year-old sample. Functional near-infrared spectroscopy, 10 channels over Fp1–F8 (10–20 system), 45 channel-pairs, five minutes, Fisher-*z* averaged to a single number per subject.

| Measurement | Result |
|---|---|
| Adults (*n* = 30), mean prefrontal resting connectivity vs. Stroop incongruent reaction time | `r = −0.45` (naturalistic film), `r = −0.44` (static crosshair) — higher coupling, faster conflict resolution; not a speed–accuracy trade (accuracy↑ with speed, `r = −0.53`) |
| Children (*n* = 53 usable of 55, ages 4–5), same scalar vs. Day–Night incongruent accuracy | `r = +0.70` |
| Developmental difference in the scalar itself | children `0.57 ± 0.16` → adults `0.75 ± 0.26` |
| **Paradigm invariance** — blank crosshair vs. a five-minute nonsocial, nonverbal animation | Per-subject representational similarity over the 45-pair distance matrices: mean `r_z = 0.90` (range 0.64–0.97, every subject *p* < 0.001); scalar-to-scalar `r = 0.88` |

**The invariance row is the one a builder should keep.** The same coupling structure is recovered whether the system is staring at nothing or being driven by five minutes of moving imagery. That licenses treating idle-state covariance as a read-out of the **weights**, not of the stimulus currently being processed — which is precisely the assumption [[wiki/entities/fcann.md]] makes when it takes `J = −Σ⁻¹` from resting time series and then uses it as a fixed landscape that tasks merely *perturb*. Eng et al. supply the behavioural half of that assumption's warrant: the matrix is not just stable, its aggregate strength tracks how well the owner performs conflict tasks.

**(brainstorm) This is a training-free control probe and it costs one forward pass over noise.** Run a trained model with no task — random or naturalistic input, it does not matter per the invariance row — take the unit-by-unit correlation matrix of the putative control module, average the off-diagonal. If the biology's relation transfers, that scalar should rank instances of the same architecture by their control-task scores *before any control task is run*. It is the missing member of [[wiki/concepts/representation-probing.md]]'s instrument list: every probe there needs labels, a decoder, or a task; this one needs none. Cheap enough to run across a seed sweep, which is exactly the population the bifactor test above needs anyway.

**Three caveats that bound how far the scalar can be pushed.**

- *It is one scalar of a graph whose predictive content is supposed to be topological.* The modularity row above says control improves with **strong within-network and weak between-network** coupling. Eng et al. measure only the within-prefrontal term, so "more coupling is better" is established only inside a module — a system that raised the score by coupling everything to everything would, on the modularity result, get worse. Any machine use of this probe must report the between-module term alongside it or it is measuring the wrong direction.
- *Coverage is anterior prefrontal cortex only* — optical imaging reaches a few millimetres of cortex, so there is no subregional resolution to separate the dorsolateral/ventrolateral/orbitofrontal dissociations this page's lesion table rests on, and no subcortex at all.
- *One task per age group.* This is exactly the task-impurity confound the top of this page names: the adult correlation is with Stroop, the child correlation with Day–Night, and both are single measures rather than latent variables. That the association survives a change of task *and* a change of age group is the argument that it is not an artefact of either, but it is not the same thing as a correlation with the common factor.
...
- **[[wiki/entities/fcann.md]]** — supplies the behavioural warrant for that model's central assumption: the resting coupling matrix it treats as a fixed landscape is recovered near-identically under a blank fixation cross and under a five-minute animation (per-subject representational similarity `r_z = 0.90`), and its aggregate strength predicts conflict-task performance — so `J` is a property of the weights that tasks perturb, not of the stimulus (Eng et al. 2025).
- **[[wiki/concepts/representation-probing.md]]** — contributes the one instrument on that page's list that needs no label, decoder or task: the mean off-diagonal of a module's idle-state covariance, which tracks control performance in humans at ages 4–5 (`r = 0.70`) and 18–22 (`r = −0.45` against conflict reaction time) and rises across that developmental interval (Eng et al. 2025).
...
- **[[wiki/concepts/connectome-state-transformation.md]]** — the same bifactor count reached without any behavioural measurement: correlating the connectome-transformation coefficients across seven task states yields one state-general component, a handful that reorganise identically in every task, and a residue that reorganises differently in each — one common plus a few specific, from whole-brain connectivity alone.
- **[[wiki/concepts/emergent-modularity.md]]** — uses this page's components as the evolutionary knobs: Sherwood et al. 2008 propose that human domain-specific skills arise from shifted relative weights on attention, updating, working memory and inhibition during a prolonged ontogeny, which is only a coherent proposal because this page shows those components have separable genetic and pharmacological parameters rather than being one resource.
