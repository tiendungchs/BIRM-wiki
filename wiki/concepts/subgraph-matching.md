...
- **A hit is a yes/no, not an alignment.** The matcher reports that the query is contained in the target and never which target node plays which query role, so the retrieved structure arrives unanchored to the present situation (`G39`) and the node correspondence a schema needs is left to a separate step nobody has built.
...
- **[[wiki/concepts/node-definition-problem.md]]** — the biological case that forces this operation on cross-individual comparison: cortical area 55b splits or swaps position in 11% of subjects, so no smooth spatial warp can put the units in correspondence and matching must run on connectivity fingerprints — discrete graph matching above the unit, continuous alignment only inside it.
- **[[wiki/concepts/connectome-state-transformation.md]]** — the operation smuggled into a published method as an admitted expedient: rest-state and task-state principal components are two independently derived bases, and matching them greedily by maximal spatial similarity under an assumed one-to-one map is exactly this page's problem, with the authors stating outright that a task subsystem may be a merge or split of several rest subsystems.
- **[[wiki/entities/sme.md]]** — the classical alternative this page's cost argument is aimed at, priced from the primary source: structural consistency enforced as a bit-vector algebra (`Emaps`, `Conflicting`, `NoGood`) rather than as an embedding geometry, `O(N²)` up to an optional maximality merge, and it returns the correspondence plus a transferable remainder where containment returns a bit.
- **[[wiki/concepts/controlled-semantic-cognition.md]]** — this page's constraint failing online rather than in search: the semantic-aphasia fluency profile ("cat, dog, horse, saddle, whip") is a walk that leaves the task-specified subgraph after three steps, so semantic control is the operation that keeps a traversal inside a specified region — the same constraint applied step-by-step to a walk instead of all at once to a candidate match.
- **[[wiki/concepts/visual-routines.md]]** — the perceptual form of a connectivity query, with a cost curve in the other currency: boundary tracing answers "same curve?" in time linear in path length (~24 ms/degree) and, if activation spreads along many contours at once, near-independent of the number of distractor paths.
- **[[wiki/concepts/incremental-grouping.md]]** — the connectivity query run as a physical flood rather than as a match, with one addition this page has no analogue for: the traversal *step size* adapts to local clutter, so cost is linear in distance travelled but the distance-per-step rises wherever no distractor is close enough to cause confusion.
