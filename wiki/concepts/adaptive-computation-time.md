...
- **[[wiki/entities/rims.md]]** — the same conditional-computation question rotated onto the width axis, with the halting test present and thrown away: each module scores the input against an appended null row, which is a per-module "is there anything here for me" test exactly analogous to `h^n_t`, but the architecture then takes a fixed top-`k_A` over the result and so hand-sets the budget this page learns — selecting every module that beats its own null row would make the active-set size adaptive per step at no ponder cost, and nobody has run it (Goyal et al. 2019).
- **[[wiki/concepts/controlled-semantic-cognition.md]]** — the same halting decision with the trigger moved into the representation: semantic control is recruited by ambiguity, weak encoding or conflict with context in the store itself rather than by a separate difficulty estimator, so "when to think harder" becomes a property of the activation pattern — and the framework's own admission that what recruits control is unknown is this page's missing stopping rule stated in neural terms.
- **[[wiki/entities/recursive-npi.md]]** — the non-differentiable rival for unbounded iteration, and the contrast that names what this mechanism lacks: ACT repeats a transition inside one accumulating state (and then mean-fields the intermediates together), while a recursive call opens a *fresh* state with the caller's context on a stack — which is what makes the space of reachable inputs to one call finite, and hence provable, where ponder traces can only be inspected.
- **[[wiki/concepts/visual-routines.md]]** — supplies the theoretical reason a learned step count is needed rather than merely efficient (connectivity predicates are denied to order-limited fixed-depth readers) and a biological ponder curve to match: internal contour tracing costs ~24 ms per degree of visual angle, linear in path length along the curve.
- **[[wiki/concepts/incremental-grouping.md]]** — the one variable-depth computation in the wiki with an external cost curve to match: the step count is set by stimulus geometry rather than by a learned halting unit, and it is scored by how well it predicts human reaction times (`R² = 0.55` against a `0.67` noise ceiling on scrambled shapes) rather than by accuracy at a compute budget.
- **[[wiki/entities/blindtest.md]]** — the negative control for a ponder budget: slow-thinking VLMs are no better than their base models on low-level visual tasks (71.59 vs 72.75; 42.48 vs 54.13) because the extra steps run in token space over a failure that lives in an image-addressed representation, so an input-dependent step count only buys something when the steps run over the representation holding the answer.
- **[[wiki/entities/multiscale-tracing-network.md]]** — the implementation behind that cost curve, and the reason its step count needs no halting unit at all: the number of steps is however long a disinhibitory tag takes to flood the figure through a scale hierarchy, so compute is set by stimulus geometry rather than learned, and it extrapolates from 7 px of training width to 30 px at 100% (Mollard, Bohte & Roelfsema 2026).
- **[[wiki/concepts/learned-industriousness.md]]** — the biological denial of this page's fixed ponder penalty: the price charged for exertion is a conditioned quantity, so an agent whose extra computation reliably paid off should come to charge itself less for depth — everywhere at once, including on tasks never trained. This page's `τ` is the same on the last episode as on the first, which makes every adaptive-compute result here a policy over a fixed price where biology adjusts the price (`G128`).
