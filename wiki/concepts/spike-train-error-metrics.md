...
- **Still one postsynaptic neuron.** Every rule in the inventory, derived or heuristic, trains a single output neuron from many inputs. Deep credit assignment in the timing domain is untouched by the whole family — the residue of `G76` after this source ([[wiki/concepts/biologically-plausible-credit-assignment.md]]).
