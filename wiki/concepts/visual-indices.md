...
- **[[wiki/entities/blindtest.md]]** — the machine limit that lands near this page's number without being this page's mechanism: VLMs count 5 overlapping circles at ≥83% and collapse at 6, but the 5 is a retrieved Olympic logo (Gemini-1.5 answers "5" on 99.74% of its errors) and the same models are worse on pentagons at every `N`, while *nested* squares — the arrangement Trick & Pylyshyn show humans cannot subitize — are the counting task they do best.
- **[[wiki/entities/varc.md]]** — what a solver looks like with no indices at all: the copy relation is a pixel-to-pixel attention edge recomputed in every forward pass, so nothing is individuated, nothing persists across the computation, and nothing can be counted or re-referred to — and it still reaches the reported average human on ARC-AGI-1, which bounds how much of that benchmark needs a referring device.
- **[[wiki/entities/multiscale-tracing-network.md]]** — the running implementation of the complementary mechanism, which makes the unbound pair concrete: the tag establishes content-free *sameness* over locations and individuates nothing, where an index individuates and describes nothing, and the model has no term into which an index could be written.
