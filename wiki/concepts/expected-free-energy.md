...
- **[[wiki/concepts/intrinsic-motivation-typology.md]]** — where this objective sits in the space of intrinsic rewards: a preference term plus a state-entropy term places it between the uncertainty and information-gain cells, and the derivation supplies exactly the thing that typology cannot — a weight for combining the epistemic term with the pragmatic one, since a typology of reward formulas offers no principle for mixing two of them.
- **[[wiki/concepts/information-sampling-vs-search.md]]** — the tractability objection to this objective, stated from the curiosity literature: the derivation assigns value over a hypothesis space it never counts ("a decision problem with two alternatives is not distinguished from one with 2¹⁰⁰"), so it scales poorly exactly in the open-ended regime it is most often invoked for — and the proposed replacement there is a set of non-commensurable heuristics, which is what a single derived objective is designed to rule out (Gottlieb & Oudeyer 2018).
