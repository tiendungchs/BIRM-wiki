...
### The silent store measured in humans, by driving it with an input it does not care about

Everything above is a model or a monkey. Wolff et al. 2017 (`raw/wolff-2017-dynamic-hidden-states-working-memory.md`, Nat. Neurosci. 20:864–871) run the activity-silent design against human EEG with a **perturbation read-out**: during the delay, flash a fixed high-contrast stimulus carrying no task content — the *impulse*, or "ping" — and decode the memorandum from the response to it ([[wiki/concepts/representation-probing.md]]). Orientation gratings, parametric Mahalanobis decoding, three experiments.

| Experiment | Manipulation | Delay activity | Impulse response |
|---|---|---|---|
| 1 (`n` = 30) | retro-cue: one of two items becomes irrelevant mid-delay | alpha lateralises with the cue (attention shift confirmed) | **cued item decodable**, 148–398 ms (`P` = 0.002); **uncued item at chance** (average `P` = 0.687) |
| 2 (`n` = 19) | both items relevant, probed in a fixed order, so one is *unattended but still required* | prioritised item decodable to end of epoch; **deprioritised item falls to chance** | **both decodable** at the first ping (attended `P` < 0.001; unattended `P` = 0.045), attended higher (`P` = 0.031) |
| 2, second ping | first item already probed and now useless | — | now-irrelevant item **fails to reach significance** (`P` = 0.112); still-relevant item decodable (`P` = 0.012) |
| 3 (`n` = 20, behaviour only) | impulse–probe stimulus-onset asynchrony 0/50/100/250/500 ms | — | **no accuracy effect** (`F`(4,76) = 1.184, `P` = 0.325) |

Four things this adds to the rows above, in descending order of how much they change a design.

- **Attention and contents are two different variables with two different read-outs.** Standard delay decoding tracks the *focus of attention* — the unattended item is at chance there while still being reported correctly at the end of the trial. The impulse response tracks what is *in the store*, attended or not, and trial-wise alpha lateralisation (the attention index) does not predict trial-wise impulse decodability at all. This is the human, non-invasive form of the pointer/content split ([[wiki/concepts/memory-read-and-erase.md]], gap G48): the two signals are separable by *which instrument you use*, not by which region you record from.
- **The read is not a reinstatement.** A decoder trained on the encoding epoch does not transfer to the impulse epoch in either direction (all corrected clusters `P` > 0.32). The hidden state does not replay the item's encoding pattern; the fixed input is *transformed by* the state, and the memorandum is recoverable from the transform. **(brainstorm)** For a machine store this is a read with a signature no wiki store exposes: `read(M) = f(M, u₀)` for a fixed, content-free `u₀`, decoded downstream — the store never emits the item in the item's own basis, and the "key" carries no information. It is the store as a *filter* rather than as a table, which is what the dynamic-coding framework claims and what a match-filter read-out would need ([[wiki/concepts/fast-weight-programming.md]] is the closest machine object, since `W` there is also read by pushing a vector through it).
- **Erase is fast, complete, and instruction-driven.** One retro-cue is enough to leave *no* decodable trace of the discarded item — and the same wipe happens in Experiment 2 to an item that has simply been used. This is a relevance-addressed clear measured on the content rather than on the control signal ([[wiki/concepts/memory-read-and-erase.md]], gap G49), and it puts a hard constraint on the carrier: whatever holds the trace must be erasable in under ~1 s by a top-down instruction, which a `τ_f` decay constant cannot do.
- **Trace strength buys *precision*, not presence.** Trial-wise decoding predicts accuracy (`P` = 0.043), and fitting the behavioural profile over probe angle localises the effect to the slope parameter (`P` = 0.023) with no effect on the guess-rate asymptote (`P` = 0.867; replicated in Experiment 2, `P` = 0.006 vs. `P` = 0.942). A weak hidden state yields a *blurred* item, not a missing one — so the store is graded, not slot-like, on the dimension its own read-out measures.

**The instrument's warrant, and its weak point.** Experiment 3 is the control that makes the other two readable: if the ping had *reactivated* the latent item into an active state, the whole result would be a retro-cue effect in disguise. It found no behavioural consequence at any stimulus-onset asynchrony — but it is a **null**, `n` = 20, 280 trials, a single angle difference (±16°), with no positive control establishing that the design could have detected a reactivation had one occurred (gap G113). Everything downstream of "the ping reads without writing" inherits that null's power.

**What it does and does not settle for `G112`.** The ping is a *decoding* instrument, not a behavioural protocol: Experiment 3's whole point is that the manipulation leaves behaviour unchanged, so the carrier signal exists only in the decoded response and never in the scored output. `G112` therefore stays open — this is a fourth non-behavioural instrument beside the neuron-budget sweep, cross-task decoding and slot algebra, and much the cheapest of them, but it is on the same side of the line. Note also that the opposing synaptic-theory position (Mongillo et al. 2008) is not itself in the wiki, so the duality argument still has only one half in hand.
...
**And a fourth possibility: the contents are not most readable where the delay signal is.** Emrich et al. 2013, as reported by Boly et al. 2017 (`raw/boly-2017-ncc-front-or-back.md`): the identity of the held item decodes well from the *posterior* region of interest activated during the sample period and poorly from the frontoparietal region of interest activated during the *delay*, with occipital beating parietal within the posterior set. So the region whose sustained activity defines the store on every design above is not the region from which the stored content is most decodable — the delay signal and the delay contents are in different places, which is what a pointer-plus-sensory-storage architecture predicts and what a slot in prefrontal cortex does not ([[wiki/concepts/constitutive-vs-enabling.md]], [[wiki/empirical-tensions.md]] T88, T92).
...
| **Hold `4 items + 3 relations` vs hold `7 items`** (Wendelken et al. 2008a, [[wiki/entities/lateral-frontoparietal-network.md]]) | Rostrolateral prefrontal cortex **higher for the relational load**, and **flat between 7 and 4 items**; dorsolateral prefrontal cortex scales with the item count and does not separate the two conditions |

The last row is the one that makes this a claim about *currency* rather than about difficulty: three relations over four items are the cheaper condition behaviourally and the more expensive one for the integrator, so an item-count capacity and a relational capacity are not two readings of one resource, and a system can be well inside its item budget and outside its binding budget.

Two things follow for a builder. **(1) The bound is on simultaneous *bindings*, not on stored symbols** — [[wiki/entities/lisa.md]] derives it rather than stipulating it, by carrying role-filler bindings in mutually out-of-phase time slots, so ~2–3 propositions can be live and a complex mapping must be serialised across load cycles.

**The derivation, written out** (Knowlton, Hummel & Holyoak 2012). The phase-set size is a **ratio of two time constants** — `period of the carrier / duration of one phase` — with the carrier taken as gamma (> 30 Hz, one burst per population every ≈25 ms) and one phase holding one role binding: **4–6 bindings ≈ 2–3 propositions**. This is the only capacity number in the wiki that names the quantities you would have to change to move it, and it makes two predictions the slot accounts do not: capacity should rise with a *slower* carrier or with *sharper* population timing precision, and the same limit should appear in low-level posterior tasks that bind by synchrony, since nothing in the arithmetic is prefrontal. It competes directly with the noise-driven-attractor derivation of `7 ± 2` above ([[wiki/empirical-tensions.md]] T32) — the two put the same behavioural constant on opposite mechanisms, one needing an oscillatory scaffold and one forbidding a clock. **(2) Exceeding the bound does not produce a miss, it produces a confident wrong answer** drawn from direct similarity — the degradation mode is a fallback onto the cheaper route, which no capacity curve in this page's designs predicts and every one of them could be tested for.

---

## The carrier is a nested rhythm, and which rhythm frames it sets the slot count

> Roux & Uhlhaas 2014 (`raw/roux-2014-wm-oscillations-alpha-gamma-theta-gamma.md`), an opinion piece over ~30 EEG/MEG/ECoG delay-activity studies. The band claims are governed by [[wiki/concepts/inter-areal-synchrony.md]]; what is new here is that they are assigned to *subprocesses of this page's store*.

| Band | Job during the delay | Diagnostic result | What it is *not* |
|---|---|---|---|
| **gamma** 30–100 Hz (parvalbumin-interneuron/AMPA–NMDA loop; superficial layers) | the generic **maintenance carrier** — one item, any modality, any format | amplitude parametric in item count; single-trial gamma predicts how many items are held; different gamma frequencies carry different items | not content-typed: the same carrier serves auditory, visual, somatosensory and spatial material |
| **alpha** 8–13 Hz (thalamic generators; layer V) | **protection** — inhibition of task-irrelevant regions | lateralised to the *irrelevant* hemifield and scaled by the number of **distractors**, not of targets (Sauseng et al.); 10–14 Hz delay power correlates *positively* with reaction time | not a store, on this reading it carries no WM content at all |
| **theta** 4–8 Hz (septal pacemaker; hippocampal–entorhinal and prefrontal) | **temporal ordering** of items | preferentially present in sequential tasks (Sternberg); frontal power parametric in load | not maintenance itself |

**The design that makes the assignment identifiable.** Varying load alone confounds items held with attention demanded, so no load curve can separate a carrier from a filter. Holding the relevant items fixed and varying the distractors does: 3 relevant vs 3 relevant + 3 irrelevant vs 6 relevant. Only 60–80 Hz amplitude (parietal and prefrontal sources) tracked the count of *relevant* items; 10–14 Hz did not, and localised to premotor cortex — read as suppression of motor plans during the delay. **The alpha half is causal, and it says part of the capacity limit is a distractor problem**: 10 Hz repetitive transcranial magnetic stimulation over the *task-irrelevant* hemisphere **raised** WM capacity (Sauseng et al.). No account on this page — slots, facilitated populations, softmax competition, stability margin — predicts that driving inhibition into a region holding nothing increases the span.

**Capacity as a duty cycle.** If one gamma cycle is one slot and a slower rhythm supplies the frame, the slot count is a frequency ratio and nothing else: `slots = period of frame / period of gamma`. **(brainstorm)** The arithmetic the source leaves implicit: a 7 Hz theta frame (143 ms) holds ≈5–6 of the ≈25 ms gamma cycles, a 10 Hz alpha frame (100 ms) holds ≈4 — which is offered as the origin of the psychological finding that *sequentially* presented material has a higher span than *simultaneously* presented material. The two are the same store with a different frame.

**The frame is selected by the format of the content, not by the task's difficulty.**

| Material | Frame | Where the coupling is measured | Cognitive-model counterpart |
|---|---|---|---|
| Sequentially ordered items (Sternberg, verbal) | theta–gamma | fronto-temporal, hippocampal — load-dependent theta–gamma phase–amplitude coupling in human hippocampus (Axmacher et al.); coupling enhanced for *correct* items (Holz et al.) | phonological loop |
| Simultaneous visual/spatial arrays | alpha–gamma | parieto-occipital; thalamo-cortical | visuospatial sketchpad |

Switching the material from ordered items to a discrete visual array **replaces delay theta with delay alpha, at different electrode sites** — so the store's code and the network carrying it are both chosen at runtime by what is being held.

Three things this changes for fast **M**:

- **The slot count becomes a settable parameter rather than a constant.** Every capacity account above ties the ceiling to a fixed property (states, populations, softmax mass, a stability margin); here it is the ratio of two clocks, either of which the system may change between trials. A machine analogue is direct — a frame counter whose period is chosen by a content-format classifier — and no design in the wiki has one.
- **The nesting is inverted relative to [[wiki/entities/lisa.md]]**, which takes gamma as the *frame* and a sub-gamma phase as the slot, and reaches the same 4–6. The two derivations therefore disagree about the lever: LISA raises capacity by slowing the carrier or sharpening population timing, this raises it by choosing a slower *frame* — and only the second predicts a capacity difference between sequential and simultaneous material, which is observed. Both are Position A of [[wiki/empirical-tensions.md]] T32 against the clock-free `7 ± 2` derivation.
- **Protection and storage are separate budgets.** Distractor count drives one band and target count another, so "how much can be held" and "how much can be kept out" are separately measurable and separately manipulable — a split none of this page's designs makes, and the one the rTMS result exploits.

**What the framework does not have.** The alpha–gamma code is *unmeasured* — the authors state no DMS study has looked for it, so half the proposal rests on the theta–gamma half plus a band substitution. The functional reading of alpha is itself contested in the same literature (inhibition of irrelevant regions vs. direct maintenance of WM content), with source-localisation outside the WM network the main evidence for the first. Beta is unassigned. And whether long-range synchrony and cross-frequency coupling are one mechanism or two is listed as open.
...
- **Stability and flexibility may not be separable knobs.** In a persistent-activity circuit, the amplitude of the synaptic drive shifts the *induction* threshold and the *disruption* threshold in the same direction, so "easy to write" and "easy to clobber" are one parameter — and it is the same parameter that sets how long the attached decision circuit integrates ([[wiki/concepts/excitation-inhibition-balance.md]], Schirner et al. 2023). No store in the wiki reports these two thresholds separately, so none can say whether it has the same coupling.
...
- **[[wiki/concepts/inhibitory-control-of-coding.md]]** — supplies the channel that decides *when* a held item is expressed: prefrontal beta bursts suppress gamma and informative spiking at exactly the sites that carry information, drop when a read is due and rise when the content stops being needed, so "which item is readable right now" is set by an inhibitory rhythm rather than by the store (Lundqvist et al. 2018) — with alpha as the same trick addressed by *place* rather than by time: it scales with the distractor count, sits over the task-irrelevant hemifield, and driving it there with 10 Hz stimulation raises this page's capacity (Roux & Uhlhaas 2014).
- **[[wiki/concepts/population-geometry.md]]** — the level at which this page's store can be read at all: when two same-type variables share one prefrontal population (the held item and the attended location), what keeps both decodable is anti-aligned mixed selectivity in hybrid cells, not separate cells for each (Lebedev et al. 2004) — and the delay-period code has a failure mode no capacity or content measure sees: on error trials the held items stay fully decodable while the population's dimensionality collapses, the collapse being specifically a loss of non-linear mixing (Rigotti et al. 2013).
- **[[wiki/concepts/representation-probing.md]]** — supplies the instrument behind this page's removal-operations table (per-site representational similarity matrices clustered by their mutual similarity), and takes back the lesson that a distinction can exist in one subsystem and not another: four cortical communities represent the same four operations under four different quotient maps, and only the frontoparietal control network keeps all four apart (DeRosa et al. 2024). It also takes from this page the one instrument that can read a carrier holding no activity at all — drive the store with a fixed content-free input and decode the *response* — together with the condition that makes it an instrument rather than an edit (Wolff et al. 2017, G113).
...
- **[[wiki/concepts/inter-areal-synchrony.md]]** — the measurement layer under this page's phase dissociation: the encoding requirement lives on the direct link (gamma, abolished by terminal silencing) and the choice-point requirement on an indirect route (theta, untouched), so a spatial working-memory deficit can be a lesion of either pathway or of the clock that times them — and it is where this page's band assignments are licensed, including the rule that the frame band identifies the *format* of what is held (theta–gamma fronto-temporal for sequential material, alpha–gamma parieto-occipital for visual arrays; Roux & Uhlhaas 2014).
...
- **[[wiki/entities/fluxx.md]]** — the reference system's capacity limit read off a domain built to stress it: ten to eleven face-up, English-language rules exceed what experienced players combine correctly, and one of them created a live win condition that nobody noticed for several turns, so tracking a legible rule-config is a maintenance cost and not a perception cost `(tentative)`.
- **[[wiki/entities/nomic.md]]** — a rule-set designer's own capacity ceiling, set by fiat: at most 25 concurrent mutable rules, roughly 2.5× the only human tracking anchor in the wiki (10–11 live legible rules, `(tentative)`, [[wiki/entities/fluxx.md]]).
- **[[wiki/entities/spiking-neural-networks.md]]** — where a coarse spiking code fails first, and it is exactly this page's component: porting an LSTM to TrueNorth, every variable tolerated rate coding *except the cell state*, which needed a spike-burst code for the extra precision — so maintenance, not computation, sets the substrate's precision requirement (Shrestha et al. 2017, via Tavanaei et al. 2019).
- **[[wiki/concepts/spike-frequency-adaptation.md]]** — a third maintenance design beside persistent firing and activity-silent synaptic traces: the item is held in a per-cell slow conductance and read with **no retrieval operation at all**, because the memory *is* the depressed firing rate the next input meets; span, interference and gradient flow are all set by one hand-tuned time constant (G78).
- **[[wiki/entities/liquid-state-machine.md]]** — the strongest fading-memory horizon in the wiki and the proof that it stops there: four readouts recover which template generated each of four independent 250 ms segments of a spike train at `t = 1000 ms`, so a 750 ms-old input survives three uncorrelated overwriting segments in a circuit with `τ_m = 30 ms` — while that framework's theorem 1 converse says an LSM approximates a filter *iff* it has fading memory, so no amount of liquid complexity converts the horizon into a non-fading store.
- **[[wiki/concepts/cross-paradigm-interface.md]]** — a working memory whose contents are **edges** rather than activity: a graph of integrate-and-fire symbol nodes initialised from long-term knowledge, filled by Hebbian binding from perception, and read by instruction-driven spike propagation — offered as a simplified model of prefrontal working memory coordinating symbolic operations with sensory grounding, and scoring 78.8–95.3% on CLEVRER's four question types (Zhao et al. 2022).
- **[[wiki/concepts/excitation-inhibition-balance.md]]** — the mechanism that collapses stability and flexibility into one parameter: lowering the amplitude of a persistent-activity circuit's synaptic drive raises the threshold to *induce* a memory and the threshold to *disrupt* it together, so a store that is hard to write is hard to overwrite — and it is the same variable that lengthens the evidence-integration time of the decision circuit sharing that drive (Schirner et al. 2023).
...
- **[[wiki/concepts/relational-reinterpretation.md]]** — why the capacity bound is the bound on *reasoning* and not merely on storage: every property above the human/nonhuman line requires several constituents to stay simultaneously and separably addressable, so relational complexity (relations that must be jointly bound) is the load variable rather than item count.
- **[[wiki/entities/lru.md]]** — the horizon/variable separation at its cheapest: 16,384-step dependencies fall to `N` independent scalar complex exponentials with `2N` recurrent parameters, three hand-set initialisation scalars, no gate, no write rule and no addressable slot — so a long horizon is not merely achievable without working memory ([[wiki/entities/s4.md]]) but nearly free, and what stays diagnostic is holding a value against interference and reading it on demand.
- **[[wiki/concepts/constitutive-vs-enabling.md]]** — the partition that reads the delay-period literature's confounds as one problem: attention, expectation and report are modulator and readout cells that co-vary with the held item in nearly every task design, and working-memory contents decode better from the posterior sample region than from the frontoparietal delay region (Emrich et al. 2013).
- **[[wiki/concepts/tensor-product-representation.md]]** — the capacity axis this page collapses into one number: how many bindings can be *generated* per step is sharp and set by parallel binder hardware, how many can be *maintained* is graded and set by the role-space dimension (`~√N`), and the two are independent only in a fully distributed code — a slot architecture forces them equal. The reported human asymmetry is `N ≪ n`, which is what illusory conjunctions under time pressure measure (Treisman & Schmidt 1982, reported via Smolensky 1990).
- **[[wiki/concepts/timescale-hierarchy.md]]** — where the capacity to hold anything comes from, allocated structurally: persistent activity appears only above a threshold in recurrent excitatory gain, that gain is a scalar function of hierarchical position, and the timescale of *small* perturbations around baseline predicts which areas have the capacity before it is ever used (Chaudhuri et al. 2015).
- **[[wiki/entities/cpc.md]]** — an auxiliary objective that doubles as a memory-demand detector: predicting 30 steps ahead in latent space improves 4 of 5 DeepMind Lab tasks and does exactly nothing on the one whose optimal policy is purely reactive.
- **[[wiki/entities/rims.md]]** — a store whose slots carry **their own transition dynamics** instead of being passive registers: an unselected module is untouched memory whose state persists by identity and is still readable by the selected modules through attention, so maintenance and computation share one substrate and the capacity limit is a hand-set active-set size `k_A` rather than an emergent one (Goyal et al. 2019).
- **[[wiki/entities/esbn.md]]** — a store its own authors classify as *episodic* rather than working memory: bindings are semi-permanent rows disambiguated by retrieval and context rather than maintained by activity, which is why no unbinding mechanism appears anywhere in the architecture.
- **[[wiki/concepts/intrinsic-timescale-measurement.md]]** — a horizon read-out that does not require using the store: in macaque prefrontal cortex the decay constant of seconds-long reward memory traces is ordered by a hundreds-of-milliseconds noise statistic taken with no task at all (LIP < LPFC < ACC in both), so a module's maintenance capacity may be rankable before it is exercised — and the offset of the same fit reports variance persisting *across trials*, i.e. past the boundary a store is usually assumed to reset at.
- **[[wiki/concepts/output-null-subspace.md]]** — the gate-free alternative to this page's contested gating anatomy: delay activity that cannot act because it lies in the read-out's kernel rather than because a basal-ganglia gate is closed, which removes the synchronisation requirement every design here carries and re-types the stored item as a **pre-computed initial condition** to be released by subtracting an input rather than as content to be read out `(tentative)`.
- **[[wiki/entities/lateral-frontoparietal-network.md]]** — the design that shows items and relations are separate load currencies: holding 4 items + 3 relations engages rostrolateral prefrontal cortex more than holding 7 items does, while dorsolateral prefrontal cortex scales with the item load and does not separate the two, so a capacity stated in items does not bound relational integration (Wendelken et al. 2008a, via Vendetti & Bunge 2014).
- **[[wiki/concepts/relational-bottleneck.md]]** — a *normative* origin for this page's capacity limit rather than another mechanism for it: the bound is neither a slot count nor a noise floor but the interference cost of binding over a compositional vocabulary that is shared by definition, which predicts the same limit shape for subitizing and absolute judgment and makes it inescapable for any architecture that gains flexibility by sharing representations (Frankland et al. 2021, via Webb et al. 2024).
- **[[wiki/concepts/activity-slots.md]]** — the mechanism behind this page's prefrontal subspace results, and a re-typing of its central split: the concurrent orthogonal delay subspaces are *slots* whose contents are copied and shifted past a fixed read-out by a velocity signal, so "no synaptic plasticity is needed to hold a novel binding" becomes a derived property rather than an assumption — and the store's capacity ceiling is a neuron budget, `O(|states|²)`, not a slot count. It also removes a whole class of evidence from this page: the activity solution and the synaptic solution are proven to be the same function on the whole task family, so no delay task can say which carrier a system uses (`G112`; Whittington et al. 2025).
- **[[wiki/entities/recursive-npi.md]]** — the case where *scoped forgetting* is the mechanism rather than the failure: zeroing the controller's hidden state at every subprogram call, with the caller's context pushed on a stack, is what removes the length dependence from a learned program — a memory contract of nested fresh states instead of one continuing state.
- **[[wiki/concepts/perturbation-elicitability.md]]** — the instrument that reads this page's silent carrier, run in the direction that page treats as failure: a content-free impulse delivered through the sensory port recovers the memorandum from the evoked response precisely *because* the perturbation changes no reportable content, so a low elicitation rate is the precondition for a clean read rather than a limit on the method (Wolff et al. 2017).
- **[[wiki/entities/dendritic-integration-theory.md]]** — a maintenance substrate that is neither persistent cortical firing nor a synaptic trace: an excitatory recurrent loop between a column's layer-5 pyramids and the matching higher-order thalamic neurons, where the structure that holds the content is also the one that licenses its integration.
- **[[wiki/entities/bb-model.md]]** — a systems model that names this page's function as its own hole: egocentric scene representations persist only while something drives them, so anything held across a gap has to be maintained by a mechanism the model explicitly declines to include — and the review-level proposal that the frame converter doubles as a short-term buffer is the alternative it does not take ([[wiki/concepts/reference-frame-transformation.md]]).
- **[[wiki/entities/subiculum.md]]** — the hippocampal exit assigned a task phase by cell class: bilateral chemogenetic silencing of the *distal* subicular class abolishes the **encoding** of a new match-to-sample water-maze goal while leaving **retrieval** of the previous day's goal intact, and silencing the entire *proximal* class — the origin of the subiculum→prefrontal wire — costs neither phase, so the "which phase does the store's output matter for" question (`T100`) is answered per output class rather than per region (Cembrowski et al. 2018).
- **[[wiki/concepts/numerosity.md]]** — a distractor inserted into the delay of a delayed-match-to-number task reverses this page's usual region assignment: the lateral-prefrontal representation is **overwritten** by the distractor and regenerated in the second delay, while the ventral-intraparietal one is unaffected — read as prefrontal cortex being a selection stage that must represent the irrelevant item in order to filter it, with maintenance living parietally (`T385`). It also supplies a microcircuit for tuned persistent activity: recurrent excitation among equally-tuned pyramidal cells carries the persistence, flank-tuned interneurons carry the selectivity (Nieder 2016).
- **[[wiki/concepts/mental-imagery.md]]** — splits visual working memory into two mechanisms that performance alone cannot distinguish: only good imagers are impaired by irrelevant visual input during the delay, and aphantasics pass easy and medium loads with no sensory render at all — so a V1 decode of delay content may be a measurement of one subpopulation generalised to the task (`T386`).
- **[[wiki/concepts/visual-routines.md]]** — names a store none of this page's carriers is: the *incremental representation* is goal-dependent (same input, different question, different contents), addressed by location in the base representation, and holds **partial** parses rather than completed items — the thing a KV cache is not, because a cache is input-determined.
- **[[wiki/concepts/visual-indices.md]]** — the pointer half of this page's stores isolated to the point of holding nothing: a FINST is a reference with no content register, so its 4–5 limit is a bound on *how many things can be referred to* rather than on how much can be held, and it is lost the moment its referent leaves the field — a working parse, not a memory (Pylyshyn 2001, `G48`).
- **[[wiki/concepts/effort-based-decision-making.md]]** — the account that makes this page's capacity limit the *source* of a price rather than only a bound: allocation to any goal precludes allocation to every other, so the cost of control is the value of the next-best use of the store, and subjective effort is the felt representation of that opportunity cost. Two consequences. The limit becomes a decision variable — the same task is worth running under one set of alternatives and not another with the store's capacity unchanged — and the metabolic rival is ruled out on this page's behalf, since global cerebral glucose consumption rises ≤ ~1% during vigorous engagement (Westbrook & Braver 2015).
