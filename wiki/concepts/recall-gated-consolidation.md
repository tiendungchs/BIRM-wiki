...
- **[[wiki/entities/medial-prefrontal-cortex.md]]** — the same gating logic with the two stores swapped: successful cortical retrieval is proposed to suppress hippocampal activity so a known episode is not re-encoded, which is this page's valve pointed at the fast store's write instead of the slow store's.
