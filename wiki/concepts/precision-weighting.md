...
- **[[wiki/concepts/metacognitive-efficiency.md]]** — a confidence report is precision read out and reported, and that report factors into a gain term (bias — where the confidence criteria sit) and an information term (sensitivity — whether correct and incorrect outputs are separable at all); only the second says the precision was ever computed, and the wiki's three confidence numbers are all the first or an uncorrected version of the second.
- **[[wiki/concepts/selective-prediction.md]]** — weakens what a precision estimate has to achieve to be usable: a selection function needs `κ_f` to be **ordinally** correct in the loss, not numerically correct as a probability, so a miscalibrated precision that still ranks is sufficient for abstention and insufficient for anything that arithmetically combines precisions.
- **[[wiki/concepts/confidence-calibration.md]]** — a measured precision register in the cheapest possible form: `1/T` is a single global gain on the logits, and the finding that a full diagonal gain (vector scaling) degenerates to a near-constant vector says a trained network's precision defect is **rank-1**, which is this page's low-rank-register argument confirmed from the machine side; the optimal `T` also drifts upward through training, so a fixed gain is wrong even for a stationary task.
- **[[wiki/concepts/prediction-error-neurons.md]]** — supplies the behavioural condition that turns `Π` from a free parameter into an experimental target: the gate must act on the error→representation *coupling* and must be movement-correlated, because passive observation of a conspecific's vocalisation demonstrably fails to update the motor program — so a scheme with no gate cannot explain why listening is safe.
- **[[wiki/entities/a24b-m2-v1-projection.md]]** — separates `Π` from the channel it is often conflated with: a gain can legitimately be low-dimensional and neuromodulatory, but the *content* prediction cannot, and this projection's bandwidth, density, topography and state-dependence are exactly the design that distinguishes the two hypotheses experimentally.
- **[[wiki/concepts/affective-opponency.md]]** — an unoccupied slot this page's quantity fits: two opponent populations with non-zero baselines constrain only their *difference* by value, leaving the **sum** free to carry a second non-negative quantity — precision being the obvious candidate, at the price of a second baseline that must be held stable (Boureau & Dayan 2010).
- **[[wiki/concepts/incentive-salience.md]]** — the rival identity for the same non-signed mesolimbic gain: a cue association scaled by appetite, stress and intoxication rather than an inferred inverse variance. The two agree against `T122` position A that the quantity is a run-time gain and disagree about what it is computed from — dissociable by varying predictability and appetite independently, which no cited study does.
- **[[wiki/concepts/schema-assimilation.md]]** — the longest-lived instance of `Π` in the wiki, and one with a lesion: schema instantiation is a *pre-stimulus, sustained* prefrontal setting that biases which posterior associations may be expressed, and removing it abolishes the earliest sensory signature of expertise (the N170 modulation) while *protecting* against schema-induced false memory — so this quantity's gain and its distortion are the same edge (Gilboa & Marlatte 2017).
- **[[wiki/concepts/mental-imagery.md]]** — a case where one gain is demonstrably not enough: the strength of an internally generated sensory signal is set by top-down drive *and* by intrinsic cortical noise, only the first of which a controller writes, and the two produce a strength/precision trade-off (the strongest imagers have the smallest and least precise visual cortices) that a single precision term cannot reproduce.
