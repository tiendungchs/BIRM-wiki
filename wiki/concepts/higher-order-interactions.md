...
- **Nothing learns hyperedges.** Every method here *analyses* a complex that is already given by thresholding a measured matrix. There is no estimator that infers a three-way relation directly from data the way covariance infers a pair — which is the actual missing component. **Partly answered at the measurement level**: the O-information `Ω(Xⁿ) = C(Xⁿ) − B(Xⁿ)` is computed from the joint distribution with no thresholding and no predictor/target split, costs `2n+1` entropy terms, and returns a signed scalar saying whether a group is redundancy- or synergy-dominated (Rosas et al. 2019, [[wiki/concepts/o-information.md]]). It scores a *given* set rather than searching for one, so it is a discriminator a hyperedge learner could be trained against, not the learner — `G105` stays open.
- **A worked case of the blindness, with a number.** An `n`-bit xor has **zero** pairwise mutual information at every pair while attaining the most negative O-information possible (`Ω = 2−n`): a covariance, precision-matrix or attention-score estimator returns the empty graph on a system whose entire regularity is present. This is the cavity complaint above restated as a distribution rather than a geometry.
...
- **[[wiki/concepts/o-information.md]]** — the measurement half of this page's structural argument: where simplicial complexes and persistent homology analyse a complex obtained by thresholding an already-estimated pairwise matrix, `Ω = C − B` reads the redundancy/synergy *kind* of a group of `n` variables straight off the joint distribution — so topology says where the non-dyadic structure sits and `Ω` says what it is, and the `n`-bit xor gives this page's cavity argument a distribution with a number attached.
...
- **[[wiki/entities/dense-associative-memory.md]]** — this page's `n`-bit-xor blindness argument answered on the energy side rather than diagnosed: a degree-`n` term in `E = −Σ_μ F(ξ^μ·σ)` *is* an `n`-body interaction whose coefficients are fit by gradient descent, and a 3-body energy on `N = 3` units solves XOR where the quadratic (= pairwise = linear perceptron) model provably cannot. The restriction is what keeps `G105` open: the `n`-way tensor is a sum of rank-one terms `(ξ^μ·σ)^n`, so an arbitrary hyperedge is still not in the hypothesis space (Krotov & Hopfield 2016).
- **[[wiki/entities/two-body-dense-associative-memory.md]]** — the constructive answer to "an `n`-body term needs an `n`-body synapse": every energy of the rank-one-sum form `Σ_μ F(ξ^μ·σ)` is *exactly* the integrated-out form of a bipartite network with strictly two-body synapses and `N_h` hidden units, so higher-order structure is realisable in pairwise hardware at the price of neurons. The restriction that keeps `G105` open is untouched — the hypothesis space is still sums of rank-one terms, not arbitrary hyperedges (Krotov & Hopfield 2021).
- **[[wiki/entities/neuron-astrocyte-associative-memory.md]]** — the elimination argument generalised past neurons: a quartic interaction among firing rates is what lateral calcium transport between the astrocytic processes wrapping different synapses looks like after the chemistry is integrated out, so a higher-order term is the generic trace of *any* eliminated fast variable, cellular or not — while the realisable class remains sums of rank-one terms, so `G105` is unmoved (Kozachkov, Slotine & Krotov 2023).
