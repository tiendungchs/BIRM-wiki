...
- **[[wiki/entities/blindtest.md]]** — the deficit this architecture's scale hierarchy is the candidate repair for, measured on frontier models: inter-primitive clearance is the quantity that selects a processing level here and the quantity that moves VLM accuracy from floor to ceiling there (≥96% at `dx = 0.75`, 0.95–0.99 at straight-run probability 0.9), and a VLM has no scale to move grouping into.
