...
- **[[wiki/concepts/topological-latent-decoding.md]]** — the categorical case of this page's graded warning: a global embedding of a ring is not merely neighbour-corrupted, it cannot return a one-dimensional latent at all, so the fix there is to skip the global embedding and fit local on-manifold coordinates of the measured topology.
