...
|---|---|---|
| **Learned industriousness** (Eisenberger 1992) | a value for *exertion itself* is acquired over a lifetime in which effort usually paid | is effort endowed with intrinsic positive value, cancelling part of the inherent cost — or is the value attached only to effort's extrinsic products? **The fork is settled on the transfer test below** — see [[wiki/concepts/learned-industriousness.md]] |
...
**That test has been run, repeatedly, and the intrinsic side wins** ([[wiki/concepts/learned-industriousness.md]], Eisenberger 1992 — not cited by either review above). Reinforcing high performance on one task raises performance, persistence and effort-self-control on *different* tasks, in a different apparatus, in a different response dimension and in a different modality, against yoked controls that equate reward count, success rate, response count and temporal density. Three qualifications the transfer result carries and this page's static `SV` cannot express: the learned quantity is **dimension-indexed** (rewarding accuracy transfers to accuracy, not to speed), **cue-conditioned** (which experimenter assigns the transfer task changes how hard students work), and **label-indexed** (a broad verbal self-attribution widens transfer beyond what the same reinforcement produces untagged). The consequence for this page's instrument: a COGED curve is a *snapshot of a conditioning history*, so eliciting it before and after a high-demand-rewarded curriculum should move it — never done in either field (`G128`).
...
- **[[wiki/concepts/learned-industriousness.md]]** — where this page's curve comes from: the aversiveness it measures is a conditioned quantity, written by a history in which reinforcement was contingent on high performance, and the value attaches to the response-produced *sensation* of effort rather than to the task — which is why it transfers to tasks never trained and settles this page's intrinsic-vs-extrinsic fork on this page's own named test. It also adds three structural facts the elicited scalar cannot carry (dimension indexing, cue conditioning, label-set indexing) and one dissociation by training rather than correlation: free-delay training moves delay self-control and leaves effort self-control untouched, and ratio training does the reverse.
- **[[wiki/concepts/explore-exploit-division-of-labour.md]]** — the subsidy term no cost model here has: play is *defined* as an activity requiring surplus resources, so the developmental explore phase is the case where the cost of a broad search is paid by someone other than the agent (alloparents, grandmothers, >60% of a 4-year-old's resting calories going to the brain), and nothing prices what that subsidy buys (Gopnik 2020).
