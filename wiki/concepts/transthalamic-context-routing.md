...
- **[[wiki/concepts/canonical-cortical-microcircuit.md]]** — resolves half of that page's open transthalamic question with a measurement rather than an anatomy: the disynaptic mirror of a feedforward cortico-cortical edge does not carry a re-typed copy of the content, it carries the sender's arousal state, so a model that treats inter-areal edges as direct is not merely missing a route but missing a *different variable* ([[wiki/empirical-tensions.md]] T276). That page also supplies the anatomical typing this channel rides on — core-type thalamic axons to L4, matrix-type to L1 and L5A — plus the caution that the type is a property of the edge rather than of the nucleus: matrix-type cells modulate primary cortex and *drive* secondary sensory cortex.
...
- **[[wiki/concepts/timescale-hierarchy.md]]** — the run-time gate that a fixed anatomical gradient cannot supply: a cortico-cortical-only model reproduces the full timescale hierarchy but has no way to set network state or gate inter-areal interaction by task, and its authors name thalamocortical projections as the missing component (Chaudhuri et al. 2015).
- **[[wiki/concepts/sleep-oscillation-nesting.md]]** — the same thalamic gating circuit claimed to run offline: sensory-projecting reticular neurons lock to alpha during waking attention and to spindles during sleep, limbic-projecting ones are suppressed in slow-wave sleep to release the hippocampal route, so attention and consolidation-addressing may be one mechanism (T373).
- **[[wiki/entities/dendritic-integration-theory.md]]** — the same organ with a third cargo, at a compartment this page could not image: higher-order thalamic input to layer 5a sets whether apical activity reaches the soma at all, so "carries the sender's state" and "gates the receiver's coupling" may be one signal read at layer 2/3 versus layer 5a — and the target-dependence caution here applies to the gating claim too.
