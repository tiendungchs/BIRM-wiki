...
- **[[wiki/entities/default-mode-network.md]]** — the biological route by which this page's parameter is set without any synaptic change: chronic childhood cortisol inhibits oligodendrocyte proliferation and differentiation, so conduction delays and integration timescales across a large-scale network are written by *myelination* on a developmental schedule driven by the environment (Azarias et al. 2025).
- **[[wiki/concepts/broadcast-hierarchy.md]]** — where this page's parameter becomes hierarchical position: apex cortex is hypomyelinated relative to sensory and motor areas, so its long integration window — and therefore the level of abstraction it can hold — is set by conduction speed rather than by any synaptic weight.
