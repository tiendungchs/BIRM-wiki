...
- **[[wiki/entities/m2-auditory-cortex-filter.md]]** — the *forward* direction of an arbitrary edge, priced separately: nothing says running should produce a 16 kHz tone, and cortex takes **5–9 days** to install a movement-gated notch at that frequency (one hour, ~1,000–3,000 pairings, does nothing) — one to two orders of magnitude slower than the tens of trials this page reports for cue→action binding. It also sharpens what the learner needs: mere co-occurrence of movement and sound fails; the sound's rate had to be a *function of* running speed, so the acquirable quantity is a parametric contingency, not an association between two states.
...
- **[[wiki/concepts/prediction-error-neurons.md]]** — the same mapping recast as the unit of inter-areal communication: a transformation between two areas' coordinate systems *is* an internal model — forward when motor → sensory, inverse when sensory → motor — and it is what licenses activity in one area to serve as a prediction of input in the other.
- **[[wiki/entities/cerebellum.md]]** — the structure in which this page's sensory→motor transform is argued to be *instantiated and learned*, with the learning problem named: the error is in sensory coordinates and the controller's error is in motor coordinates, and feedback-error learning bridges them by using a crude legacy feedback controller's output as the training signal. It also supplies the efference-copy consumer this page's binding argument asks for — a forward model whose input *is* the copy of the command.
- **[[wiki/entities/a24b-m2-v1-projection.md]]** — this page's motor→sensory forward model with its location settled and its plasticity demonstrated: the transform is completed in the sending area (anterior cingulate / secondary motor cortex) and delivered to V1 already in retinotopic coordinates, and it is **re-learned within ≤10 sessions** when left–right visuomotor coupling is inverted, by an amount that scales with behavioural performance — the wiki's only measured rewrite of an action→sensory-consequence map.
