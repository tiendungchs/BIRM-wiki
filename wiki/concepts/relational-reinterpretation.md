...
- **[[wiki/concepts/tensor-product-representation.md]]** — the structural code that delivers most of what this page classes as human-only, and its price: an uncompressed tensor product keeps both role and filler separately readable while bound (an exact partial inner product returns either factor without destroying the compound, and works from the filler side too), and a filler pattern can be reused as the *role* pattern of a second binding — so "a relation tokened as an object filling a role" is available in the algebra. What it costs is a binder count multiplicative in rank, and the read is still a computation rather than a comparison (`G104`); Smolensky's own open problem is doing the value-as-variable move without first unbinding.
- **[[wiki/entities/esbn.md]]** — a candidate for the operation this page places above the line, built without symbolic machinery: role-filler independence held during binding, demonstrated by generalising same/different to 98 unseen entities after training on 2.
