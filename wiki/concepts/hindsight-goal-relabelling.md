...
- **[[wiki/entities/option-critic.md]]** — the other way to get hierarchy's benefit without a subgoal vocabulary, from the opposite side: relabelling manufactures dense signal inside a flat learner, option-critic manufactures temporal commitment without dense signal, and neither has been run against the other at matched interaction budget (Bacon, Harb & Precup 2017).
- **[[wiki/entities/feudal-networks.md]]** — the other way to avoid the all-or-nothing sparsity this page's trick exists for, and the comparison neither literature runs: a directional goal scored by cosine is *always partially achieved*, so dense signal comes from the goal's form rather than from rewriting labels after the fact — online and critic-side, where relabelling is offline and label-side (Vezhnevets et al. 2017).
- **[[wiki/entities/diayn.md]]** — the other way to make a coverage problem supervised without a subgoal vocabulary, and the comparison neither source runs: relabelling harvests goals from the agent's own realised future and stays flat, DIAYN samples an index from a fixed prior and trains a classifier on fresh rollouts to produce a policy library — dense signal from stored data versus dense signal from a manufactured discrimination task.
- **[[wiki/entities/go-explore.md]]** — the other cheap non-hierarchical answer to terminal-only reward, with the identical hole: this page manufactures dense signal by relabelling under a designer-given projection `m : S → G`, Go-Explore manufactures it by keying an archive on a designer-given cell map `φ`, and neither infers its own abstraction — two independent arrivals at the conclusion that the missing piece is the state-to-target map rather than the goal generator (`G33`).
- **[[wiki/entities/asymmetric-self-play.md]]** — the same reachability constraint met by a second body instead of by the agent's own past, and the result that contests this page's reading of it: goals proposed by a policy *paid for the solver's failure* are reachable by construction (same action space, same initial state) but lie deliberately far outside the solver's coverage, and with the accompanying trajectory withdrawn the solver scores **zero** on every holdout — so reachability may be necessary rather than sufficient, and a goal may need a witness in proportion to how far it was pushed (`T403`).
