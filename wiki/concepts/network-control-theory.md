...
- **[[wiki/concepts/empowerment.md]]** — the same controllability/observability pair priced in bits rather than in energy: `𝔈 = max_{p(a)} I(S_{t+1}; A_t)` asks how many *distinguishable* outcomes bounded action power can reach and be sensed, with no target state and no linearity assumption, where this page asks the minimum energy to reach a named one — and the two degenerate symmetrically, the Gramian through a numerically unusable inverse, empowerment through a flat landscape whenever sensing or acting is unbounded (Salge et al. 2013).
