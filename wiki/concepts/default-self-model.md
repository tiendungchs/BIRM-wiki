...
2. **The uncorrected default is optimistic, not noisy.** The failure is a signed bias, not a variance increase. More self-evaluation by the same machinery does not fix it; the evaluator and the evaluated are the same estimator. **This reading is not identified by the measurement**: the lesion result is a *one-shot discrepancy score* (a single self-rating against an external criterion), which Fleming & Lau 2014 show cannot separate metacognitive bias from a loss of metacognitive sensitivity — and Kruger & Dunning 1999, the behavioural version of the same claim, is partly predicted by the null, since type 2 sensitivity is mathematically bounded by type 1 performance ([[wiki/concepts/metacognitive-efficiency.md]], `T315`). The two readings prescribe different fixes: a signed bias is repaired by a post-hoc calibrator, a sensitivity loss is not repairable downstream at all. The lesion **specificity** — lateral-frontal damage does not produce the effect — is the one part of the result the ceiling objection does not reach.
...
- **[[wiki/concepts/metacognitive-efficiency.md]]** — the measurement theory this page's central result is missing: a self-report scored once against an external criterion cannot separate an inflated estimate from an uninformative one, so "the uncorrected default is optimistic rather than noisy" is a reading the lesion design cannot license (`T315`), and `meta-d′/d′` is the statistic that would.
...
- **[[wiki/entities/integrated-world-modeling-theory.md]]** — the self/world partition derived instead of installed: bodily states are uniquely perceivable *and* uniquely controllable, so the boundary follows from a controllability asymmetry an agent with an efference copy can measure — the one proposal in the wiki that makes the minimal self a computable statistic.
- **[[wiki/concepts/selective-prediction.md]]** — the minimum viable competence model this page argues for: a per-problem-class threshold vector `{θ_c}` fitted against per-class labelled samples is amortised across queries, is a statement of what the system is good at, and has its calibrator (the labels) genuinely outside the estimating loop — meeting the independence requirement in the cheapest possible way, and only for a fixed distribution.
- **[[wiki/concepts/confidence-calibration.md]]** — Beer's mechanism **A** (post-hoc correction of an optimistic prior by a module with an external referent) implemented at its cheapest: one scalar fitted against held-out labels removes a modern network's whole overconfidence term without touching a prediction — so the correction half of this page's architecture is solved and mechanism **B** (upstream suppression of negative evidence) is the half that remains.
