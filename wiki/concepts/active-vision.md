...
- **[[wiki/concepts/visual-indices.md]]** — the other side of `T387`, from the author this page quarrels with: a pointer seized by the stimulus that holds *nothing*, against a pointer bound by the current program step that holds the one or two values it needs. Pylyshyn's reply is that "deictic code" misnames a device whose function is access rather than encoding, that reference must be plural where a fovea is singular, and — conceding half the row — that indexes may also be assigned by scanning attention until an object grabs one (Pylyshyn 2001).
- **[[wiki/entities/blindtest.md]]** — names what a single forward pass denies: every failing task there is one a human solves by moving the eye or the processing focus and re-reading the display, and the spacing dial (accuracy floor to ceiling purely by separating primitives) measures how much clutter that denial can tolerate.
