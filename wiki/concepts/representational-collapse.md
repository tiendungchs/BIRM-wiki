...
**And the instrument this suggests exists — now with a cheaper rival that needs no loss at all.** The entropy of the ℓ¹-normalised singular values of the *embedding*, `RankMe(Z) = exp(−Σ_k p_k log p_k)`, selects hyperparameters within half a point of a labelled ImageNet oracle across **110 models over 5 methods**, in distribution and out, with no labels, no training and no parameters — and beats the labelled oracle out of distribution in three places (Garrido et al. 2023, [[wiki/concepts/label-free-model-selection.md]]). So the fourth column of the taxonomy above has a label-free read-out after all: *how much collapse happened* is measurable at scale, and within one method less of it is monotonically better. Two riders that keep it from settling T314: it is computed on the embedding, and it is licensed only for comparing runs of a *given* method, which is exactly the comparison B's projector ablation is not.

LeJEPA's training loss has Spearman correlation **≈0.85** with downstream linear-probe accuracy across architectures, datasets, learning rates and epoch budgets, rising to **≈0.99** under the rescaling `train_loss / λ^0.4` — a label-free ranking of checkpoints, i.e. the first thing certifiability *buys* rather than promises. **(brainstorm)** It is calibrated on exactly the read-out that survived DINOv3's partial collapse, so the monitor and the known failure have not yet met, and the obvious experiment is to run the rescaled-loss ranking against the *patch-token* probe on a long run and see whether the correlation survives.
...
- **No collapse monitor is calibrated against partial collapse.** There are now three label-free monitors and the same hole runs through all of them: RankMe is validated against linear, MLP-head and `k`-NN read-outs and never against a dense one; the rescaled LeJEPA loss is calibrated on the read-out that partial collapse spares. **The third, and oldest, may not be**: `(L_align, L_uniform)` measured on validation embeddings tracks quality across 521 encoders spanning four datasets, two modalities and every hyperparameter, and was validated against a **dense conv-layer depth regression** as well as linear and 5-NN probes — so unlike the scalar it has been scored on the kind of read-out DINOv3 destroys ([[wiki/concepts/alignment-uniformity.md]]). Untested on patch tokens over a long run, which is the cheap experiment.
...
- **[[wiki/concepts/label-free-model-selection.md]]** — this page's provisions turned into an instrument: every working label-free selector is an anti-collapse statistic (effective rank of the embedding, `(L_align, L_uniform)`, a rescaled loss), so "how much collapse happened" is currently the only thing that can be measured about a run without labels — and RankMe's 110-model sweep is the largest measurement of embedding rank against downstream performance the wiki holds (T314).
