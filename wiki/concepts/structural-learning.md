...
- **[[wiki/concepts/inductive-bias.md]]** — the formal name for what this page's variance-only result demonstrates: a task family sampled symmetrically about the identity has an uninformative mean yet installs a usable bias, which is `er_Q(H)` depending on the *support* of the environment `Q` and not on its average — the statistical statement of why the acquired object is visible in the variance.
