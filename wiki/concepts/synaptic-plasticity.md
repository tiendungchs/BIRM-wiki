...
- **`A₊, A₋, τ₊, τ₋` are not four free parameters.** They are consequences of membrane dynamics and firing threshold, so the "kernel shape as design parameter" claim above is really a claim about the *neuron model*, not about the synapse.
...
**Rule 1 is the only entry that currently meets the demand outright**, and its answer is unflattering: plain Hebb ascends *retained input variance* (it converges to the top eigenvector of `C`). So the family's founding rule does learn by Richards et al.'s criterion — it simply learns the wrong objective for a reasoning system. For rules 2–7 the objective is still named only informally.
