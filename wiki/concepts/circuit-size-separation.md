...
- **No learning, and the missing half of it is now a gap row.** The paper proves what small spiking networks *can* compute and says nothing about reaching those weights and delays. The delay half is [[wiki/architectural-gaps.md]] `G80`: the `Θ(n log n)`-vs-`Θ(n)` bound makes a delay the most expressive parameter per unit count in the wiki, the CD_n construction uses delays alone with every weight fixed at 1, and nothing in the wiki optimises one. Every construction is hand-built, and the delay assignments in CD_n are precisely the objects [[wiki/concepts/temporal-coding.md]] can only *select* from a random repertoire, never construct.
