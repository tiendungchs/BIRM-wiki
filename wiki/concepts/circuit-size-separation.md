...
- **[[wiki/concepts/neuron-complexity-index.md]]** — the empirical mirror of this page and the other end of the same bracket: where this page proves `≥1663` sigmoidal units are *necessary* for one dendritically-corrected witness function, that page fits the whole I/O map of an L5 pyramidal compartmental model and finds ~7 layers × 128 channels *sufficient* — so the wiki now has a lower bound on a chosen function and an upper bound on the full mapping, on one axis (units-for-a-function), with the gap between them unclosed.
- **[[wiki/concepts/adaptive-computation-time.md]]** — the same function priced in the other currency, with an empirical shadow of this page's theorem: where this page counts *units at fixed depth*, a halting unit counts *sequential transitions at fixed units*, and parity — the canonical function whose feedforward cost explodes with input width — is solved by a small recurrent net given a learned, input-dependent number of steps, with ponder time growing linearly in the number of bits (Graves 2016).
- **[[wiki/entities/universal-transformer.md]]** — the largest-scale instance of this page's trade in a non-spiking model: one shared attention-plus-transition block applied `T` times reaches 0.35–0.46 sequence accuracy on length-400 copy and reverse where a parameter-matched fixed-depth stack is at 0.03–0.06, and the accompanying universality argument is precisely the claim that only the *sequential* axis, not the width axis, can be traded against input size.
- **[[wiki/concepts/width-versus-parameter-count.md]]** — the confound in this page's currency, measured: unit count and parameter count come apart by up to 7.8× in a trained network at matched accuracy (static random sparsity at fixed weights), so an `Ω(n)`-units bound and a parameter count are not interchangeable, and every units-to-competence number the wiki asks for (`P5`, [[wiki/empirical-tensions.md]] T234) must state the weight budget it was measured at.
- **[[wiki/concepts/visual-routines.md]]** — the same technique one class harder, and the result this page's density claim should be contrasted with: Minsky & Papert show inside/outside and connectedness are computable by *no* diameter- or order-limited perceptron at any size, so for those predicates the separation is not `Ω(n)` units but unavailability of the whole class.
