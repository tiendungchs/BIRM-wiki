...
| Which cells actually have a tuft in layer 1 — cell filling of 46 retrogradely identified macaque V2 projection neurons (Markov et al. 2014, [[wiki/concepts/counterstream-organization.md]]) | **The port is not universal.** All supragranular cells are tufted whether they project up or down; layer-6 cells of both directions and layer-5 feedback cells have slender apicals that never reach layer 1; 4 of 9 layer-5 *feedforward* cells do reach it and branch there |

**The denial that follows, and it is architectural.** Layer 6 carries the long-range feedback — and layer-6 cells have no apical tuft in layer 1. Layers 2/3 and 5 are the only layers reported to respond to layer-1 stimulation. So the apical port is read by the **ascending** stream and by **short-range** feedback cells, and is not available to the long-range descending stream. Feedback can gate what goes up; it cannot gate the long-range broadcast arm the same way. Any architecture that implements "top-down modulates top-down" through this mechanism is asserting something the anatomy denies precisely where the reach is longest — which sharpens the `T259` row above rather than softening it: the broadcast port is real, but it is an *input* port for feedforward and local cells, not a port the global broadcaster itself listens on.
...
- **[[wiki/concepts/counterstream-organization.md]]** — the anatomical boundary on this mechanism: cell filling of retrogradely identified projection neurons shows layer-6 cells, which carry the long-range feedback, have slender apical dendrites that never reach layer 1, while all supragranular cells and some layer-5 feedforward cells are tufted — so the coincidence port is available to the ascending stream and to short-range feedback, and architecturally denied to the long-range descending stream.
