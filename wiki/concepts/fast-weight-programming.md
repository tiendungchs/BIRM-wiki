...
- **[[wiki/entities/esbn.md]]** — the rival self-invented-key store: both let a network emit its own keys and values, but a delta-rule write superposes them into one matrix (inheriting crosstalk, gaining a correction operation) where the ESBN appends a row (no crosstalk, no correction, no bound vector).
- **[[wiki/concepts/activity-slots.md]]** — this page's design proven to have an activity-only twin: a state held as a weight matrix and a state held as partitioned recurrent activity compute the same function on structured sequence memory, so "where the state lives" is a neuron-count and sample-complexity decision rather than a capability one — and the weight version is the cheaper of the two by a factor of the state count (`G112`; Whittington et al. 2025).
- **[[wiki/entities/continuous-modern-hopfield-network.md]]** — the same attention layer read with the softmax *kept* instead of deleted, and the two readings price different resources: this page's linearised head is a rank-limited programmable matrix with capacity `d_dot` and a correctable delta write, that page's softmax head is a non-parametric store over an explicit pattern list with capacity exponential in `d_k`, no write operation at all, and a retrieval error `∝ exp(−βΔ_i)` in the separation of the keys (Ramsauer et al. 2020).
- **[[wiki/entities/neuron-astrocyte-associative-memory.md]]** — a biological circuit that *is* an outer-product fast-weight store: the synaptic bias `c_ij` accumulates `x_i I_j` during a write phase and is read linearly during a read phase, with an astrocyte supplying the normaliser — so the circuit is linear attention and inherits this page's `d_dot` orthogonal-key bound and random-feature noise floor rather than the exponential-separation bound of the softmax form (Kozachkov, Slotine & Krotov 2023).
- **[[wiki/entities/three-factor-key-value-memory.md]]** — the same one-step key-value write over a fast matrix with the controller-issued write replaced by two biological gates and the sum-over-writes replaced by an *overwrite*: the interference that bounds an outer-product store at `d_dot` associations is traded for the loss of exactly one slot, which is why overload here is graded at any size rather than a mixture over every non-orthogonal key (Tyulmankov et al. 2021).
- **[[wiki/concepts/key-value-memory.md]]** — the `σ = identity` corner of the key-value family, plus the dual that extends this page's claim past attention: *any* linear layer trained by gradient descent is an outer-product fast-weight store with its training inputs as keys and its error signals as values, so the interference analysis here applies to ordinary feedforward weights.
