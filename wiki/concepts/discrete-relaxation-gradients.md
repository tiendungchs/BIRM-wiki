...
**The recipe, stated generally** (Kirsanov 2026, `raw/kirsanov-2026-surrogate-gradients-spikes.md`, secondary **(tentative)**): *when a computation contains a non-differentiable operation, replace its derivative — and only its derivative — with something smooth during backpropagation.* Two further literatures instantiate it, bringing the independent-rediscovery count to five: **binary neural networks**, where weights are quantised to `±1` by `sign(·)`, and **differentiable rendering**, where visibility is a step function. What the substitute must satisfy is minimal — non-zero in a neighbourhood of the discontinuity, decaying away from it — and its shape beyond that is reported not to matter, which is the ST-family analogue of this page's finding that Gumbel-Softmax tolerates a wide range of `τ` schedules.

**Why an admittedly wrong derivative beats a correct one (brainstorm).** The true derivative at a threshold is not *wrong*; it correctly answers a question about infinitesimal perturbations. An optimizer takes **finite** steps, and near a discontinuity the two questions come apart completely: a neuron 1 mV below threshold has exact derivative zero and a large finite-step effect. Read this way the whole family is not an approximation of the true gradient but a *different and better-posed estimator* for the quantity actually used, which is why bias measured against the exact gradient (the framing of `T298` position B) may be scoring the wrong target.
...
- **[[wiki/concepts/biologically-plausible-credit-assignment.md]]** — the other half of the "wrong backward pass" question, and the one with the scaling evidence: that page's rules falsify the error *routing* (`B` random instead of `Wᵀ`) and fail at ImageNet, while every method on this page falsifies only a *local derivative* and scales — so relaxation bias is the cheap kind of wrongness and transport bias is the expensive kind.
...
- **[[wiki/entities/switch-transformer.md]]** — the measured case against relaxing one particular discrete decision: with the routing argmax replaced by a *sample* from the router softmax, quality drops from −1.471 to −1.570 negative log perplexity, while multiplicative jitter on the router's *input* is neutral-to-marginally-better (−1.468) — so for a router the useful noise is in the input, not in the decision, and the stochastic-selection family here is not merely unnecessary but harmful.
- **[[wiki/concepts/adaptive-computation-time.md]]** — a fifth family this taxonomy omits: neither relax the sample nor score it, but take the **mean field** over the entire discrete distribution — `s_t = Σ_n p^n_t s^n_t` over halting steps — which is exact, temperature-free and noise-free, and is available only because the mixed objects are state vectors on a path rather than incompatible branches. Graves's stated reason for preferring it over sampling is this page's variance argument applied along a sequence: each halting decision conditions every later one, so estimator noise compounds.
