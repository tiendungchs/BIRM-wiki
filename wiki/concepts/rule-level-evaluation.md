...
- **[[wiki/concepts/certification-instruments.md]]** — this page is instrument `I15`, the only one in the inventory that asks the solver what it did rather than scoring what it emitted.
- **[[wiki/concepts/human-baseline.md]]** — the rule-level score needs a baseline of a different kind — coverage, not accuracy: humans reach the intended abstraction on 476/480 tasks against o3's 412, which is a denominator no accuracy protocol supplies.
- **[[wiki/entities/math-perturb.md]]** — the instrument this page would make mechanical: MATH-Perturb's memorisation signatures (assumption reversion, unchecked technique transfer, answering the original question) are read out of 20 hand-inspected traces per model, where requiring the solver to state the method *and its preconditions* would score the same thing at scale.
- **[[wiki/entities/hle-verified.md]]** — this page's instrument turned on the benchmark instead of the solver: a 10-category taxonomy applied to the *reference* rationale finds it invalid more often than valid (65.9–70.3% in mathematics, biology/medicine and chemistry), dominated by missing prerequisites and unstated intermediate structure rather than contradiction — so the authoritative rule a rule-level grader would classify against is itself frequently unsound.
- **[[wiki/entities/hle.md]]** — right-answer-wrong-reasoning treated as an authoring problem rather than a scoring one: HLE contributors are told to defeat it by editing the item (adding answer choices) instead of by reading the rule, which is the cheapest possible response to what this page measures and gives no read-out.
- **[[wiki/entities/shortcut-suite.md]]** — the natural item supply for this instrument: a construction in which the unintended rule is installed by the experimenter (`q ∧ s ≡ q`) and therefore known in advance, so a stated rule can be classified against a shortcut whose identity was not inferred after the fact; its own hand-classification of CoT traces yields three error types — distraction, disguised comprehension, logical fallacy.
- **[[wiki/entities/baba-is-ai.md]]** — the complement of this instrument: rather than asking a solver to state the rule it used on a fixed-rule task, it makes the rule a movable object and reads the solver's understanding off the edits it plans, so the framing/optimisation split is visible with no rater and no author-declared abstraction.
- **[[wiki/entities/prm800k.md]]** — this page's proposal executed at industrial scale in the one domain where a derivation decomposes into enumerable steps: 800K human labels on the *process* rather than the answer, with the resulting scorer beating an answer-trained one at every difficulty and under distribution shift. The limit is the same one this page names — a step label certifies local validity, never that the derivation answers the question asked.
- **[[wiki/concepts/abstraction.md]]** — this instrument is the sharpest test of that page's behavioural family (abstraction as generative competence over varied instantiations), and its manual classification is why the family has no scalable instrument.
- **[[wiki/concepts/reasoning.md]]** — the only measurement in the wiki that puts a number on each half of the framing/search split that page argues every reasoning definition is silent about.
- **[[wiki/entities/blindtest.md]]** — the same factorisation of accuracy into perception and rule, obtained by removing the rule instead of by classifying it: this page's separately-scored 49.1–77.3% visual-error rates and BlindTest's 58.07% with no rule in the task are two measurements of one quantity, one manual and task-coupled, one automatic and rule-free.
- **[[wiki/concepts/perception-reasoning-dissociation.md]]** — the other half of the same factorisation, obtained by repairing the parse instead of classifying the rule: this page measures rule-correct-grid-wrong from a run where perception is broken, that one substitutes a natural-language parse and finds the residual failures turning *into* reasoning errors (Mini-ARC inductive 4 → 9 as perception is eased) — so the two instruments read the same product from opposite ends and both bottom out in a manual rater.
