...
**Correction to the InfoNCE cell: it is not usefully read as a bound on anything** (Wang & Isola 2020, [[wiki/concepts/alignment-uniformity.md]]). The mutual-information-bound interpretation is known to be inconsistent with practice — optimising a *tighter* bound on `I(f(x);f(y))` gives worse representations. What the loss provably does instead, at `M → ∞` negatives, is minimise `−(1/τ)E_{p_pos}[f(x)ᵀf(y)]` (zero iff `f(x)=f(y)` a.s.) plus `E_x log E_{x⁻} e^{f(x⁻)ᵀf(x)/τ}`, and on a finite dataset the second term is exactly a **resubstitution entropy estimator** `−Ĥ(f(x)) + log Z_vMF` under a von Mises–Fisher KDE with concentration `κ = 1/τ`. So the quantity is `H(f(x))`, estimated non-parametrically, not `I(f(x);f(y))` bounded — and against `I = H(f(x)) − H(f(x)|f(y))`, the alignment half is *stronger* than the conditional-entropy term it replaces. The temperature is a kernel bandwidth, which is the one design parameter this whole page's vocabulary otherwise has no name for.
...
- **[[wiki/concepts/epistemic-value.md]]** — where this page's question stops being about fitting and starts deciding whether the agent moves: four defensible readings of one epistemic divergence produce four different agents, one of which collapses the policy and one of which diverges numerically.
...
- **[[wiki/entities/vicreg.md]]** — the estimator argument's boundary: the batch-size independence transfers intact (±1.3 over a 32× range against SimCLR's −4), but a per-branch covariance penalty saturates in embedding width at 8192 where the cross-branch cross-correlation kept paying to 16384 — so "Gaussian-parametrised proxy" fixes the sample cost and not the dimension return.
- **[[wiki/entities/barlow-twins.md]]** — the estimator reading of the same choice: a contrastive term is a *non-parametric* entropy estimator of the embedding distribution (sample-hungry, dimension-cursed) while a cross-correlation penalty is a **Gaussian-parametrised proxy** for it, which is one statistical fact predicting both scaling curves — flat to batch 256 where SimCLR loses 4 points, and monotone gains in embedding width to 16384 where InfoNCE saturates.
...
- **[[wiki/concepts/alignment-uniformity.md]]** — the correction to this page's InfoNCE entry: the contrastive loss is a non-parametric *entropy* estimator of the embedding marginal (vMF-KDE, bandwidth `κ = 1/τ`) rather than a mutual-information bound, so temperature is this page's missing name for a kernel bandwidth.
- **[[wiki/concepts/representational-collapse.md]]** — the same objects one level up — the anti-collapse terms are divergences between an embedding distribution and a fixed target, and the moment ladder is a statement about which of them are estimable in high dimension.
- **[[wiki/concepts/environment-invariance.md]]** — the constructive complement to this page's negative result: since a shortcut and the intended rule inducing the same `Q` have identical divergence, the separating term must not be a functional of the pooled predictive distribution — an invariance penalty across environments is the one such term the wiki has.
- **[[wiki/entities/mae.md]]** — the reconstruction pole of this page's pipeline/loss split, with the pipeline term priced: the transformation `f` is random masking (75%, and the ratio alone is worth ~19 linear points) while the loss is plain MSE, and truncating the target to its top-96 PCA coefficients — deleting exactly the component no context can predict — *lowers* accuracy, which bounds the "the loss should not be charged for irreducible detail" argument.
- **[[wiki/concepts/information-bottleneck.md]]** — the rate–distortion split above with the distortion *derived* rather than authored: naming a relevance variable `Y` fixes `d(x,x̂) = D_KL[p(y|x)‖p(y|x̂)]`, which is why `β` there is the negative inverse slope of a curve rather than a free weight — and the one place a divergence in this page's vocabulary scores a *hidden* layer, since it is defined against `p(y|·)` and not against a target vector.
- **[[wiki/entities/cpc.md]]** — the primary source of the InfoNCE cell in the rate–distortion table above: the temporal pairing is the transformation `f` that sets the rate, the density ratio `p(x_{t+k}|c_t)/p(x_{t+k})` is the distortion term, and the `log N` ceiling on its bound is why the loss value carries no absolute reading of how much structure was recovered.
- **[[wiki/concepts/intrinsic-motivation-typology.md]]** — the choice hidden inside that page's information-gain cell: "the decrease in uncertainty caused by an event" is a directed quantity, so which divergence and in which direction decides which of several curiosity drives an agent actually runs.
- **[[wiki/concepts/default-policy-regularization.md]]** — the second instance of this page's "divergence as a *term*, not the objective": `KL(π_i ‖ π₀)` puts neither the data nor the world in either argument, charging a task policy for departing from a distilled default. The direction is fixed by exactly the criterion stated above — which phase you want to become the default — and here the charged quantity is read as the subjective cost of cognitive control.
- **[[wiki/entities/diayn.md]]** — the variational lower bound used as a reward rather than as a loss: replacing the intractable `p(z|s)` with a learned `q_φ(z|s)` costs exactly `E[KL(p(z|s) ‖ q_φ(z|s))]` of slack by Jensen, and because the agent is *paid* by the approximation it is simultaneously training tighter, the two players push the same way — the structural reason the scheme is cooperative where adversarial curriculum objectives are saddle-point unstable.
- **[[wiki/concepts/realised-information-gain.md]]** — a third position for this page's "divergence as a *term*" argument, and the only one where the KL is neither fitted nor paid but **read**: both arguments are the same agent's own successive posteriors, `D_KL(p^j ‖ p^{j−1})`, so the model/world direction-choice does not arise and the asymmetry reappears as a different question — whether the update is scored from the new belief's perspective (as the measured version is) or the old one's, which decides whether a large move *away* from a confident wrong belief counts as much as a large move toward a new one. Fitted as the dominant determinant of infant disengagement (Poli et al. 2020).
