...
- **[[wiki/concepts/hippocampal-indexing-theory.md]]** — the rival reading of the same tagged cells: an index is also necessary and sufficient for the memory while holding none of its content, so this page's tag-and-manipulate criterion cannot distinguish a content-bearing trace from a pointer (`T28`).
- **[[wiki/concepts/key-value-memory.md]]** — the computational reading of silent-engram reactivation: in the key-value dual of a trained network a catastrophically forgotten task is restored by multiplying its stored *keys* by a scalar, which is optogenetic reactivation done in parameters — the trace was present and only its address was swamped.
