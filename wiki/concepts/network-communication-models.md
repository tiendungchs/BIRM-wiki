...
- **[[wiki/concepts/inter-areal-synchrony.md]]** — the mechanistic account this page's models deliberately abstract away: communication models say which polysynaptic path carries a signal but not what gates it, while coherence-based accounts say what gates a link but only for small motifs of directly connected elements — the review names bridging the two as its crucial open direction. The nearest thing to a bridge the wiki holds is frequency-division multiplexing on a fixed edge: three narrow bands over one anatomical pair, each with a fixed direction label and a task-set gain, which is a routing policy implementable without changing any weight (Bastos et al. 2015).
- **[[wiki/concepts/timescale-hierarchy.md]]** — the variable every policy here assumes away: all of these models score paths on a graph of identical nodes, and the same macaque connectome with a gradient of local recurrent gain produces entirely different global dynamics from the homogeneous version — including a structure–function `r²` that *falls* from 0.83 to 0.53 when the nodes are made realistic (Chaudhuri et al. 2015).
