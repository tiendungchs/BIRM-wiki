...
- **[[wiki/concepts/relational-bottleneck.md]]** — the same superposition crosstalk this page engineers around, read as an explanation instead of a defect: a compositional vocabulary is shared across bindings by construction, so `blue⊗upper-left` and `blue⊗lower-right` must interfere, and the capacity limits of working memory, subitizing and absolute judgment become the price of the flexibility rather than independent facts (Frankland, Webb & Cohen 2021, via Webb et al. 2024).
- **[[wiki/concepts/displacement-codes.md]]** — the same "a relation is a vector" commitment with the opposite choice of role code: this page's roles are random and deliberately similarity-destroying, while a displacement code's role is a metric offset in a shared modular basis, so similar arrangements get similar compositions and no unbinding is needed to compare two bound structures (Hawkins et al. 2019).
- **[[wiki/concepts/activity-slots.md]]** — the combination this page's compressed code cannot offer: both constituents stay separately addressable while bound (the slot is the role, its contents the filler) *and* the bound state is still a vector with a measured linear algebra on it — bought by paying a slot per relative position rather than by compressing, and limited by the role being a position rather than a name (`G104`, [[wiki/empirical-tensions.md]] T293; Whittington et al. 2025).
- **[[wiki/entities/thousand-brains-theory.md]]** — the architecture that makes the metric alternative to this page's role code necessary rather than optional: with thousands of concurrently active object-anchored frames, relating two objects *is* converting between two frames, so the compose operator must return something comparable to other compositions — which a similarity-destroying random role cannot, and a displacement in a shared modular basis can (Hawkins et al. 2019).
- **[[wiki/concepts/apical-amplification.md]]** — binding bought with hardware instead of algebra: coincidence at the two poles of one pyramidal cell fuses a feedforward and a feedback stream with no operator, no tag and no read-out stage, since the matching cells simply fire hardest — at the cost that only the pairs anatomy already delivers to that cell can be bound, which is precisely the generality an algebraic operator buys.
- **[[wiki/concepts/visual-indices.md]]** — the opposite trade on the same job: binding by maintained causal contact rather than by algebra, so nothing is spent on representational dimensions and the referent's properties may change arbitrarily without breaking the bond — at four or five items, with no operator for binding two indices to each other and no way to store the result (Pylyshyn 2001).
