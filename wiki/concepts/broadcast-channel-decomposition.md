...
|---|---|---|---|
| Value | lateral habenula, via `GPb → LHb → RMTg → DA` (LHb neurons are the sign-inverse of value DA neurons — at shorter latency **only in the negative direction**: on no-reward trials habenular excitation precedes the dopamine dip, on rewarded trials the dopamine burst precedes the habenular inhibition, so the source of the burst is unidentified; Matsumoto & Hikosaka 2007, [[wiki/entities/lateral-habenula.md]]); parabrachial nucleus supplies the **aversive-outcome component** specifically | orbitofrontal / ventromedial prefrontal cortex, nucleus accumbens **shell**, dorsal striatum | outcome evaluation, value learning, stimulus–response habits |
...
**The value channel's source is a funnel, and it may not carry the whole negative half.** Hu, Cui & Yang 2020 ([[wiki/entities/lateral-habenula.md]]) give the lateral habenula an afferent map — more than a dozen pathways carrying reward omission, pain, thirst, sound, touch, light and social defeat — and one lesion dissociation that cuts this page's value channel in two: habenular lesion abolishes the dopamine dip on **reward omission** and leaves intact the dip on **aversive events** (`T361`). So "the value channel" has at least two upstream routes for bad news, only one of which passes through the node named here, and the inversion itself is done by an interposed GABAergic population rather than by any cell holding a negative rate.
...
- **Splitting the channels does not split the quantity** (`G118`). Every channel here carries news about one reward scalar; the hedonic/motivational dissociation says the quantity being taught about is itself two, and no decomposition of the *carrier* supplies that.
...
- **[[wiki/concepts/homeostatic-need-signal.md]]** — extends the channel count past the dopaminergic split on a different principle: hunger and thirst are two further broadcast negative-valence scalars with their own sources (arcuate, subfornical organ), their own hormonal inputs and their own need-specific outputs, so *how many* channels an architecture needs is answered partly by how many internal variables it regulates rather than by how many aspects one event has.
- **[[wiki/concepts/general-danger-channel.md]]** — the measured content of a source this page names on inference: the parabrachial nucleus appears here as the supplier of the value channel's "aversive-outcome component specifically" and as a salience candidate, and recording from its CGRP population shows it is neither — it carries meal-termination satiety and food novelty alongside pain, and is driven *below* baseline by appetitive anticipation, so the input to the value/salience split is already a multiplexed signed scalar with modality discarded (`T357`).
- **[[wiki/entities/lateral-habenula.md]]** — the source this page names for the value channel, unpacked and partly corrected: it is not a single sign-inverting node but a funnel taking reward omission, pain, thirst, sound, touch, light and social defeat from more than a dozen afferents, with valence separated by *transmitter* within a pathway rather than by target, and with a slow burst-mode gain set by an astrocytic potassium channel — a second code dimension on the same wire that none of this page's three channels has.
- **[[wiki/concepts/pseudo-reward-prediction-error.md]]** — the address problem raised one level up: this page splits the signal by *what is signalled about the outcome*, and a subgoal-level error adds a split by *which value function the error belongs to* — with the observation that at 3 mm voxels and scalp EEG the two are co-localised in all four structures, so the second split, if it exists, is invisible at the resolutions used (`T367`, Ribas-Fernandes et al. 2011).
- **[[wiki/concepts/sign-tracking-and-goal-tracking.md]]** — the case that most needs this page's answer and is not settled by it: an animal that learns a CS→US association shows no prediction-error signature in accumbens core, which is either a second chemistry or a channel this page's partition predicts and that electrode did not sample — and the systemic-antagonist arm argues against the address-by-axon rescue without closing it (`T404`).
