...
- **[[wiki/entities/cscg.md]]** — the boundary case that shows what the loops cost: grouping the action with the next hidden state collapses that model to a chain, so one forward and one backward sweep give the *exact* posterior and every query — localisation, `k`-step prediction, planning to a clamped goal — is the same algorithm with different evidence clamped.
