...
- **~~No ingested machine-learning source in this family.~~** Closed by [[wiki/entities/neo-neural-theorizer.md]]; the ARC-specific files below are still unread, so the wiki still cannot say how far cost 1 has been pushed *on a benchmark with task grouping*. Original statement: Everything above is second-hand or adjacent, and the gap is an *unread-file* gap rather than an acquisition gap: `raw/` already holds inductive-logic-programming and neurosymbolic attacks on ARC (`rocha-2024-ilp-program-synthesis-arc.md`, `das-2025-compositional-neurosymbolic-arc.md`, `franzen-2025-product-of-experts-arc.md`) and a human-side study of the same task (`johnson-2021-human-program-induction-arc.md`). Until they are ingested the wiki cannot say how far cost 1 has actually been pushed.
