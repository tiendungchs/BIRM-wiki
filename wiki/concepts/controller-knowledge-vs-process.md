...
- **[[wiki/concepts/controlled-semantic-cognition.md]]** — the strongest lesion evidence this page's debate has, and it favours the processing side: damage to the control network leaves item knowledge intact and abolishes frequency and typicality sensitivity, while damage to the store does the reverse — and the miscue probe (a misleading phonemic cue *drives* a control-lesioned patient to the wrong answer, where a store-lesioned one produces nothing) is the one measurement whose two lesions give responses of opposite sign, so it cannot be produced by a single capacity variable. The wiki's own controllers have never been scored on it (`G109`).
