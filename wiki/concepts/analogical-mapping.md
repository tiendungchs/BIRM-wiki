...
**The fourth process, and the reason the table has three rows.** The architectures name a fourth — **schema induction**, in which the completed analogy (source, target, and the correspondence itself) is written back to memory as a reusable structure. LISA has it, SME's family has it, the Companion architecture drops it. The wiki treats it on [[wiki/concepts/schema-assimilation.md]] rather than here.

**And the decomposition itself is contested** (Parsons & Davies 2022, [[wiki/entities/lateral-frontoparietal-network.md]]). Hummel & Holyoak's own position is that retrieval, mapping, inference and schema induction are "all special cases of the same sequential, directional algorithm" — ARCS and MAC/FAC need access and mapping to run *simultaneously*, because the constraints that pick a source have to shape the memory search while it runs. On the neural side no study has measured all four, access has never been imaged, and the two studies that split a trial into encoding and mapping phases found **more** frontoparietal activation in the phase where no mapping was required. So the row structure above is a convenience of exposition and of the wiki's build plan, not a finding — `T330`.
...
| Problem solved when source retrieval was cued by **relational** similarity (Gentner 2010; Gentner & Smith 2012, via Parsons & Davies 2022) | **~10%** |
| Same, when retrieval was cued by **semantic / surface** similarity | **~60%** |

The last two rows are the same claim with the sign made uncomfortable: **the cue that finds a usable source is the shallow one.** Depth of the analogy is a property of the mapping; *availability* of the analogy is a property of surface semantics, and a system that indexes memory by relational structure alone would retrieve the right kind of thing and solve fewer problems. This is the empirical basis for MAC/FAC's structure-blind first stage being a design choice rather than a concession (`T197`).
...
**The table above is upgraded from single studies to a 27-experiment meta-analysis, and the upgrade adds one structural claim** (Hobeika et al. 2016, 506 subjects, 351 foci, activation-likelihood estimation; via [[wiki/entities/lateral-frontoparietal-network.md]]). The frontopolar integrator survives as a coordinate — left BA 10 at the rostral end of the inferior frontal sulcus, MNI −48/44/−8, family-wise-error corrected, and the *only* cluster shared by semantic and visuospatial analogy maps. The right homologue is significant in neither. The structural claim: **the integrator is domain-general and the content stores flank it on a dorsoventral axis** — semantic analogies additionally recruit BA 47 ventral and posterior to it, visuospatial analogies additionally recruit anterior middle frontal gyrus dorsal to it plus superior/inferior parietal cortex and fusiform. One comparison site; per-domain afferents. So the row "spatial relations → parietal cortex" above is not a separate mechanism for spatial analogy but the visuospatial *feed* into the same mapper, and a machine analogy system should be a single relational head over per-modality encoders rather than one mapper per content type.

**The within-subject primary for that axis says the last clause is not yet earned** (`T338`; Wendelken, Chung & Bunge 2012, `n = 23`, semantic and visuospatial relational matching in the same subjects; via [[wiki/entities/lateral-frontoparietal-network.md]]). Three results tighten the domain-general claim and one loosens it. Tightening: the whole-brain interaction for semantic-specific integration returns **no voxels anywhere** at power where the main effect is robust; left rostrolateral cortex is the only one of five regions with a main effect of integration demand; and unlike its ventrolateral and dorsolateral inputs it shows **no** left-right verbal/non-verbal split. Loosening: *inside* left rostrolateral cortex the visuospatial integration peak sits dorsal to the semantic one in almost every subject (`F₁,₂₁ = 9.1`, `P = 0.007`), and the offset is absent for first-order processing of the same stimuli — so it is a property of the integration operation. The authors read this as cells that are domain-general in principle but domain-sensitive by the **provenance of their inputs**, which is a mixture of experts with a wired gate rather than one shared head. The two readings diverge exactly where a builder cares: whether a new content domain inherits the mapper for free once its encoder is trained. The discriminating test — halve the head and score each domain separately — has never been run on either tissue or model.

**And the frontopolar row of that table is now the one entry with a necessity result behind it** (Urbanski et al. 2016, 27 focal frontal-lesion patients, voxel-based lesion-symptom mapping; via [[wiki/entities/lateral-frontoparietal-network.md]]). A **0.33 cc** cluster in left BA 10/47 (MNI −31/51/−3) predicts an analogy deficit; the same patients are normal on a perceptual-match control built from the same stimuli and format, and the whole-brain map for that control is empty. Three things the lesion data add that no activation result can. **(i)** The deficit is graded by *afferent count*: severing the left occipital (inferior fronto-occipital), temporal (uncinate), thalamic (anterior thalamic radiation) or fronto-marginal feeds each impairs analogy, and the number of severed tracts **that terminate in the cluster** predicts accuracy (`rs = −0.553`) while the number that do not predicts nothing — the integrator is a fan-in point, and its afferent diversity is the resource. **(ii)** The impairment does not respect the Find/Apply distinction: patients fail equally when the relational schema is stated for them in words, so what is lost is **relational matching/integration**, not schema induction and not remote (cross-dimensional) mapping — the two stages Stage 3 of this page and [[wiki/entities/lisa.md]] both treat as downstream. **(iii)** The deficit is orthogonal to the standard executive battery (Frontal Assessment Battery, Stroop interference, fluency, Mini-Mental State), which removes the inhibition-failure account of frontal analogy deficits and makes the suppressor and the integrator separately damageable in the tissue as well as by manipulation.

**And the same meta-analysis says the psychometric proxy is not this task** (`T335`). Holding domain constant at visuospatial, four-term analogies engage the left rostrolateral cluster and matrix problems do not; matrix problems instead engage the dorsal attention and fronto-parietal control networks bilaterally. The frontal-patient row in the table above — deficits on *two-relation Raven's-type problems* — is therefore evidence about a format the integrator is least engaged by, and the confound that could dissolve the whole dissociation (evaluate-vs-complete response mode, collinear with format across the 27 studies) is unexamined.

**And the load variable is separable from difficulty, by two designs that reverse it** (Wendelken et al. 2008a, 2008b, via [[wiki/entities/lateral-frontoparietal-network.md]]). Holding `4 items + 3 relations` engages rostrolateral prefrontal cortex more than holding `7 items`, and rostrolateral cortex does not distinguish `7` from `4` at all — while dorsolateral cortex scales with the item load. And an *easy* problem that invites a relation-between-relations (does "uses" describe writer–pen?) engages rostrolateral cortex more than a *hard* problem solvable by constrained retrieval (painter : brush :: writer : ?). So relational complexity is not a re-description of effort: the two dissociate in opposite directions in the same subjects. Exported as `I36` on [[wiki/concepts/certification-instruments.md]]. The same source adds the parietal term this table omits — inferior parietal cortex represents the *first-order* relation, scales with the number of relations, and prefers specific relations over bare association (Wendelken & Bunge 2010).

**The suppressor has a mechanism claim attached, and its timing is awkward** (Knowlton, Hummel & Holyoak 2012). Cho et al. 2010's right inferior frontal gyrus signal *scales with the amount of interfering information*, and scalp EEG puts the right-prefrontal modulation at **late** stages of processing, with its magnitude interacting with the individual's working-memory capacity (Sweis et al. 2012). The model this is offered in support of ([[wiki/entities/lisa.md]]) needs the filter to act *before* admission — propositions tagged low in goal-relevance are supposed never to enter the phase set, which is what buys the signal-to-noise gain. A late, capacity-dependent modulation looks instead like **eviction of something already loaded**, which is a different operation with a different cost: pre-admission filtering is free at the bottleneck, eviction spends a slot first. Nothing in the analogy literature separates the two, and the discriminating measure is cheap — an early filter predicts no interference cost at all in reaction time once capacity is controlled, an evictor predicts a cost that grows with phase-set occupancy.

**And the necessity result does not replicate in the largest series available, on the same side of the brain** (`T336`; Mole et al. 2025, 247 unilateral focal patients, graph lesion-deficit mapping; via [[wiki/entities/lateral-frontoparietal-network.md]]). On an analogy task explicitly built from Urbanski's, **right** frontal patients are worse than left (63.19% vs 73.19%), and the inferred network is right middle and inferior frontal gyri centred on *caudal* middle frontal gyrus — the opposite hemisphere and posterior to the frontal pole. Three implications for this section's table. **(i)** The frontopolar row's necessity claim is contested rather than settled; the two studies differ in method (mass-univariate mapping at `n = 27` vs stochastic block modelling at `n = 247`) and in score (difference index vs raw accuracy) and have never been run through one pipeline. **(ii)** The "spatial relations → parietal cortex" row loses its necessity: 74 posterior patients, 32 of them left, are unimpaired, and posterior cortex contributes minimally to the inferred network — so the visuospatial feed is either reconstructible elsewhere or not loaded by a two-set comparison. **(iii)** The laterality is not uniform across items even inside the disagreeing study: an *odd-one-out* rule is right-lateralised and a *progression* rule is not. The authors' reading of the common factor is not relational integration at all but **holding several novel, mutually irreducible representations active in parallel** — a capacity claim about occupancy rather than a claim about a convergence point, and the first rival on this page to the integrator account that is stated at the same grade of evidence.
...
- **Offload** (Wendelken et al. 2011, [[wiki/entities/lateral-frontoparietal-network.md]]): neither. Across ages 6–18 the integrator's activation on second-order trials is **flat**; what falls is its activation on *first-order* trials, and structural equation modelling attributes part of that fall to cortical thinning in inferior parietal cortex. The 7–10 group engages the same regions as adults but engages them equally for first- and second-order problems. Nothing about the second-order machinery changes; the first-order store gets good enough to stop calling it.

See [[wiki/empirical-tensions.md]] T193. For a builder the distinction is the difference between scaling the relation vocabulary and scaling the binding budget, and the second is a *capacity* parameter no wiki architecture currently exposes. The third option changes the prescription again: raise effective relational capacity by making the first-order representation self-sufficient rather than by widening the binder, since an integrator whose *call rate* is ungated looks capacity-limited whether or not it is `(brainstorm)`.
...
| The stage decomposition (retrieve→map→infer→abstract) is "oversimplified"; real use is cyclic, interactive, open-ended, with the source's representation *developed during* reasoning, and multiple or imagined sources | G5 (no joint discover-and-navigate loop); `T330` now carries the stronger form of the same complaint, with the neural evidence |
...
- **[[wiki/concepts/reasoning.md]]** — Holyoak's role-based criterion filed as the one definition that constrains the *representation* rather than naming an operation, and the reason it is the sharpest of the seven: its central step is the mapping this page records as assumed everywhere and performed nowhere.
- **[[wiki/entities/lateral-frontoparietal-network.md]]** — supplies the fifth term missing from this page's functional decomposition and one instrument: parietal cortex holds the *first-order* relations that the frontopolar integrator compares, and the developmental series says the relational shift is the integrator being *freed* — activation on second-order trials is flat with age while activation on first-order trials falls, mediated by parietal cortical thinning — which is a third mechanism for `T193` alongside knowledge accretion and executive maturation (Vendetti & Bunge 2014).
- **[[wiki/entities/lateral-frontoparietal-network.md]]** *(fifth link, Mole et al. 2025)* — the contradiction to the entry below it, at ten times the sample: 247 focal patients on an analogy task built from Urbanski's put the deficit in a **right** frontal network centred on caudal middle frontal gyrus, leave posterior patients unimpaired, and recover left-lateralised networks for Stroop and fluency in the same cohort — so this page's integrator has two rival locations (`T336`) and its parietal feed has a causal null.
- **[[wiki/entities/lateral-frontoparietal-network.md]]** *(fourth link, Urbanski et al. 2016)* — turns this page's frontopolar integrator from an activation correlate into a required component, and prices it: a 0.33 cc left BA 10/47 lesion impairs analogy and not a matched perceptual match, the deficit scales with how many of the cluster's own afferent tracts were cut, and it is present even when the relational schema is supplied in words — so the integrator's job is the comparison, not the induction.
- **[[wiki/entities/lateral-frontoparietal-network.md]]** *(third link, Hobeika et al. 2016)* — turns this page's prefrontal dissociation table from single studies into a 27-experiment coordinate map, and adds the geometry: one domain-general integrator (left BA 10, rostral inferior frontal sulcus) with semantic and visuospatial content feeds flanking it ventrally and dorsally, so mapping is one head over per-modality encoders rather than one mapper per content type — and matrix-format problems, this page's psychometric proxy, do not recruit that integrator at all (`T335`).
- **[[wiki/entities/lateral-frontoparietal-network.md]]** *(second link, Parsons & Davies 2022)* — adds the node that decides *what content* the integrator operates on (ventrolateral prefrontal cortex: anterior retrieves semantic knowledge, posterior selects among it, and it tracks the associative strength of a verbal analogy while the frontopolar integrator tracks integration demand in the same trials), and then withdraws the page's stage decomposition: no imaging study has measured all four subprocesses, access has never been imaged, and the phase where no mapping is required out-activates the phase where it is (`T330`).
- **[[wiki/entities/lateral-frontal-pole.md]]** *(Chau et al. 2025)* — the causal-lesion status of this page's frontopolar integrator, and the challenge to it: 27 focal frontal patients put analogy in the left frontal pole (Urbanski et al. 2016), 247 patients put analogy impairment in a frontal network *posterior* to it (Mole et al. 2025), with *identifying* an analogy versus *applying* one the proposed reconciliation — which Urbanski's own supplied-versus-derived-schema null does not support, and which in any case addresses the rostro-caudal half and not the laterality half (`T336`). It also offers a non-relational reading of the same region's job — decompose high-dimensional input into several low-dimensional features — which would make relational integration a special case of projection onto a learned basis rather than a primitive.
- **[[wiki/concepts/relational-bottleneck.md]]** — the open conjecture that would place this page's mechanism downstream of an architectural constraint: because a relational bottleneck re-represents its inputs as patterns of similarity (inner products), it may bias a network toward *learning to implement* structure-mapping rather than having the mapping algorithm authored — stated by Webb et al. 2024 as a link to be established, not one demonstrated.
- **[[wiki/concepts/displacement-codes.md]]** — the cheapest possible structure-mapping substrate, available only because the relation code is metric: if "B relative to A" is a modular vector, then "A is to B as C is to D" is a comparison of two such vectors rather than a graph match — which would move this page's core operation from search to a distance, and is untested because the cell class is a prediction with no recordings (Hawkins et al. 2019).
