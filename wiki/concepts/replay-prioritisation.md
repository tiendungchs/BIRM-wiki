...
- **[[wiki/concepts/multi-horizon-value-learning.md]]** — where a value ensemble breaks this page's rule: averaging temporal-difference errors across ten discount factors to set replay priority *hurts* 4/10 Atari games, which is expected here since `Need` is a row of `(I − γT)⁻¹` and is therefore defined per-`γ` — a mean over horizons ranks nothing (Fedus et al. 2019).
- **[[wiki/entities/state-space-composition.md]]** — a third candidate utility for ordering replays alongside `Gain × Need` and recall-gating, and the only one with no reward term: replay where a *composition is missing*, with the surprising property that ordering does not matter at all — construction by path integration is trajectory-independent, so coverage is the entire criterion (Bakermans et al. 2025).
- **[[wiki/entities/model-free-episodic-control.md]]** — the rival answer to the same sample-efficiency problem, and the cheaper one where the environment is deterministic: prioritisation changes which transitions a parametric learner fits, MFEC stores the return itself and fits nothing, and on the five Atari games they share MFEC leads through the whole initial phase (Blundell et al. 2016).
- **[[wiki/concepts/intrinsic-motivation-typology.md]]** — names the quantity `|δ|` prioritisation borrows: it is the predictive-novelty cell of the intrinsic-motivation typology, a reward formula defined without saying which module consumes it — and the wiki's record is that the same number works as an action bonus and fails as a rehearsal priority, a distinction no typology of rewards can express.
- **[[wiki/concepts/hindsight-goal-relabelling.md]]** — the second, orthogonal knob on the same buffer: prioritisation reweights *which* stored tuples are sampled, relabelling rewrites *what a stored tuple is an example of* by substituting a goal the episode actually reached and recomputing its reward offline. The two are stated to compose and the combination has never been run — which matters here because relabelling changes the `|δ|` distribution this page's rule reads (Andrychowicz et al. 2017).
- **[[wiki/concepts/learning-progress.md]]** — the derivative of this page's quantity, read by the actor instead of by the rehearsal scheduler: `|δ|` ranks *what to replay*, `|Δ PC|` over the last 15 attempts at an activity ranks *what to attempt next*, and the human fit shows the derivative is exactly what keeps a learner out of a stochastic trap (unlearnable activity: 34.98% vs 64.29% full mastery for competence-driven vs progress-driven participants) — the failure mode an error-magnitude priority has no defence against (Ten et al. 2021).
- **[[wiki/concepts/sleep-oscillation-nesting.md]]** — the rival selection story: priority as a stale local tag left by waking attention in the thalamic gating circuit, spent later as a spindle addressed to that cortical network, rather than a `Gain × Need` quantity recomputed at sample time.
- **[[wiki/entities/go-explore.md]]** — the converse of this page's central negative result, completing the pair: `|δ|` works as an action bonus and fails as a rehearsal priority, while a count bonus `1/√(C+1)` fails as an action reward (12,240) and works as a memory-selection priority (1.7M) on the same representation — two data points for the rule that an intrinsic quantity must be specified together with the module that consumes it.
