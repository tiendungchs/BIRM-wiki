...
- **It needs *full* observability to expose modularity.** Community detection and multidimensional scaling on an SR built over observations recover a maze's room structure only when observations identify nodes; under aliasing they recover nothing, because a first-order model of an aliased observation stream has no modularity in it to find. A clone model trained on the identical random walks does recover the rooms and the hyper-rooms above them (George et al. 2021, [[wiki/entities/cscg.md]]) — so this page's spectral machinery is downstream of de-aliasing for hierarchy just as it is for state identity.
- **The grid-like eigenvectors may be an artefact of having a transition matrix at all.** George et al. 2021 report grid-like structure in the eigenvectors of a clone-structured HMM's transition matrix too, and argue the property therefore belongs to any method with a transition operator and "might not have any behavioural relevance" — a suspicion with a supplementary result behind it, not a refutation, but it targets the derivation this page shares with [[wiki/concepts/abstract-structural-codes.md]].
...
- **[[wiki/entities/cscg.md]]** — supplies the state space this page assumes: learn a de-aliased discrete graph first, then run the SR over its clone states. It is also this page's sharpest rival on three axes its own authors name — policy dependence (dynamic planning means recomputing `S`), full observability (an SR over aliased observations yields no communities), and the suspicion that grid-like eigenvectors come free with any transition matrix.
...
- **[[wiki/entities/pfc-columnar-planning-model.md]]** — the on-demand counterpart of the cached matrix: the same retrograde value propagation recomputed as a wave at decision time, which is why an inserted obstacle stops the wave but breaks the successor matrix (Martinet et al. 2011).
- **[[wiki/concepts/learned-world-models.md]]** — the same policy contamination proved for a latent predictor: without the action in its input, the learned transition is provably `E_π[z′|z]`, the behaviour-policy-averaged future, which is what makes `S` policy-dependent restated in representation-learning terms — and the fix is symmetric (condition on the action / use a symmetrised or uniform-policy write rule) (Zhang et al. 2026).
- **[[wiki/concepts/function-to-structure-inference.md]]** — the same path-weighted object appearing as a *nuisance* rather than a code: resting correlations between cortical regions with no tract are predicted by the two-step path sum `Σ_k w_ik w_kj`, so a correlation matrix is closer to a communicability/successor-style summary than to an adjacency matrix, and recovering adjacency means deconvolving it.
- **[[wiki/entities/global-neuronal-workspace.md]]** — the same object read as a workspace step operator: medial-temporal cells code *transitions* between working-memory states and activate prospectively for upcoming list items, so the association network supplies the successor and the broadcast supplies the step.
- **[[wiki/concepts/multi-horizon-value-learning.md]]** — the same many-horizons-one-basis structure on the value side rather than the occupancy side: `ϒ = Σ_n γⁿΛⁿ` re-weights one eigenbasis per discount, a multi-horizon agent re-weights one shared encoder per discount, and both defer the choice of horizon to read-out time.
- **[[wiki/concepts/network-communication-models.md]]** — the home taxonomy of this page's parameter-free sibling: `e^A` is the *broadcasting* communication model, and the property that makes it attractive as a brain code is the one that makes it win in connectomics — diffusing along all walks reaches the target by the shortest path first while requiring no node to know the global topology, so a representational metric shaped like communicability is evidence of a decentralised protocol rather than of an optimiser with a map.
- **[[wiki/entities/state-space-composition.md]]** — the rival answer to the same generalisation demand, and the one that does not relearn: an SR is policy-dependent and brittle to a moved reward or a new wall, where a state space composed from pre-credit-assigned vector bases changes only in *which blocks are active*, so credit assignment becomes retrieval-and-compose rather than propagation (Bakermans et al. 2025).
- **[[wiki/concepts/empowerment.md]]** — the policy-free counterpart of an SR row: occupancy here is discounted expected visitation *under the current policy*, while empowerment maximises over the action distribution and so scores what the agent **could** reach and distinguish rather than what it does reach — in a deterministic world exactly `log` of the `n`-step reachable-set size, which is the same local statistic that makes both quantities track global graph centrality (Salge et al. 2013).
- **[[wiki/concepts/optimal-hierarchy-criterion.md]]** — the job proposed for predictive state statistics that no SR result in the wiki has yet been asked to do: Solway et al. 2014 deny humans compute their model evidence and nominate ordinary predictive learning as the approximator, since recovering community structure and bottlenecks from transition statistics would make the optimal subgoal parse a by-product of an SR rather than a separate optimisation.
- **[[wiki/concepts/eigenoption-discovery.md]]** — what this page's eigenbasis is worth on the action side: the eigenvectors of the diffusion operator are exactly the proto-value functions whose differences `eᵀ(φ(s′) − φ(s))` define eigenpurposes, so an agent already maintaining an SR is already holding a reward-free option library and needs only to solve each eigenpurpose, not to estimate anything new (Machado et al. 2017).
