...
- **[[wiki/concepts/priority-map.md]]** — the acting-to-observe loop in its cheapest measured form: the argmax over a similarity map fixes the next fixation, hence the next observation and the next query, so the evidence gathered about a scene is selected by a policy the scene's own contents wrote (Bichot et al. 2015).
