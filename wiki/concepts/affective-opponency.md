...
- **[[wiki/concepts/higher-order-conditioning.md]]** — a fourth way to build an opponent pair, made by a *training procedure* rather than by chemistry, projection target or tissue: second-order conditioning is a feature-negative discrimination, so excitation and inhibition to the same stimulus are acquired on the same trials, with different time constants (inhibition slower and ultimately stronger, giving an inverted U in training amount) and different positions *within* the trial — probing during the stimulus versus after its offset returns opposite signs in the same animals (`G126`).
- **[[wiki/concepts/token-reinforcement.md]]** — the measurement context this page's two axes need: a token currency denominates gain and loss in one unit, removing the food-versus-shock scaling problem, and token removal is a punisher beyond the reinforcement-density change it causes (yoked control, Raiff et al. 2008). The magnitude is contested — a loss weighted ≈ 3× a gain in one human matching experiment, no bias at all in another (`T396`), which is the difference between a movable origin on one signed channel and two separately-weighted ones.
