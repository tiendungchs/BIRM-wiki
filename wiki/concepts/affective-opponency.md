...
- **[[wiki/entities/lateral-habenula.md]]** — two of this page's missing mechanisms measured in one structure: a sign carried as the GABA/glutamate **ratio** at a single co-releasing pallidal terminal — so a signed error crosses one connection with non-negative quantities on both sides, and the ratio is itself slowly plastic (it tilts toward excitation under chronic stress and is normalized by selective serotonin reuptake inhibitors) — and a slow, glially-written burst gain that is a literal context baseline, whose elevation alone produces anhedonia and behavioural despair with no change in input.
