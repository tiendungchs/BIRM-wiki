...
- **[[wiki/concepts/key-value-memory.md]]** — a confidence signal that does not require an answer: key-query match strength predicts retrievability before the value is read, evidenced by feeling-of-knowing preceding recall and by cue familiarity inflating it without improving recall — the one estimate in the wiki available upstream of the output it would qualify.
