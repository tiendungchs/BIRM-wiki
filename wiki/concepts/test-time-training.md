...
- **[[wiki/concepts/rule-level-evaluation.md]]** — a bound on where inference-time compute pays: on identical ConceptARC tasks, raising reasoning effort buys 9–19 accuracy points in the textual modality and ~0 in the visual one, where the same budget is spent invoking computer-vision libraries instead — so test-time scaling is a lever on search and not on perception.
- **[[wiki/entities/math-dataset.md]]** — the cheapest possible version of "spend inference on the task" (emit more tokens before answering), measured in 2021 as *harmful*: 6.9% → 5.3% on MATH. The difference from this page's method is the only one that matters — TTT's extra computation is scored against the task's demonstration pairs and a generated chain of thought is scored against nothing ([[wiki/empirical-tensions.md]] T217).
- **[[wiki/entities/gsm8k.md]]** — the negative control for adaptation-by-conditioning: GSM-NoOp's drop persists within one standard deviation when the context supplies 8 instantiations of the *same question with its correct reasoning chain*, so the defect is below the in-context-learning layer and no amount of task-matched demonstration reaches it.
- **[[wiki/entities/math-perturb.md]]** — the sharpest prediction against this page, from the cheapest version of it: one-shot conditioning on the nearest solved problem repairs 24–40% of errors and creates 18–40% more, with the misleading half *growing with model capability*, from which the authors hypothesise that any naive fine-tuning over a narrow distribution of problem settings degrades robustness to method-invalidating shift — untested in the weight-update setting.
- **[[wiki/concepts/analog-in-memory-computing.md]]** — this page's operation priced on hardware where a weight write is the dominant energy cost, and the ablation it never ran: restricting the inner loop to the last dense layer turns the update into a delta rule needing no backward pass, and on few-shot image classification that restriction costs nothing measurable (<1% of weights updated, chip on par with software).
- **[[wiki/concepts/loopy-belief-propagation.md]]** — the same inference-time compute lever aimed at a different target: rounds of message passing improve the *estimate* on a fixed graph where this page improves the *parameters*, and the two compose if the loop is initialised from an adapted model.
- **[[wiki/entities/integrated-world-modeling-theory.md]]** — the argument for why iterating is not equivalent to deepening: feedforward unrolling of a recurrent system is an equivalence of input–output maps on-distribution and from matched initial conditions only, and diverges under intervention, so every property that matters off-distribution lives in the recurrence.
- **[[wiki/entities/dreamcoder.md]]** — the induction pole's constructive answer to what an explicit program object is *for*: solutions get stored, compressed and recombined into a growing library, which is the operation transduction has no analogue of (gap G14).
- **[[wiki/concepts/adaptive-computation-time.md]]** — the disjoint way to spend compute at inference: this page changes the *weights* per instance and leaves the forward pass fixed, a halting unit freezes the weights and changes the *number of forward steps* per input — so the two are stackable in principle, no reported system does both, and only the second leaves a readable per-instance record of how much thinking was done (Graves 2016).
- **[[wiki/entities/varc.md]]** — this page's recipe with the pretrained library deleted: TTT over 51 augmentation-derived auxiliary tasks on an 18M ViT trained only on ARC-1 + RE-ARC reaches 54.5% (60.4% ensembled), which the page's "vast latent building blocks dredged from a pretraining corpus" account cannot explain — and it supplies two numbers the page lacked, that independent-per-task TTT beats joint-over-all-test-tasks TTT by ~10 points, and that TTT from *random initialisation* alone reaches 26.4%.
- **[[wiki/entities/vdm-i2i.md]]** — this page's loop run on a frozen *generative* backbone: a per-task LoRA fit on 2–5 ARC demonstrations, with the paper naming the absence of an in-context route as video models' main deficit against LLMs — so here the technique is not an optimisation but the only adaptation channel that exists.
