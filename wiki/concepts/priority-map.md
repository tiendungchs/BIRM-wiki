...
- **What decides the template's *content*?** (`G96` is the delivery half of the same question — a template is useless unless the control layer holds an address for the population it biases; this page's evidence is that biology loads the register with a learned associate chosen for detectability, so neither the content nor the address is read off the goal.) Zhou & Geng 2025 shows it need not be the target: the register can be loaded with a learned associate instead, and the same laboratory's behavioural work says the swap happens only when the target is hard to discriminate and the associate is easy. Nobody has measured the trade-off curve — vary target discriminability and associate predictive validity independently and find the crossover at which the proxy is loaded. Until that exists, "chosen by expected guidance value" is a description, not a policy.
...
- **[[wiki/concepts/broadcast-channel-decomposition.md]]** — a second readout of this page's selection, taken from the same substrate: the superior colliculus is the proposed source of dopamine's *alerting* burst, which is short-latency, keyed to rough features only, correlated with orienting speed, and relocates to the earliest unpredictably-timed event — so "what to look at next" and "broadcast that something important happened" are one computation emitted twice, and the map's argmax has a modulatory consumer nothing in the wiki wires up.
- **[[wiki/concepts/reward-prediction-error.md]]** — the cheapest alternative to a map: instead of ranking candidate locations and taking an argmax, a gaze-moving agent emits `δ = V(t+1) − V(t)` on each shift and hill-climbs the value surface — selection by sampled gradient rather than by comparison, which needs no representation of the candidates but pays one wasted move per rejected direction (Schultz, Dayan & Montague 1997).
...
- **[[wiki/concepts/information-sampling-vs-search.md]]** — a second scoring function for this page's map, measured in the same station family: lateral intraparietal neurons rank candidate cues by *expected reduction of uncertainty*, learned from prediction errors on each cue's own predictions and encoded independently of the reward expected from acting on the cue — so `priority(ℓ)` is template similarity **and** epistemic worth, with a valence-driven third term (reward-based salience) competing in the same stage (Gottlieb & Oudeyer 2018, `T360`).
- **[[wiki/concepts/visual-routines.md]]** — the same two-stage decomposition proposed from task analysis 30 years earlier, with the argmax read differently: local comparison → difference signal → winner inhibits the rest → *only the winner's features reach a central common register*, which makes selection a gate on a narrow read-out channel and predicts the conjunction-search failure from bandwidth rather than from capacity.
- **[[wiki/concepts/active-vision.md]]** — the map with a stated goal and a stated destination: a remembered 81-element feature vector is correlated against every location to yield the saliency map, and the search is confined to the task-relevant region by a stored object→scene template mapped through the scene→retina transform — so a top-down *query* and a top-down *spatial constraint* enter the same map by two different routes (Ballard et al. 1997).
- **[[wiki/concepts/visual-indices.md]]** — the same winner-take-all circuit read as an assignment mechanism for *reference* rather than for action: the winner receives an enabling signal that lets it be probed, using no encoded property of the target and not even its location, and neighbourhood enhancement makes the probe follow the object when it moves — so selection-for-action and reference-without-description are one stage under two descriptions (Koch & Ullman 1985; Pylyshyn 2001).
