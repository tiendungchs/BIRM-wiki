...
- **[[wiki/concepts/learned-industriousness.md]]** — this page's pairing mechanism pointed at an **interoceptive** CS, which converts it from a subgoal-value rule into a cost-learning rule: where an external stimulus paired with reward becomes something worked *for*, the response-produced sensation of high effort paired with reward becomes something that makes working *cheaper*, and the acquired value rides the sensation rather than the task, so it transfers to behaviours never trained. That page inherits this page's unresolved dynamics — whether an unbacked exertion extinguishes the value, and at what exchange rate — neither of which has been measured for a sensation rather than a stimulus.
- **[[wiki/concepts/self-boosting-knowledge-acquisition.md]]** — a manufactured want that claims to outrun this page's audit: a knowledge reward has no primary reinforcer behind it and should extinguish on every unbacked delivery, and the proposed escape is that the store regenerates gaps faster than delivery consumes them — an empirical race nobody has run, with that source conceding that the reinforcement properties of curiosity (resistance to extinction, selective devaluation) are untested (Murayama 2022).
- **[[wiki/entities/option-critic.md]]** — the machine architecture that forgoes this page's mechanism entirely, with both consequences: having no manufactured intermediate reward, it cannot suffer Cronin's subgoal capture, and equally cannot hold an intermediate target that survives devaluation of the primary reward — its options are shadows of the current return and re-shape whenever it changes (Bacon, Harb & Precup 2017).
- **[[wiki/entities/feudal-networks.md]]** — the architecture most exposed to this page's capture result and the one that manages it by dilution rather than by structure: FuN's Worker is paid a cosine for following the Manager's direction whether or not that direction pays, so a manufactured reinforcer competes with the primary one by construction, and the only bounds are a hand-set gain `α < 1` and the Worker's retained access to the environment reward — which the source's "purely feudal" ablation removes, and performance falls (`T401`).
- **[[wiki/entities/diayn.md]]** — a want manufactured with nothing to inherit value from: the pseudo-reward is computed entirely inside the agent from a discriminator over its own states, so subgoal capture is not a failure mode but the entire pretraining phase — and the `log p(z)` baseline, which keeps that manufactured reward non-negative, is what stops the agent ending the episode immediately.
- **[[wiki/entities/asymmetric-self-play.md]]** — **(brainstorm)** the same intermediate layer populated from the opposite end: this page propagates *value* outward from a primary reinforcer already encountered, while an asymmetric proposer propagates *reachability* outward from the initial state with no environment payoff anywhere in the system. An agent given both would have value flowing in from the goal and reachability flowing out from the start; where they meet is what a subgoal is, and no source in the wiki runs the combination (`G33`).
- **[[wiki/concepts/sign-tracking-and-goal-tracking.md]]** — this page's manufactured want measured as an *individual difference*: two rats given the same pairing both nose-poke for the paired stimulus in extinction, but the one that also approaches it works for it significantly harder, so the exchange rate this page reads off the pairing is partly a property of the learner — and the difference is carried by whether accumbens dopamine was recruited during acquisition (`T404`).
