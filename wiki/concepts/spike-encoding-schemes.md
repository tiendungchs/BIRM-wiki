...
- **No compositional encoding.** Every scheme listed encodes a *scalar* or a *vector of scalars*. None encodes a relation, a binding or a structure, so the interface into every spiking architecture in the wiki is flat ([[wiki/concepts/vector-symbolic-binding.md]] is the only wiki mechanism that would need otherwise). **One counter-example now exists and it is outside this taxonomy.** Kleyko et al. 2025 transmit a sparse-block-code VSA vector on Loihi 2 as 24-bit **graded spikes** — fixed-point vector components carried by event packets — so what crosses the wire is a *binding*, not a scalar ([[wiki/entities/sigma-pi-reservoir.md]]). It costs `DL` multiplicative (Pi) neurons and `dD/L` embedding synapses. But a graded spike is neither a rate nor a timing code: the payload is a number in the packet, so the compositional encoding is bought by giving up the property that makes this taxonomy a taxonomy. Whether a relation can be encoded in *spike timing alone* is still open.
...
- **[[wiki/entities/sigma-pi-reservoir.md]]** — the counter-case to this page's "no compositional encoding": a sparse-block-code vector carried as Loihi 2 graded spikes encodes a *binding* rather than a scalar, so the interface into a spiking architecture need not be flat — with a resource price attached (`DL` Pi neurons, `dD/L` embedding synapses) and a graded-spike payload (24-bit fixed point) that is neither a rate nor a timing code, i.e. outside this page's taxonomy entirely (Kleyko et al. 2025).
- **[[wiki/concepts/synaptic-plasticity.md]]** — an argument for this page's timing family from the *write* side rather than from the reader: with a per-neuron plasticity efficacy that zeroes at each spike and recovers over 34–75 ms, spike-timing plasticity records only the arrival order of the leading spikes of a volley and is deaf to the rest of the burst (Froemke & Dan 2002, [[wiki/empirical-tensions.md]] T329) — so the biological learning rule is a rank-order/time-to-first-spike consumer whatever the downstream reader does.
