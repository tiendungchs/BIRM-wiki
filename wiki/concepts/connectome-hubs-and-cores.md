...
- **The weak edges are not the disposable ones, and every pruning scheme assumes they are.** Modal controllability — the capacity to drive the system into states no cheap trajectory reaches — is `r = −0.99` with weighted degree, so the sparsely connected non-hubs are exactly the population that makes hard transitions possible (Gu et al. 2015, [[wiki/concepts/network-control-theory.md]]). The converging biological evidence is that weak-connection topology in resting fMRI classifies schizophrenia and correlates with IQ *better* than strong-connection topology. Magnitude pruning, top-`k` routing and threshold-based graph construction all delete this population first, and none of the wiki's pruning results report what happens to the network's reachable set as opposed to its accuracy.
...
- **[[wiki/concepts/network-control-theory.md]]** — the target-relative successor to this page's six converging measures, and the page that mostly re-derives them: minimum control energy asks which nodes drive the system to a *named* target state where degree and betweenness ask nothing of the sort, but on a measured connectome the node-level diagnostics collapse onto this page's weighted-degree column — average controllability `r = +0.91`, modal `r = −0.99`, both replicated in an 85-subject diffusion-tensor cohort and in macaque tract tracing (Gu et al. 2015, [[wiki/empirical-tensions.md]] T324). Two consequences: this page's core (precuneus, posterior cingulate, superior frontal, paracentral) *is* the set of regions that move the brain cheaply to many states, and the only control diagnostic carrying information this page does not already have is **boundary** controllability (`r ≈ 0` with degree), which ranks nodes by position between communities rather than by edge mass and is the one statistic here with no counterpart in the six.
...
- **[[wiki/concepts/timescale-hierarchy.md]]** — a rival ranking of which nodes matter, computed on the same kind of graph with heterogeneous nodes: lesion impact on the global correlation structure is predicted by an area's **intrinsic time constant**, hierarchy alone gives `r² = 0.18`, and the top influencers (STPc, PBr) sit at intermediate depth — so the six graph measures here miss the dynamical variable (Chaudhuri et al. 2015).
