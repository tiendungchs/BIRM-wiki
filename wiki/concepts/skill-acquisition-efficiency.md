...
- **[[wiki/entities/gpqa.md]]** — the direct challenge to this page's exclusion of crystallized ability: PhDs in other sciences, paid to succeed, with unrestricted web access and a 37-minute mean, reach 34% where in-domain experts reach 65%, so the excluded "accumulated knowledge" is not a store a retriever can substitute for — and its size runs opposite to machine difficulty across the three domains ([[wiki/architectural-gaps.md]] G17).
- **[[wiki/concepts/certification-instruments.md]]** — supplies instrument `I5`, developer-aware generalization difficulty — the only quantity in the inventory rather than a genre — and the conditioning that the inventory's second conclusion generalises into its organising principle.
- **[[wiki/concepts/human-baseline.md]]** — the human is the only reference system whose priors are enumerable but not authored by the benchmark's builder, so this page's central quantity has no scale until the baseline protocol is fixed.
- **[[wiki/entities/frontiermath.md]]** — the regime where the `P` term cannot be purchased at any price: >200 solution techniques with the most common pair appearing in three problems, and a contributor's estimate that the relevant training data for a given item is "a dozen papers" — the exact complement of MATH's AMPS result that a curated corpus over 100 named topics is worth a 130× parameter increase.
- **[[wiki/entities/hle.md]]** — the design rule inverted: HLE selects items that are *hard for experts too*, the quadrant "easy for humans, hard for AI" exists to exclude, so on this page's framing it measures the size of a retrievable store — except its own searchability audit finds that removing retrievable items barely changes frontier scores (G17).
- **[[wiki/concepts/discrete-infinity.md]]** — a priors-to-skill conversion rate measured across species rather than across models: chimpanzees need thousands of trials and years per integer with each numeral costing the same, while a child who has 1–3 acquires all the rest from one induction over the successor function.
- **[[wiki/concepts/emergent-modularity.md]]** — adds a term this page's conversion rate does not carry: the human neonatal brain is ~27% of adult size against 36% in chimpanzees and 70% in macaques and keeps its fetal growth rate through the first year, so priors, experience and skill are causally sequential through a **growth schedule** — two learners with identical priors and identical environments can differ in acquired skill purely by when their capacity arrived.
- **[[wiki/concepts/motivation-representation-synergy.md]]** — this page's decomposition taken to its limit by the desert-island argument (Tomasello & Rakoczy): with priors and architecture held at the great-ape value, a human raised without culture would score close to a great ape, so nearly all measured human skill sits in the experience term — and the two factors of that page are what make the experience stream cumulative.
- **[[wiki/concepts/shared-intentionality.md]]** — the origin of the desert-island argument this page already cites, with its mechanism: the experience term is large because it is *culturally assembled*, and the two operations that make it cumulative (imitative learning through the demonstrator, and derived normativity fixing which use is correct) are dated to ~1 year and absent in apes.
- **[[wiki/concepts/developmental-heterochrony.md]]** — supplies a mechanism and a first magnitude for the schedule term: two learners with identical priors and identical experience can differ in acquired skill by a per-module time-warp alone, measured here as a ~4% subset of prefrontal genes shifted late, concentrated in gray matter and in early adolescence.
- **[[wiki/entities/neural-episodic-control.md]]** — the wiki's largest Atari data-efficiency gain attributable to *where the value is stored* rather than to schedule, exploration or depth: 54.6% median human-normalised at 10M frames against 22.4% for prioritised replay — and the gain reverses by 40M, so storage location buys early efficiency and costs asymptote.
- **[[wiki/concepts/replay-prioritisation.md]]** — a confound for every claim that a sparse-reward failure is representational: an exponential sample-complexity gap is produced by the sampling schedule alone, with architecture, loss and buffer contents held fixed.
- **[[wiki/concepts/abstraction.md]]** — extreme generalization defined as handling tasks that share only *abstract* commonalities makes the generalization spectrum an abstraction ladder read from behaviour, which is that page's family U turned into a score.
- **[[wiki/concepts/reasoning.md]]** — what a score means once framing and search are separated: a benchmark that supplies the representation measures skill, and only the developer-aware condition makes a result informative about the contested half.
