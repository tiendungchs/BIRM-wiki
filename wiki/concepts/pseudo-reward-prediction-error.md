...
- **[[wiki/entities/option-critic.md]]** — the architecture in which this page's quantity does not exist: options are learned from the task reward alone, with the option identity entering as an *index* on the critic (`Q_U(s,ω,a)`) rather than as a second reward, so there is one error to route and `T367` does not arise — which makes it the constructive single-channel pole and the null hypothesis any own-channel hierarchy owes a comparison against (Bacon, Harb & Precup 2017).
- **[[wiki/entities/feudal-networks.md]]** — the machine instance of this page's signal, with the ratio it flagged as unasked actually swept: the Worker's `r^I_t = (1/c) Σ d_cos(s_t − s_{t−i}, g_{t−i})` is a pseudo-reward against a subgoal the *upper level invented*, and the weight `α ~ U(0,1)` is the `r̃` : `r` ratio, whose optimum is domain-dependent (high on Gravitar and Amidar, below 1 elsewhere) rather than `1` : `1` by fiat. Two structural differences from the measured signal: the subgoal is a direction rather than a state, so the reward is graded and never collected, and it is re-set every step with no termination (Vezhnevets et al. 2017).
- **[[wiki/concepts/state-prediction-error.md]]** — the wiki's other non-primary error term, and the contrast that sharpens both: that one is an error about *which event occurred* and writes a transition edge, this one is an error about *value* over a subgoal and writes a policy — yet both are carried by the same structures and both raise the same unanswered question, what stops a non-primary error from updating the root value function (`T367`).
