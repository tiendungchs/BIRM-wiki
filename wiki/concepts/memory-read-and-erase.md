...
| **Replace** | Write over a matched slot — the standard case; in a *distributed* store, `W ← W + β(v − v̄) ⊗ φ(k)`, which provably leaves every orthogonally-keyed association exact ([[wiki/concepts/fast-weight-programming.md]]) | Separable in cortical representational geometry from both suppress and clear |
...
| **Reverse read** (given content, return every item holding it) | Two stores, and they price it differently. **A duplicate matrix**: a second STDP matrix wired `content → cue` with a two-interneuron arbiter, answering the union over any-overlap in 6 steps ([[wiki/entities/spiking-hippocampal-cam.md]]). **Or no extra matrix at all**: where the content layer is *bidirectionally* plastic the reverse direction is native to the weights already there — the hippocampal↔sensory layer of [[wiki/entities/vector-hash.md]], whose memory-palace result runs precisely that way, recalled sensory states cueing the scaffold that then addresses new content. Every other store here exposes one direction and answers the reverse only by scanning | Not measured; the operation is proposed as CA3's, not observed as a separable signal |
...
**The read anticipation is not stimulus anticipation, and the founding measurement says so with a delay manipulation** (Lundqvist et al. 2016, `raw/lundqvist-2016-gamma-beta-bursts-underlie-working-memory.md`, the earlier recording this section's 2018 result extends). Two further monkeys, a **delayed saccade** task in which nothing appears at the end of the delay — the go cue is the *removal* of the fixation dot, so there is no stimulus to anticipate and no comparison to make, only a stored location to use. Gamma burst rate rose and beta fell over the end of the standard 750 ms delay; on the 1/7 of trials whose delay was doubled to 1500 ms, the elevated gamma and suppressed beta were **maintained across the entire second half** rather than decaying after 750 ms. So the schedule is indexed to *the memory becoming due*, and it holds the read regime open until the read happens rather than firing once at a predicted time.

**And the reader is a different population from the holder.** In the same 2016 dataset, the 71 informative prefrontal units split by peak time into 50 **encoding/decoding** cells — active at presentation, suppressed at delay onset, ramping again over the second half of the delay in lockstep with gamma burst rate — and 21 **maintenance** cells, active mid-delay when beta burst rate is high. Their information time courses track their firing. This is the sharpest form of this page's claim: the access protocol is not a phase of the store's activity, it is a separate set of cells whose recruitment is what a read *is* ([[wiki/empirical-tensions.md]] T86).
...
**One store deletes the erase operation entirely, which is the other way to satisfy the specification** ([[wiki/entities/three-factor-key-value-memory.md]], Tyulmankov et al. 2021). The write is `K_{t+1} = (1 − η_t) ⊙ K_t + η_t ⊙ [1 x_tᵀ]` with `η_t = q_t γ_t` — a global write-enable times a local slot selector — so the `(1 − η)` factor zeroes exactly the synapses the second term fills. Three consequences against the table above:

- **There is no decay constant and no eviction policy.** A slot is emptied *by being chosen*, and a slot nobody chooses is never touched, so retention is bounded by the allocation rate rather than by a schedule. That is the cheapest possible answer to the row `G42` complains about — forgetting asserted everywhere without a rate.
- **It is not relevance-addressed, and cannot be.** The gate selects a slot by cycling or by a coin, never by what the slot holds; the store can say *how long ago* but not *what for*. So the erase is free precisely because it is the least informed kind.
- **It supplies the one removal primitive the table has no column for — *refuse to be erased*.** A per-synapse metaplasticity bit latches when `[η_t]_ij > 1`, reachable only under a large global third factor (`q_t = 10`), and thereafter suppresses all plasticity at that synapse. Flashbulb memories survive indefinitely at minimal cost to ordinary recall. In a superposed store the same protection trades directly against capacity, so this is a property of slot storage, not of the latch.
...
**(brainstorm) The type of the removal may not belong in the store at all.** Sensory cortex carries only the occupancy bit while the frontoparietal network carries the full four-way identity, so *why* an item was removed never reaches the carrier. That argues for keeping the operation code in the controller and shipping only its effect downstream, rather than tagging memory entries with a removal reason — the opposite of what a usage/priority field in a machine store does. **A built store now argues the other way**, and it is worth the qualification: [[wiki/entities/spiking-hippocampal-cam.md]] recovers its operation code *inside* the carrier, from the relative arrival time of the cue and content volleys, so the controller emits content and nothing else. The two positions are compatible only if the biology's four-way type is doing something the timing-recovered type is not — and the engineering case is the cheaper one whenever the operations differ in the temporal structure of their inputs, which `replace` / `recall` / `store` naturally do.
...
- **[[wiki/entities/spiking-hippocampal-cam.md]]** — the closest existing implementation of this page's specification, and the one that contradicts its control-layer conclusion: the erase is addressed at the cue being overwritten and made selective at unit granularity by the refractory period, and the *operation type* is recovered inside the carrier from a 1 ms delay differential between the cue and content volleys rather than supplied by a controller. It implements `replace` and still has no `suppress`, and its read has no schedule (G49).
- **[[wiki/entities/vector-hash.md]]** — the cheap route to this page's reverse-read row, and the fidelity asymmetry that decides what the reverse read is *for*: because the addressing stage is frozen and content is hung on a plastic bidirectional layer, a content cue returns its scaffold state **exactly** while the scaffold returns content only approximately — so running the store backwards is worth doing to recover an *address*, and running it forwards is worth doing to recover an *item*. It also bounds what a reverse read can mean in this family: a frozen random projection leaves no similarity structure, so "given content, return what is like it" degenerates to "return whose basin this cue is in".
- **[[wiki/entities/neural-episodic-control.md]]** — the null protocol, argued for explicitly: write every step with no gate (learning *when* to write is judged too slow), read on demand with no schedule, and erase demoted to a heuristic keyed on retrieval recency — the least-recently-returned-as-a-neighbour entry is overwritten, which is the wiki's only eviction rule addressed by *reads* rather than by write age.
- **[[wiki/concepts/fast-weight-programming.md]]** — the `replace` this page specifies, built and proved: read the value currently at the key, subtract it, write the interpolant (`W ← W + β(v − v̄) ⊗ φ(k)`), and every association with an orthogonal key is left *exactly* unchanged — where the gated alternative `(1−β)W + βv⊗φ(k)` decays the whole store, i.e. is a global clear disguised as a targeted write. The controller also emits a write *strength* `β = σ(W_β x)`, a fourth output field alongside address, lead time and removal type. Still no `suppress`, no reclaimed capacity, and no read schedule (Schlag et al. 2021).
- **[[wiki/entities/model-free-episodic-control.md]]** — the same ungated write as its successor with the content-sensitivity moved into the *value* rather than the decision: everything is written, eviction is least-recently-used, and the only selectivity anywhere is `max{Q, R_t}`, a write that can raise a stored value and never lower it.
- **[[wiki/concepts/sleep-oscillation-nesting.md]]** — an erase that shares its trigger with the read: the ripple that transports an ensemble to cortex downscales the hippocampal synapses that generated it, inside a global sleep downscaling in which retention is the local exception.
- **[[wiki/concepts/visual-routines.md]]** — this protocol's two halves implemented in a spatial frame: *marking* is an address that must survive an eye movement, and the counting routine's "switch off the element under the focus" is a `clear` addressed by current location rather than by content — the cheapest relevance-addressed erase in the wiki.
- **[[wiki/concepts/visual-indices.md]]** — an address with nothing behind it: the index is a pure pointer, its `clear` is triggered by the *world* (the referent leaving the field) rather than by the controller, and its decay-and-reactivation cycle is named by the source and left unmodelled — the read/erase protocol with the content side deleted (Pylyshyn 2001).
- **[[wiki/entities/continuous-modern-hopfield-network.md]]** — the cheapest forgetting operator in the wiki, and it is one addend: subtract `γ(t−i)` inside the log-sum-exp and the store becomes exponentially recency-weighted with its convergence argument untouched, so "forget" is a term in the energy rather than an eviction policy over a buffer. Nothing is actually removed, `γ` is never scheduled, and the same construction with causal masking makes the pattern list unbounded (Ramsauer et al. 2020).
- **[[wiki/entities/three-factor-key-value-memory.md]]** — the one store in the wiki where erase is not a primitive: the `(1 − η)` factor zeroes exactly the synapses the write then fills, so forgetting is free, synchronous with the write, and scheduled entirely by the allocation gate rather than by a decay constant or an eviction policy. It also supplies the removal-policy operation this page's inventory lacks — *refuse to be erased*, a metaplasticity bit latched by a large global third factor (Tyulmankov et al. 2021).
- **[[wiki/concepts/key-value-memory.md]]** — the position that would delete one of this page's primitives: if forgetting is always retrieval failure, removal is not an operation a store performs and the typed family here collapses to gain control on keys — logged as `T392`, with this page's separable suppress/clear/replace geometry as the evidence against.
