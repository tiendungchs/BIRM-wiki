...
| **Reverse read** (given content, return every item holding it) | One store: a second STDP matrix wired `content → cue` with a two-interneuron arbiter, answering the union over any-overlap in 6 steps ([[wiki/entities/spiking-hippocampal-cam.md]]) | Not measured; the operation is proposed as CA3's, not observed as a separable signal |
