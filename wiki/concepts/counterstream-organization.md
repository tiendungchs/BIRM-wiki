...
- **[[wiki/concepts/inter-areal-synchrony.md]]** — the dynamical counterpart the anatomy predicts: supragranular gamma and infragranular beta map onto the two compartments rather than onto the two directions, so a frequency-tagged direction estimate and an SLN-derived one measure different things wherever a pathway mixes compartments.
