...
- **[[wiki/concepts/visual-indices.md]]** — the third account of the same ~4 signature, and the one with a discriminating prediction: a fixed pool of *referring devices* rather than binding interference or a separate counting regime, which predicts the limit applies to bare reference — items merely tracked, with no property bound to them at all — where an interference account predicts the cost should scale with how much is bound (Pylyshyn 2001).
