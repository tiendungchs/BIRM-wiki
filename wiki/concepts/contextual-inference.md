...
- **[[wiki/entities/fluxx.md]]** — the domain that needs this page's machinery to be doing hardness-6 work rather than search: when other agents rewrite the legible rule set, the live rule-config is exactly a context latent with its own dynamics, and the generator (a fixed shuffled deck) is countable, so the posterior over "which rules are in force and how long will they last" is estimable by counting.
- **[[wiki/entities/nomic.md]]** — where this apparatus runs out: `rule_config` is still a context latent, but the transition kernel governing that latent is itself part of the latent, so there is no fixed generator left to infer online.
- **[[wiki/concepts/connectome-state-transformation.md]]** — the same context variable split across the two directions: the generative half must be *told* which of seven task states to transform into, while the state-fingerprinting half recovers the state from the connectome at 74% of seven-way choices — a context estimator and a context-conditioned generator that were fitted separately and never closed into a loop.
- **[[wiki/entities/mediodorsal-thalamus.md]]** — the same context latent obtained without any posterior, which prices what the inference machinery buys: the context variable is *pooled from the content population* (>75% of the thalamic delay-period variance explained by prefrontal cue-selective cells, confirmed by matching a model ablation against a terminal suppression), so it carries no uncertainty, cannot allocate a new context, and cannot signal a context the content cells have never expressed — and its whole output is a two-signed gate on reachability rather than a mixture weight (Rikhye et al. 2018).
- **[[wiki/concepts/transthalamic-context-routing.md]]** — the degenerate limit of this page's latent: a context variable that is neither inferred nor pooled from the gated population but simply *relayed* from an upstream area's layer-5/6 output, arriving as a slow scalar with no uncertainty, no allocation capacity and no dependence on the receiver's own evidence (Neske & Cardin 2025).
- **[[wiki/entities/cn-dpm.md]]** — this page's machinery with the component's contents scaled from a scalar to a deep classifier plus density model, which forces two amendments the formalism does not contain: allocation must be **deferred** (a network cannot be fitted to the single sample that triggered it, so novel data buffer until there are `M` of them and the component is then trained offline to convergence — stream autocorrelation substituting for the task label), and the responsibility likelihood becomes a *learned* generative model whose discriminability then caps the system, measured at 48.18% correct retrieval over five components against components that forget 0.0 points (Lee et al. 2020).
- **[[wiki/entities/ch-hnn.md]]** — the same job done discriminatively and amortised: one feedforward pass over the stimulus emits the mask that selects which units may fire and learn, instead of maintaining a posterior over which stored context generated the data — cheap and task-oracle-free, but it produces no uncertainty and has no way to report that *no* stored structure applies.
- **[[wiki/concepts/environment-invariance.md]]** — the missing half of each other: invariant risk minimisation is *handed* its environment partition and cannot infer one, while this page's machinery infers context membership and has no criterion for which correlations should survive across contexts **(brainstorm)**.
- **[[wiki/entities/elastic-weight-consolidation.md]]** — the importance-gated method this page's relevance-gated alternative is defined against, and the reason the contrast is narrower than it looks: EWC's own agent infers its task context as the latent of a hidden Markov model rather than being handed it, so the difference is not inference-vs-oracle but *what the inferred signal gates* — a per-parameter anchor there, a per-memory update scale here.
- **[[wiki/concepts/independent-causal-mechanisms.md]]** — the consumer this page's shift detector would serve: the Sparse Mechanism Shift criterion scores a factorization by how few factors a distribution change touches, which presupposes machinery that detects *that* a shift occurred — the loop `G6` leaves open **(brainstorm)**.
- **[[wiki/entities/cerebellum.md]]** — the ancestor of this page's responsibility signal, 23 years earlier and with a controller attached: MOSAIC computes responsibility as prior × likelihood where the **prior** is a learned cue→responsibility predictor and the **likelihood** is a paired *forward model's* prediction error, and the same signal then gates that module's **inverse model** — so a policy is trained only in the region where its own world model predicts well, a coupling COIN drops when it shrinks a module to a scalar state.
- **[[wiki/concepts/structural-learning.md]]** — the rival decomposition of this page's own data, and the direct answer to its "the contexts are atoms" open problem: ±60° visuomotor rotations read as two settings of one continuous metaparameter on a learned subspace rather than as two allocated contexts, evidenced by an exploration anisotropy — variance compressed orthogonal to the structure, and search running *along* it even on off-structure probes — that an exchangeable atom has no way to express (Braun et al. 2009, [[wiki/empirical-tensions.md]] T350).
