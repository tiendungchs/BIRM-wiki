...
- **[[wiki/concepts/spike-encoding-schemes.md]]** — the same ratio failure one level down: bits-per-spike (`log₂(N+1)` for count rate against `log₂(N!)` for rank order) is disowned by the survey that quotes it because a code's capacity is only realisable by a reader that can decode it — capability-per-parameter has the same defect and the same fix, score the pair.
- **[[wiki/concepts/circuit-size-separation.md]]** — the same scoring move applied to a substrate rather than a system, and the only place in the wiki where the ratio is *lower-bounded* rather than estimated: Maass 1997 proves `Ω(n)` sigmoidal hidden units for functions a single spiking neuron computes, which is a density separation between unit types with no reference to accuracy on any task.
- **[[wiki/entities/cn-dpm.md]]** — the memorising asymptote reached by an explicitly principled route: a Bayesian-nonparametric criterion allocates a whole new classifier-plus-density-model per detected regime, with lateral feature reuse but no factorisation, so description length grows with the number of environments and the learner buys zero forgetting at `ℐ → 0`.
- **[[wiki/concepts/memorisation-vs-generalisation.md]]** — the same knowing/memorizing axis with the interpretability cost attached: at the `ℐ(n) → 0` end there are no macrofeatures, hence no intermediate states to name, so a system's position on this page's scale predicts whether a mechanistic account of it can exist at all.
- **[[wiki/concepts/convergent-circuit-motifs.md]]** — where `ℐ` should be maximal in nature: under severe miniaturisation selection acts directly on `C(S)` with `log₂N` held by ecological demand, making small-brained planners (salticids at <5×10⁵ neurons) the empirical high-density case this page predicts but never names `(tentative)`.
- **[[wiki/concepts/cellular-scaling-rules.md]]** — fixes the units of the denominator: `C(S)` measured in substrate (mass, volume, metabolic cost) and `C(S)` measured in units are related by a clade- and structure-specific exponent (`M ∼ N^1.06` in primates, `N^1.55` in rodents), so a density comparison that switches currencies between systems is reporting the exponent rather than the systems — and the human brain's position on the primate line is the worked case of a value that looks exceptional in one currency and predicted in the other.
- **[[wiki/concepts/abstraction.md]]** — the identity neither page had recorded: Barsalou's concept as "a competence for generating infinite conceptualizations" is `C` fixed while `log₂N` diverges, so this measure is the formalism the wiki's behavioural abstraction tests were already using in words.
- **[[wiki/concepts/width-versus-parameter-count.md]]** — a measurement that forces the denominator to be declared: at a constant weight count, accuracy rises with unit count alone, so `C(S)` in parameters and `C(S)` in units are separately purchasable and a density score that does not say which it divides by is unreadable (Golubeva et al. 2021).
