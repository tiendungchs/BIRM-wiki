...
- **[[wiki/entities/dorsal-visual-stream.md]]** — the pathway the operation is carried on, and a constraint on any single-site implementation of it: the conversion has at least three serial stages whose lesions fail independently in humans, and a *direct* caudal-inferior-parietal → hippocampal-formation projection runs alongside the relay — so the store can be addressed without the conversion at all (Kravitz et al. 2011).
- **[[wiki/concepts/mental-imagery.md]]** — the empirical warrant for this page's bidirectional-weight premise and the price it charges: classifiers trained on perception decode imagery even when constrained by voxel-wise perceptual feature models, so the two directions share a format, while the reverse pass is weaker and less perception-like the further it travels from the driver.
- **[[wiki/concepts/active-vision.md]]** — the operation used to store a *task constraint* rather than a pose: "the resource area" is held as a template in object→scene coordinates and positioned retinotopically by the scene→retina transform, and composing the two reaches a remembered object outside the current field of view — which is the deictic answer to "people reach for things they cannot see, so they must hold a 3-D model" (Ballard et al. 1997).
