...
- **[[wiki/concepts/information-bottleneck.md]]** — the same "read the architecture off a spectrum" move in the statistical rather than the dynamical setting: modes above this page's spectral gap are the ones worth keeping, as the eigenvalues of the conditional second-order correlations are claimed to decide how many representational levels to keep (`G67`, `G29`).
- **[[wiki/concepts/timescale-hierarchy.md]]** — this ladder run at rung 4 across 29 areas rather than within one: threshold-linear E/I masses (and the Wong–Wang reduction for the nonlinear case) coupled by tract-tracing weights, with the eigenvalues of the *inter-areal* coupling matrix playing the role that page's operator `Q` plays for a single ensemble — the spectrum is the timescale bank (Chaudhuri et al. 2015).
- **[[wiki/concepts/timescale-estimator-bias.md]]** — the same spectrum estimated from the opposite end: there the time constants are *inferred* from data by fitting a deliberately generic Ornstein–Uhlenbeck mixture through approximate Bayesian computation, here they are eigenvalues of an operator *derived* from a hand-specified neuron model, and nothing has run both on one system to see whether the inferred mixture recovers the derived spectrum.
