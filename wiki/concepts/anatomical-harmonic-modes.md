...
- **[[wiki/entities/integrated-world-modeling-theory.md]]** — the same word for a different object, and the joint test nobody has run: harmonic modes here are a *fixed* spectral basis generated from wiring before any activity, there they are *transient self-organizing* complexes formed by activity over that wiring, so the open question is whether the transient complexes are excitations of this page’s basis.
- **[[wiki/concepts/eigenoption-discovery.md]]** — the same operator read as a goal rather than as a coordinate: there the eigenvectors of a *transition* Laplacian are turned into intrinsic rewards to climb (`rᵉ = eᵀ(φ(s′) − φ(s))`), here the eigenvectors of an *anatomical* Laplacian are a basis to express state in — so one spectral decomposition could in principle supply both the coordinate system and the purposes defined over it (Machado et al. 2017).
