...
| **Never publish the items** | A closed submission channel | Nothing — it *prevents* rather than detects, and makes third-party audit impossible (G17) | — | [[wiki/entities/frontiermath.md]] |
...
- **[[wiki/architectural-gaps.md]]** — G17 absorbed both contamination disputes this page carried: a near-zero train−test disparity is the both-splits-leaked signature as readily as the clean one, and a permanently private set relocates the failure into unauditability rather than solving it.
...
- **[[wiki/entities/prm800k.md]]** — a leak this page's whole detector table is structurally blind to, because it is neither accidental nor hidden: 4,500 of MATH's 5,000 test problems were *documented as moved into* a public process-supervision training corpus to avoid overfitting the train split. `δ_train−test` returns nothing, since both splits are training data for anyone using PRM800K — which is most of the process-supervision literature. The defect this page names as opacity is here replaced by disclosure that nobody downstream propagates.
- **[[wiki/entities/blindtest.md]]** — the leakage channel closed by construction rather than by audit: tasks authored from scratch, absent from the internet, and requiring no retrievable world knowledge, which is why its null is stronger evidence than a null on a knowledge-loaded benchmark — and the authors reject human acuity charts as targets precisely because those *are* on the internet.
