...
|---|---|---|---|---|
| **CS → US(motivational)** | the current motivational value of the *specific* predicted outcome, retrievable by its cue | reinforcer devaluation after first-order conditioning; second-order conditioning; conditioned reinforcement | tracks motivational state **directly**, with no relearning | basolateral amygdala ([[wiki/entities/amygdala.md]]) |
| **CS → affect** | a US-*general* value state ("pure" good/bad), carrying no outcome identity | **transreinforcer blocking** — a shock-paired CS blocks conditioning to a CS for omitted food, two reinforcers sharing nothing but aversiveness | unknown; the source calls this representation "poorly understood" | unassigned |
...
- **How central states of "affect" relate to the values governing instrumental action is unstated by the source** — the CS→affect representation has an assay (transreinforcer blocking) and no substrate and no update rule.
