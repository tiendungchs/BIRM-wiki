...
- **Nothing here makes exploration an output of the system.** The itinerant wandering through the repertoire is produced by a swept noise amplitude and a swept global coupling, both set from outside, so metastability supplies the *mechanism* `G61` wants (a selector that leaves its own attractor) and never the closed loop.
...
- **[[wiki/entities/affordance-active-inference-model.md]]** — a stable heteroclinic channel used as a *prior belief* rather than as network dynamics: the itinerancy is what the agent expects the world to do, so the same trajectory that generates behaviour also generates an error when the world departs from it, and nesting two of them at an 8:1 period ratio makes the slow one set the fast one's rate.
...
- **[[wiki/concepts/cortical-traveling-waves.md]]** — the same Kuramoto-on-a-connectome system measured spatially instead of globally: `σ_R` says how much the coalition structure churns and says nothing about where activity goes, while the instrength gradient of the *same* weight matrix says every wave runs from low- to high-instrength cortex — and both statistics require the identical interior regime, since waves vanish below critical coupling and at full synchrony alike (Koller et al. 2024).
- **[[wiki/concepts/parallel-timescale-streams.md]]** — the coalition churn `σ_R` quantifies, resolved into symbols: fitting an authored alphabet of 126 network-combination masks gives *which* coalition and *in what order*, and the ordering rule — ~70% of transitions differing by one network, ~10% long jumps — is preserved across three orders of magnitude of churn rate (Alderson et al. 2026).
- **[[wiki/concepts/small-world-topology.md]]** — the statistic this page's coupling-gain result invalidates as an instrument: `σ` on a *functional* graph falls when one global scalar is lowered with structure untouched, so a reported small-worldness confounds topology with coupling.
- **[[wiki/concepts/ignition.md]]** — the ignited state is metastable rather than a lock-in: conscious access is described as a trajectory through a series of metastable states, and the dwell time in each is what sets the step rate of serial computation over parallel modules.
- **[[wiki/concepts/cortical-state-bistability.md]]** — the churn this page measures as `σ_R`, resolved into two named regimes on one cortical patch: gamma-distributed beta occupancy (1.2 s at rest, matching 1.54 s perceptual dominance) alternating with low-frequency release bursts, so the dwell statistic has an explicit hold/release mechanism behind it rather than an order parameter alone.
- **[[wiki/entities/global-neuronal-workspace.md]]** — conscious access described as a trajectory through a series of metastable states rather than entry into a single attractor, with seen and unseen trials diverging as trajectories, which is what makes ignition decodable rather than merely energetic.
- **[[wiki/concepts/stationary-surrogate-null.md]]** — the baseline `σ_R` and "fluidity" are missing: a covariance- and spectrum-matched stationary surrogate reproduces windowed-connectivity variability on infra-slow signals, so a variance-of-connectivity statistic is only an operating-point measurement once the surrogate's value is reported next to the data's (Laumann et al. 2017).
- **[[wiki/concepts/timescale-estimator-bias.md]]** — the minimal generative alternative this page's statistics have never been scored against: a mixture of leaky integrators fitted so that synthetic and observed summary statistics carry the same finite-sample bias, with a Bayes factor for how many components are warranted — so a claim that dwell times or coupling variance indicate an operating point is testable against "this is a sum of Ornstein–Uhlenbeck processes" rather than against nothing.
- **[[wiki/concepts/attractor-identification.md]]** — names the discrimination this page rests on as the one criterion that does the work: localization and flow-back are satisfied by a trajectory deflected by ghost attractors as well as by one captured in a basin, so multistability is claimed only on long-dwell invariance across conditions and across the removal of tuned input.
