...
- **[[wiki/concepts/width-versus-parameter-count.md]]** — the same squeeze evaluated instead of merely priced: this page reads falling density under growth (52% → 31–37%) as a cost paid in lost redundancy, while a controlled machine sweep finds the optimum for convolutional models *at* 1–10% connectivity with the weight count fixed — so thinning the edges while adding units may be where the arithmetic wants to be, and the biological compensation pattern is what an optimum looks like from inside **(brainstorm)**.

- **[[wiki/entities/ligo.md]]** — units and edges as two separable *growth operators* rather than two scaling exponents: LiGO factorises expansion into a width operator and a depth operator that are ablated independently (51.7% FLOPs saved growing depth only, 41.6% growing width only) and compose multiplicatively, so an architecture can be given different schedules for adding units and for adding the connections between them.
