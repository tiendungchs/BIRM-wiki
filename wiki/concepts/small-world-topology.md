...
- **[[wiki/concepts/network-control-theory.md]]** — the one place a *function* of this topology is put to a test rather than asserted, and the answer is unfavourable: in Wilson–Cowan simulations of seizure dynamics, blanket distributed stimulation quiets a *random* structural graph and is less effective on a small-world one, where the input has to be aimed at specific driver nodes — so the biologically realistic regime is the one in which broadcast control fails (Ching, Brown & Kramer 2012, via Bassett & Sporns 2017).
- **[[wiki/concepts/network-communication-models.md]]** — the communication model `L` was silently computed under: average shortest-path length and global efficiency are only claims about signalling if traffic actually takes shortest paths, which requires every node to hold a map of the whole graph — so this page's "fast integration" benefit describes what the topology *permits*, and the transferable version is the broadcast result, where diffusing along all walks reaches the shortest path first with no node knowing the topology at all.
