...
| Human entorhinal/subicular + posterior parietal + medial parietal + lateral temporal + medial prefrontal | Six-fold directional modulation during **plain virtual navigation**, all aligned to the entorhinal orientation, fast runs only, 4-/5-/7-/8-fold null | fMRI activation + 60°-shifted adaptation (Doeller et al. 2010, [[wiki/concepts/hexadirectional-signal.md]]) |
...
**The null case, and it predates the claim by fourteen years** (Ji & Wilson 2007, `raw/ji-2007-coordinated-replay-visual-cortex-hippocampus.md`). In freely running rats on a cue-rich figure-8 maze, **54 of 116** deep-layer V1/V2 cells had spatially localised firing fields, consistent across laps within a session and across days — the same phenomenology as the V2M row above, in overlapping tissue. The authors read them as responses to the intra-maze visual cues (black/white stripes, geometric shapes on floor and inner walls), and ran **no darkness or cue-removal control**. Two things this fixes for this page. It shows that "localised firing fields in a sensory area" is a cheap and long-known observation, so the replication claim rests *entirely* on the deprivation controls rather than on the tuning; and it names the confound those controls address — a cue-rich environment makes a sensory response spatially localised for free. The same recording is why the sensory-cortical replay in [[wiki/concepts/offline-replay.md]] cannot be read as offline reactivation of a reference frame.
...
**The primary for this proposal adds two things this page cannot get from Chen et al. 2022** (Hawkins et al. 2019, [[wiki/entities/thousand-brains-theory.md]]):

| Addition | Content | Consequence for this page |
|---|---|---|
| **An inter-frame operator** | Displacement cells: subtract two grid states module by module and the result is a vector in the same modular format — a *movement* if both locations are in one space, a *composite object* if in two ([[wiki/concepts/displacement-codes.md]]) | Relating two frames is now a named operation rather than a gap; but it is only defined if the two spaces share moduli, scale and orientation, so the arbitration problem (G43) moves from "reconcile disagreement" to "establish commensurability" |
| **A testable scale prediction** | Input convergence × grid scale sets the *range of object sizes* a region can model; LGN→V2 is more divergent than LGN→V1, so **V2's cortical grid scale should be larger than V1's** | The first falsifiable quantitative prediction about cortical grids in the wiki that does not depend on the detector caveat of T37, since it is a *relative* comparison between two areas measured the same way |

It also relocates the claim: the prediction is specifically **L6a cortico-cortical cells**, argued from the reciprocal L6a↔L4 loop (~45% of L4's synapses arrive from L6a, against <10% from thalamus) and from motor projections terminating in L6. The S1 and V2 recordings in the table above were not layer-restricted, so that prediction is untested by the very evidence usually cited for it.
...
**Prediction 4 now has evidence, and it arrives as a negative about the rival.** The localised alternative — one region owning the egocentric↔allocentric conversion ([[wiki/entities/retrosplenial-cortex.md]]) — has been reviewed twice with opposite verdicts (`T383`), and the later review's two load-bearing findings both favour *this* page. Gain-field-like conjunctive egocentric–allocentric cells, taken as the signature of a converter, are reported in **many structures beyond retrosplenial cortex**, so conjunctive coding is not diagnostic of a localised transform. And retrosplenial inactivation has little effect on boundary-anchored responses in medial entorhinal cortex, which is the downstream consequence a localised converter is supposed to produce. What the distributed reading inherits with the win is the arbitration problem in full: if the conversion is everywhere, `G43`'s question of how concurrently held frames are reconciled cannot be answered by pointing at a converter, and the same review adds a third frame to reconcile — a **route-centered** one, carried in retrosplenial ensembles both invariant to and modulated by the route's allocentric placement (Alexander et al. 2023).


**And the conversion machinery has now been recorded rather than inferred — in a form that is neither localised nor replicated-per-area.** Alexander & Nitz 2015 dissociated three frames in 228 retrosplenial neurons (egocentric turn direction, route-centric position, allocentric room position) by running two turn-sequence routes on a track that was then moved between room locations. The frames are combined **multiplicatively within single cells**, and the ensemble reconstructs route position to 4.51 cm within a room location while discriminating which location it is in — with no place fields anywhere in the population ([[wiki/entities/retrosplenial-cortex.md]]). For this page the result is two-edged. It supports Prediction 4's *distributed* reading in that no conversion event happens anywhere: the population is a basis and the map is whatever a downstream reader's weights extract. But it does not support the *replication* reading either — this is one population spanning several frames, not several areas each running its own frame. The third arrangement the evidence now forces onto the table is **one mixed population, many readouts**, which is a different architecture from both "one converter region" and "a converter per sensory area" (`T47`, [[wiki/concepts/population-geometry.md]]).
...
- **[[wiki/concepts/canonical-cortical-microcircuit.md]]** — the circuit this page's central claim assumes and never specifies: cortical columns are canonical because the laminar excitatory graph is conserved across areas and species, and "columns voting on a consistent pose" cashes out as soft winner-take-all among superficial pyramids, propagated between columns by lateral patches and between areas by layer-5 feedback (Douglas & Martin 2004) — and the price on the uniformity premise: V1 and A1 have the same excitatory and inhibitory hodology and respond to the same top-down drive with *opposite* sign, so a column copied everywhere is right about the wiring and underdetermined about the computation, the difference living in per-area scalars (Harris & Shepherd 2015, [[wiki/empirical-tensions.md]] `T371`).
...
- **[[wiki/concepts/hexadirectional-signal.md]]** — the instrument that produced most of this page's evidence table, and the earliest datapoint in it: five regions six-fold and all registered to one entorhinal angle during ordinary navigation, which is simultaneously the strongest support for replication and the strongest evidence against the *independence* half of the claim (G43).
- **[[wiki/entities/retrosplenial-cortex.md]]** — the measured third arrangement between this page's replicated frames and a single converter: one population whose units are multiplicative conjunctions of three frames, from which any frame or combination is linearly recoverable — so the conversion is neither localised in a module nor replicated per area, it is a readout choice (Alexander & Nitz 2015, `T47`).
- **[[wiki/entities/entorhinal-cortex.md]]** — the anatomical case *against* frames being separate module types: the lateral and medial divisions run near-identical circuits (same layers, same principal-cell chemistry, same three interneuron families) and differ mainly in which cortical areas project to them, so the *what*/*where* split is a fan-in difference over one repeated operator — which pushes `T47` toward the registered/shared-code reading and turns "how many frames" into a question about input partitions (Witter et al. 2017).
- **[[wiki/concepts/displacement-codes.md]]** — the operator that makes this page's concurrent frames usable rather than merely numerous: a difference between two frames' location codes is itself a modular vector, which is the coordinate conversion this page's Prediction 4 asks for, run between objects instead of between egocentric and allocentric — and it relocates G43 from reconciling disagreement to establishing that two frames are differenceable at all.
- **[[wiki/concepts/offline-replay.md]]** — the null case for this page's replication claim, and the reason it matters: deep-layer V1/V2 cells with spatially localised fields on a cue-rich maze, uncontrolled for the cues, whose firing *order* replays during subsequent slow-wave sleep — so a sensory response can look like a frame online *and* reactivate offline, which is the confound the darkness controls here exist to rule out (Ji & Wilson 2007).
- **[[wiki/entities/retrosplenial-cortex.md]]** — the localised rival to this page's Prediction 4, now contested from within its own literature (`T383`): Vann et al. assign the egocentric↔allocentric conversion to *one* region applying a rotation parameter delivered by an external head-direction population, while Alexander et al. 2023 find conjunctive ego–allo cells in many structures and no effect of retrosplenial inactivation on entorhinal boundary coding — and the region's lesion profile (near-normal until cues conflict) remains the arbitration signature `G43` had no positive case for.
- **[[wiki/concepts/reference-frame-transformation.md]]** — this page's Prediction 4 stated as a circuit and priced: a `K`-element gain-field bank costs `K × |population|` units plus one parameter feed, so replicating a frame per cortical area replicates the bank too, and the localised/distributed choice becomes a unit-count difference rather than a preference.
- **[[wiki/entities/bb-model.md]]** — the localised rival, implemented, and the sharpest available statement of what it denies: one bank at one site, gated by a single head-direction bump, so the concurrent frames this page posits cannot disagree in it at all (`G43`).
- **[[wiki/entities/dorsal-visual-stream.md]]** — the anatomy behind this page's Prediction 4: posterior parietal cortex is named here as the candidate site of a universal egocentric→allocentric conversion, and the pathway makes it the *source* of a three-stage route (`cIPL` → posterior cingulate → retrosplenial → hippocampal formation) with world- and object-centred cells already present in the caudal inferior parietal lobule — so the conversion is staged along a route rather than performed at a site, and no stage owns one frame (Kravitz et al. 2011, `T47`).
