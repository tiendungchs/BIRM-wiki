...
- **[[wiki/concepts/projection-distortion.md]]** — the out-of-domain prior on this page's subspace parameter: where the ambient space is available for comparison, a fitted 2-D summary loses local neighbourhoods (Jaccard > 0.7), reverses group-level rankings and inflates distance ratios 4–200×, and an arbitrary-shape embedding scores as well on the fidelity metrics — so the 0.993-projected-against-0.395-raw composition number is the expected shape of the artefact, not an anomaly (T159).
