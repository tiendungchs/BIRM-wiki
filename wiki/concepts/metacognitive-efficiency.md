...
- **[[wiki/concepts/confidence-calibration.md]]** — the machine instance of this page's calibration/resolution dissociation, with the correction priced: a modern network's entire calibration term is removed by fitting **one scalar** on held-out labels, and because that transform is rank-preserving it moves `AUROC2` and `meta-d′`'s sensitivity numerator by exactly zero — so this page's "the scale-learning problem is barely studied" open problem is answered for closed-set classifiers, and bias is confirmed as the cheap half.
- **[[wiki/concepts/information-sampling-vs-search.md]]** — the precondition an uncertainty-driven sampling policy assumes and this page measures: the agent must know how uncertain it is, and measured metacognitive accuracy is imperfect and matures slowly with age, so a policy reading its own uncertainty to decide where to look reads a biased estimate (Gottlieb & Oudeyer 2018).
- **[[wiki/concepts/learning-progress.md]]** — a self-assessment used for *allocation* rather than for a confidence report, and measured without any of this page's corrections: a fitted `w_LP` says a participant's choices track a statistic of their own recent accuracy, but type-1 `d′` is never computed, so a null progress weight is ambiguous between having no drive and having a noisy competence estimate — the exact confound this page exists to remove (Ten et al. 2021).
- **[[wiki/concepts/effort-based-decision-making.md]]** — a revealed-preference readout that beat the self-report on the same construct: cognitive effort discounting detected an aging group difference in cognitive motivation that the Need for Cognition questionnaire did not, so an agent's *choices* between priced task bundles index an internal cost more sensitively than its statement about that cost — the same instrument-validity argument this page makes for confidence, run on effort (Westbrook & Braver 2015).
- **[[wiki/concepts/self-boosting-knowledge-acquisition.md]]** — a second self-model whose error is *signed* and which this page's decomposition has no terms for: the forecast is of the agent's own future motivational state rather than of an answer's correctness, the bias runs downward for self-generated reward and upward for delivered reward, and it collapses when a reward is promised — a bias-term result with no sensitivity or efficiency analogue yet defined (Kuratomi et al. 2018, via Murayama 2022).
- **[[wiki/concepts/realised-information-gain.md]]** — the earliest-age instance of this page's presupposition, and the one with the least room to hide it: an `e^{|β|} = 7.02` on realised information gain means an 8-month-old's stay/leave decision tracks the size of its own belief move, which requires a read-out of an internal quantity, and there is no confidence report available at that age from which any sensitivity or bias term could be estimated. So the wiki has a behaviourally-strong type-2 read in a population where the whole decomposition is unmeasurable by its standard instrument (Poli et al. 2020, `G89`).
