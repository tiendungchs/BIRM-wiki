...
- **[[wiki/concepts/shared-intentionality.md]]** — the direct opponent on the same two experiments: this page reads the Hare et al. (2000, 2001) results as a representation present and gated by motivation, Tomasello & Rakoczy 2007 grants those exact results and locates the human addition in self–other equivalence, which they argue is deployed in *no* ape frame — no declarative pointing, no pretense, no shared symbol ([[wiki/empirical-tensions.md]] `T292`, the hardest instance of this page's `T291`).
- **[[wiki/concepts/relational-reinterpretation.md]]** — the strongest representational reading of the comparative nulls this page re-reads motivationally, and the reason the two are hard to separate: a chunk-based deflation and an undeployed competence predict the same data, and only the motivational account predicts a frame in which the behaviour appears (T291, T292).
- **[[wiki/concepts/developmental-heterochrony.md]]** — constrains this page's re-weighting account at the molecular level: whatever parameter shifted on the human lineage, the transcriptional evidence excludes a global change of developmental pace and localises it to a small, gray-matter-specific gene set peaking in early adolescence (Somel et al. 2009).
- **[[wiki/concepts/cellular-scaling-rules.md]]** — supplies the primary measurement this page's outlier test is worked on: the primate cellular scaling rules are fitted *without* human values and then predict the human brain's 86 billion neurons to within 7%, which is what makes "extreme but not exceptional" a result rather than a rhetorical move — and the same rules explain the residual, since brain mass tracks units linearly in primates and hypermetrically in rodents.
- **[[wiki/concepts/incentive-salience.md]]** — a mechanism, a pathology and a non-choice measurement for this page's *motivational* factor: a cue-triggered mesolimbic multiplier scaled by the current state, which sensitizes independently of the hedonic term, and which is read from affective orofacial reactions rather than from anything the agent chooses or reports.
- **[[wiki/concepts/expected-value-of-control.md]]** — the one normative rule for `G102`'s missing state: under `signal* = argmax [Σ Pr·Value − Cost(signal)]` a held capability goes unexercised whenever its control signal loses to the untaxed default — competence intact, expression withheld on price. It supplies the form and not the dial, since the cost function is asserted monotonic and never measured, and it re-derives the refusal per decision rather than holding it as a standing disposition.
