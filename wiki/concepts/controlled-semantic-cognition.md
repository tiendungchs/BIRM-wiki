...
**The adapter has now been caught being recruited by content, from the other literature** (Wendelken, Chung & Bunge 2012; via [[wiki/entities/lateral-frontoparietal-network.md]]). A single left rostrolateral prefrontal seed — defined as the *intersection* of semantic and visuospatial relational-integration clusters, so domain-general by construction — is more strongly coupled to **left MTG** (−60/−30/−3) when the relations being integrated are semantic than when they are visuospatial, in the same 23 subjects on the same trials. This is the three-layer arrangement measured: general controller, semantic adapter, hub. Two things it settles that a co-activation map cannot. pMTG's absence from relational-reasoning **activation** maps is not evidence that the relational system bypasses the semantic store — the adapter shows up in the *connectivity*, and only for semantic content. And the recruitment is driven by what the content **is**, not by the task instruction, which the two arms hold constant.
...
- **[[wiki/entities/lateral-frontoparietal-network.md]]** — the overlapping and the non-overlapping halves: intraparietal sulcus and inferior frontal sulcus appear in both networks as the domain-general term, while the semantic-control-specific nodes (pMTG, ventral prefrontal) are absent from the relational-reasoning *co-activation* maps and present in its *connectivity* map whenever the content is semantic (Wendelken et al. 2012) — evidence that the control network is per-representation-store rather than one supermodule, and that its adapter is recruited by content rather than by task.
...
- **[[wiki/concepts/schema-assimilation.md]]** — where a schema ends up once it is over-trained: after three months of training on specific object–location associates, retrieval runs on ventrolateral prefrontal cortex + this page's anterior-temporal hub + angular gyrus with **no** ventromedial prefrontal involvement, so repeated encoding of fixed associations converts schematic structure into denotational semantic content — while reinstating a *template* still requires ventromedial prefrontal cortex (Gilboa & Marlatte 2017).
