...
- **[[wiki/concepts/controlled-semantic-cognition.md]]** — a bottleneck justified the other way round: the anterior temporal hub is required not to discard nuisance information but because conceptual similarity is a complex, variable and nonlinear function of every input space, so the metric has to be learned somewhere that sees all of them at once — compression as the only place a shared metric can exist, rather than as the objective.
- **[[wiki/concepts/relational-bottleneck.md]]** — this principle with its intractable half deleted: instead of estimating `I(X;Z)` and `I(Z;Y)` to find a minimal sufficient statistic, restrict `Z` architecturally to relations `{r(x_i,x_j)}` — a space guaranteed to contain a sufficient statistic for any relational task and to exclude most nuisance-carrying representations, so the finite-sample benefit is had with no `β`, no estimator and no named relevance variable (Webb et al. 2024).
- **[[wiki/concepts/empowerment.md]]** — the same Arimoto–Blahut alternation pointed the other way: this page compresses an input subject to preserving information about a named relevance variable, empowerment *maximises* the capacity of the agent's own actuator→sensor channel with no relevance variable at all — and it inherits the same non-convexity, plus a continuous case where the objective diverges without an assumed noise level (Salge et al. 2013).
