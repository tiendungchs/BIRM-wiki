...
- **[[wiki/concepts/small-world-topology.md]]** — the same weight matrix summarised by a property that discards this page's result: `C` and `L` are symmetric and directionless, whereas the instrength gradient over those edges says which way traffic runs through the shortcuts.
- **[[wiki/concepts/stationary-surrogate-null.md]]** — why this page's observable is the durable one: windowed correlation structure at rest is largely reproduced by stationary surrogates, while lag-based propagation is *consistent* with stationarity and still carries state information — sensitive to eye state, task history and slow-wave sleep, and separating autism from controls where conventional functional connectivity shows no difference (Mitra et al. 2014–2017, via Laumann et al. 2017).
