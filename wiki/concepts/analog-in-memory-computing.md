...
- **The generated teaching signal is emitted by a separate meta-network, not by the controller that consumes it.** Parameter-generation L2L is the wiki's nearest approach to `G50` — a learned signal replacing a hand-set learning rate — and the gain still arrives from outside the adapting system rather than from a learned edge inside it.
