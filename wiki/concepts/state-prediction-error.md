...
- **What this result does to `G119`.** The row asks for a reinforcer admissible to one learning rule and inadmissible to another, and the sign-tracking evidence reads the dopamine transient as inadmissible to the goal-tracker's predictive learning. This source reads the same transient as *necessary* for predictive learning of exactly that kind, with no value in the product at all — so it is the row's counter-evidence, and it forces admissibility to be set by something finer than the signal's chemical identity (timing, terminal field, or the receiving rule's own window) unless one of the two nulls is a dose artefact (`T404`).
