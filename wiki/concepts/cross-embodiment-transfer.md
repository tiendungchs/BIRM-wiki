...
- **[[wiki/entities/thousand-brains-theory.md]]** — supplies a mechanism for *why* an embodiment change breaks a learned model rather than merely degrading it: learning pairs a movement space with an input space, and a module fed one modality's input while integrating another effector's movement learns nothing at all, because the traversed locations do not map onto the space the inputs vary over (Hawkins et al. 2019).
- **[[wiki/entities/feudal-networks.md]]** — a mechanism for body-independence at the top level rather than a dataset for it: the Manager's policy is stated in *latent state displacements*, so it is invariant to how primitive actions translate into them; transferring an Atari agent from action-repeat 4 to 1 needs only rescaled discounts, dilation radius and goal horizon, and beats every baseline on every environment where the transferred flat LSTM manages positive transfer on one (Vezhnevets et al. 2017).
