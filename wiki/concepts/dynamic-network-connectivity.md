...
- **[[wiki/entities/salience-network.md]]** — the network-scale switch this register could implement: mode reassignment within seconds without rewiring, driven by a detector rather than by the diffuse arousal signal this page's gain usually carries.
- **[[wiki/concepts/transthalamic-context-routing.md]]** — a second, anatomically addressed substrate for state-dependent coupling change alongside the diffuse neuromodulatory one: a higher-order thalamic projection carrying arousal to a named cortical area, separable at the target from the locomotion-linked modulation that reaches the same cells by another route (Neske & Cardin 2025).
