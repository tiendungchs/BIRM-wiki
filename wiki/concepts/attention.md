...
- **[[wiki/concepts/priority-map.md]]** — the two-stage decomposition of this page's spotlight: a persistent template register and a per-location similarity computed in ventral prearcuate cortex, an argmax over a retinotopic map taken in the frontal eye field, with a causal double dissociation between them — the operations `softmax(qKᵀ)V` fuses into one expression (Bichot et al. 2015); the 2019 follow-up carries the same lesion into V4 and shows feature gain and spatial gain in one sensory population run through different controllers (Bichot et al. 2019).
