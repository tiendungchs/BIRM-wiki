...
- **[[wiki/entities/lateral-frontoparietal-network.md]]** — the evolution/development coincidence measured at the level of cortical surface area: the regions most expanded from macaque and chimpanzee to human overlap those most expanded across human childhood (Fjell et al. 2013; Buckner & Krienen 2013), which is this page's timing-shift hypothesis stated on a spatial rather than a transcriptional axis.
- **[[wiki/concepts/curriculum-learning.md]]** — the data schedule to this page's capacity schedule, and possibly not a separate lever: phase offsets decide when a block exists, a curriculum decides what it is shown, and the lineage both descend from (Elman 1993's *starting small*) attributed its result to expanding resources rather than to example order — which is `T351` and the reason `G100` and `G32` are carried as two rows.
- **[[wiki/entities/saycam-baby-vision.md]]** — the same 6–32-month window seen from the data side, and the null for this page's thesis: the model is shown the whole two years as one undifferentiated training set, with no capacity schedule and no input schedule, and still acquires the categories.

- **[[wiki/entities/ligo.md]]** — this page's per-module phase offsets, made runnable and then priced: LiGO's depth operator is a learned matrix `w ∈ R^{L₂×L₁}` blending each donor layer into each grown layer, fitted in 100 SGD steps — the first machine object with the shape DH5 asks for — but it is fitted *dense* where the transcriptional measurement says the real object is ~4% sparse, it fires once rather than over a window, and the payoff it delivers is 45% of the training compute with no competence gain at all (`T354`).
- **[[wiki/concepts/explore-exploit-division-of-labour.md]]** — the behavioural readout for this page's sparse per-component warp: hypothesis-search breadth falls on *different* curves per domain (adolescents cool on unlikely physical hypotheses while staying preschool-flexible on unlikely social ones), landing in the same ~10-year synaptic-elimination window this page dates the human-neotenic delay to — so what the delayed capacity commitment buys is a prolonged high-temperature search, and `Δ` acquires a behavioural target (Gopnik 2020).
