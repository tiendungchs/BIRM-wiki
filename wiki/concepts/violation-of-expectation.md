...
|---|---|---|
| **Error statistic** `max_t` vs `mean_t` | Moves IRENE's Blocking Barrier 83.5 → 99.4 and its Multi-Agent 74.9 → 63.6; rival groups justify opposite choices | Still open, and now traceable to its origin: BIB chose `max_t` because the mean "consistently resulted in lower VOE scores" ([[wiki/entities/bib.md]]). Garrido et al. state the choice explicitly (`max` for single-video classification, generalises better; the pairwise numbers use the same statistic throughout) rather than defending it |
...
**What the null does *not* license** ([[wiki/entities/byol.md]]). An untrained network is a floor for a *read-out*, not a statement that its activations are structureless. BYOL trains an encoder by regressing onto the projections of a **fixed randomly-initialised** target and reaches 18.8% ImageNet linear-eval top-1, where the random target itself reads out at 1.4%. So a randomly-initialised network at chance under a surprise metric may still carry enough input-dependent structure to teach a downstream encoder — the correct reading of the null is "this much of the score survives without training *on this read-out*", and any claim that untrained activations are uninformative needs the target-network test, not the classifier test.
...
- **[[wiki/entities/byol.md]]** — the qualification on this page's untrained-network null: an encoder regressed onto a *fixed random* target reaches 18.8% ImageNet linear-eval where the target itself reads out at 1.4%, so activations that score at chance under a surprise metric are not thereby structureless.
- **[[wiki/entities/autotom.md]]** — the other readout of an inverse-planning posterior, and the one that needs no authored contrast pair: correlate the model's confidence against published human confidence profiles (r = 0.93 / 0.88 / 0.73 on Baker 2009/2017, against GPT-4o at 0.81 / 0.30 / 0.04) — stronger evidence per stimulus, but only available where a psychophysics study already exists, and the free parameter this page records for VoE reappears as the un-swept `α` and `U_min` of the model-utility criterion.
- **[[wiki/entities/agent-benchmark.md]]** — the protocol's largest agent-domain deployment, and the source of two additions to this page's instrument list: a *rater-ensemble ceiling* (10 raters per trial ⇒ 100% pairwise, single rater .91), which separates "the stimulus pair is unambiguous" from "an individual observer gets it"; and **per-type correlation against the human profile** as a second read-out on top of accuracy, which catches a model at aggregate parity (.90 vs .91) whose profile correlation is 0.06.
- **[[wiki/entities/bib.md]]** — the primary source for the `max_t` free parameter recorded above (chosen because `mean_t` "consistently resulted in lower VOE scores"), and the supplier of the design rule that makes the protocol diagnostic rather than merely label-free: author the implausible continuation to be the *perceptually more familiar* one, so a similarity-matching model lands below chance instead of at it.
- **[[wiki/concepts/certification-instruments.md]]** — paired VoE is instrument `I6`, the only one that can register a below-chance score, and this page supplies both of its defects to the inventory: `F9`'s unfitted error statistic and `F8`'s missing randomly-initialised null.
- **[[wiki/concepts/human-baseline.md]]** — the developmental protocol whose human data is a *direction* rather than a rate, which is exactly why it needs the randomly-initialised-network null to recover a reference point at all.
- **[[wiki/concepts/prediction-error-neurons.md]]** — types the quantity this protocol aggregates: cortical error comes as two sign-specific signals with opposite effects on the model, and mouse V1 mismatch responses are retinotopic and feature-specific, so a single scalar surprise score discards both the sign and the *what* that the brain's own error code carries.
- **[[wiki/entities/a24b-m2-v1-projection.md]]** — locates what a surprise score is a residual *of*: the expectation being violated is manufactured in a distant motor-related area and delivered as a prediction, so the protocol measures the sender's learned action→consequence map, not the sensory area's — and inactivating the sender shrinks the violation response without changing the stimulus.
- **[[wiki/concepts/reference-frame-transformation.md]]** — why the frame a mismatch is computed in decides what it is worth: differencing top-down reconstruction against bottom-up drive at the *egocentric* end returns a signal carrying a location in peri-personal space, so expectation violation becomes a pointer for attention rather than a scalar surprise; computed inside the allocentric store it reports only *that* something changed ([[wiki/entities/bb-model.md]], whose hippocampal-lesion simulation is that signal with its *where* removed).
