...
- **[[wiki/concepts/parallel-timescale-streams.md]]** — what the operator averages over, and how much of it there is: the rest and task connectomes it maps between are window-averages of a 126-state walk that runs simultaneously at six speeds, so a single learned deformation is a statement about one stream's equilibrium and not about the trajectory or the other five (Alderson et al. 2026).
