...
- **"Integration" here is a strength statistic, and strength does not distinguish kind.** Participation coefficient, global efficiency `E` and modularity `Q` all score how much cross-module traffic there is, not what it carries. The classical complexity measure in the same family, TSE, approximates `C + B` and takes the *same value* on a system of exact copies as on one whose information exists only in the combination ([[wiki/concepts/o-information.md]], Rosas et al. 2019). So an integrated state in which every module broadcasts a copy of the same variable and one in which the modules jointly code something none of them holds are indistinguishable under every statistic on this page.
...
- **[[wiki/concepts/connectivity-scaling-bottleneck.md]]** — where this page's axis is anchored, and which way scale slides it: across primates the anatomy itself moves toward segregation with size (path length β = +0.74, clustering β = +0.75, density falling), so a larger network negotiates its integration/segregation trade from a more segregated set point and with a shorter lever (Ardesch et al. 2022).
- **[[wiki/concepts/network-control-theory.md]]** — the actuator half of the `G85` pair this page supplies the measurement half of: where this page reads a scalar position on the integration↔segregation axis, control theory names what would move it deliberately — a driver set `B`, an input trajectory `u(t)`, a target state, and a minimum control energy `E*` as the cost — and it warns that the broadcast neuromodulatory-gain candidate proposed here is a *distributed* control strategy applied to a small-world graph, the regime where distributed control is the weaker option (Bassett & Sporns 2017).
- **[[wiki/concepts/network-control-theory.md]]** — names the nodes that would move this page's axis, and they are not the ones a gain signal would reach: **boundary controllability** ranks regions by their position between network communities, is the only control diagnostic uncorrelated with node degree (`r = +0.13` / `+0.008` / `−0.19` across three datasets, against `+0.91` and `−0.99` for the other two), and is enriched in the dorsal and ventral attention systems — so the integration↔segregation actuator this page asks for has an anatomical address that is neither the hubs nor a diffuse broadcast (Gu et al. 2015, [[wiki/empirical-tensions.md]] T324).
- **[[wiki/concepts/network-communication-models.md]]** — the generative parameter behind this page's measured axis: moving along participation coefficient `B` under task demand *is* a change of communication regime, and the biased-random-walk `λ` — one scalar sliding a network from unbiased diffusion to selective shortest-path routing by biasing local transition probabilities — is the mechanism that would produce that movement in a model rather than describing it after the fact.
- **[[wiki/concepts/o-information.md]]** — the coordinate this page's axis is missing, and a warning about the classical statistic beside it: TSE complexity, the standard integration/segregation measure, approximates `C + B` (correlation > 0.97) and takes the *same value* on a 3-bit copy as on a 3-bit xor, so an integration score cannot separate a network that duplicates information across modules from one whose information exists only in the combination — `Ω = C − B` is the orthogonal reading, and it is what would decide whether a high-`B` integrated state is doing useful joint coding or broadcasting copies (Rosas et al. 2019).
