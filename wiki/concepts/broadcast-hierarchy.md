...
- **[[wiki/concepts/counterstream-organization.md]]** — an independent anatomical route to the same resource profile, measured by retrograde tracing rather than by effective connectivity: descending pathways outnumber ascending ones ~2:1 and reach further at equal cumulative weight, so top-down is many low-weight wide-fan channels — and the same dataset finds no consistent depth coordinate for association cortex at all, which is what a node outside the chain would look like (`T378`).
...
- **[[wiki/concepts/canonical-cortical-microcircuit.md]]** — the other structural hierarchy metric: SLN% grades depth by the superficial fraction of a projection's source cells, laminar differentiation grades it by the source area's own layering, and the laminar-type rule adds a coupling *constraint* that SLN% does not express — with a third, non-laminar candidate now beside them: inter-areal connectivity generated by a small set of genetically defined intratelencephalic subclasses (each gene module fixing a projection role) times an areal gradient fixing the endpoint, which would make hierarchy a consequence of port typing rather than a measured depth (Harris & Shepherd 2015).
...
- **[[wiki/concepts/connectivity-scaling-bottleneck.md]]** — what scale does to this page's two layers: the sparse long-range overlay is the part that is rationed as a brain grows (long connections fall from 63–71% of the distribution in small primates to 40% in humans), so the broadcast channel thins while local model detail thickens — a bandwidth prediction about how a scaled-up hierarchy must reallocate (Ardesch et al. 2022).
- **[[wiki/concepts/information-bottleneck.md]]** — the information-theoretic reading of this page's laminar ordering: convergence onto fewer, slower, broader units is a rate reduction per level and the data-processing inequality makes relevant information lost at one level unrecoverable above it, so hierarchical position *is* a point on an information curve — with the cortex's lossiness targeted at sufficiency for action, i.e. a relevance variable stated behaviourally.
- **[[wiki/concepts/intrinsic-timescale-measurement.md]]** — the observation this page's conduction-physics account is competing to explain: seven macaque areas ordered by across-trial spike-count autocorrelation, `50–350 ms`, matching the laminar-projection hierarchy at `r_s = 0.97` — and a constraint on the account, since the measured *intrinsic* spread is only ~7× while the functional horizons it orders are seconds, so hypomyelination must seed an ordering rather than supply the whole range.
- **[[wiki/concepts/timescale-hierarchy.md]]** — the rival mechanism for this page's slow apex: a gradient of *recurrent excitatory gain* along the SLN hierarchy reproduces the full timescale spread with conduction delays explicitly omitted, so hypomyelination is not needed to get long integration windows — two accounts of one observation, logged at [[wiki/empirical-tensions.md]] T319 (Chaudhuri et al. 2015).
- **[[wiki/concepts/abstraction.md]]** — abstraction-as-anatomical-dimensionality-reduction filed against the other eight senses: it is the order family implemented in wiring, and the one place the level structure is a property of endpoints rather than of a code.
- **[[wiki/entities/lateral-frontal-pole.md]]** — this page states abstraction as anatomical dimensionality reduction fixed by endpoints; that page adds that the *filters* of the reduction are set per task by a separate controller, so the wiring fixes where compression happens without fixing what is compressed — and `T332` warns that the level ordering itself changes with the criterion used to recover it.
- **[[wiki/concepts/hierarchy-of-associativity.md]]** — the same laminar ranking instrument applied past the last cortical stage: perirhinal/parahippocampal → entorhinal is feedforward by the Felleman–Van Essen criteria and the medial-temporal return is compressed-in/diffuse-out in its strongest form — one amodal representation redistributed to much of neocortex, unimodal sensory areas included.
- **[[wiki/concepts/apical-amplification.md]]** — the destination a broadcast needs, at cellular resolution: layer-1 horizontal fibres deliver feedback from many higher areas and from thalamus onto the apical tuft of every level's pyramids, where it gates that level's own output — so addressing a level costs no relay stage, which is what the scope variable presupposes and no chain permits.
