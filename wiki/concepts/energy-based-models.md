...
| Instances | Siamese nets, DrLIM, PIRL, MoCo, SimCLR, CPC; unnormalised maximum likelihood; MCMC / contrastive divergence; GANs (`ŷ` from a trainable generator); **denoising and masked auto-encoders** (`ŷ` by corrupting `y`) | Sparse modelling, sparse and noisy auto-encoders, VAE, VQ-VAE, implicit rank minimisation; VICReg, **Barlow Twins** ([[wiki/entities/barlow-twins.md]]), whitening, maximum coding-rate reduction |
...
1. **The two anti-collapse strategies are not composable.** Adding BYOL's predictor and stop-gradient to Barlow Twins costs **10 points** (71.4 → 61.3; either alone ≈ −1). An update-rule asymmetry inside a system that already has a well-defined minimum moves the dynamics *away* from it. The reverse control agrees: BYOL with Barlow Twins' 8192-wide projector and embedding drops 74.1 → 72.3. These are alternative whole designs, not a menu of stackable safeguards.
2. **There is a fourth locus this table does not have — a normalisation inside the loss.** Deleting the off-diagonal term entirely leaves **57.3%** top-1, not collapse, because `C` is formed from embeddings standardised *along the batch dimension* and a constant unit has zero batch variance. Deleting the standardisation as well (cross-covariance instead of cross-correlation) costs 18 points. So part of the anti-collapse provision sits in a normalisation layer that is certifiable like a term but appears in no coefficient count — and the same quantity penalised by a cross-entropy-with-temperature instead of a squared error loses 8 points, meaning the *shape* of the penalty is a second free choice on top of which quantity is penalised.

| | Contrastive | Dimension-contrastive | Distribution-matching | **Dynamical (BYOL)** |
|---|---|---|---|---|
| Provision lives in | loss | loss | loss | **update rule** |
| Coefficients | temperature | 6 | 1 | 0 in the loss (`τ`, predictor LR ratio instead) |
