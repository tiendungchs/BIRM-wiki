...
**The posterior cingulate row is contested, and from single units.** Macaque dorsal posterior cingulate firing does track the variance of risky offers — which varies with subjective but not objective expected value — but a study that explicitly dissociated value (preference for a choice) from **salience** (unsigned deviation of an outcome from baseline) found salience the better explanation, and the region's signal outlasts the trial by seconds to several trials, rises at context change points and on explore trials, and is *required for learning* (muscimol inactivation reduces it). Read together, that is an update-size signal rather than a valuation; the two claims are not measured on the same thing — haemodynamic, undifferentiated human posterior cingulate here, macaque area CGp (dorsal only) there — and the human effect could sit in the **ventral** subregion, which no single-unit study has recorded from ([[wiki/entities/posterior-cingulate-cortex.md]], [[wiki/empirical-tensions.md]] T382).
...
| The kernel is hyperbolic | `SV = A/(1+kD)` is **time-inconsistent**: preferences between the same two rewards reverse as both approach, so no *single* stationary `V` exists over which dynamic programming converges. Every value function in the wiki uses `γ^k`. **This costs much less than it appears**: `1/(1+kD) = ∫₀¹ w(γ)γ^D dγ` with `w(γ) = (1/k)γ^{1/k−1}`, so the measured kernel is a mixture of exponentials and is recovered exactly by a bank of ordinary temporal-difference learners recombined at read-out ([[wiki/concepts/multi-horizon-value-learning.md]], Fedus et al. 2019). What survives is the empirical question of which kernel the brain uses, not an architectural obstruction — T141 |
...
- **[[wiki/concepts/simulation-based-planning.md]]** — a horizon that is neither derived nor regulated but *fixed per agent*, and a non-exponential one: a hyperbolic kernel has no *single* stationary value function, so the wiki's `γ^k` rollout scoring is at best one term of what this brain does — the rest of the mixture is supplied by [[wiki/concepts/multi-horizon-value-learning.md]] (gap G24).
...
- **[[wiki/concepts/perturbation-elicitability.md]]** — the causal evidence that this region's affective signal is constitutive rather than downstream: orbitofrontal and anterior cingulate stimulation elicits graded emotional and motivational states with subjective intensity rising linearly over 2–8 mA, while the same protocol elsewhere in prefrontal cortex elicits nothing at all.
- **[[wiki/concepts/motivation-representation-synergy.md]]** — the nearest existing machinery to that page's missing factor, and the reason it does not suffice: valuation here is a scalar attached to an option at choice time, while what is needed is a standing disposition (social tolerance, prosociality) that gates whether a whole class of inferences is run before any option is valued.
- **[[wiki/concepts/multi-horizon-value-learning.md]]** — how the kernel measured here is computed: `A/(1+kD)` is the exponential-hazard-prior member of the family `d(t) = ∫₀¹ w(γ)γᵗ dγ`, so the per-subject `k` is a prior over how fast the chooser expects the promise to be reneged on, and a bank of exponentially-discounted value heads reproduces the hyperbola without any time-inconsistent recursion (T141).
- **[[wiki/entities/lateral-frontal-pole.md]]** — the boundary condition on this page's common-currency scalar: ventromedial prefrontal cortex carries the value signal only when the option's decomposition into probability and magnitude completes automatically, and is replaced by a lateral-frontal-pole + posterior-cingulate signal when the option is a 20-gamble bundle — so a single represented subjective value is the easy-case architecture, not the general one.
- **[[wiki/concepts/affective-opponency.md]]** — the reference-point objection to a common-currency scalar: if the origin of the valence axis moves with the current threat or reward prediction, then this page's absolute, idiosyncratically-discounted value and a value scored against a context-set zero cannot both be what a valuation module computes — and the two differ only in conditions that deliberately move the baseline, which no source here does.
- **[[wiki/concepts/homeostatic-need-signal.md]]** — the state dependence this page's values lack: an outcome's worth is measured there as the fall it produces in a need variable, so a value estimate or a discount rate obtained in one need state does not transfer to another, and the same food can be devalued to worthlessness with the item, the policy and the environment untouched (`T356`).
- **[[wiki/concepts/expected-value-of-control.md]]** — the other term of the same subtraction: this page measures `Value(outcome)` with the chooser's own discount kernel already applied, that page's dorsal anterior cingulate signal is the `− Cost(signal)` charged for deploying control to obtain it — and the common currency this page establishes is the precondition for the two to be subtracted at all, though no design measures both terms together.
- **[[wiki/entities/posterior-cingulate-cortex.md]]** — the challenge to this page's third region: macaque single units in dorsal posterior cingulate are better explained by unsigned **salience** than by value once the two are dissociated, and the signal outlasts the trial by seconds, rises at context change points, and is *required* for learning (muscimol inactivation reduces it) — which places that tissue at the update-size stage rather than the valuation stage ([[wiki/empirical-tensions.md]] T382).
- **[[wiki/concepts/conditioned-reinforcement.md]]** — the same kernel question at a different timescale, species and reward type, with an incompatible answer: pigeon schedule choice is fitted by pure reciprocal summation `Σ 1/t_i` with no residual effect of reinforcement rate, so one reinforcer at 30 s beats two at 60 s and 90 s — which this page's finite fitted `k` predicts the wrong way round. No registry row (the kernel is an `L3` realization choice); the claim both sources license is the weaker one, that the aggregator is not exponential.
- **[[wiki/concepts/token-reinforcement.md]]** — this page's kernel appearing inside a cost model rather than a choice: accumulating a token balance trades a longer delay to the next reward against a lower long-run cost per reward, and the only account that fits the observed intermediate batching is a unit-price model with a discounting term bolted on (Yankelevitz et al. 2008; Foster & Hackenberg 2004).
