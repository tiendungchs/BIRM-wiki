...
- **[[wiki/concepts/error-consistency.md]]** — the fourth adequacy question, and the same discipline on a different object: this page asks whether *our description* of one system is right (per neuron, per stimulus, against a noise ceiling), that page asks whether *two systems* are the same (per trial, against the agreement chance would produce). Both refuse aggregation, and both make the ceiling an estimated quantity — explainable variance here, `c_exp` there — so the residual becomes attributable instead of being charged to the model.
...
- **[[wiki/concepts/memorisation-vs-generalisation.md]]** — the third and hardest failure of description, and the only one that is not a property of the describer: this page's depth gradient says the *unit* of description fails above LGN, that page says a task with no macrofeatures has no description of the wanted kind at any unit or any depth, so the residual diagnostic (correlated = missing term, uncorrelated = missing mechanism) has no third case for "there was no mechanism".
