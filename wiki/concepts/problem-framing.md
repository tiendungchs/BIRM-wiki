...
| **Rebuilt per step and discarded** | [[wiki/concepts/active-vision.md]] (Ballard et al. 1997) — the representation is two or three currently-bound deictic pointers plus what the current step reads through them, so it is never a persistent object that could be wrong. The block-copying learner's state is **16 bits**, and every one of those features was chosen by the authors, so the framing is *small* rather than constructed |
...
- **[[wiki/entities/math-perturb.md]]** — framing as an act of *rejection* inside a fully specified problem: nothing is underdetermined and no representation must be constructed, and the only question is whether the framing the model already holds still applies — which no model tested performs as a separate step.
- **[[wiki/entities/baba-is-ai.md]]** — framing measured without asking for a rule: across the distractor ladder the optimisation problem is identical at every rung and only the relevance judgement changes, and accuracy falls at the rung where a rule is active but its noun has no referent, and again when a causally *inert* wall is added.
- **[[wiki/entities/fluxx.md]]** — the framing axis with discovery and perception removed entirely: every rule is face up and in the instruction language, so nothing is latent and the whole difficulty is *maintaining* the frame while other agents rewrite it.
...
- **[[wiki/concepts/active-vision.md]]** — the same framing language with an effector and a bill attached, and the clearest demonstration that a *small* framing is not a constructed one: the representation is rebuilt per step from two or three bound pointers and thrown away, which is why it is never wrong — while the 16-bit feature set those pointers read through was hand-designed, leaving this page's last row exactly as empty as before (Ballard et al. 1997).
...
- **[[wiki/concepts/perception-reasoning-dissociation.md]]** — this page's split given a protocol and a number on visual tasks: a four-step taxonomy (perceive demonstrations → induce → perceive test → apply) attributes 84–89% of one-stage failures to the two framing steps, with the caveat that earliest-step attribution over a sequential dependency graph awards every joint failure to framing by construction.
