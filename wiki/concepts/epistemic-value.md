...
This page is the measurement half of [[wiki/concepts/expected-free-energy.md]]. That page holds what the objective *is* — the decomposition `G(π) = ⟨ℓ, μ^π⟩ + Φ(μ^π)`, the convex-MDP class it belongs to, and the ρ-POMDP identity that fixes the exploration weight at `w = 1` without tuning — a *derived* weight rather than an absent one, which is the whole of [[wiki/empirical-tensions.md]] T123, and the derivation is over E2 while the term this page ranks first is E3. This page holds what happens when the term is switched off: four independent ablation literatures across message passing, deep networks, gradient descent and a human electroencephalography study, which between them are the wiki's only evidence that a derived exploration drive earns its place.
...
- **[[wiki/concepts/broadcast-channel-decomposition.md]]** — the cheap biological rival to every derived term on this page: an orienting burst keyed to rough sensory features leaks additively into the *value* channel, so information-bearing states acquire positive value with no posterior, no entropy and no weight to tune — an exploration bonus obtained by contamination rather than derivation, and one that dopamine neurons also show directly, firing to cues predicting the future *availability* of information about a reward while conveying nothing about the reward itself. Its predicted failure is where this page's ablations cannot look: novelty and uncertainty are correlated in ordinary tasks and the cheap version should only come apart when they are deliberately decoupled.
...
- **[[wiki/entities/lateral-frontal-pole.md]]** — a causal ablation this page's record lacks, on the human side: inhibitory TMS to the lateral frontal pole removes **directed** exploration and leaves **random** exploration intact, so the strategic and the stochastic terms are separately damageable, and the region tracks the *difference* in uncertainty between options rather than total uncertainty — an epistemic quantity defined comparatively rather than absolutely.
- **[[wiki/concepts/general-danger-channel.md]]** — the opposite sign on this page's novelty term, measured in a brain: an unfamiliar food or object activates the same parabrachial neurons that carry footshock, malaise and itch, silencing them abolishes neophobia, and the response inverts to appetitive inhibition once the food is familiar — so the biological drive to sample looks like an *annealing penalty* on the unfamiliar rather than an additive bonus for it (`T358`), with the practical difference showing up on irreversibility rather than on eventual behaviour.
- **[[wiki/concepts/intrinsic-motivation-typology.md]]** — the map this page is four ablated points in: E3 is that page's information-gain motivation and Plan2Explore's disagreement is its predictive-novelty cell, while E2 and E4 have no cell there at all because the typology scores the observable flow rather than a belief over a hidden state — and the whole competence family, which rewards *achieving* self-set goals rather than predicting anything, has no counterpart anywhere in this page's literature.
- **[[wiki/concepts/information-sampling-vs-search.md]]** — the regime label all four quantities on this page are silently defined in: `E1`–`E4` price a query against a known goal in a known state space, which is *sampling*, so nothing measured here speaks to the *search* case where the task itself is unknown — and it supplies the one neural dissociation this page lacks, lateral intraparietal neurons encoding a cue's reliability independently of the reward expected from acting on it (Gottlieb & Oudeyer 2018).
- **[[wiki/concepts/empowerment.md]]** — the same arrow reversed, and a quantity none of `E1`–`E4` can express: channel capacity from the agent's own actions to its later sensors scores what the agent *can perceptibly do* rather than what it will come to know, needs no belief over a hidden state (only the interventional transition kernel), and is therefore the one exploration-adjacent term here that survives in an environment with no hypothesis space to hold a posterior over (Salge et al. 2013).
- **[[wiki/concepts/expected-value-of-control.md]]** — where this page's quantities get spent rather than defined: an epistemic payoff enters the control decision through `Σ Pr·Value` and is compared against a subtracted effort cost, and because state-uncertainty and parameter-novelty terms have separate addresses here, that sum is at least two signals converging on one subtraction rather than the single scalar the criterion assumes.
- **[[wiki/concepts/learning-progress.md]]** — the competence-side counterpart to `E1`–`E4`: those four are defined over beliefs about the world, `PC`/`LP` over the agent's own achievement record, and the human fit shows the achievement-side signal is what rejects an unlearnable activity — a state where every belief-side quantity stays maximal forever. The two families have never been fitted to the same behaviour (Ten et al. 2021).
- **[[wiki/concepts/realised-information-gain.md]]** — `E3` with the expectation deleted, and a human measurement of the cheap version. `D_KL(p^j ‖ p^{j−1})` is the same divergence over the same kind of object (the parameters of a predictive distribution) computed on the observation that *actually arrived* instead of over hypothetical `y` under each candidate action, so it costs one subtraction rather than one belief update per branch. It consequently fails this page's impostor audit for the opposite reason to the deep estimator — a frozen policy in a stationary environment drives it to zero not by self-consistency but because there is genuinely nothing left to learn — and it cannot select an action at all, only gate engagement with the one already running. In 43 8-month-olds it dominates the disengagement decision (`e^{|β|} = 7.02`) while a surprise term in the same model carries the *opposite* sign (Poli et al. 2020).
