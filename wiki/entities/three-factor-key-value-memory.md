...
- **[[wiki/concepts/associative-memory.md]]** — the synthesis that types this page as the family's only **write** decision: the modern-Hopfield line derives capacity and supplies no write rule, this one supplies a synapse-executable write and matches the classical capacity per connection, and nothing in the wiki holds both at once (`T62`) — it also carries the 6× allocation gap against the measured, circuit-regulated plateau gate (`T391`).
