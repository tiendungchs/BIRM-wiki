...
- **[[wiki/entities/scan.md]]** — the failure this architecture was proposed as the fix for: SCAN's authors name "learned functions for interpreting individual modifiers, composed by the network" as their second route out of the length and add-primitive failures.
- **[[wiki/entities/rims.md]]** — the other escape from token-granularity routing, and the opposite trade: this system gets *relational* specialisation and typed arity by having the layout supplied by an unlearned dependency parser, RIMs learns the selection end to end from each module's own recurrent state and gets only temporal-phase and spatial specialisation — which decays over training. Relational modules and a learned router have never been obtained together (Goyal et al. 2019).
