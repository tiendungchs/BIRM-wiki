...
| **The teacher is still unexplained** | Same problem [[wiki/concepts/synaptic-plasticity.md]] logs for BTSP generally: modelling plateaus as an input-independent Poisson process is a *convenience that does the work*. If real plateaus are driven by entorhinal content, every uniformity argument on this page has to be redone. **The primary source models exactly that** — plateau probability raised by entorhinal drive and lowered by local feedback inhibition, with goal-directed field translocation as the result (Milstein et al. 2021, `T391`) — so the antecedent of that conditional is now on the table |
| **Binary weights may be a modelling choice, not biology** | Justified by positing several release sites per connection, each binary. The average number of release sites at a CA1 synapse is unknown. The continuous rule fitted to in vivo `V_m` uses `W_max ≈ 4` in units of the baseline weight and converges each synapse to a graded, timing-dependent `W_eq` — so the binary case is a limit, and the involution's `0 ↔ 1` symmetry is additionally broken by `k⁻/k⁺ ≈ 0.15` (Milstein et al. 2021) |
...
- **[[wiki/entities/spiking-hippocampal-cam.md]]** — the other explicitly-CAM hippocampal store, with disjoint strengths: this page has capacity theory, one-step reads and no addressing decision; that one has a second read direction (`content → {cues}`), a four-operation set typed from input spike timing, and an erase emitted by the write — at a store size equal to its one-hot cue count, which is no capacity claim at all.
- **[[wiki/concepts/analog-in-memory-computing.md]]** — the cost model behind this page's hardware argument, stated once and measured: reads by Ohm's and Kirchhoff's laws are nearly free while writes are stochastic, energy-dominant and ≈4-bit, which is exactly why a one-shot two-state local rule is the one a crossbar can run on-chip.
- **[[wiki/entities/three-factor-key-value-memory.md]]** — the same plateau-gated, postsynaptically-blind write applied to a *slot* rather than to a distributed trace, which is where the two disagree about the gate: `f_q` here is an independent per-cell coin and that independence is the feature (uniform load, no retrograde interference), while there an independent gate costs 6× capacity against a sequenced least-recently-used gate (`C ≈ 0.16N` vs `C ≈ 1.0N`) — the candidate reconciliation being trace width, not gate policy (`T391`).
