...
- **[[wiki/entities/bib.md]]** — the primary source for the benchmark described second-hand above, and the page that names what this page's baseline column actually was: a nearest-object heuristic applied in ~70% of trials, a goal-*location* preference in BC-MLP, and an efficiency law over-generalised onto the irrational agent — plus the origin of the `max_t` surprise statistic every number here is computed with.
- **[[wiki/concepts/certification-instruments.md]]** — supplies instrument `I6`, paired violation of expectation — the only protocol in the inventory that can register a model scoring *below* chance, and so detect the opposite rule acquired.
- **[[wiki/concepts/motivation-representation-synergy.md]]** — the same bound on the inverse-planning formulation: a Dirichlet prior over per-agent goal preferences models variation in what observed agents want, never variation in the *observer's* motivation to model them, so the framework occupies only the representational column of that page's two-factor dissociation.
- **[[wiki/concepts/shared-intentionality.md]]** — the missing frame beneath the inverse-planning formulation: a Dirichlet prior over per-agent goal preferences represents *other* agents' variation and has no term for a referent jointly indexed by observer and observed, which is the operation the comparative record marks as the human-specific one and dates three years before belief attribution.
