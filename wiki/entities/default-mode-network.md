...
| **Which grain the network is** | Node-level meta-analyses give nine nodes nine different preferential functions, and the posterior medial cortex fractionates into dorsal-PCC / ventral-PCC / RSC subnetworks with distinct control-network couplings — so "the default mode network" may name a family of systems sharing convergence zones rather than one system. **Now resolved for this page's single most consistent node** (Foster et al. 2023, [[wiki/entities/posterior-cingulate-cortex.md]]): the three subregions are separated by three thalamic afferent sets, three cortical afferent profiles, group *and* precision individual resting-state parcellation, task meta-analysis, and human intracranial recording down to single units — with only the ventral subregion in this network, the dorsal one in frontoparietal/executive networks, and the precuneus not cingulate cortex at all ([[wiki/concepts/node-definition-problem.md]]). **And the grain is not stable in time**: with a fixed vertex set, one third of the map changes membership between rest and a visual classification task and changes back afterwards (Gao et al. 2013), so any region list is indexed by the state it was measured in |
...
- **[[wiki/entities/global-neuronal-workspace.md]]** — a competing functional assignment for an overlapping hub set (medial prefrontal, posterior cingulate, precuneus): the workspace claims those hubs for broadcasting *current* content, this page's literature claims them for *internally generated* content, and no source in the wiki reconciles the two ([[wiki/empirical-tensions.md]] T267).
- **[[wiki/concepts/perturbation-elicitability.md]]** — the causal null over this network: in a 1537-site stimulation survey projected onto the Yeo 17-network parcellation, default and limbic networks sit at the bottom of the elicitation-rate gradient, and the effects that do appear (in orbitofrontal and cingulate sites) are content unrelated to the environment rather than perturbations of ongoing perception.
- **[[wiki/entities/integrated-world-modeling-theory.md]]** — the evolutionary role assigned to this network: cortical expansion producing association cortex and a more freely operating default mode is nominated as what allowed internal dynamics to decouple from immediate environmental engagement at all, which is the precondition for counterfactual simulation.
- **[[wiki/entities/lateral-frontoparietal-network.md]]** — shares the inferior parietal lobule with this network under a different functional attribution (first-order relational representation, scaling with the number of relations), which is the node-definition problem arriving at the level of *names*: two literatures assign incompatible jobs to overlapping coordinates and neither cites the other.
- **[[wiki/entities/lateral-frontoparietal-network.md]]** *(second link, Parsons & Davies 2022)* — the sign of this network's contribution to explicit reasoning, measured: across four coactivation networks recovered by MVPA during analogy, default-mode activation is **negatively** related to accuracy while executive frontoparietal activation is positively related — with the untested exception that an analogy requiring spontaneous retrieval of its own source should need the two to cooperate rather than trade off (Hammer et al. 2019).
- **[[wiki/concepts/retrieval-mediated-learning.md]]** — a coupling result that disconfirms its own network-level reading: hippocampal connectivity rose across encoding repetitions with ventromedial prefrontal cortex, frontal pole, precuneus and superior parietal cortex alike, and only the ventromedial region's activation predicted behaviour (others all r < 0.14).
- **[[wiki/entities/posterior-cingulate-cortex.md]]** — finishes the split this page's open-problem list calls for: the "PCC/Rsp" hub is *three* regions sitting in three different large-scale networks — dorsal posterior cingulate with frontoparietal/executive, ventral posterior cingulate with this network, retrosplenial with the medial temporal lobe — separated at the single-unit level by human intracranial recording, which makes this network's central hub a **pair of ports** (the medial temporal store writes to the ventral one, the control layer reads the dorsal one) rather than one shared bus, and supplies the node-definition fix [[wiki/empirical-tensions.md]] T267 was blocked on.
- **[[wiki/entities/retrosplenial-cortex.md]]** — splits this page's "PCC/Rsp" hub into at least two nodes: hippocampal and anterior thalamic connection densities are far higher to areas 29/30 than to area 23, and within one fMRI study areas 23/31 track real-vs-imagined while retrosplenial cortex tracks whether the event involved the participant personally (Vann et al. 2009).
- **[[wiki/entities/anterior-thalamic-nuclei.md]]** — puts a diencephalic node inside this network: human anterior thalamic nuclei are functionally connected to several of its components, their damage disrupts its activity and deep-brain stimulation modulates it — a dependency this network's cortical-connectivity methodology cannot see.
- **[[wiki/concepts/mental-imagery.md]]** — what this network is credited with constructing, and why the credit is weak: dorsolateral prefrontal, posterior parietal, precuneus and occipital cortex act together to construct and deconstruct even *non*-episodic abstract images, but nearly every study uses complex episodic scenes, so the sensory-generation contribution cannot be separated from the memory and temporal components.
