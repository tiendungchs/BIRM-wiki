...
- **[[wiki/concepts/curriculum-learning.md]]** — the wiki's only *automatic* curriculum, and one that anneals the target rather than the data: the EMA raises the predictor's achievable goal `τ/(1+σ²)` as the representation stabilises, which is a continuation method with `λ` read off the learner's own state — the self-pacing Bengio et al. propose and do not build (`T305`).
- **[[wiki/entities/saycam-baby-vision.md]]** — the datapoint this page's crop-only penalty (−13.1) should be read against: on longitudinal egocentric video the augmentation list buys little, suggesting the list is a synthetic stand-in for variability a richer stream already supplies (G95).
- **[[wiki/concepts/label-free-model-selection.md]]** — the lineage the loss-value selector cannot serve, and why the spectrum selectors matter: with no jointly-descended objective there is no loss whose value orders checkpoints, leaving an embedding-spectrum statistic as the only label-free instrument available to this family (`G108`).
