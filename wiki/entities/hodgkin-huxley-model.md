...
- **[[wiki/entities/early-visual-system.md]]** — the systems-level cash value of the shunting result above: because a rest-reversal conductance divides, the divisive-normalization arithmetic that carries V1 simple-cell saturation and nonspecific suppression is biophysically available without a dedicated mechanism — and it survived its own proponents changing their minds about which mechanism implements it.
- **[[wiki/concepts/predictive-adequacy.md]]** — this model is the precedent cited for the *constrain-simple-test-natural* position: voltage clamp produced the equations that naturalistic synaptic current would not have, which is the strongest available argument that reduced stimuli are a method rather than a bias.
- **[[wiki/concepts/neuron-complexity-index.md]]** — retires this page's cost limitation and turns its compartmental extension into a measuring instrument: a temporally convolutional network fitted to the compartmental model's input/output pairs reproduces somatic voltage (94.6% variance) and single spikes at ~2000× less compute, and the *size* that fit requires becomes a complexity score for the cell — 5–8 layers with NMDA conductance present, one hidden layer without it.
