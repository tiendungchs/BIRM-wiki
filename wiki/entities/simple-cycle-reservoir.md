...
- **[[wiki/entities/ms-ssm.md]]** — Def. 7 built and measured: a Multi-Cycle Reservoir with non-identical blocks read out by a linear mixture is defined on this page and never used, because every theorem here requires identical blocks; MS-SSM supplies the two missing pieces — a learned undecimated wavelet cascade so the blocks receive genuinely different band-limited signals, and an input-dependent mixer `E_t = Linear_E(x_t)` in place of the fixed coefficients `a_i` — and reports exactly what this page's width-is-a-rate result predicts size alone cannot buy: redistributing a fixed state across bands is worth 25 points on ListOps against 4 for doubling the state (Karami et al. 2025).
- **[[wiki/concepts/autonomous-pattern-generation.md]]** — the same boundary reached from the generation side: this page proves a contractive reservoir cannot carry a non-fading dependency, and that page shows the corresponding way out for *output* — drive the reservoir with a periodic clock, whereupon ESP guarantees a unique periodic orbit and an unbounded emission at the price that everything produced is locked to the clock's phase and can depend on nothing else.
- **[[wiki/entities/conceptor.md]]** — what can still be bought inside this page's boundary: many patterns internalised into one contractive recurrence and selected among by a positive semi-definite filter in the feedback loop, with logical operations and an abstraction order over the selectors. The universality result is untouched — a conceptor chooses which fading-memory orbit is expressed, and cannot make a non-fading dependency expressible.
- **[[wiki/entities/liquid-state-machine.md]]** — this page's theorem 22 years earlier with the existence quantifier still in place: Maass, Natschläger & Markram 2002 prove that *some* composition of separating basis filters plus an approximating readout hits any time-invariant fading-memory filter (and any such filter on **spike trains**, which this page's linear theory does not cover), and draw the identical boundary; what this page adds is the construction. Read together they say the LSM's biological richness — heterogeneous neurons, dynamic synapses, distance-decaying connectivity — cannot be buying representable functions, only separation quality at a given `n`.
- **[[wiki/entities/sigma-pi-reservoir.md]]** — the same demotion of the reservoir carried one step further, and to the same permutation ring: this page proves the recurrent *coupling* is expressively free inside the fading-memory class, that one argues the *pointwise nonlinearity* does not deliver the higher-order features it is credited with (saturating reservoirs mainly cause memory fading, leaving a feature space close to a linear recurrent network's) and moves them into a separate multiplicative circuit — so between the two, almost nothing of the classical reservoir's design is left load-bearing, and the exclusion list here applies unchanged since the added stage is memoryless (T238).
- **[[wiki/entities/lru.md]]** — the empirical answer to this page's central trade, and it is favourable: this page pays state dimension `n' ≈ (N+1)nk` to unitarily dilate an arbitrary `W` into a perfectly conditioned ring, and LRU shows that at 16,384 steps a **normal diagonal** recurrence with `2N` recurrent parameters not only suffices but *beats* the dense non-normal alternative at matched initialisation spectrum (sCIFAR 72.2 → 86.5, ListOps 50.4 → 58.8) — so the conditioning this page buys by construction was not purchased at the cost of a memory mechanism, and Prop. 12's "the state basis is unobservable" is joined by an empirical "and the simplest basis is the best one" ([[wiki/empirical-tensions.md]] T311, T174).
- **[[wiki/concepts/parallel-timescale-streams.md]]** — the target specification for this page's Def. 7 multi-ring construction: stacking rings with different contraction rates buys a bank of decay constants over one input stream, and the connectome result says the arrangement to aim at is instead several concurrently-running streams sharing one state alphabet and one transition prior, which no ring stack currently provides.
