...
- **[[wiki/entities/corethink-compositional-reasoner.md]]** — the same benchmark from the opposite end of the DSL grain axis: 22 coarse macro-patterns that neither execute nor reject, against these 7 fine predicates that do both for free — the trade being search cost against coverage ceiling, with neither source setting the grain by any principle.
- **[[wiki/entities/arc-vsa-solver.md]]** — the same object-centric commitment in the opposite representation: graded dot products instead of exact unification, and the parse handled by ranking six hypotheses and committing to one rather than by keeping a typed superset of overlapping parses ([[wiki/empirical-tensions.md]] T191).
- **[[wiki/concepts/reasoning.md]]** — the other page carrying logical machinery the census cannot classify: inductive logic programming is definition R3 in a different notation, and the deductive half of what it runs has no home in the wiki.
- **[[wiki/entities/recursive-npi.md]]** — the answer to the hole this page names and defers (`G70`): where a clause is unrolled `n` times and silently caps the test instance at `≤ n` repetitions, a tail call re-enters the same program with no count stored anywhere — at the price of per-step trace supervision this page does not need.
