...
Criteria 1, 2 and 4 are collectively what prevents collapse; each blocks one of the collapse modes tabulated in [[wiki/concepts/energy-based-models.md]]. The reason four criteria are needed at all is `G34` — the cheapest minimiser of a joint-embedding objective is a constant encoder, so every one of them is a patch on an objective whose optimum is to represent nothing.
...
- **[[wiki/concepts/representational-collapse.md]]** — the page that states why this design's training criteria come in exactly four parts — one per place free capacity can sit — and what the thirteen collapse measurements did to that recipe.
- **[[wiki/entities/global-neuronal-workspace.md]]** — the biological framework this design's configurator is credited to, and the measure of what it omits: it has the shared bus and the top-down modulation but no threshold, no exclusivity and no broadcast, so it is a router without an admission gate.
- **[[wiki/concepts/reasoning.md]]** — the architecture in which reasoning-as-energy-minimisation is the operating definition and the Mode-1/Mode-2 split is a wiring diagram, supplying the census's architectural criterion for System 1 versus System 2.
