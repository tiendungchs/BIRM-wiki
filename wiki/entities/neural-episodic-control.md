...
- **NEC > MFEC because the embedding is trained by TD, not chosen** — though the two agents also differ in the write rule (`max` vs a tabular update that can decrease, `T352`) and in the read (unweighted mean vs kernel), so the attribution is confounded three ways. MFEC keys on random projections of pixels or on a VAE latent; the former cannot resolve the few pixels that determine value, the latter spends capacity on reward-irrelevant detail like the on-screen score. Learning the metric from the reward signal is the paper's sole architectural claim against MFEC and it is worth ~20 median points at 10M.
...
| [[wiki/entities/model-free-episodic-control.md]] (Blundell et al. 2016) | Random projection or VAE latent, **fixed** | **Unweighted** mean of `k` NN; exact match returns the stored value | Monte Carlo return, keep the max | Episode end, backward sweep | Is the value function |
...
- **The Atari margin is not stratified by how often the agent had already seen the state, and the number exists.** [[wiki/entities/model-free-episodic-control.md]] measures exact state-action revisit rates of 10–60% across five of these same games (60% on Q\*bert) against <0.1% in a 3-D environment, so an unknown and game-varying fraction of NEC's early lead is literal lookup rather than one-shot generalisation. Nothing in the 57-game table is conditioned on it (`T353`).
...
- **[[wiki/entities/model-free-episodic-control.md]]** — the architecture this one is the successor to, and the baseline every number above is measured against: the same store with a fixed embedding, an unweighted `k`-NN read and a `max`-over-returns write, which is where NEC's three simultaneous changes (`T352`) and the only published revisit statistics (`T353`) come from.
...
- **[[wiki/entities/go-explore.md]]** — the same bet on non-parametric memory with the contents swapped, and the score gap is the argument: this page stores *returns* for states it happens to revisit and gets 42.1 on Montezuma's Revenge, Go-Explore stores *states and the routes back to them* and gets 1.7M — so for exploration an episodic store should hold the address, not the value.
