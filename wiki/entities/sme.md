...
- **[[wiki/entities/lateral-frontoparietal-network.md]]** — the attempt to find this engine's stage decomposition in tissue, and its failure: access, mapping, inference and schema induction are separable in the code, no imaging study has measured all four, access has never been imaged, and the trial phase with no mapping demand out-activates the one with it (`T330`, Parsons & Davies 2022).
