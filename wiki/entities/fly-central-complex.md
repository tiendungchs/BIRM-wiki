...
- **[[wiki/concepts/activity-slots.md]]** — this circuit's motif turns out to be the one that preserves compositionality: the `BioRNN` that integrates velocity in a *separate population* from the stored activity is modelled on the fly head-direction circuit, and it is the only conventional variant that obeys slot algebra on every task — mixing velocity into the memory units destroys the algebra while leaving task accuracy intact (Whittington et al. 2025).
- **[[wiki/entities/retrosplenial-cortex.md]]** — the same two-input heading architecture at cortical scale: a self-motion-integrated estimate plus a sensory signal that resets it, with a whole visual scene through V2/V4/parietal input in place of this circuit's single bar, and thalamic head-direction input in place of its ring.
- **[[wiki/concepts/attractor-identification.md]]** — the one system in that page's scoreboard satisfying the *supporting* anatomical criteria outright: a physical ring of neuropil and a fully traced connectome implementing the copy-and-offset double-ring construction, so ring-attractor claims here rest on structure as well as on state-space invariance — with the full state-space dimensionality still uncharacterised, and 2-D path-integration results suggesting the manifold may not be the 1-D ring the anatomy suggests.
