...
| **The decomposition is a partial answer to `G33` and an unpriced one** | Skills are obtained with no reward, no transition graph, no demonstrator and no task ensemble — which is more than any other family on that row asks for — but the count `N` is a hard-set integer and the objective supplies no principle for it, so the row's *selection* half is untouched: DIAYN decomposes the reachable set, not the task |
...
- **[[wiki/entities/asymmetric-self-play.md]]** — the opposite corner of reward-free goal generation: a latent index that names no state and whose ant skills score below doing nothing, against literal reachable object configurations that transfer zero-shot to hand-written tasks. Both need a hand-authored non-degeneracy clause to keep the self-generated objective from collapsing — the `log p(z)` baseline here, three legality predicates plus a `−3` out-of-zone penalty there — which is where the designer's task knowledge re-enters after being removed from the goal distribution (`G72`).
