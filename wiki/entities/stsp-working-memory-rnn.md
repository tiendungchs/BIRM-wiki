...
- **[[wiki/entities/spacetime-attractor.md]]** — the same comparison run on a different objective and coming out the other way: on planning tasks with within-trial reward change, trained networks converge on attractor dynamics and networks too small to build them fail outright, so "attractors are the fragile, less cortex-like solution" may be a fact about delay tasks rather than about prefrontal recurrence ([[wiki/empirical-tensions.md]] T106).
- **[[wiki/concepts/spike-frequency-adaptation.md]]** — the postsynaptic sibling of this page's mechanism: the review of adaptive neurons treats synaptic depression and an intrinsic adaptive conductance as interchangeable substrates for the same adaptation signature, so the activity-silent-memory argument is not specific to synapses.
- **[[wiki/concepts/activity-slots.md]]** — re-types this page's result rather than contradicting it: under the episodic/working duality, "prefrontal cortex stores in short-term synaptic plasticity" is the claim that prefrontal cortex runs the *synaptic* algorithm, whose activity-side dual computes the same function — so the delayed-match evidence bears on the carrier only through decoding and scaling, never through behaviour (`G112`; Whittington et al. 2025).
