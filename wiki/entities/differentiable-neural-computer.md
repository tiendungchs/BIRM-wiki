...
- **[[wiki/entities/transformer.md]]** — the near-contemporaneous architecture that dropped everything this page keeps: attention survived as the read, while the addressable memory, the gated writes and erases, and the write-order link matrix that lets a read traverse *temporal* rather than content structure did not — leaving a model whose only memory is its own input, re-scored from scratch each step (Vaswani et al. 2017).
- **[[wiki/concepts/memory-read-and-erase.md]]** — the closest existing read/erase machinery in the wiki and the measure of what is missing: the free list frees by *usage* and the temporal-link matrix orders by *write time*, so this model has an erase with the wrong address and a read with no lead time.
- **[[wiki/entities/neural-module-networks.md]]** — the opposite answer to composition over data structures: this page keeps one controller and makes *memory* addressable, NMN keeps memory implicit and makes the *computation graph* per-instance and typed — which is why this page's learned program reads as assembly while NMN's is a parse tree by construction, and why NMN extrapolates to graph depths it never trained on.
- **[[wiki/concepts/discrete-relaxation-gradients.md]]** — names the principled alternative to this page's handling of the allocation sort (ignore the gradient discontinuities), and this page returns the most useful evidence against it: ignoring them is "apparently harmless" here, which bounds when a relaxation's bias and temperature schedule are worth paying for.
- **[[wiki/entities/neural-episodic-control.md]]** — the design that rejects this one by name: no learned write head (writing is unconditional and append-only), no per-episode wipe (the store persists across the whole run and grows to `5×10⁵` entries per action), on the stated grounds that learning *when* to write is slow to train — so the two pages bracket the write-gating question with a measured cost on neither side.
- **[[wiki/concepts/fast-weight-programming.md]]** — the distributed alternative to this model's slotted store: a targeted erase obtained from linear algebra alone (read the key, subtract what is there, write the interpolant) with no addresses, usage weightings or free list — and the trade is exact, since the delta rule reclaims no capacity and has no `suppress`, which is what the bookkeeping here buys (Schlag et al. 2021).
- **[[wiki/entities/scan.md]]** — the third proposal in SCAN's discussion, built: quasi-discrete external memory for separate variable storage, offered as the route to abstract rule learning — with no systematicity number ever reported on those splits.
- **[[wiki/entities/universal-transformer.md]]** — the architecture that claims to subsume this one's read step: a single addressed read per step is expressible as a global parallel revision of every position, and its authors' stated advantage is competitiveness on realistic sequence-to-sequence tasks (28.9 BLEU WMT14) rather than on algorithmic ones — bought by giving up the explicit, erasable, order-linked memory this page keeps.
- **[[wiki/entities/rims.md]]** — the memory-model contrast that beats it on held-out sequence lengths (90.0 / 73.4 / 38.1 vs 87.2 / 44.1 / 19.8 on MNIST at 16×16 / 19×19 / 24×24 after training at 14×14) and inverts the design: an external memory applies **one shared update rule** to passive cells under a central controller, where RIMs gives each cell its own recurrent parameters and no controller, so the memory *is* the computation (Goyal et al. 2019).
- **[[wiki/entities/esbn.md]]** — the opposite bet about what an external memory needs: this page invests in addressing (content, temporal link, usage) and lets the controller see the input, while the ESBN keeps naive content addressing and instead *cuts* the controller's line to the values — and on withheld-entity relational tasks that one difference beats the NTM by 40+ points.

- **[[wiki/entities/world-models-vmc.md]]** — named there as the fix for a world model's capacity limit: an LSTM's weights cannot hold what an iterative real-environment collection procedure records, so the proposal is to move the model's memory out of the weights and into an addressable store.
- **[[wiki/concepts/curriculum-learning.md]]** — the strongest necessity claim the concept page carries (a 25-lesson schedule essential everywhere except bAbI; 98.8% against an LSTM's 37%) and its clearest violation of the nesting condition: 10% of exemplars must be drawn from earlier lessons to stop regression, so Bengio et al.'s non-decreasing-weight requirement is load-bearing rather than formal.
- **[[wiki/entities/recursive-npi.md]]** — the same era's other differentiable-programming system with the opposite memory contract: an external *environment* plus a stack of reset controller states, against this page's external memory plus one continuing controller state — which is why one admits a finite verification set for unbounded inputs and the other does not.
- **[[wiki/entities/remerge.md]]** — the same content-addressable store with the recirculation loop present but untrained for inference: read heads here can already chain, and nothing in this objective asks the chain to run to a fixed point, which is precisely what that model adds.
