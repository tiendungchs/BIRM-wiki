...
- **[[wiki/entities/universal-transformer.md]]** — the strongest of the three architectures scored here and the one whose failure is most informative: recurrence in depth buys large gains on length extrapolation and algorithmic tasks elsewhere, and buys almost nothing at high compound divergence (98.0 i.i.d. → 18.9 at MCD, 1.2 on SCAN MCD), so length generalisation and compositional recombination are separable failures and this model separates them.
