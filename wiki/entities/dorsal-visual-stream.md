...
- **[[wiki/concepts/incremental-grouping.md]]** — the early-visual half of a loop this page describes only from the parietal end: same-object judgements and tracing are parieto-prefrontal traffic, and the tag that makes them answerable is written back down in V1 — which also means the model that implements the tag replaces this entire pathway with a read-out of one output unit.
- **[[wiki/entities/multiscale-tracing-network.md]]** — what it costs to implement the tag and skip this pathway entirely: the same-object judgement this page carries as parieto-prefrontal traffic is replaced by a direct read-out of one output unit over a tag written in early visual cortex, so the model reproduces the behaviour and the reaction time while leaving this anatomy with no job in the loop.
