...
| Ablations | Replay, target network and the deep convolutional network are each shown load-bearing by disabling them (Extended Data Table 3); a linear function approximator is far worse (Table 4). **The main text reports no numbers for either** — the tables were not in the ingested body. One number is recoverable second-hand: disabling experience replay drops performance to **at best ~30%** of the with-replay score (Kumaran, Hassabis & McClelland 2016, `raw/kumaran-2016-complementary-learning-systems-updated.md`) |
...
| **Episodic control** ([[wiki/entities/neural-episodic-control.md]], MFEC) | Same pixels, **same conv stack**, embedded | `(s,a,Q^{(N)})` tuples in one dictionary per action, kernel-weighted 50-NN read, tabular update at a large `α` | Retrain | The store *is* the policy; no target network and no reward clipping are needed |
...
- **[[wiki/concepts/complementary-learning-systems.md]]** *(also)* — supplies this page's missing ablation magnitude and the theory's own reading of why the buffer works: data efficiency (each experience reused in many updates, the stated analogue of time-compressed hippocampal replay) plus removal of the policy's bias on which samples are seen — two functions the paper's own instability analysis states separately (Kumaran et al. 2016).
...
- **[[wiki/entities/model-free-episodic-control.md]]** — the first agent to move this one's value function into a table, and the source of the statistic that reprices every Atari comparison including this page's: the emulator is deterministic with 30 initial states, and exact state-action revisit rates run 10–60% across five games (`T353`).
- **[[wiki/entities/neural-episodic-control.md]]** — this architecture's perception kept and its value function moved out of the weights: the same convolutional stack feeds one differentiable dictionary per action, which removes both devices this page's instability analysis is built around (no target network — stored `N`-step returns are the labels — and no reward clipping, since a table has no shared learning rate to protect) and beats prioritised replay by 2–3× in median human-normalised score below 20M frames before being overtaken at 40M.
- **[[wiki/entities/elastic-weight-consolidation.md]]** — the direct attack on this architecture's one-network-per-game constraint: the same agent plus a Fisher-weighted penalty, an inferred task context and per-game gains learns 10 games in sequence, and still scores below 10 separately trained DQNs — which prices what the null architecture's per-task duplication was buying.
- **[[wiki/concepts/hindsight-goal-relabelling.md]]** — the buffer-side intervention that needs none of this architecture's other machinery: with the network, loss and interaction budget unchanged and only the goal field of each stored tuple rewritten, this agent goes from solving `n`-bit flipping at `n ≤ 13` to `n = 50`, where count-based and bootstrapped exploration bonuses buy nothing (Andrychowicz et al. 2017).
- **[[wiki/entities/option-critic.md]]** — this architecture used as a substrate and beaten by a head on it: the same three convolutional layers and 512-unit dense layer carry a policy over options, eight linear-softmax intra-option policies and eight sigmoid termination functions, learned end-to-end within 200 episodes and surpassing the original in Asterix, Seaquest and Zaxxon — so temporal abstraction is measured here as an output-layer change rather than an architecture change (Bacon, Harb & Precup 2017).
- **[[wiki/entities/feudal-networks.md]]** — uses the same convolutional front end under A3C instead of Q-learning, and supplies a methodological warning about this page's role as a yardstick: hierarchical gains reported against DQN are not commensurable with gains against an A3C-LSTM baseline, which is the source's stated reason for re-running its comparison with [[wiki/entities/option-critic.md]] itself (Vezhnevets et al. 2017).
- **[[wiki/entities/go-explore.md]]** — the same benchmark with one non-structural component added, and the wiki's null recalibrated: this page's `0` on Montezuma's Revenge has been read as evidence that sparse long-horizon tasks need a model or a hierarchy, and Go-Explore adds neither — only an external archive of visited states and the ability to re-enter one — reaching 1.7M, which relocates the missing ingredient from decomposition to a stored frontier (`T402`).
