...
- **[[wiki/entities/waterbirds.md]]** — the opposite control policy on the same nuisance problem: randomise the nuisance at capture so that no group is rare, versus plant the correlation at a chosen rate (95%) so that the rare group *is* the measurement — and the second, unlike this page, also ships a training objective rather than a test-only set.
