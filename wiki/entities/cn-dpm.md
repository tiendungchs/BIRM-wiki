...
- **[[wiki/entities/elastic-weight-consolidation.md]]** — the routing contrast that turns this page's gating measurement into a design rule: the same kind of inferred task variable costs almost nothing when it selects a *penalty anchor* (EWC's oracle is worth only a modest improvement) and costs the whole result when it selects *which component answers*, as here — so an unreliable gate is safest spent on regulating plasticity rather than on selection (G37).
- **[[wiki/entities/sparsely-gated-moe.md]]** — the two halves of the cost of a growing expert library, separated: this page prices *indexing* one (gating accuracy 48.18% at five experts with no labels), while the sparsely-gated MoE prices *estimating* one — 65,536 experts help and 131,072 hurt at fixed data, because each expert's batch is `kbd/n` and past some `n` no expert sees enough examples, a ceiling that binds regardless of how good the router is.
