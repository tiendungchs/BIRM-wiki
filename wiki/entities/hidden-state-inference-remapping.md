...
**The morph row has since been reproduced within one lab — by the wrong variable** (Colgin et al. 2010, `raw/colgin-2010-attractor-map-versus-autoassociation-hippocampal-dynamics.md`). Both regimes were obtained on the same morph-day protocol at the same training stage, separated by whether the two enclosures had earlier been placed apart and joined by a walkable corridor. The model's prediction that the divergence is an early-vs-late position on the evidence continuum is therefore not what produced it here, and two features of the result are awkward for the account as stated. (i) The manipulation adds **no observations that distinguish the enclosures** — the corridor group saw the same square and the same circle — so whatever split the partition was the animal's own path-integrator coordinate, a variable the generative model has no feature slot for. (ii) After training, returning both enclosures to a **common physical location** — an observation that should raise the evidence for a 1-state partition — left the two representations essentially independent, i.e. the binding resisted contradicting evidence rather than being reweighted by it. The model can absorb (i) by admitting self-motion coordinates as a feature dimension, which is free but untested, and (ii) only through a prior stiff enough to defeat the likelihood. Carried as [[wiki/empirical-tensions.md]] `T394`.
...
- **Exact inference is intractable**, so all results compare a handful of hand-picked partitions. The authors call the whole paper **"an analytical heuristic rather than an algorithmic theory"**. *Proposal generation* — where the candidate partitions come from — is named as the unsolved key step. Proposal generation is `G37` (which stored state applies now) and the partition prior is `G38` (when to allocate a new one instead), and this model supplies the Bayesian statement of both while running neither.
...
**The primary data say "sample" more strongly than this page states it.** Jezek et al. 2011 (`raw/jezek-2011-theta-paced-flickering-between-place-cell-maps.md`) is usually cited here for the *rate* of switching; its load-bearing result for a sampling account is the **shape** of what a cycle contains. CA3 theta cycles correlating with both stored charts at once are 1.25% of the total, and that is *below* a null in which each unit independently expresses one chart or the other (970/1000 shuffles exceed it, `p < 0.03`), with the margin tightening to `p < 0.001` in the second half of each cycle. A posterior *mean* over two hypotheses is exactly the mixture that is suppressed. So whatever the hippocampus does with its uncertainty, it is not representing it as a blend — which is the specific claim a sampling code makes and a rate code does not, and it is the strongest evidence this page has. Two things it does not supply: the sampling rate is never shown to track a probability (no manipulation grades the evidence ratio and measures the ratio of cycles), and the pre-switch baseline flicker rate of 1–3% is not tied to any residual uncertainty about the environment.
...
- **[[wiki/concepts/path-integration.md]]** — the rival account of this page's central phenomenon (`T394`): discreteness as separation between two anchored coordinates on one preconfigured attractor map, with the cue→coordinate binding as the only learned object — which explains the morph disagreement by a manipulation this model's feature space cannot represent, and is in turn silent about the graded and partial regimes this page's posterior predicts.
- **[[wiki/concepts/attractor-identification.md]]** — the battery this page's central phenomenon is scored against, and the one criterion it contests: criterion five reads an abrupt transition under a graded morph as evidence for two anchored coordinates on one landscape, where this page reads the same abruptness as the log posterior odds of a hidden-state partition crossing a decision boundary — so the instrument cannot separate the two accounts and `T394` carries the fork.
