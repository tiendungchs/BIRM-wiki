...
- **[[wiki/concepts/neuromodulatory-metaparameters.md]]** — the same measurement obtained two ways: this agent's emergent learning rate rising in volatile blocks is, in the neuromodulatory account, an explicit cholinergic metaparameter with its own broadcast channel and its own set-point rule. One is implemented and opaque, the other is legible and unimplemented — and they are not exclusive, since a recurrent inner learner could just as easily emit the metaparameters as consume them (Doya 2002).
- **[[wiki/concepts/reward-prediction-error.md]]** — the page that carries this model's sharpest result as a general claim: the same scalar trains the recurrent weights and, delivered as an input to those weights once frozen, still moves behaviour, so no dopamine manipulation separates the plasticity role from the inference role.
- **[[wiki/entities/dqn.md]]** — the same reward-driven training at one timescale instead of two: what DQN writes into weights (and must retrain to change), this model leaves in recurrent activity, which is precisely where the re-goaling DQN cannot do reappears.
