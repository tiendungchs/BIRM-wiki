...
- **Nothing tests self-amendment (G9).** The activation predicate is fixed; no level lets the agent edit the rule about what makes rules active. That third tier — self-amendment, gap **G9** — is what a rule-rewriting game like [[wiki/entities/nomic.md]] supplies and this environment does not. [[wiki/entities/fluxx.md]], now in the wiki, turns out not to supply it either — its recency-override rule and turn machinery are never editable — but it does supply the two tiers between: rewrites performed by *other* agents, and a win condition that is itself a card any opponent can replace.
...
- **[[wiki/entities/nomic.md]]** — the specification of the third tier this benchmark's limitations section names: there the activation predicate is not fixed but is a numbered rule that a rule-change can amend, and mutability is a per-rule bit any unanimous vote can flip — bought by giving up the bounded rule vocabulary, the implementation and every measurement.
