...
| **Clinical evidence puts the switch in a different failure mode from the one the model predicts** | If this network sets the mode, a disorder of switching should show altered *salience-network* connectivity. What is reported instead is mostly a default-mode-side change — increased default↔insula coupling in schizophrenia (read as diminished internal/external switching), increased default↔salience coupling in Alzheimer's, increased default↔salience coupling during and after meditation, and in major depression a **spatial expansion of this network that shrinks the default network's territory** rather than a change in its coupling (Tripathi et al. 2025, [[wiki/entities/default-mode-network.md]]). The last case is the sharpest: a topographic change is not a switching change, and — since group-level connectivity differences are majority topography ([[wiki/concepts/node-definition-problem.md]]) — the two are not distinguishable in most of this literature |
...
- **[[wiki/concepts/broadcast-channel-decomposition.md]]** — the same functional role one level down and in a different substrate: a dedicated population whose output is an unsigned salience estimate reassigning processing mode rather than being scored as an outcome, implemented as a topographically separated subcortical modulatory channel instead of a cortical network — and it shares this page's unresolved question of what consumes the estimate.
...
- **[[wiki/concepts/node-definition-problem.md]]** — why this page's clinical evidence is weaker than it looks: up to 62% of group-level connectivity-matrix variance is cross-subject spatial topography, so a reported change in this network's coupling to the default network and a change in where the two networks' borders fall are the same measurement (Tripathi et al. 2025).
...
- **[[wiki/entities/lateral-frontoparietal-network.md]]** — the network this page's switch engages, described from the reasoning literature instead of the attention literature: the same lateral frontoparietal system carries a rostrolateral node and a relational-integration function that the switching account has no use for, which is a warning that "engaged by the switch" is a coarse description of a system with an internal division of labour.
- **[[wiki/concepts/expected-value-of-control.md]]** — the same dorsal anterior cingulate tissue given an incompatible job description: a normative evaluator running `argmax` over control signals net of cost, against this page's bottom-up detector switching the system between internally and externally directed modes — the two differ in whether the region's output is a decision variable or a switch.
- **[[wiki/concepts/pseudo-reward-prediction-error.md]]** — this page's two cortical nodes are exactly the two regions surviving every control in that experiment (dorsal anterior cingulate and bilateral anterior insula, after regressing out displacement magnitude), and what they report there is a *purely internal* bookkeeping error: the two compared conditions are matched visual events with no exogenous salience difference, which the bottom-up detector framing has no way to produce (Ribas-Fernandes et al. 2011).
- **[[wiki/entities/posterior-cingulate-cortex.md]]** — a name collision worth keeping straight: this page's salience is a stimulus-relevance detector that switches the default network off, while the salience that best explains dorsal posterior cingulate firing is the unsigned deviation of a decision *outcome* from baseline, carried by a region inside the system this one is said to switch.
