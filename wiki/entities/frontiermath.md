...
- **[[wiki/architectural-gaps.md]]** — G17 absorbed this page's contamination row: a permanently private benchmark relocates the failure from leakage into unauditability rather than solving it.
- **[[wiki/entities/hle-verified.md]]** — the auditability inversion of G17 made concrete: this benchmark's ~10% critical error rate is unauditable by anyone outside because the items are unpublished, while HLE's 2,500 published items were re-audited by a second organisation, found ~26% scoring-relevant-defective, repaired, and released with the full defect record — the price of publication is contamination, and the return is a defect rate somebody else can check.
- **[[wiki/entities/hle.md]]** — the other benchmark that floors every solver and disclaims its own ordering (`F7`), and the auditable counterpart of this page's unauditable ~10% error rate: HLE publishes all 2,500 items and measures its disagreement rate at 15.4%, showing in the process that the number is a property of the review protocol (18% → 25% under a single-reviewer rule) rather than of the dataset — which weakens both sides of G17, since a *measured* error rate is not comparable across benchmarks that audited themselves differently.
- **[[wiki/entities/aime.md]]** — the precedent for this page's guessproofness bound: a 000–999 integer answer sheet gives `P(correct | no work) = 0.1%` by format rule alone, tighter than the `<1%` criterion audited per item here, and achieved in 1983 without an audit — so the contribution is the *auditing*, not the threshold.
- **[[wiki/concepts/benchmark-contamination.md]]** — the maximal prevention on that page's defence table (never publish the items) and the audit it forecloses; the residual channel it does not close is that every evaluation of a closed model sends the items through a provider's API, which is itself a leakage event.
- **[[wiki/concepts/reasoning.md]]** — one of the two pages that use proof machinery while the wiki holds no definition of deductive inference: all seven of its reasoning definitions select a structure that fits observations, and none names a truth-preserving step.
