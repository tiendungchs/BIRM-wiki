...
- **[[wiki/entities/simsiam.md]]** — the minimal version of the skeleton this page inherits, and the control on how much of it is load-bearing: the EMA target, the batch coupling, the BatchNorm and the cosine are each deleted without collapse, leaving the predictor and the stop-gradient.
...
- **[[wiki/concepts/alignment-uniformity.md]]** — independent support for this page's lever from the loss side: replacing a contrastive loss with its own alignment/uniformity decomposition wins on every image benchmark and loses 3–4 points on both sentence tasks, and the only thing that changed is the pair sampler (augmentations vs. neighbouring sentences).
- **[[wiki/concepts/representational-collapse.md]]** — the lever that is on none of the tables and outweighs all of them: holding the loss fixed and varying only the `(x, y)` sampler moves ImageNet-1% linear evaluation from 15.5 to 54.2.
- **[[wiki/concepts/manifold-untangling.md]]** — the vision-neuroscience statement of what a joint-embedding objective is buying: flat, separated, aligned object manifolds, scored by a fixed-complexity reader on held-out *transformation* conditions rather than held-out samples.
- **[[wiki/entities/mae.md]]** — the arm this page's flagship ablation beats, now held as a source: same masking, same ViT, the visible-patches-only encoder trick inherited wholesale, and one difference (pixel target vs EMA-encoder target) worth +26.2 — measured with a frozen linear probe that MAE's own partial-fine-tuning sweep shows inverts method rankings at read-out depth 1 (T310), and against which MAE wins Clevr counting 90.5 to 86.7 and every dense transfer number in its own table.
- **[[wiki/entities/cpc.md]]** — the ancestor of the "predict a representation, not an observation" move, differing on exactly the two axes T167 is about: the pair is temporal rather than spatial, and the anti-collapse provision is negatives rather than the EMA/predictor asymmetry.
- **[[wiki/concepts/curriculum-learning.md]]** — the reminder that the wiki's biggest ordering effect is not an ordering: a single fixed context/target mask sampler moves ImageNet-1% linear evaluation 15.5 → 54.2 with no schedule at all, so "design the experience stream" and "sequence it easy-to-hard" are separable levers and the first is currently worth more.
- **[[wiki/entities/saycam-baby-vision.md]]** — the same pair-construction lever on an axis that has a shape: segment length sweeps to an interior optimum at ~288 s where this page's mask sampler was found by search, so the two bracket T167's open half (is the lever settable by a rule?).
