...
- **[[wiki/entities/visual-predictive-coder.md]]** — a formal limit on the same-target objective this page occupies: where the observation map is non-injective, no left inverse exists, so no reconstruction objective on a single stationary observation recovers the latent state however good its pixels are — and the 0.039-vs-0.094 MSE inversion against a next-frame predictor that maps far better is that statement in one number.
- **[[wiki/entities/saycam-baby-vision.md]]** — the second route to small augmentation dependence: this page reaches it by *abstention* (a fresh random mask supplies the pair), that one by a stream whose natural variability already covers the list — same insensitivity, opposite mechanism.
- **[[wiki/concepts/label-free-model-selection.md]]** — the selection-side version of this page's read-out-depth finding: if two of this model's own ablations are visible only to the frozen probe, then a *selector* built on that probe inherits the defect, and the label-free alternative is a spectrum statistic that never sees a label (T310, `G108`).
