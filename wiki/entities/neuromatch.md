...
- **[[wiki/entities/learningmatch.md]]** — the same amortisation of a pairwise query with the opposite ablation result: here the fixed order-embedding geometry beat both learned comparators (MLP, neural tensor network) on accuracy *and* was 10× faster, there the learned comparator is the only component whose depth changes the error. The discriminator is the target relation — subgraph containment is transitive, so a geometry can encode it exactly; a curved metric has no such algebraic handle ([[wiki/empirical-tensions.md]] T173).
- **[[wiki/concepts/representational-collapse.md]]** — the asymmetric hinge in a working retrieval system, and therefore the wiki's cleanest instance of a design that needs no anti-collapse provision because its energy shape punishes the degenerate solution.
- **[[wiki/concepts/analogical-mapping.md]]** — the untrained rival on the same relational-comparison problem, and the sharper version of this page's missing certificate: a sparse binary code has a closed-form chance-overlap baseline, so "no correspondent exists" is a computable verdict rather than a validation-set threshold.
- **[[wiki/entities/macfac.md]]** — the same filter-then-verify architecture with the opposite attitude to error and a different kind of filter: functor counts derived to *over*-estimate the exact matcher's match-hypothesis count (so failures cost time, never recall), and un-flagged false positives treated as the human behaviour to reproduce rather than the defect to remove.
- **[[wiki/concepts/curriculum-learning.md]]** — the concept page's cleanest ablation (−6% and higher variance without the schedule) and its only instance with both missing pieces present: the pace is *self-set* (increment on 20 plateaued epochs) and the difficulty ordering is *derived* (composition under sum aggregation) rather than chosen by an oracle.
