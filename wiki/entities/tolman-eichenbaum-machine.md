...
| **Evidence accumulation** | On a cue-counting T-maze, a 2-D map spanned by physical position × accumulated evidence, as recorded. The recording is Nieh et al. 2021, and it matches TEM at both levels the model commits to: cells are ~90% **conjunctive** for the two variables (the `g̃ ⊙ x̃` prediction) with 1.7 fields each and 53% multi-field, while the two variables separate as independent gradients on a ~5-dimensional population manifold ([[wiki/concepts/population-geometry.md]]) |
...
| Limit | Consequence |
|---|---|
| **Slow to learn a novel map** | Transfer is bought by gradient training over many environments; a genuinely new structure cannot be acquired in one traversal ([[wiki/entities/cscg.md]] is the mirror image — one-traversal maps, with transfer available only when a modeller freezes the transition matrix by hand) |
...
| **Fixed configuration of factors** | The learned latent space for a T-maze task would not survive the T-maze becoming a W-maze: space and task were learned in one fixed combination, not factorised (gap G40). The biology gives the limit a measurable form the model has not been held to: intrinsic dimensionality of the CA1 manifold *drops* on a simpler version of the same maze, so how many factors the circuit tracks follows the task rather than the architecture ([[wiki/concepts/population-geometry.md]]) |
...
- **[[wiki/entities/cscg.md]]** — the complementary architecture, and the proposed merge (a TEM whose hippocampus also predicts future hippocampal states) that would give fast novel-map construction and transfer at once. The complementarity is narrower than it looks: the clone model *can* reuse a transition matrix across worlds of the same shape, so what TEM uniquely supplies is the automatic part — deciding that a stored structure applies — not the reuse itself (George et al. 2021).
...
- **[[wiki/entities/fly-central-complex.md]]** — the same integrate-then-correct logic wired rather than learned, and a discipline this model does not implement: the fly resets its structural code to the landmark by preserving an offset, where TEM corrects drift by memory recall of the sensorium — a blend rather than a reset ([[wiki/empirical-tensions.md]] T46).
- **[[wiki/concepts/population-geometry.md]]** — the recording behind this page's evidence-accumulation row, and the only place its two representational commitments are checked at once in the same neurons: conjunctive cells (~90%) *and* factorised population geometry (two independent gradients on a ~5-dimensional manifold), which is the pattern the `g̃ ⊙ x̃` outer product predicts and which no cell-level comparison alone would have distinguished from a splitter-cell account.
- **[[wiki/entities/thousand-brains-theory.md]]** — the unbuilt many-frame version of this model's factorisation: thousands of concurrent object-anchored frames voting, against one structural code rebound per environment — and the contrast that prices it, since this model states its binding as an equation and reports results while the theory states a wiring diagram.
- **[[wiki/concepts/schema-assimilation.md]]** — this model delivers the capability that *defines* a schema (first-presentation transitive inference from a learned structural code) while having neither half of the mechanism biology uses to build one: nothing detects that a new observation conflicts with stored structure, and nothing rewrites the structure when it does.
- **[[wiki/entities/spacetime-attractor.md]]** — the same structural-code-plus-binding idea moved from *where am I* to *where will I be*: one state at a time in a sequence versus one population per future timestep, which is what buys planning under reward that changes within the horizon (Jensen et al. 2026).
- **[[wiki/entities/gcq.md]]** — the installed counterpart of this page's learned structural code: the same path-integrated `g` and the same observation-prediction objective, but the code is the fixed attractor set of a CANN rather than something trained, and drift correction by memory retrieval is replaced by a single batch fit at initialisation with no correction thereafter.
- **[[wiki/concepts/vector-symbolic-binding.md]]** — the compressed version of this model's binding step: circular convolution sums the `g ⊗ x` outer product along wrapped diagonals back down to `n` dimensions, so nesting costs nothing and the price is decode noise requiring a cleanup memory.
- **[[wiki/concepts/analogical-mapping.md]]** — the same structure/content factorisation with the arrow reversed and no learning: rather than binding a content code into a learned structural address, an element's content code is superposed with the *labels* of the roles it fills in the current episode, which is enough to make relational correspondence readable by a dot product.
- **[[wiki/concepts/fast-weight-programming.md]]** — the general form of this model's Hebbian memory, with the two things it lacks: a derived capacity (`d_dot` orthogonal keys, with the measured failure point matching) and an error-correcting write that re-binds one `g`–`x` pair without touching the others — which is what its hand-designed memory-management heuristics are standing in for (Schlag et al. 2021).
- **[[wiki/concepts/hexadirectional-signal.md]]** — the instrument this model's `g` has never been scored with: TEM's grid-like units are certified per unit, while the human evidence it is built to explain is an aggregate, split-half, speed-gated population statistic — an apples-to-apples comparison nobody has run.
- **[[wiki/concepts/tensor-product-representation.md]]** — the formal name and the theory for this model's binding layer: `g ⊗ x` is a tensor-product binding with `g` the role and `x` the filler, so its capacity is the `~√N` graceful-saturation law, its Hebbian memory is the activity/weight duality (binder activities *are* the weight matrix), and its retrieval is the self-addressing unbinding procedure — which is exact only if the `g` codes are orthogonal, where the exact procedure needs a separate dual-basis probe and merely linear independence.
- **[[wiki/entities/esbn.md]]** — the same two streams with the opposite join: TEM binds `g ⊗ x` into a conjunctive hippocampal code and learns `g` from transition statistics, where the ESBN keeps `g` (keys) and `x` (values) in adjacent columns that never combine and gets `g` from the controller's task loss.
- **[[wiki/concepts/retrieval-mediated-learning.md]]** — the query-time pole of `T334`: TEM answers first-presentation transitive inference from a structural code with no write-time integration of the queried pair, so it and the reinstatement result bracket the two places an unobserved path can be paid for.
- **[[wiki/concepts/relational-bottleneck.md]]** — the brain-side candidate this model is nominated for: binding abstract to perceptual pathways by rapid hippocampal conjunction of medial-entorhinal structure with lateral-entorhinal content is the mechanism Webb et al. 2024 propose for the framework's missing binder, which would make an ESBN-style external memory a hippocampus — contested in the same source by developmental-amnesia evidence that abstract reasoning survives hippocampal damage (Dzieciol et al. 2017).
- **[[wiki/entities/entorhinal-cortex.md]]** — the anatomy this model's interface contradicts on one structural point: its `g`/`x` loop is a pair of one-way wires into and out of a store, where the tissue returns the store's output to a deep entorhinal sublamina that also holds a **copy of the write** and projects that back up into the input layers — a comparator the model has no place for (Witter et al. 2017).
- **[[wiki/concepts/activity-slots.md]]** — derives this model's algebraic dual: the same computation with the abstract position code removed and the memory moved out of synapses into activity subspaces, exactly as behaviourally capable on novel state–observation pairings. That makes "cortex holds the structure, hippocampus holds the binding" a claim about a *carrier* rather than about a computation, and it is not decidable from behaviour ([[wiki/empirical-tensions.md]] T28, `G112`; Whittington et al. 2025).
- **[[wiki/entities/state-space-composition.md]]** — the same conjunctive architecture with the other operand swapped, and the swap is the whole point: conjoining the structural code with *sensory* codes zero-shot predicts what you will see, conjoining it with *structural* vector codes (object-, border-, reward-vector) zero-shot predicts what you will **do** — which requires global relational structure to be present locally at every state, and therefore requires an offline construction process this model does not need (Bakermans et al. 2025).
- **[[wiki/entities/visual-predictive-coder.md]]** — the ablation nobody ran on this model: keep next-observation prediction, remove the path-integrating structural module, remove the action input and the relational store, and a coarse distance-preserving map still appears in the latent (mutual information 0.627 bits vs 0.227 for a same-frame control) — though no grid-like unit is reported, and none was looked for.
- **[[wiki/concepts/hippocampal-indexing-theory.md]]** — the theory this model implements, now with its primary source: the model's `g̃ ⊙ x̃` conjunction is step 2 of the four-step claim, and the theory adds two things the model assumes away — a learned CA1→entorhinal return address, and the retraction that the index addresses association cortex rather than all of neocortex.
- **[[wiki/entities/remerge.md]]** — the same transitive-inference test paid for at the opposite end: this page pays at pretraining with a learned generative structural code, that one pays at retrieval by settling a loop over raw traces, and together they bracket the cost space for the behaviour.
- **[[wiki/entities/bb-model.md]]** — the same `g`/`x` factorisation reached from the opposite end, and the pair brackets how much of a spatial system can be derived versus installed: this page's code is *learned* end-to-end from next-observation prediction and never converts between frames, while the BB-model *wires* its structural code in a supervised setup phase and spends its entire architecture on the egocentric↔allocentric conversion this page has no need for.
- **[[wiki/concepts/key-value-memory.md]]** — this model read as a key-value memory (via Whittington et al.): the medial-entorhinal structural code is a **learned position encoding** — an address space adapted to the environment's structure — and its structure-sensitive cell types do not emerge under the fixed positional encodings a standard transformer uses.
