...
- **[[wiki/entities/alphazero.md]]** — the same planner shape with the model's status inverted (exact vs learned), and the warning that follows: this page's cross-entropy-method search selects by `argmin` over sampled action sequences, i.e. the backup operator AlphaZero's Methods argue propagates a learned evaluator's largest error to the root.

- **[[wiki/entities/continual-dreamer.md]]** — the same world-model-for-control question with a continual axis added, and it moves the advantage elsewhere: training the policy *inside* the model buys 10× sample efficiency and positive forward transfer across tasks, neither of which is a property of the prediction space.
...
- **[[wiki/entities/cpc.md]]** — the 2018 ancestor with the same temporal pairing and the same latent target, run with negatives and an 8-GPU batch; the horizon there is an ablated scalar (optimum 12 steps) where here it is fixed by the tubelet mask.
- **[[wiki/entities/world-models-vmc.md]]** — the opposite settlement of every choice here: a decoder, a stochastic MDN transition with a tunable temperature, and a controller compiled into 867 parameters rather than a planner run at decision time — plus the demonstration of the planner exploitation this page's deterministic predictor has no defence against.
- **[[wiki/concepts/hindsight-goal-relabelling.md]]** — the same manipulation regime with the goal machinery inverted: this system plans to hand-chosen goal *images* on a fixed timetable with no achievement detector, where HER requires an explicit predicate `f_g` and a state→goal projection `m` and then generates its whole goal curriculum from its own failures — so the missing achievement detector is exactly what buys the curriculum (`G33`).
