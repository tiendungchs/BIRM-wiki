...
- **[[wiki/concepts/attractor-dynamics.md]]** — the non-recurrent relative of relaxation, and the source of the per-read confidence signal that settling itself cannot produce.
- **[[wiki/entities/btsp-cam.md]]** — the same feedforward address-then-store design with the address *learned* instead of frozen, which is precisely the difference that buys cue robustness: a data-independent random projection leaves many stored items sitting just above the read threshold, while one-shot gated learning makes each unit's weighted-sum distribution bimodal and puts the threshold in the gap.
- **[[wiki/entities/differentiable-neural-computer.md]]** — the same random-access-memory ambition with addressing made learned and dynamic rather than random and frozen: that buys pointers, reuse of freed locations and write-order recall, and loses this page's closed-form capacity, since nothing about a learned address distribution is countable.
- **[[wiki/entities/hami.md]]** — this design taken to its discrete limit: address-then-store with the Hamming ball and the distributed vote deleted, leaving exact match on a short symbol key, so the read becomes one content-addressable-memory clock cycle — and it depends on the same precondition stated here, an encoder that maps semantic similarity onto the address metric (Poursiami et al. 2025).
- **[[wiki/entities/cerebellum.md]]** — the same circuit read as a *controller* rather than as a store, with a precisely opposed claim about the climbing fibre: here it is an error-gated write enable (Albus' CMAC), there it is a **signed error already expressed in the output's own coordinates** — the training signal of a learned inverse model. A write enable need only say *whether* to write; feedback-error learning needs direction and magnitude, and the ocular-following preferred-direction data are the measurement that separates the two readings.
- **[[wiki/entities/dense-associative-memory.md]]** — the other way to buy capacity without changing the write rule: this page enlarges the address space and schedules sparsity by load, that one leaves the code untouched and steepens the energy exponent to `K^max = α_n N^{n−1}` — and both end at the same unbuilt object, a load-dependent schedule for their single scalar knob.
- **[[wiki/entities/continuous-modern-hopfield-network.md]]** — the two available fullness signals, at different granularities: this page's `|s_u|` is a per-*read* vote margin requiring a query, while the separation `Δ_i` there is a per-*store* number computable from the keys alone, with every retrieval bound exponential in it. Neither system gates on its own number, and `Δ_i` costs `O(N²)` to maintain exactly — which is the awkward part, since the point of an exponential store is a large `N` (Ramsauer et al. 2020).
- **[[wiki/entities/three-factor-key-value-memory.md]]** — this design with the frozen half unfrozen: the address matrix is written by a gated presynaptic-only plasticity rule instead of being drawn at random, which replaces the derived-but-static sparsity `p = (2MT)^(−1/3)` with an allocation *gate* and makes erase free (the write's `(1 − η)` factor clears the slot it fills) — and it independently reproduces Keeler's capacity-per-storage-element equality against Hopfield, at `C ≈ 0.16N` for twice the connections (Tyulmankov et al. 2021).
- **[[wiki/concepts/key-value-memory.md]]** — this design named as the **fixed random** address space in a four-way scaffold taxonomy: similar keys deliberately do not index similar values, which is what buys RAM-like addressing and what forfeits any structure a query could exploit — the opposite pole from [[wiki/entities/vector-hash.md]]'s structured scaffold.
