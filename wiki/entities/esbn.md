...
- **[[wiki/concepts/relational-bottleneck.md]]** — the principle this architecture is the wiki's first-hand instance of, and the reframe it forces: the controller's entity-blindness *is* the bottleneck, the memory read is one row of a relation matrix, and the external store is therefore incidental — CoRelNet computes the whole matrix feed-forward and gets the same abstraction with no memory at all (Webb et al. 2024).
- **[[wiki/concepts/activity-slots.md]]** — this page's binding with the algebra put back: the role is a subspace of one activity vector rather than a row index in an external store, so the bound state can be superposed and compared where a key/value row cannot — at the cost of `O(|states|²)` units. The shared limitation is exact: both label the role by *position*, so neither reaches a role that is a type (Whittington et al. 2025).
