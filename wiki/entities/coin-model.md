...
- **[[wiki/entities/cerebellum.md]]** — this model's direct predecessor by the same first author: MOSAIC (Wolpert & Kawato 1998) already computes responsibility as a cue-driven prior times a prediction-error likelihood and uses it to weight expression, learning and the output mixture — but over a **fixed** module set with no allocation rule, and with each module holding a forward model *and a controller* rather than a scalar state. The page carries the term-by-term comparison of what each buys.
- **[[wiki/concepts/structural-learning.md]]** — the competing decomposition of the same paradigm from the same laboratory: this model buys reduced ±60° interference by allocating two non-competing contexts, structural learning buys it from a short path between two settings of one metaparameter on a learned subspace — so the discriminating test is extrapolation to a parameter outside the trained range, which neither has run (T350).
