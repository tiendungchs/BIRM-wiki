...
**The primitive is measured; its load-bearing property is not.** Recordings confirm the map, the generalisation across objects and environments, and even the endpoint decomposition the encoding below hand-codes — an elongated object's fields anchor preferentially to its *ends* (Høydal et al. 2019, [[wiki/concepts/vector-coding.md]]). What they do not confirm is path-integrability: the same cells degrade in darkness while grid cells in the same region do not, which is what an online measurement from a visible referent would also look like. Logged as `T348`, and it is the assumption everything from §2's latent learning to §3's constructive replay rests on.
...
- **[[wiki/concepts/vector-coding.md]]** — supplies the measurement behind this model's building block, including the endpoint decomposition of extended objects it assumes, and the one result that threatens it: the block may be re-derived from the visible referent rather than carried by self-motion, in which case there is nothing to construct offline with (`T348`).
