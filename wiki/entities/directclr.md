...
- **[[wiki/concepts/label-free-model-selection.md]]** — the opposing measurement in the same space: this page finds the trained embedding rank-deficient and reads the projector's discarding capacity as what buys performance, while a 110-model sweep finds higher embedding rank monotonically better *within one method* — compatible only under that restriction, and the crossed experiment is unrun (T314).
