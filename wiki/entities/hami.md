...
- **[[wiki/entities/model-free-episodic-control.md]]** — the other store in the wiki that keeps the *maximum* return rather than an expectation, reached from the opposite direction: MFEC states the near-determinism that rule assumes and this page leaves it implicit, and MFEC supplies the revisit statistic (`T353`) that would say how much of any exact-match store's advantage is lookup (`T352`).
