...
| Mole et al. 2025 | **247** unilateral focal patients (frontal 102, posterior 74) + 81 matched controls; graph lesion-deficit mapping by layered Bayesian stochastic block modelling | Analogy impairment mapped to a **right** frontal network — right middle and inferior frontal gyri, hierarchy dominated by **caudal** middle frontal gyrus. Right frontal patients worse than left (63.19% vs 73.19%, `P < 0.05`); posterior patients unimpaired. Primary read at [[wiki/entities/lateral-frontoparietal-network.md]] |

**The row is now `T336`, and the reconciliation is aimed at the smaller half of the discrepancy.** Chau et al.'s proposal — and they have an interest in it — is that Mole's task required *identifying* an analogy rather than *applying* one to a new problem, with only the latter needing the high-dimensional decomposition they attribute to FPl. That addresses the rostro-caudal difference. It says nothing about the **hemisphere**, which is the larger disagreement: Urbanski found no right-hemisphere tract disconnection predicting any deficit, and found the analogy-*preserved* patients' lesions overlapping in right prefrontal cortex, i.e. the opposite sign in the same tissue Mole implicates. Three of the exculpations one would try next are closed by Mole's own design — the Analogical Reasoning Test is explicitly modelled on Urbanski's task, the right frontal network is recovered on an *entirely verbal* deductive task as well, and right frontal patients are worse than left on easy and hard trials alike. What is not closed is **method** (mass-univariate VLSM at `n = 27` versus stochastic block modelling at `n = 247`, never run through one pipeline) and **score definition** (Urbanski's localiser is the analogy − perceptual-match difference index, `I39`; Mole reports raw accuracy). Mole's cohort has the matched control already collected, so the second is answerable without new patients.

**Reading the Urbanski primary weakens the reconciliation from the other side too.** That study crossed its analogy task with exactly one supplied-versus-derived manipulation — *AnalogyApply* states the schema in words, *AnalogyFind* requires extracting it — and the frontopolar-lesion patients were impaired **equally** on the two (and equally on cross- versus intra-dimensional trials). The deficit therefore survives handing the subject the decomposition, which is the opposite of what a demand-for-decomposition account predicts, and is why the authors read the region as doing *relational matching/integration* rather than inference or schema induction. Find/Apply is not identify/apply, and the null rests on `n = 5` patients in the cluster, so this bounds the reconciliation rather than refuting it. Two further facts from the same study sit on Mole's side of the discrepancy: analogy-preserved patients' lesions overlapped in **right** prefrontal cortex, and no right-hemisphere tract disconnection predicted any deficit.
...
| **The one large lesion series in this literature puts analogy in the other hemisphere** | `T336`. Every FPl result on this page is left-lateralised or laterality-agnostic; Mole et al. 2025's 247 patients are not, and their same-cohort Stroop and fluency networks come back left, which removes the coverage explanation. Whichever way it resolves, the reconciliation this page carries was written before the primary was read here and does not survive contact with the laterality half |
...
- **[[wiki/concepts/analogical-mapping.md]]** — the causal lesion evidence for that page's frontopolar integrator, and the challenge to it: 27 patients put analogy in a 0.33 cc left frontopolar site, 247 patients put it in a right frontal network centred on caudal middle frontal gyrus, and the task difference proposed to reconcile them (identify vs apply) is untested and addresses only the rostro-caudal half (`T336`).
...
- **[[wiki/concepts/relational-bottleneck.md]]** — the ambiguity this region sits inside once the framework is taken seriously: prefrontal lesions severely impair relational reasoning, but a relational bottleneck has two prefrontal-shaped jobs — *representing* the abstract pathway's content-free values, and *binding* those values to perceptual embeddings by attention or gating — and no lesion result in the wiki separates them (Webb et al. 2024).
