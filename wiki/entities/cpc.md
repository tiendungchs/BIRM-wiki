...
- **[[wiki/entities/visual-predictive-coder.md]]** — the same next-observation premise with the residual actually computed in pixel space rather than replaced by a contrastive discrimination, and with the map it produces measured; both are cited as "predictive coding" and `T349` questions whether either owes its representation to the forward target at all.
- **[[wiki/entities/saycam-baby-vision.md]]** — the other temporal-pairing system in the wiki, using time for *equivalence within a window* where this page uses it for *prediction across a gap*; its segment length (~288 s optimum) is the same kind of object as this page's horizon — one scalar that replaces the augmentation list and has an interior optimum (T167).
