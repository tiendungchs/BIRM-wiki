...
- **The gain cannot be the whole slow code.** A multiplicative burst gain is non-negative and scales the channel's impact; it cannot make a reward-evoked *decrease* into an increase. The offset measured by Shabel et al. 2019 (below) does exactly that, on the same structure, under the same stressor — so the slow write-port has at least two independent knobs.
...
|---|---|---|
| **Negative reward prediction error** | LHb cells are excited by reward omission and punishment and inhibited by unexpected reward — the mirror image of dopamine cells, in monkey; confirmed in human functional imaging for both omitted positive and unexpected negative feedback. LHb also encodes reward **probability** and **magnitude**. Primary record below: Matsumoto & Hikosaka 2007, where the inversion is causal (single-pulse habenular stimulation inhibits 82% of reward-responsive dopamine cells) but **directional** — habenular excitation leads the dopamine dip, the dopamine burst leads the habenular inhibition | the sign-inverse of the dopamine value signal, upstream of it |
...
## The primate record the inverse-of-dopamine claim rests on

> Matsumoto & Hikosaka 2007, *Lateral habenula as a source of negative reward signals in dopamine neurons*, Nature 447(7148):1111–1115 (`raw/matsumoto-2007-lateral-habenula-negative-reward-signals.md`). Primary data: 49 task-related lateral-habenula neurons and 62 dopamine neurons (substantia nigra pars compacta) in two rhesus monkeys, plus electrical stimulation of the habenula during dopamine recording. This is the source the reviews above credit; the numbers and — more importantly — the **asymmetries** are only in the primary paper.

**Task.** Reward-biased visual saccade: a target appears left or right, the monkey must saccade to it within 500 ms either way, and one fixed direction is rewarded (0.3 ml juice) for a block of 24 trials before the contingency reverses with no external instruction. The target therefore instructs *which action* and *what outcome* independently, and an incorrect saccade repeats the trial even when unrewarded — **the required action is held constant across reward contingency**.

| Measurement | Lateral habenula | Dopamine neurons |
|---|---|---|
| Post-target response (150–350 ms) | **excited** by the no-reward-predicting target, **inhibited** by the reward-predicting one; 43/49 neurons show a significant main effect of reward contingency, only 10/49 of target position, reward index > position index at `P < 10⁻⁸` | exact mirror, same cells' task |
| After contingency reversal | flips within a few trials, then stable; the neuronal change **leads** the change in saccade latency | flips in the opposite direction on the same trials |
| Reward on–off response (250–700 ms after tone) | excited by **omission**, inhibited by delivery; largest on the first trials after reversal, then declines | mirror image, also declining |
| Graded, not binary | with a smaller reward bias (both targets rewarded, different amounts) both populations still discriminate, but **more weakly** | same |

**Two findings that are not in any review, and both constrain the architecture.**

| Finding | Numbers | What it fixes |
|---|---|---|
| **The causal arrow is one-directional.** On unrewarded trials habenular excitation *precedes* dopaminergic inhibition; on rewarded trials dopaminergic excitation *precedes* habenular inhibition | latency analysis by bootstrap, response onsets compared | the habenula can be the source of the dopamine **dip** and cannot be the source of the **burst**. The wiki's shorthand "sign-inverse of the value neurons, at shorter latency" ([[wiki/concepts/broadcast-channel-decomposition.md]]) is true only of the negative half; the positive half's source is unidentified |
| **Habenular drive is sufficient, anatomically specific, and graded.** A *single* biphasic pulse in the habenula inhibits dopamine neurons at 10–40 ms in 18/22 cells (82%); ipsilateral 9/10, contralateral 9/12; still 6/8 at 40 µA, 6/12 at 20 µA. Stimulation of mediodorsal thalamus **1 mm away** affects 0/8. Cells inhibited more strongly by stimulation are the cells inhibited more strongly by the no-reward target (`r = 0.56`, `P < 0.01`); there is **no** such correlation with the excitation to the reward-predicting target | | a per-cell gain on one wire — the same dopamine neuron's dip depth and its habenular sensitivity are the same parameter. The absent correlation on the excitation side is the same asymmetry read from the causal experiment rather than from latencies |

**The channel is not a pure prediction error.** After the prediction is established, the habenular reward on–off response *declines* — the error signature — but a significant residual difference between unrewarded and rewarded trials survives (`P = 0.016`), which the dopamine neurons do not show. So this channel carries a rectified outcome/state term **added to** its error term, in a purely appetitive task with no aversive event anywhere in it. This is `T361`'s Position A holding on the dissociation and failing on purity: the structure is a disappointment channel with a persistent outcome residue, not `[−δ]⁺`.

**Selection bias, and it is the wiki's problem twice.** 49 of 74 isolated habenular neurons were kept — the criterion was an on-line task-related response — and the 62 dopamine neurons were selected for *phasic excitation to reward-predicting stimuli*, with "dopamine-like neurons that were not sensitive to reward-predicting stimuli" explicitly not examined. The second exclusion removes exactly the population `T355` and `G117` are about. The record that founds the value channel was built by discarding the candidate second channel.

**What it does not address.** No aversive outcome is delivered anywhere in the study — "no reward" is the whole negative axis — so it cannot separate disappointment from pain (`T361`) at all, and the opponent-process reading the authors offer is their own inference, flagged as untested outside appetitive procedures.

---

## The rodent record in which the channel's sign is a state variable

> Shabel, Wang, Monk, Aronson & Malinow 2019, *Stress transforms lateral habenula reward responses into punishment signals*, PNAS 116(25):12488–12493 (`raw/shabel-2019-stress-transforms-habenula-reward-to-punishment.md`). Primary data: two-photon calcium imaging (GCaMP6s normalized by co-expressed tdTomato, through an implanted gradient-index lens) of individually identified lateral-habenula neurons in 4 head-fixed mice, tracked across ~40 daily sessions — the same cells before and during a stress protocol. 42 neurons before stress, 34 during.

**Task.** 10-s auditory CS⁺ → one drop of 10% sucrose 0.5 s after CS offset; interleaved sucrose-*omission* trials and a CS⁻ control. Stress = intermittent 0.3-s tail shock delivered after the **CS⁻**, to keep contextual fear off the reward cue. Licking is the behavioural read-out, with bout duration taken as the hedonic measure and inter-lick interval as the motor control.

| Before stress | Count | Post-CS response profile |
|---|---|---|
| **Reward-selective** (sucrose ≠ omission) | 18/42 (43%) | **decrease** to sucrose, **increase** to omission — prediction-error polarity, matching the monkey record above |
| Nonselective (respond, no discrimination) | 19/42 (45%) | responsive relative to pre-CS baseline, same on both trial types |
| Nonresponsive | 5/42 (12%) | — |

**What stress does, measured on the same cells.**

| Quantity | Before | During stress |
|---|---|---|
| Licking during sucrose | 48 ± 10% of the time | **28 ± 10%** (`P < 0.05`, n = 4 mice) |
| Licking **bout** duration (hedonic measure) | 3.4 ± 0.8 s | **1.7 ± 0.8 s** (`P < 0.02`) |
| Inter-lick interval within a burst (motor control) | 12.7 ± 0.8 ms | 12.2 ± 1.2 ms (n.s.) — **not a motor deficit** |
| Reward-selective response to **sucrose** | negative (decrease) | **sign-inverted** — an increase, i.e. punishment-like |
| Reward-selective response to **omission** | positive | **larger** positive |
| Nonselective cells, either trial type | — | **unchanged** across sessions |
| Trial-by-trial correlation, reward-selective response × licking | — | mean Pearson `R = −0.24 ± 0.04`, significant in 11/14 cells; nonselective `R = −0.01`, 1/11 |

Onset of the neural change and of the anhedonic behaviour coincide (within the first stressed session) and both persist for sessions 0–5. Shock trials themselves excite the habenula, as expected.

**The transform is an additive offset, and this is the wiki's first neural measurement that separates the two operators of `T362`.** A multiplicative gain `κ > 0` on the channel — the burst/Kir4.1 knob this page documents above — scales both responses and **cannot invert one of them**; `κ < 0` would invert *both*, making the omission response reward-like. What is observed is reward inverted *and* omission enlarged, both moved in the same direction by roughly one constant:

```
a_LHb'(outcome)  =  a_LHb(outcome) + c ,   c > 0        (offset, written by stress)
       reward:   negative  →  positive          (sign crosses zero)
    omission:    positive  →  more positive     (rank order preserved)
```

That is the **movable origin** of [[wiki/concepts/affective-opponency.md]] — shifted the wrong way, on a slow timescale, by a stressor rather than by a threat prediction, and read out on an identified population rather than inferred from behaviour. `T362` asked for a state manipulation over a graded family; this is the two-point version of it on a channel instead of on a choice.

**It also breaks the prediction-error reading of the channel, by the authors' own argument (`T369`).** Under stress the reward's value to the animal *falls* (licking, bout duration). A signal computing `actual − expected` must then produce a **smaller** omission response, because less is being missed. The measured omission response gets larger. The authors state the consequence directly: the result "challenges the view that LHb signals simply reflect the difference between actual and expected value". Their own alternative is two separate modifications — reduced reward value plus a pathologically amplified omission term — which is the position the offset reading has to beat.

**What a builder takes.** Every trial type now returns a punishment-like signal, so the channel's output stops depending on the outcome at all: an agent in this state gets the same bad news whatever happens, which is exactly a policy-gradient signal with its informative variance removed. The failure is not a wrong value estimate — nothing has been relearned — it is a **write to the channel's origin by a controller outside the learning loop**, and its behavioural signature is an agent that keeps sensing and keeps acting and stops pursuing. Note this offset is separable from the burst gain of the section above: one crosses zero and one cannot, they have different measured writers (stress acts on both), and no architecture here has either.
...
| stress-written offset on the outcome response (Shabel et al. 2019) | `a'(outcome) = a(outcome) + c`, `c` set by a slow state controller — a movable origin on the *channel*, not on the value function | the wiki's origin shifts ([[wiki/concepts/affective-opponency.md]]) are inferred from behaviour and triggered by a threat *prediction*; this one is measured per cell and written by an unrelated stressor, and it can make every trial type return bad news |

---

## Limitations

- **Review, not primary data** for everything outside the section above, and the strongest mechanistic claims (bursting, Kir4.1, ketamine site of action) come from the authors' own laboratory.
- **The primate section's two populations are both selected samples** — habenular cells kept only if a task-related response was seen on-line (49 of 74), dopamine cells kept only if phasically excited by reward-predicting stimuli — and electrical stimulation drives fibres of passage as well as cell bodies, so the 82% inhibition bounds the effect of *the site*, not of habenular somata.
...
- **The stress record is four mice and 34–42 neurons**, unilateral, head-fixed, in one appetitive task with the aversive event confined to the CS⁻ trials; the offset reading is the wiki's arithmetic over two outcome types, not a fitted model, and the authors' own two-modification alternative (`T369`) is not excluded by their data. Calcium imaging is a slow, non-linear proxy for firing, so a sign inversion of a `ΔG/T` response is a sign inversion of a calcium transient.
...
- **[[wiki/concepts/broadcast-channel-decomposition.md]]** — that page's inferred `GPb → LHb → RMTg → DA` value-channel source, given its own afferent map, the primary latency and stimulation data behind its "sign-inverse at shorter latency" shorthand (which holds only in the negative direction), and a correction: the habenula is not a single sign-inverting node but a convergence funnel whose inputs carry reward omission, pain, thirst, sound, touch, light and social defeat, and whose valence within a pathway is separated by transmitter rather than by target.
- **[[wiki/concepts/affective-opponency.md]]** — supplies three of that page's missing mechanisms from one structure — and one of them is that page's central operator, measured: under stress the same imaged neurons' reward response inverts in sign and their omission response grows, which is an **additive offset on the channel** and not a gain, so the movable origin is a physical write to an identified population rather than an inference from behaviour (Shabel et al. 2019, `T362`, `T369`). The other two: a sign carried as the GABA/glutamate *ratio* at a single co-releasing terminal, which is how a signed error survives non-negative firing rates without an opponent pair; and a slow, externally-written burst gain that is a literal context baseline — raising it alone produces anhedonia, which is the movable origin failing in the direction that page predicts.
- **[[wiki/concepts/reward-prediction-error.md]]** — the upstream inverse of that page's `δ`, and the source of the wiki's sharpest evidence that the quantity is not an error at all under a state change: when stress lowers the reward's measured value, `actual − expected` must shrink on omission trials and the recorded omission response instead grows (`T369`). Also a dissociation that page does not carry: and a dissociation it does not carry: the habenular route is necessary for the dopamine dip on *reward omission* and not for the dip on *aversive events*, so the negative half of `δ` is not one signal with one source (`T361`). The primary monkey record adds the other half of the same point: the habenular signal leads the dip and *lags* the burst, so `δ`'s positive half has no identified upstream source at all.
...
- **[[wiki/concepts/incentive-salience.md]]** — the anti-reward channel moving in lock-step with that page's 'liking' measure: acute stress shortens licking **bout duration** (that page's hedonic read-out, not a choice) at the same session the reward response here inverts, with the inversion correlated trial-by-trial with the licking drop (`R = −0.24`, 11/14 cells) — so a hedonic loss has an identified upstream signal, where that page's dissociations are defined by dopamine manipulations that leave 'liking' alone. Also the proposed joint address for that page's two dissociable quantities: dopamine carries value *change* ('wanting') and serotonin a tonic value *state* ('liking'/mood), and this is one of the few structures that writes to both — without computing either.
...
- **[[wiki/concepts/pseudo-reward-prediction-error.md]]** — extends this structure's remit past outcomes the environment pays for: habenular activation follows a jump that increases distance to a *subgoal* carrying no primary reward at all (`n` = 30, region of interest, `p` < 0.05), so the anti-reward hub reports errors against an internally supplied target — which puts a second, level-indexed variable on a channel this page otherwise describes as one funnel collapsing many sources onto one sign (`T367`, Ribas-Fernandes et al. 2011).
