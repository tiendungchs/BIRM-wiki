...
**A second measured row, and it disagrees about where the map is kept.** [[wiki/entities/m2-auditory-cortex-filter.md]] is the same computation in audition — secondary motor cortex delivering a movement-conditioned prediction into a primary sensory area — and it supplies the two things missing here: a **de novo** acquisition schedule (5–9 days of an arbitrary running→tone contingency; 1 h insufficient) rather than a relearning schedule, and a behavioural price for the cancellation (detection of *non*-reafferent tones improves during movement while the reafferent tone stays impaired). Where it parts company is the disambiguation this page's table above was designed around. There, M2 was stimulated **electrically in bulk** — which drives all M2 output equally and therefore bypasses any sender-side selectivity — and reafferent-frequency-tuned interneurons were *still* recruited preferentially, in experienced but not naive animals. That is anatomy **A** ("transform at the synapse"), the option the axonal imaging here rejects for V1. Recorded as [[wiki/empirical-tensions.md]] `T346`; the honest reading is that neither result excludes the other's mechanism in its own preparation, and that the two cases also differ in *what* is predicted — a graded flow magnitude here, an address on a labelled line there.
...
- **[[wiki/entities/m2-auditory-cortex-filter.md]]** — the same computation in a second modality, and the wiki's only disagreement about where a cortical forward model is kept: bulk electrical stimulation of the sender there still selectively recruits reafferent-tuned interneurons, placing the map in the sender→receiver synaptic matrix, while the axonal bias measured here places it in the sender (`T346`). It also supplies the two schedules this page lacks — *de novo* acquisition over 5–9 days for an arbitrary contingency, and a behavioural price: cancelling the predictable frequency improves detection of every other one during movement.
