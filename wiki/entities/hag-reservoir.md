...
**The premise now has a formal counterweight.** Li, Fong & Tiňo (2024) prove that a *linear* reservoir wired as a single ring with one shared weight and `±1` input signs approximates any time-invariant fading-memory filter to arbitrary precision ([[wiki/entities/simple-cycle-reservoir.md]]) — so within that class, recurrent connectivity carries no expressive power and whatever HAG's growth buys must be conditioning, effective dimension or sample efficiency instead ([[wiki/empirical-tensions.md]] T174). Two escapes are open and neither is tested: HAG's units are `tanh`-nonlinear while the theorems are linear, and variance-HAG's best reservoirs sit at `ρ(W) = 1.24–2.89`, i.e. **outside** the contractive class the theorem covers.
...
- **[[wiki/concepts/population-geometry.md]]** — a model-side measurement of the same quantity that page measures in brains, with the opposite sign of conclusion about expansion: raising the linear effective dimensionality of a code past a dataset-dependent threshold *degrades* class structure, so a high participation-ratio code is not a better code ([[wiki/empirical-tensions.md]] T75) — and that page now supplies the mechanism this reversal lacked: the non-linear terms generating the extra dimensions are also the ones most exposed to variability outside the task (Rigotti et al. 2013).
...
- **[[wiki/entities/simple-cycle-reservoir.md]]** — the direct antagonist on the same substrate: this page grows the recurrent graph because a random static one is "the antithesis of the optimal", that page proves a one-parameter ring loses nothing expressively over fading-memory filters, and the two are reconcilable only outside expressivity — via conditioning, dimension-at-fixed-`ε`, `tanh` nonlinearity, or the fact that variance-HAG's `ρ(W) = 2.89` leaves the contractive class entirely (`[[wiki/empirical-tensions.md]]` T174).
- **[[wiki/entities/ltc.md]]** — the orthogonal lever on the same substrate, untested together: HAG grows the recurrent *graph* under a fixed neuron model, LTC leaves the graph alone and makes each *unit's rate* a learned function of its input — and LTC's driving-force term `(A − x)` bounds the state for unbounded input with no constraint on `W` at all, which is the guarantee this page currently obtains by rescaling a `ρ(W) ≈ 2.89` reservoir back into range.
- **[[wiki/concepts/autonomous-pattern-generation.md]]** — the opposite verdict on the untrained `W` this page discards: there the random tangle *is* the asset, supplying a random temporal basis whose span the linear readout searches, so "the antithesis of the optimal" and "the thing that makes the span rich" are two readings of the same matrix — and this page's best reservoirs (`ρ(W) = 1.24–2.89`) sit in the self-sustaining regime that page calls chaotic and unreproducible.
- **[[wiki/entities/liquid-state-machine.md]]** — the ancestor of this page's thesis, with the graph swept rather than grown: at a fixed 135 neurons, a connection-probability kernel `C·exp(−(D/λ)²)` has an interior optimum — `λ = 0` fails for lack of recurrence, large `λ` homogenises the circuit into chaos — and the paper's own conclusion is that separation is better bought by **replicating columns** than by adding connections, because density amplifies noise and signal together. That is a fixed-`n`, within-class datum for T174 that this page's growth results do not supply, and an untested alternative to growing edges at all.
- **[[wiki/entities/sigma-pi-reservoir.md]]** — the rival diagnosis of the same complaint: this page accepts that the reservoir must supply the feature space and improves it by growing the recurrent graph, where Kleyko et al. 2025 conclude the recurrent circuit cannot supply higher-order features at all and add an orthogonal multiplicative stage while leaving connectivity maximally trivial — so the two disagree about *where* a random reservoir's expressive deficit lives, and only one of them is falsifiable by a dimension sweep (T238).
- **[[wiki/concepts/effective-connectivity.md]]** — the standing objection to growing a graph from co-fluctuation statistics: an edge set thresholded from a symmetric similarity measure can carry neither direction nor sign nor a self-weight at any resolution, whereas the same three quantities come for free once the edge is a parameter of an inverted forward model — at the price of committing to that model.
- **[[wiki/entities/cn-dpm.md]]** — growth as the learning mechanism at the opposite granularity and under the opposite criterion: one synapse per step from a local homeostatic band here, one entire expert per offline consolidation phase from a Bayesian-nonparametric novelty test there — and both leave the growth-rate parameter (homeostatic band, concentration `α`) to the designer.
