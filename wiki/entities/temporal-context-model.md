...
| **The composed item may not be in the medial temporal lobe at all** | The model derives transitive association from reinstatement *within* entorhinal-hippocampal circuitry. When the intermediary and the inferred outcome are dissociated experimentally, hippocampus during the inference cue carries the intermediary and shows no trace of the inferred outcome, which appears in medial prefrontal cortex and midbrain — so contextual overlap may deliver the link while the composition is completed elsewhere (Barron et al. 2020, [[wiki/empirical-tensions.md]] T48) |
...
- **[[wiki/entities/hidden-state-inference-remapping.md]]** — the discriminating experiment between a retrieved-context account and a generative-state account: recall here is strictly similarity-based, so it cannot express the confirmed prediction that a cue which has *historically varied* within a context is tolerated when it varies again. Sanders et al. otherwise treat the two as analogues, mapping retrieved temporal context onto hidden-state assignment.
...
- **[[wiki/entities/tolman-eichenbaum-machine.md]]** — the modern successor with the responsibilities re-assigned: both put a reusable code in entorhinal cortex and a binding operation in hippocampus, but TEM's code is path-integrated and content-blind while TCM's is a decaying trace of content itself, and TEM predicts forward while TCM embeds backward. The opposition is narrower than it looks: TEM's *sensory* input is exponentially filtered before binding — an approximate Laplace transform, matching the same LEC cells — so TCM's decaying content trace is present inside TEM as `x`, and the disagreement is only over whether entorhinal cortex *also* carries a content-blind path-integrated `g` alongside it (Whittington et al. 2020).
...
- **[[wiki/concepts/offline-replay.md]]** — and the recorded version of that propagation, which lands partly against this page: ripples do write a direct link between items never experienced together, but only when the chain ends in reward, and the inferred outcome itself is represented outside the medial temporal lobe ([[wiki/empirical-tensions.md]] T48).

- **[[wiki/concepts/memory-allocation-excitability.md]]** — the same contiguity phenomenon with the drifting context vector replaced by a decaying per-cell excitability tag: overlap of contexts becomes overlap of allocated populations, and `β` becomes the CREB decay constant, which would make the free-recall conditional response probability and the 5 h vs. 7 d fear-transfer result two measurements of one parameter **(brainstorm)**.
- **[[wiki/entities/differentiable-neural-computer.md]]** — the same contiguity operator in discrete, controllable form: a temporal link matrix gives `L w` forward and `L^T w` backward recall, but the forward/backward mixture is emitted per read per step rather than fixed by a drift rate, and what is linked is the *writer's trajectory* rather than a decaying trace of content.
- **[[wiki/concepts/retrieval-mediated-learning.md]]** — the opposing account of the same behaviour and the reason `T334` exists: this model derives inference across non-co-occurring items from retrieved-context overlap *at test* with no composite ever stored, where the classifier read-out of an absent element during encoding says a composite is built before the query.
- **[[wiki/concepts/hippocampal-indexing-theory.md]]** — the same commitment compressed into one scalar: the hippocampus stores nothing and acts as a reinstatement operator on entorhinal state, with `α_N` setting how strongly a repeated item pulls back the context that surrounded it.
- **[[wiki/entities/remerge.md]]** — the other retrieval-time account of inference in the wiki, differing only in what is recirculated: retrieved *context* here, retrieved *items* there, with the shared commitment that no composite is ever stored.
- **[[wiki/concepts/key-value-memory.md]]** — this model located as the **slowly drifting random** address space in a scaffold taxonomy: random keys that change slowly make temporally adjacent items similar in address space, which is the minimal structured scaffold and needs no learned key mapping at all.
