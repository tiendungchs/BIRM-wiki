...
- **[[wiki/entities/bib.md]]** — the benchmark page: it supplies the design reason this page's Preference floor is a floor rather than noise (the implausible test trial is authored to be the perceptually *familiar* one), and the composition rule behind the curriculum ablation here (each evaluation is a conjunction of two withheld training tasks).
- **[[wiki/concepts/curriculum-learning.md]]** — the non-monotone counter-case to the definition: adding a training task *removes* capability (80.4 → 51.4), so no ordering over task identities can satisfy the entropy-and-weight conditions, and the curriculum unit has to be the distribution of latent-variable variation a task induces.
- **[[wiki/concepts/hindsight-goal-relabelling.md]]** — the opposite sign on the same design question (`T366`): HER reports that training on many goals is *faster* than training on the single goal being evaluated, while this study's third task removes a capability the previous two conferred (80.4 → 51.4) — the candidate discriminator being whether the added objectives share the evaluation task's latent variables.
