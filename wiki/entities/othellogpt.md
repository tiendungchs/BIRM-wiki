...
- **[[wiki/entities/transformer.md]]** — a use of the founding operator its own paper never describes: attention is introduced purely as a weighted *read* over values, and `Emb[m] @ Att_h.V @ Att_h.O` anti-aligned with the `Empty` probe direction at cosine −0.862 shows first-layer heads **writing** a fact into every downstream position — so a head is a broadcast channel as much as a retrieval (Vaswani et al. 2017).
- **[[wiki/concepts/certification-instruments.md]]** — supplies failure mode `F4`: the same activations read out at 75% under human colour labels and 99.6% under mover-relative ones, so the labels a probe-based certification needs are two guesses deep, not one.
