...
| **Requires action labels** | Inverse dynamics named as the escape. [[wiki/entities/adaworld.md]] is the wave's answer, on the pixel branch |
...
- **[[wiki/concepts/simulation-based-planning.md]]** — the cheapest instance of MPC over a learned latent in the wiki (<1 s per plan), which converts the wave's planning-cost argument from "latent beats pixels" to "token count beats both".
