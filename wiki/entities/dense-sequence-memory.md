...
- **[[wiki/concepts/attractor-dynamics.md]]** — the sequential regime, and the read-out-nonlinearity knob that raises capacity without touching the code, the sparsity or the rule.
- **[[wiki/entities/trnn.md]]** — the trained counterpart of this page's asymmetric release term: there the dwell timer is a low-pass copy of each unit's own activity subtracted from its drive, so the sequence is generated by self-destabilisation rather than by a stored transition, and the resulting trajectory carries a working memory (Liu et al. 2025).
- **[[wiki/entities/dense-associative-memory.md]]** — the symmetric origin of this page's move, supplying the *item* capacity `K^max = α_n N^{n−1}` (and `N^{n−1}/(2(2n−3)!!·ln N)` for error-free recall) that this page's *sequence* capacity is measured against; its `(2n−3)!!` denominator is the closed form of the `d_max(N)` non-monotonicity reported here empirically, and its `f = F′` duality with a one-hidden-layer network is the bipartite two-body reformulation read backwards.
- **[[wiki/entities/two-body-dense-associative-memory.md]]** — the source of the bipartite two-body reformulation this page adapts: there the two matrices are a matrix and its transpose, which buys a Lyapunov energy for arbitrary time constants; dropping the symmetry to get transitions is exactly what costs this page the energy function, and the `N_mem ≤ N_h` ceiling stated there is this page's "one hidden unit per stored pattern" seen from the capacity side (Krotov & Hopfield 2021).
