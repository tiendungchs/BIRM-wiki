...
- **[[wiki/entities/vdm-i2i.md]]** — the same image-to-image reformulation of ARC with the prior sourced from a pretrained video corpus instead of installed as an invariance group, and it scores 16.75% against this page's 60.4% — so the two together read as *format matters, corpus is an expensive way to deliver it* (`T154`, `G95`).
- **[[wiki/concepts/perception-reasoning-dissociation.md]]** — the same attribution reached by deleting the reasoner rather than by easing the perceiver, and the magnitude disagreement is the finding: describing the grid in language to a frontier VLM buys Mini-ARC 8.05 → 20.13, while handing an 18M ViT the grid as an image with a metric and no reasoner at all reaches 60.4% on ARC-1 — so what perception supplies is a re-computable metric, not a description.
