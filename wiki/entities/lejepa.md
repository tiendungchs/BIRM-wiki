...
- **[[wiki/concepts/label-free-model-selection.md]]** — the register this page's rescaled-loss monitor belongs to, and its two competitors: a spectrum statistic needs no loss and therefore works on the EMA lineage this one cannot serve, while this one ranks by predicted probe accuracy rather than by surviving capacity (`G108`).
