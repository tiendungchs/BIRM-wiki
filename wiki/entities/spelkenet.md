...
- **[[wiki/concepts/visual-routines.md]]** — the opposite commitment on the same question: objectness computed once per input by a counterfactual probe, against a region computed only when a routine needs one and retained only in the incremental representation — scene-relative objects versus query-relative ones.
- **[[wiki/concepts/active-vision.md]]** — the same disagreement one step further: here no scene representation is built at all, so objects are not computed once per input *or* per query but exist only as the referents of the two or three pointers currently bound — and the block-copying data say humans decline to build more even when doing so would save a second per block (Ballard et al. 1997).
- **[[wiki/concepts/visual-indices.md]]** — maintained versus re-derived, on the same question: an objectness map is recomputed per input by a counterfactual probe, where an index is assigned once and *persists* through exactly the changes that would break a re-derived segmentation — an object tracked through feature space at a shared spatial locus, or through colour and shape changes made behind an occluder and never noticed (Pylyshyn 2001).
- **[[wiki/entities/multiscale-tracing-network.md]]** — the opposite extreme on `T151`'s question, and the one with a clock: objectness as a fixed point of a small disinhibitory recurrent circuit seeded at one point, returning a single connected component in a number of timesteps that predicts a human reaction time — against a whole partition read from `R × T` rollouts of a 7B model with no cost curve and no time.
- **[[wiki/concepts/incremental-grouping.md]]** — the rival account of objectness stated independently of any implementation: a fixed point of a small recurrent circuit seeded at one point, returning one connected component at a cost in steps, against a statistic over sampled counterfactual rollouts returning a whole partition at a cost of `R × T` forward passes.
