...
- **[[wiki/concepts/vectorized-instructive-signals.md]]** — the engineered and the measured answer to the same problem: this model *manufactures* a vector from a scalar by multiplying the broadcast `δ` with each stripe's own gating activation, while cortex appears to receive an already-addressed, oppositely-signed signal per neuron on the apical tuft — so the anatomical multiplication may be a workaround for a channel the biology has.
...
- **[[wiki/entities/medial-prefrontal-cortex.md]]** — the same cortico-striatal loop drawn plural: each medial prefrontal subregion originates its own largely closed re-entrant circuit, superficial layers target the striatal matrix while deep layers target the patch, and an extra one-way ventral → accumbens-shell → pallido-thalamic → dorsal cascade chains the tiers through the basal ganglia — none of which PBWM's single gated loop has an analogue of.
- **[[wiki/concepts/control-unity-and-diversity.md]]** — the architecture with maximal diversity and no unity: independent per-stripe gates share no control resource, so a population of trained PBWM instances should show *no* common factor across inhibition, updating and shifting tasks — a directly testable mismatch with the biological decomposition (Friedman & Robbins 2021).
- **[[wiki/concepts/dynamic-network-connectivity.md]]** — the same modulators used for a different operation: here dopamine *gates what is written* into a stripe, there it sets *how strongly the stripe's recurrence runs at all* — a gain on the store rather than a gate on its input, and one whose excess suppresses every direction rather than admitting the wrong item (Arnsten et al. 2010).
- **[[wiki/entities/meta-rl-agent.md]]** — the same dopaminergic critic training the same prefrontal–striatal loop, with the gate removed: meta-RL claims recurrence alone suffices for a fast inner learner and leaves the write-enable implicit, where this page makes learning the gate the whole model — and the striatal gating theory it belongs to was itself modelled on LSTM gating (Wang et al. 2018).
- **[[wiki/entities/c-ts-model.md]]** — this architecture nested and made unsupervised: a second, anterior loop gates prefrontal stripes that hold no item at all, only an abstract rule defined by the policy it licenses downstream, and reinforcement learning replaces the supervised motor layer at every level — so *what* a stripe stores stops being a maintained input and becomes a cluster over contexts, which is the operation this page's homogeneous slots cannot express (Collins & Frank 2013).
- **[[wiki/concepts/policy-abstraction-hierarchy.md]]** — the typing this model's stripes lack, stated as an anatomical fact: the order of policy a cortico-striatal loop can search is fixed by which rostro-caudal band sits in it, all bands run from the first trial, and the striatal update rule is identical at every level (Granger causality putamen → cortex → caudate, invariant across rule sets) — so the machine form is `k` banks of stripes differing only in their inputs (Badre et al. 2010). It also constrains what may pass between two such loops: in the monkey medial wall the caudal stage receives the abstract variable and the resolved action but **not** the sensory argument they were computed from, so binding happens at one address only — where this model makes binding a property of which stripe is gated, the biology makes it a property of which area holds all three terms at once (Awan et al. 2020, gap G59).
- **[[wiki/concepts/precision-weighting.md]]** — the rival slot for this model's dopamine term: PVLV delivers it as an exogenous teaching signal consumed by a plasticity rule, while the precision reading makes dopamine an inferred *gain* consumed by the inference dynamics — "the value of prediction error" rather than "the prediction error on value" (T122).
- **[[wiki/entities/basal-ganglia.md]]** — the biology this model compresses into `[Go_j − NoGo_j]⁺` and a scalar `δ`, and four things the compression drops. **Sign without signed weights**: every downstream node is an autonomous pacemaker, so two purely inhibitory striatal projections push the output both ways — the model buys the same opponency with a subtraction. **One scalar, two updates**: dopamine changes intrinsic excitability *and* the sign of corticostriatal plasticity in the same direction in each population, so gating and learning are one modulation read at two timescales rather than a critic feeding a plasticity rule. **The channels are asymmetric**: indirect-pathway cells have ~50% less glutamatergic input but higher intrinsic excitability, where the model's Go and NoGo units are identical. **The opponent arms have separate handles**: potentiation in a D₂ cell is driven by adenosine A2a, not by dopamine at all, so the two arms can be moved independently — a degree of freedom `δ` cannot express. And the selection story the biology supports may not be a balance at all: both pathways are reported to *increase* activity during a movement, which `[Go − NoGo]⁺` followed by kWTA cannot represent ([[wiki/empirical-tensions.md]] T129) (Gerfen & Surmeier 2011).
- **[[wiki/entities/basal-ganglia.md]]** (model census) — places this architecture among its rivals on the same anatomy: it is the *gate a cortico-cortical loop* option, one of three live and untested answers to what the basal ganglia do in working memory, and the one recent alternative (Schroll et al. 2012) makes the module the reverberating loop itself rather than a write-enable — which would mean the gate is never free during the delay ([[wiki/empirical-tensions.md]] T130). The census also names this model's weakest joint: `Random Go` is an externally scheduled exploration rule, where Chakravarthy's family makes the indirect pathway *itself* the explore regime, setting action-selection stochasticity whenever the value gradient is too flat to learn from — the same dopamine signal that sets the Go/NoGo balance then anneals exploration for free, and the prediction is that damaging the veto channel makes an agent deterministic rather than impulsive (Helie et al. 2013).
- **[[wiki/concepts/neuromodulatory-metaparameters.md]]** — names what this model's `Random Go` is a hand-set stand-in for (the inverse temperature `β`) and supplies the missing rule for setting it from the agent's own value estimate and action-value spread, plus a second modulator the model does not have at all: an acetylcholine-carried learning rate whose set-point is read off the oscillation of the same `δ` the model already computes (Doya 2002, gaps G61 and G50).
- **[[wiki/concepts/temporal-abstraction-options.md]]** — what this architecture supplies to hierarchical reinforcement learning and what it does not: a gated stripe is the natural home for "which option is currently in control", and basal-ganglia disinhibition is the initiation/termination event an option needs a signal for — but nothing here maintains a *separate policy per stripe content*, which is the option-specific action-strength table that HRL puts in dorsolateral striatum (Botvinick, Niv & Barto 2009).
- **[[wiki/concepts/reward-prediction-error.md]]** — the wiki's page for the signal this model both computes and repairs: PVLV contests where `δ` comes from, and `δ_j = snr_j · δ` is the wiki's only mechanism for making a broadcast scalar selective about which module it teaches.
- **[[wiki/concepts/vector-symbolic-binding.md]]** — the algebraic alternative to this model's routing solution to binding: here binding is *where* a filler is gated (and every reader must learn to decode every stripe), there it is *how* the filler is transformed, and the decoder is one fixed permutation shared by all readers.
- **[[wiki/concepts/memory-read-and-erase.md]]** — the read-side twin of this model's contribution: a write gate learned by reinforcement is exactly what the read side lacks, and the basal-ganglia/cortex split here matches the finding that the read scheduler is a different circuit from the store.
- **[[wiki/concepts/ignition.md]]** — the trained-gate answer to a gate with no training story: this model learns *what deserves admission* to a store by reinforcement across delayed consequences, which is exactly the credit-assignment problem the ignition threshold has no account of (`G91`).
- **[[wiki/entities/mediodorsal-thalamus.md]]** — the biological thalamus doing more than this model's SNrThal unit allows: rather than a disinhibited write-enable carrying no variable of its own, it *computes* the cueing context by pooling from prefrontal content cells and returns it as two oppositely signed channels — and it gates the content layer while leaving the rule layer untouched, a typing PBWM's homogeneous stripes cannot express (Rikhye et al. 2018).
- **[[wiki/concepts/tensor-product-representation.md]]** — the reading on which this model's stripes are not an alternative to algebraic binding but a *setting* of it: a filler pattern copied into a pool dedicated to one role is a **semi-local tensor product** (distributed filler, one-hot role), and under linear dynamics the local, semi-local and fully distributed cases are isomorphic. It derives both of this model's conceded limits from the one-hot role code — readers must learn each stripe because the role basis is the unit basis and carries no shared structure, and a finite net admits only finitely many roles because each needs its own pool — while the three ways the isomorphism fails (nonlinear units, damage locality, what a *local* learning rule can learn) are exactly where this model's per-stripe dopamine credit assignment lives (T317).
- **[[wiki/entities/esbn.md]]** — the same routing-style binding with the role code made *learned and shared* rather than one-hot over hardware: the ESBN's keys are controller-authored vectors in a single space, so one read path serves every role, which is T317's `Ψ_R` axis moved off the unit basis while keeping the gate.
- **[[wiki/concepts/activity-slots.md]]** — the same one-hot role code with the learning moved: here the write policy is trained by reinforcement and the contents sit still, there the contents are transported between slots by a velocity operator and no plasticity is needed at all — and the slot version is shown to support a linear algebra over its bound state, which this page's stripes were assumed not to ([[wiki/empirical-tensions.md]] T317; Whittington et al. 2025).
- **[[wiki/concepts/pseudo-reward-prediction-error.md]]** — the credit-assignment problem this model's `δ_j = snr_j · δ` would solve for free, and the measurement that creates it: a subgoal-level error shares all four structures with the ordinary reward prediction error, so if the two are one channel then something must decide which value table it updates — an option identifier held in a gated stripe is the cheap candidate, with the mask supplied by the hierarchy rather than by the sender (`T367`).
