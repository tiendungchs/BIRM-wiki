...
- **[[wiki/concepts/motivation-representation-synergy.md]]** — the comparative bound on what this model can express: its Bayes net recovers *what* another agent believes and wants, and has no term for whether the observer is disposed to run the inference at all — the exact factor that separates great apes (perspective taking used for private gain) from humans in the evolutionary record.
- **[[wiki/concepts/shared-intentionality.md]]** — dates what this model starts from: state/observation/belief/goal is the ~4-year competence, which the developmental record says is *constructed* over three years of perspective-shifting discourse and propositional-attitude syntax out of an earlier ~1-year operation (jointly indexing one referent with another agent) that no node of this Bayes net represents.
