...
- **[[wiki/entities/three-factor-key-value-memory.md]]** — this architecture restricted to the heteroassociative case with fixed activations and exactly one update step, given the one thing this line lacks: a write a synapse could execute. Read together they close the loop — this page derives the read's capacity, that one derives the read's *contents* from two gated local rules, and the pairing is what lets an attention-shaped store report a bound *and* be written online (Tyulmankov et al. 2021).
- **[[wiki/concepts/key-value-memory.md]]** — the family this store is one member of, and the one place it already supplies what that framing asks for: separation `Δ_i` is a store-level key-match score computable with no query, where the behavioural evidence for key-only readout (feeling-of-knowing, change detection without identification) asks for the per-query version.
- **[[wiki/concepts/associative-memory.md]]** — the synthesis that collects what this page supplies to the family: `β` and `M` as the separation decision, `Δ_i` as the only run-time occupancy candidate in the wiki's whole capacity ledger, `−γ(t−i)` as the only forgetting *rate*, and the measured metastable regime (`T390`) as the reason an attention layer's capacity number describes an operating point it avoids.
