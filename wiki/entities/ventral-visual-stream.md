...
**Limitation 1, eight years on, with a number.** The review's own throwaway — the then-current models' "confusion patterns do not match IT's" — was made into a statistic by Geirhos et al. 2020 ([[wiki/concepts/error-consistency.md]]): chance-discounted trial-by-trial agreement `κ` between sixteen ImageNet CNNs and human observers on the same images. Human observers agree with each other at `κ = .331`; ResNet-50 with humans at **.068**; **CORnet-S** — the recurrent architecture selected on Brain-Score and marketed as "the current best model of the primate ventral visual stream" — at **.066**, below AlexNet's .080 and behaviourally indistinguishable from ResNet-50 (`κ = .711` between the two). Even its *best* human match is below the *worst* human–human pair, and its relation to the six Brain-Score metrics is weak at best. Three consequences for this page. (i) The success criterion this page is built around (linear decodability of the population) and the criterion the field optimised (neural predictivity) both rise while behavioural strategy-match does not move — so limitation 1 is not merely unresolved, it is *load-bearing*, and the two criteria rank models differently ([[wiki/empirical-tensions.md]] T307). (ii) **Recurrence does not certify itself**: adding it produced no behavioural departure from a feedforward stack, which weakens the assumption on both sides of T278 that the feedforward/reentrant distinction is behaviourally visible at all. (iii) Architecture is close to irrelevant to the strategy — arbitrary CNN pairs from different families and depths reach `κ` up to .793, well above the human–human figure, so the sixteen models are one model.
...
3. **The nuisance directions are named by the experimenter.** Untangling is defined against identity-preserving transformations — position, scale, pose, illumination — enumerated in advance by whoever designed the stimulus set, exactly as an augmentation list enumerates them for a self-supervised model (`G95`). Nothing in the account says how the stream would discover which directions are nuisance from the input alone.
...
- **[[wiki/concepts/counterstream-organization.md]]** — the only part of cortex where the laminar hierarchy ruler travels: V1, V2, V4, TEO and TEpd give SLN values that correlate at ~0.73 across independent injection sites, while association areas give ~0 — so this stream is where a depth coordinate is safe to assume, and it is the stream that defines the coordinate.
- **[[wiki/concepts/error-consistency.md]]** — the instrument that priced this page's limitation 1: trial-by-trial agreement with human observers is near chance for every ImageNet CNN, is *not* improved by the recurrent Brain-Score-selected model, and is highest between CNN pairs — so the cascade's machine imitators match its decode and not its behaviour (T307).
...
- **[[wiki/entities/imagenet-c.md]]** — the 2019 repair for limitation 1's benchmark complaint: vary the nuisance parameters explicitly on a declared, severity-graded scale instead of trusting "natural, real-world" images to contain them — and the finding that six years of architecture progress bought no degradation resistance once each model's clean error is differenced out.
- **[[wiki/concepts/timescale-hierarchy.md]]** — this cascade placed inside the whole-cortex timescale gradient, with one exception that is the mechanism's best evidence: TEpd sits high in the anatomical hierarchy yet stays fast, because it is coupled to ventral-stream areas rather than to the slow prefrontal core — so the ~100 ms budget survives depth only when the graph keeps the stream out of the slow loops (Chaudhuri et al. 2015).
- **[[wiki/entities/dorsal-visual-stream.md]]** — the other stream, and the page that completes the framework this one revises: the dorsal stream trifurcates past parietal cortex into parieto-prefrontal, parieto-premotor and parieto-medial-temporal pathways, none of which has a stated success criterion, and the two streams are proposed to re-converge in the medial temporal lobe — where form information from this cascade becomes a *landmark* only once the spatial route marks it navigationally relevant (Kravitz et al. 2011).
- **[[wiki/concepts/mental-imagery.md]]** — this cascade run backwards, and the complement to its feedforward-sufficiency claim as an instrument: voluntary imagery is the only sensory representation attributable solely to feedback and obtainable with the input removed, so the pathway T278 argues about can be measured unmixed — and ventral damage disrupts visualising shape while sparing spatial transformations, so the reverse pass inherits the stream division.
