...
- **[[wiki/entities/simsiam.md]]** — the graft that isolates this page's 10-point loss: the identical predictor + stop-gradient addition is inert on SimCLR (66.5 → 66.0) and costs SwAV 1.3, so the asymmetry is not generally destructive and the surviving suspect is this page's own batch standardisation (T166).
...
- **[[wiki/entities/directclr.md]]** — the opposite bet on the same component, and the wiki's sharpest disagreement about projector design: monotone gains up to a 16384-wide embedding here, against a measured optimum of 360 dimensions out of 2048 there, with no shared account of what the width buys (T314).
- **[[wiki/entities/cpc.md]]** — the non-parametric estimator this page's cross-correlation penalty is the Gaussian-parametrised proxy for, in its original form; it also supplies the measured sensitivity to the negative-sampling distribution (7.3 points) that a negative-free objective has no analogue of.
