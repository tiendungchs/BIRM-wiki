...
For the wiki this is a discipline claim, not a historical note: every "scale will/won't solve X" argument in the ARC pages ([[wiki/entities/arc-agi.md]], [[wiki/entities/arc-agi-2.md]]) is the same shape of extrapolation, and MATH is the case where it is settled and the answer is that the curve told you nothing about the discontinuity. See [[wiki/architectural-gaps.md]] G17.
...
- **[[wiki/empirical-tensions.md]]** — T217 (a self-generated trace helps or hurts), T218 (whether normalised exact match is an adequate grader).
- **[[wiki/architectural-gaps.md]]** — G17 absorbed this page's scaling-extrapolation row: the log-linear fit projecting ~10³⁵ parameters licensed "not this method", not "not this ability".
...
- **[[wiki/concepts/confidence-calibration.md]]** — makes this page's AUROC 68.8% the wiki's one temperature-invariant confidence number: AUROC is rank-based, and the single-scalar fix that removes a network's whole calibration error preserves every ranking, so no calibration procedure could have produced or destroyed this measurement.
- **[[wiki/entities/prm800k.md]]** — the process-supervision corpus built on this dataset, and the reason its test split cannot be reused naively: 4,500 of the 5,000 test problems were deliberately moved into PRM800K's *training* set (evaluation runs on the remaining 500), so every downstream model finetuned on PRM800K inherits the leak, and the standard train-vs-test familiarity detector is blind to it because both splits are now training data. It is also the primary source for the "78% on MATH" figure this page's saturation argument quotes.
