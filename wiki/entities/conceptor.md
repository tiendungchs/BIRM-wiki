...
- **[[wiki/concepts/node-definition-problem.md]]** — the empirical case that this page's node type is *necessary* rather than merely elegant: cortical units are internally inhomogeneous and organised along more than one axis of variance, hard partitions cannot express that at any granularity, and weighted overlapping components achieve higher within-unit homogeneity than partitions optimising for homogeneity.
- **[[wiki/entities/cn-dpm.md]]** — the other continual learner that can audit its own library without labels, on the orthogonal question: quota `q = Σs_i(A^j)/N` answers *how full the shared space is*, while a `K×K` cosine matrix over per-component log-likelihood vectors answers *which stored components are redundant with each other* (threshold 0.9 removes 2 of 7 experts at a cost of 1.06 accuracy points). Neither store has the other's instrument.
- **[[wiki/concepts/output-null-subspace.md]]** — the rival selector over one recurrent net, and the trade is clean: a conceptor selects an *orbit* by inserting an `N×N` filter into the feedback loop and pays run-time cost for an algebra (`∧ ∨ ¬`, blending, cue-inference), while an initial condition selects a *transient* by placing one `N`-vector, costs nothing at run time and has no composition operator at all `(tentative)`.
