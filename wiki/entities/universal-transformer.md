...
- **[[wiki/entities/rims.md]]** — the complementary half of conditional computation, ingested alongside it: this model shares one parameter set across depth and varies the **step count per position**, RIMs holds the step count fixed and varies **which of `k_T` parameter sets run per step** — between them both axes are covered, and neither sets its own budget (a hand-set halting threshold here, a hand-set `k_A` there, both instances of `G107`) (Goyal et al. 2019).
