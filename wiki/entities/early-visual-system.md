...
- Suppression from **outside** the classical receptive field is a separate mechanism with no simple arithmetic rule, possibly carried by intra-V1 lateral connections or by feedback. The wiki's one *normative* account of it is that the surround suppression is a **prediction learned from natural-image statistics**: image autocorrelation along the locally dominant orientation stays high out to ±50 pixels (measured, at three image scales, absent in white noise), so in a hierarchical residual model the flanks predict the centre and the error units fall silent when a bar extends past the classical field. Model length-tuning curves match cat striate layer 2/3 with **no fitted parameter** — the curve is set by the training images — and orientation *contrast* between centre and surround raises the response (+19.1%) because it raises the residual, making surround facilitation and suppression one mechanism at two signs (Rao & Ballard 1999, [[wiki/concepts/predictive-coding-free-energy.md]]). Cutting the model's descending feedback removes end-stopping from 28/32 error units to 5/32; but in the linear case that same feedback term is algebraically interchangeable with within-level lateral inhibition (`W = UᵀU`), so this page's "lateral connections or feedback" ambiguity is provably not resolvable from response data alone ([[wiki/empirical-tensions.md]] **T343**).
...
- **[[wiki/concepts/counterstream-organization.md]]** — the quantitative connectivity of this system read as a hierarchy: 339 pathways over 11 macaque areas fitted to a continuous depth coordinate, with one concrete revision — area 8L of the frontal eye field sits at the level of V4 and receives direct V1 and V2 input, which is compatible with its driving attentional effects there rather than with sitting eight levels above it.
...
- **[[wiki/concepts/prediction-error-neurons.md]]** — where the framework's oldest evidence sits: end-stopping and surround suppression read as positive prediction errors — a stimulus predicting its own surround, with the prediction delivered as inhibition — and mouse V1 somatostatin interneurons shown causally responsible for surround suppression.
- **[[wiki/concepts/predictive-coding-free-energy.md]]** — the normative account of this page's hardest residual, extra-classical suppression: it is the training distribution showing through, since the model that produces it was fitted only to natural images and its length-tuning curves are parameter-free — which makes a suppression map a free read-out of what a unit's generative model has learned to be predictable, and makes the least natural stimulus the strongest drive.
- **[[wiki/entities/a24b-m2-v1-projection.md]]** — names a large share of the ≥60–80% of a V1 cell's variance this page reports as non-thalamic: one cortico-cortical projection with roughly twice the dorsolateral-geniculate cell count contacts most neurons in layers 1, 2/3 and 6 while driving **zero of 19** layer-4 cells — so the descending prediction and the ascending drive enter this page's cascade at disjoint laminar ports.
- **[[wiki/concepts/mental-imagery.md]]** — the same sheet written from the other direction, with the substrate priced: imagery strength is *negatively* predicted by V1 and V2 surface area and by resting excitability while imagery *precision* is positively predicted by it, so this stage is not only a feedforward filter bank but a shared blackboard whose intrinsic activity is one of the two parameters deciding what a top-down signal can write.
- **[[wiki/concepts/visual-routines.md]]** — the job description for what this cascade produces and what it deliberately does not: an *unarticulated*, uniform, input-determined base representation, whose parallel-computed properties (orientation, colour, motion, disparity, terminators) are exactly the ones that can serve as indexing anchors — everything articulated is assigned to a routine applied afterwards.
- **[[wiki/concepts/incremental-grouping.md]]** — the second thing this sheet carries, written by feedback onto the same cells that carry the feedforward response: a grouping tag that spreads from a cued location through whatever a non-participating feedforward population (layers 4 and 6) declares unambiguous, enhancing the firing of the grouping population (layers 2, 3 and 5) — so the parse costs no separate store and is legible to anything already reading the base representation.
- **[[wiki/entities/multiscale-tracing-network.md]]** — the model's strongest commitment about this tissue: two segregated populations within one area, one veridical and non-participating (layers 4, 6) and one modulatory and tag-carrying (layers 2, 3, 5), with the second gated by the first — which makes the parse a second signal on the same cells rather than a separate store.
