...
- **[[wiki/concepts/representational-collapse.md]]** — the control with no row in the taxonomy: regression onto an exactly computable target removes the degenerate solution outright, so every surviving design choice is about expressivity rather than anti-collapse.
