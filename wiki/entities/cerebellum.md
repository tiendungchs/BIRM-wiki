...
- **[[wiki/entities/m2-auditory-cortex-filter.md]]** — reafference cancellation done *cortically* rather than by the classical efference-copy route, and the three ways it differs: auditory thalamic suppression is flat across frequency so the filter is not subcortical, the contingency cancelled is arbitrary and newly imposed rather than a property of the plant, and what is cancelled is sensory content on a labelled line rather than a predicted plant state.
...
- **[[wiki/concepts/displacement-codes.md]]** — the competing claim about what the efference copy is *for*: here it feeds a single subcortical forward model with no map, while there the same L5 motor output is differenced against a location code so that the quantity sent onward is a spatial relation rather than a predicted sensory consequence (`T344`).
- **[[wiki/entities/a24b-m2-v1-projection.md]]** — a cortical forward model with the opposite interface, which is why the two cannot be one module in a machine version: this page's cerebellar model predicts the *plant's state* and its climbing-fibre error is claimed to arrive in motor-command coordinates, while the A24b/M2 projection predicts *sensory content* and is measured to arrive in the receiver's retinotopic coordinates. Together they split `T107`'s question in two: which structure predicts the consequence of an action depends on whether the consequence being predicted is a body state or an image.
- **[[wiki/concepts/structural-learning.md]]** — the continuous alternative to MOSAIC's decomposition, from the same first author eleven years later: a task family covered by one learned subspace plus a metaparameter rather than by a fixed set of modules mixed by responsibility, which is the difference between traversing a coordinate and interpolating inside a convex hull — and it predicts extrapolation beyond the trained parameter range, which a module set cannot do (T350).
- **[[wiki/entities/thousand-brains-theory.md]]** — the cortical theory that makes this page's own inferential move and excludes this structure from its scope: both read uniform anatomy as uniform computation (a microzone here, a column there) and both put prediction-from-efference-copy at the centre, but that theory models neocortex only, so the ~80% of neurons sitting in this circuit are outside a framework offered as a theory of intelligence.
