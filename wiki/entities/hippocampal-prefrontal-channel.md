...
- **[[wiki/concepts/sleep-oscillation-nesting.md]]** — the channel's offline phase relationship, and it is target-specific: hippocampal ripples *lead* medial-prefrontal slow-oscillation–spindle events by ~130 ms where central-parietal coupling is tight, so the same wire carries a different temporal protocol depending on the cortical endpoint.
- **[[wiki/entities/anterior-thalamic-nuclei.md]]** — a third route for this channel's traffic: prelimbic and anterior-cingulate cortices barely innervate the hippocampal formation directly, and trans-synaptic tracing makes the anterior thalamic nuclei a major monosynaptic frontal → dorsal-hippocampal route alongside nucleus reuniens.
- **[[wiki/entities/subiculum.md]]** — names this channel's origin tile: the *proximal* (prosubicular) excitatory class, defined by *Nnat*/*Chrna7*/*Nptx1*/*Robo1* and shared with the nucleus-accumbens and lateral-entorhinal projections, so the store→controller wire and the store→valuation wire leave from the same population rather than from "the subiculum" (Kinman et al. 2026).
- **[[wiki/entities/dorsal-visual-stream.md]]** — where two of the three dorsal pathways meet on this channel: the parieto-prefrontal and parieto-medial-temporal routes converge through the prefrontal ↔ hippocampal projections of the cingulum bundle, proposed as the coordination of spatial working memory with long-term-memory navigation and as cross-retrieval of either from the other (Kravitz et al. 2011).
