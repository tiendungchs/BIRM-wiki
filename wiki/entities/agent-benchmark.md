...
- **[[wiki/entities/baba-is-ai.md]]** — the same state-estimator-under-reasoner layering without the ablation that prices it: this page can put a derenderer behind BIPaCK and watch .96 → .65, whereas there the grounding and path-planning errors share a forward pass with the plan and cannot be separated from it.
- **[[wiki/concepts/motivation-representation-synergy.md]]** — the precedent for reading this benchmark's nulls cautiously: chimpanzee theory of mind was recorded as absent for two decades because the paradigms were cooperative-with-humans, and inverted as soon as the same question was asked in a competitive frame — a violation-of-expectation battery presents its agent-modelling problem in exactly one frame, so a null measures deployment under that frame rather than capability.
