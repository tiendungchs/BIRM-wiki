...
- **[[wiki/entities/shortcut-suite.md]]** — the third and most negative reading of in-context learning: there one-shot demonstrations both repair (24–40%) and mislead (18–40%), here demonstrations are uniformly worse than none even when drawn from the matched shortcut distribution — across the two benchmarks no reading of ICL as a robustness fix survives.
- **[[wiki/entities/baba-is-ai.md]]** — obtains this page's non-retrievability property for free rather than by paying for two expert rewrites per item: three layouts with identical object inventories and three different winning plans, so nearest-solved-instance matching is useless by construction.
- **[[wiki/concepts/reasoning.md]]** — the invariance definition made positive: the perturbation is built so that the correct response is to abandon a framing that no longer applies, which is the only test of that kind in the wiki with a right answer rather than only a failure mode.
