...
- **[[wiki/concepts/test-time-training.md]]** — the induction pole against which that page's transduction pole is defined: test-time training adapts weights per task and throws the adaptation away, where DreamCoder emits a program object that gets stored, compressed and recombined into a growing library — the operation transduction has no analogue of (`G14`).
- **[[wiki/entities/recursive-npi.md]]** — the complementary half of the same commitment: this system induces programs from input/output pairs and is *handed* recursion in its generic basis, while the recursive NPI is handed the program in its traces and shows what recursion buys once present — an unbounded trip count and a finite verification set.
- **[[wiki/concepts/optimal-hierarchy-criterion.md]]** — the same "compress the solved behaviour" move stated as a normative optimum over option inventories, and the page that makes the shared limit explicit: the criterion needs the solutions in hand before it can score any abstraction built over them (Solway et al. 2014).
