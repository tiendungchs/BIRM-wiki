...
| Structure is given | Walls arrive as a binary input. Learning the scaffold is not modelled; hippocampal replay into cortex is *proposed* as the training route (Bakermans et al. 2025 — the 2023 preprint is now published in Nat. Neurosci.; [[wiki/entities/state-space-composition.md]], where the proposal now has a mechanism and a recording; Ou et al. 2025) |
...
- **[[wiki/entities/gcq.md]]** — the gap this page's superset-plus-inhibition trick would fill: GCQ's toroidal codebook has *every* edge by construction, so nothing in its map represents a wall or an unreachable state, and a learned transition superset gated down by present sensory input is the wiki's only mechanism for making an attractor map know which transitions are unavailable here.
- **[[wiki/entities/autotom.md]]** — the explicit-posterior rival to this page's fixed-point account of theory of mind, now with the cost of the recursion measured: nesting run to order 4 by sampling one world per level, ~12K tokens and ~36 API calls per question, degrading 95 → 55 with depth (T183).
- **[[wiki/concepts/activity-slots.md]]** — the same architecture pointed at the past instead of the future: one population per *offset*, the whole structure read at once, but indexed by relative position to a stored observation and wired by the action group rather than indexed by delay and wired by the adjacency matrix — so this page's unification of sequence memory with planning is derived on the memory side too, and the El-Gaby/Xie subspaces this page reinterprets are given their own generative account (Whittington et al. 2025).
- **[[wiki/entities/state-space-composition.md]]** — the producer this page names and does not build: the STA's adjacency superset and its wall mask have to be taught from experience, and the cited route is hippocampal replay composing structural building blocks into memories — an operation that is now formalised and measured, though it delivers *vector-to-landmark* content rather than the transition superset the STA actually needs (Bakermans et al. 2025).
