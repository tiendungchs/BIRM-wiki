...
### 1. G17's decisive experiment, answered in three parts

**(a) The full-set ordering survives, exactly.** All seven models hold rank. This is evidence for G17's position A and it is the first such evidence in the wiki — no other self-auditing benchmark ([[wiki/entities/gpqa.md]], [[wiki/entities/frontiermath.md]]) re-scored anything after measuring its own floor.
...
- **[[wiki/architectural-gaps.md]]** — runs the decisive experiment G17 absorbed and splits it (ordering preserved, offset non-constant, sub-leaderboard spurious), and leaves G17 holding the unanswered half: whether a model-in-the-loop repair pipeline can measure its own effect on model scores.
...
- **[[wiki/entities/frontiermath.md]]** — the inversion: a ~10% self-reported critical error rate that no outside party can audit because the items are unpublished (G17), against 2,500 published items audited by a second organisation with the full defect record released.
...
- **[[wiki/entities/prm800k.md]]** — the supply this page's audit indicts, collected the other way round: step labels here are judgements by screened annotators on *model-generated* traces with the ground-truth answer in hand, not reuse of shipped reference solutions — which is the process-supervision pipeline that this page's majority-invalid rationale finding does *not* contaminate, and the practical answer to it. The residual worry transfers intact: labellers knowing the destination are doing backward reasoning the scorer must learn forward.
