...
- **[[wiki/concepts/associative-memory.md]]** — where this page's elimination argument is generalised into a rule for reading the whole family: any exotic-looking store is the trace of an integrated-out fast variable, so "biologically admissible" is a question about the chosen frame — and this page's `N_mem ≤ N_h` is what fixes the capacity denominator for the neuron-counting frame of that page's ledger.
