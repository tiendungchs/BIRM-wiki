...
- **[[wiki/concepts/spike-frequency-adaptation.md]]** — this page's `V` is that page's mechanism used with the opposite sign of intent: here the low-pass copy of a unit's own activity is subtracted to *destroy* persistence and force a moving trajectory, there the identical variable is the medium a 1200 ms working memory is stored in ([[wiki/empirical-tensions.md]] T233).
- **[[wiki/concepts/attractor-identification.md]]** — the test this page's central claim has not been put to: "no fixed points" is established by a measured transience index over activity, where the criteria that decide the question in tissue are causal — perturb the state and ask whether it flows back, and zero the tuned input and ask whether the state set survives. Both are cheaper on a trained network than on a brain and neither is reported.
