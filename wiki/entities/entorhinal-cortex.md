...
**The two streams are not equally strong, and the weaker one wins the argument.** Under a local-global cue conflict on a familiar track, superficial medial entorhinal cells follow the **global** cue set coherently while superficial lateral entorhinal cells carry only a weak spatial signal, detectable at the population level, that follows the **local** cue set (Neunuebel et al. 2013). Downstream, CA3 settles into a coherent map locked to the **local** cues at every mismatch angle — so the hippocampal attractor is placed by the afferent with the *lower* spatial information, against a conflicting afferent with higher (Neunuebel & Knierim 2014, [[wiki/concepts/pattern-separation-completion.md]]). Two mechanisms are proposed and neither is tested: hyperexcitable adult-born granule cells receive lateral- rather than medial-entorhinal input preferentially and may dominate the dentate's drive onto CA3 on entry to an environment; and an attractor needs only a small bias to place its bump, so the channel that *selects* the fixed point need carry almost no information (Zhang 1996). Independent evidence that the medial stream is not the primary spatial drive onto place cells is listed in the same source: disrupting grid cells spares much place-field tuning (Brandon et al. 2011), adult-like place fields precede well-formed grids in development (Langston et al. 2010; Wills et al. 2010), cue-card rotation can drive grids and place cells discordantly (Song et al. 2012), and inactivating the hippocampus costs medial entorhinal cells their gridness (Bonnevie et al. 2013).

**Why a builder should care.** Every model in the wiki that carries both streams reads position out of the structural one. If the *content* stream is what decides which stored state is entered and at what orientation, then the structural stream supplies coordinates *within* a state that something else selected — which makes the anchoring problem (`G39`) a question about the sensory afferent's gain, not about the positional code's accuracy.
...
- **[[wiki/concepts/pattern-separation-completion.md]]** — and prices the two streams against each other at the store's read: under cue conflict the coherent, strongly spatial medial input loses to the weak, barely spatial lateral input in deciding where CA3's attractor lands, so the afferent that *selects* the stored state is not the afferent that carries the most information about it (Neunuebel & Knierim 2014, `T393` on the dentate stage in between).
...
- **[[wiki/entities/subiculum.md]]** — the same reciprocal topology typed by cell class rather than by tracer field: proximal subicular neurons (*Nnat*/*Chrna7*/*Nptx1*/*Robo1*) both receive from and project to the lateral division, distal ones (*Nts*/*Cdh7*/*Elf1*/*Scn4b*) to the medial division — so `T340`'s Position A survives one stage further out, still as anatomy (Kinman et al. 2026). Both halves are measured in one preparation: trans-synaptic labelling puts lateral input on 91% proximal cells and medial on 99% distal, and retrograde tracing recovers the same border for the return, with a ~200 µm transition zone — though the medial-projecting fraction (74.3%) is the least clean of seven efferent classes tested, and subicular cells collateralise, so the wire back to one division is carried by cells simultaneously writing to accumbens or retrosplenial cortex (Cembrowski et al. 2018).
