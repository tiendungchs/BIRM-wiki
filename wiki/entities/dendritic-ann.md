...
- **[[wiki/concepts/width-versus-parameter-count.md]]** — the control this model never ran: a mask drawn uniformly at random and frozen at initialisation, with the weight count pinned and the width raised, already buys most of the accuracy a sparse wide net can buy — which brackets how much of the dendritic result is the *chosen topology* and how much is sparsity-at-width that any random mask supplies (Golubeva et al. 2021).
