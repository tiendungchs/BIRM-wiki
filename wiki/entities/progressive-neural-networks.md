...
- **[[wiki/entities/ligo.md]]** — the complementary growth experiment, and the other half of the control `G100` needs: this page freezes old capacity and varies stored *content* against a random-frozen-column twin, LiGO absorbs old capacity and varies *cost* against a full-size-from-scratch twin — and where growth-with-content wins here (209 vs 134), growth-with-schedule there only ties at 45% fewer FLOPs, which is why `T354` asks whether either result is about competence rather than speed.
