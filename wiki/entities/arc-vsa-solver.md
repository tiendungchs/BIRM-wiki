...
- **[[wiki/entities/arc-agi-2.md]]** — what the re-authored benchmark does to a method whose vocabulary is a fixed spatial-semantic algebra: 11.7% on demonstrations and **0.0%** on queries, against 48.8/10.8 on version 1, which is the coverage-vs-abstraction question of G17 answered by a benchmark rather than by an argument.
...
- **[[wiki/entities/dreamcoder.md]]** — the same "select by re-use, not brevity" principle at a different scope: this page charges each distinct operation across all *demonstrations of one task* by a minimum hitting set, DreamCoder charges description length across the *whole solved corpus* to decide what becomes a library primitive.
- **[[wiki/entities/resonator-network.md]]** — the opposite search discipline in the same algebra: this solver instantiates and scores six candidate parses, where a resonator holds every candidate for every factor superposed and never instantiates one — the natural missing component here, since an object's colour/centre/shape could be *factored out* of its code rather than compared against a learned prototype.
- **[[wiki/concepts/selective-prediction.md]]** — the mean max-softmax over parse similarities is a confidence rate `κ_f` defined over *discretisations* rather than answers, already computed and never thresholded; thresholding it is a selection function for "no available parse makes these demonstrations look like an object mapping", at zero extra cost.
- **[[wiki/concepts/visual-routines.md]]** — recasts this solver's six object hypotheses as a *degenerate compilation*: six pre-compiled parses over a space whose generators (shift, index, bounded activation, tracing, marking) are compositional and open-ended, which is why enlarging the menu is the wrong repair for `G75` and assembling a routine is the right one.
