...
- **[[wiki/concepts/mental-imagery.md]]** — the biology this model's imagery mode is an implementation of, agreeing on the format (imagery decoded from V1/V2 by perception-trained classifiers) and adding two things the model has no term for: intrinsic noise on the generated layer, which sets imagery strength jointly with the top-down drive, and the aphantasia result that the sensory render is dispensable for easy and medium visual working memory (`T386`).
- **[[wiki/concepts/predictive-coding-free-energy.md]]** — a generative model obtained without ever training a decoder: reading the frame-conversion interface top-down renders the store's contents into sensory populations on the *same* weights used for inference, which is the architectural claim that page makes in general, with the bottom-up/top-down gain pair standing in for the precision term that decides which direction currently wins.
