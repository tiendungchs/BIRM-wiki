...
| Second-order conditioning | **impaired** | spared | a CS can only train a second CS if it retrieves a value — though **not necessarily an outcome-specific one** (`T395`), and the lesion cannot resolve the two serial stages inside the complex |
...
- **[[wiki/concepts/conditioned-reinforcement.md]]** — the behavioural literature the conditioned-reinforcement assay this page assigns to the basolateral nucleus comes from, and the variable that assay does not control: every delivery of the conditioned reinforcer that food does not follow extinguishes it, so a lesion result read at one pairing frequency is read at one point on a curve nobody has measured.
- **[[wiki/concepts/higher-order-conditioning.md]]** — re-reads this page's second-order-conditioning row as a claim about **depth rather than content**: the `S1`/US convergence is in the lateral nucleus and second-order plasticity is proposed for the basal nucleus downstream of it, so a basolateral lesion removes second-order conditioning by removing the stage *after* outcome identity — which is why the same lesion spares sensory preconditioning, and why the deficit licenses no inference that `S2` stored a specific outcome (`T395`). It adds the posterior-insular US store (post-training lesions block expression, pre-training lesions do not), three pharmacological dissociations run on this structure (intra-amygdala AP5 blocks second-order acquisition while *enhancing* first-order expression; systemic quinpirole blocks second-order and spares sensory preconditioning; naloxone enhances second-order), and a basal/lateral split on secondary reinforcement versus freezing.
