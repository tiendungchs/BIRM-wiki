...
- **[[wiki/concepts/confidence-calibration.md]]** — the case a global temperature cannot absorb: one scalar corrects an offset, not a miscalibration *conditional on which channel the answer came from*, so this page's confidence–accuracy gap widening on the shortcut arm relative to a matched standard arm is evidence of input-conditional bias and therefore a shortcut statistic rather than a scale artefact.
