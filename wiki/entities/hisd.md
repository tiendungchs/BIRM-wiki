...
- **[[wiki/concepts/optimal-hierarchy-criterion.md]]** — the score that would adjudicate this system against the topological route: both minimise a description length, but over different streams (episode symbol strings here, walks on an adjacency matrix there), so they should agree only where those streams carry the same structure — a heavily reused corridor that is not a bottleneck is the diagnostic case, and nobody has run it (Solway et al. 2014).
- **[[wiki/entities/diayn.md]]** — the same "a skill is what separates one behaviour from another" instinct applied to data the agent generates itself: HiSD segments an unlabelled observation corpus and takes grammar rules used at least twice as options, DIAYN acts to make its own skill index recoverable and takes the discriminator's decision regions as skills — so HiSD needs a demonstrator and gets nesting, DIAYN needs only an environment and gets a flat, frozen set.
- **[[wiki/entities/option-critic.md]]** — the supervision contrast in the other direction: HiSD discovers options offline from an unlabelled observation corpus with no reward and no interaction, then still has to learn a policy for each one; option-critic discovers intra-option policies and terminations from reward-driven interaction alone and never names a skill — so what HiSD gets for free (a named, reusable, nestable library) is exactly what option-critic cannot produce, and vice versa for return-serving guarantees.
