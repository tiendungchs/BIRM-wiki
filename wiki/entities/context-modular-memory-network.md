...
Masks are per-context and independent, so nothing here carries what was learned in context `k` into context `k+1`: the store holds `s` structures and no map between any two of them (`G83`).
...
| Nothing is ever forgotten or evicted | Contexts accumulate; there is no rate, schedule, or capacity-triggered eviction. The model has the rarer half of `G42` — closed-form `α*` and `I*`, so it knows *what* full means — and not the operational half: nothing reads that number at run time and refuses a write |
...
- **[[wiki/concepts/engram.md]]** — the biological object this model's per-context mask abstracts: a fixed-fraction write set whose size is conserved across memories of different strength and valence, with the derived optimum here (~20–30% active) sitting just above the measured amygdala fraction (10–20%) and far above the dentate one (2–6%) **(tentative)**.
- **[[wiki/entities/pbwm.md]]** — control that carries no content, in the other direction: this page's context bits select which attractors are *retrievable*, PBWM's Go/NoGo bits select which stripes are *overwritable*, and both keep the controller's state a handful of bits.
- **[[wiki/entities/spacetime-attractor.md]]** — the same gating logic applied to a world model instead of a memory library: a learned superset of future transitions is made environment-specific by inhibitory input that deletes the blocked transitions from the dynamics, so structural generalisation needs no synaptic change (Jensen et al. 2026).
- **[[wiki/concepts/effective-connectivity.md]]** — the biological claim this architecture is the cheapest implementation of: a brain's directed connectome is *parameterised by its state*, so there is one effective connectivity per task and no atlas (Li & Yap 2022) — which is this page's per-context mask over a fixed weight matrix, and the neuroimaging model's `A + Σ_k B^k u_k` with a binary `B`.
- **[[wiki/concepts/connectome-state-transformation.md]]** — the measured version of this architecture's central move: context as a modulation of a fixed connectivity matrix, except learned rather than authored, continuous rather than binary, and with a rank attached — ten components suffice to carry the task-induced reorganisation of a 268-node, 35,778-edge human connectome.
- **[[wiki/entities/mediodorsal-thalamus.md]]** — supplies the controller this model declares to be outside itself, and far below its own cost estimate: the gating signal is computed by a low-dimensional thalamic relay that pools from the very cortical population the gate will act on, so no external context label is needed and the mask's source is a projection rather than an `s·N` synaptic addressing scheme (Rikhye et al. 2018).
- **[[wiki/entities/ch-hnn.md]]** — the same mask-separation mechanism with the mask made a learned function of the stimulus rather than an index: interference is still removed at recall by a per-context binary pattern over shared synapses, but the pattern is *predicted* from the input by an upstream network trained on inter-episode similarity, so it generalises to contexts never stored.
