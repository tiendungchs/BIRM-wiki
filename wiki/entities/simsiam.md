...
- **[[wiki/entities/directclr.md]]** — the component this page's subtraction never removes: every method in the hub table keeps a projector, and the projector's measured job is to host the dimensional collapse the loss cannot avoid, so "twelve lines of PyTorch" still contains one module doing anti-collapse work that no coefficient count records (T314).
