...
- **The meta-grammar is authored and fixed, so the third tier is missing** (`G9`): the model discovers grammars, nothing discovers the generator that emits them, and the boundary above is exactly what a flat `W` looks like from the outside.
...
- **[[wiki/entities/cfq.md]]** — reads this page's signature dissociation on a measured scale: buying systematicity outright while leaving the length split at 100% error is what a sampler covering one *direction* of compound divergence and not another looks like, and it is the strongest evidence against CFQ's one-scalar claim (T312).
- **[[wiki/concepts/inductive-bias.md]]** — names what this page's episode sampler is and where its guarantee stops: the grammar generator is the wiki's most explicit environment `Q`, held-out episodes are fresh draws from it and sit inside `er_Q`, so passing them certifies a small bias loss over *that* `Q` — and the facet the sampler never varied failing at 100% is `er_Q(H)` small over the wrong `Q`, not a bound violated.
