...
- **[[wiki/entities/dorsal-visual-stream.md]]** — the input side of this page's partition claim: the parieto-medial-temporal pathway targets CA1/prosubiculum and the pre-/parasubiculum *specifically* rather than the subicular complex as a whole, so the store's input stage is partitioned by source in the same way its output stage is partitioned by destination (Kravitz et al. 2011).
