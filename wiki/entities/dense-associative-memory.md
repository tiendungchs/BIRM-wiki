...
- **[[wiki/concepts/associative-memory.md]]** — the page that types this one's contribution: `n` is the **separation operator**, one of four independent decisions, so raising it is a read-out change and not a code, kernel or write change — and the `f = F′` duality is the first case of the elimination principle, where an apparently exotic store turns out to be the trace of a faster variable that was integrated out.
