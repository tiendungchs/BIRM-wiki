...
- **[[wiki/entities/cfc.md]]** — the opposite response to the shared fact that a continuous-time model's compute *is* its solver's step count: KAN-ODE keeps the solver and buys a readable vector field, CfC deletes the solver and thereby gives up having a vector field to read — its limitations section concedes the losses (no invertibility, causal inference "less straightforward", PDE/physics work stays with ODEs), which is the interpretability-versus-cost trade priced from the other direction.
