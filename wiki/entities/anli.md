...
- **[[wiki/architectural-gaps.md]]** — G17: whether a human paid to fool one model finds that model's idiosyncratic blind spots or the model class's, with below-chance base-model scores on one side and BERT/XLNet beating RoBERTa on the rounds RoBERTa seeded on the other.
...
- **[[wiki/entities/gsm8k.md]]** — the cheap route to the same non-saturation property, and its limit: GSM-Symbolic generates fresh items from a template for free where ANLI pays humans to find them, but every failure it can produce is of one kind (surface rebinding of a fixed reasoning graph) while the adversarial loop's are open-ended.
- **[[wiki/entities/gpqa.md]]** — the same protocol with the adversary swapped: difficulty defined relative to a *timed human with a search engine* rather than to the current model, bought by the same device (a bonus schedule paying jointly for correctness and for the validators' failure) — and it recovers what ANLI gives up, since each writer ships the reasoning chain, so the withheld abstraction is nameable per item.
- **[[wiki/concepts/certification-instruments.md]]** — supplies instrument `I12`, adversarial per-item authoring, the only one immune to developer-blindness depleting — and failure mode `F6`, which is the price: by round 2 a context-blind baseline is within 2.6 points of the full model.
- **[[wiki/concepts/human-baseline.md]]** — replaces the accuracy baseline with a per-item authoring cost — try count and clock — the only difficulty measurement in the wiki taken at collection time rather than inferred afterwards.
- **[[wiki/entities/olymmath.md]]** — the other escape from F1, and the cheaper one: instead of paying human adversaries in perpetuity, source items from printed non-digitised material and open-source the *construction pipeline* so the set can be refreshed — a benchmark as a renewable process. The cost is comparability, since successive editions share no anchor subset, where ANLI's rounds are explicitly nested.
- **[[wiki/entities/hle-verified.md]]** — where the cost deleted at authoring reappears: ANLI pays ~5 minutes of annotation per item *and gets validity as a by-product*, whereas an ensemble-gated benchmark pays nothing up front and pays later for two independent expert repair teams plus adjudication, on a set it can only partly fix (the domains with the worst validity are the ones excluded from repair as unverifiable).
- **[[wiki/entities/hle.md]]** — `I12` with the human adversary deleted: the same model-in-the-loop principle run once, automatically, against a frozen six-model ensemble at one API call per attempt instead of ~5 minutes of paid annotation, which buys 2,500 items and loses the try count, the clock and the retrain-and-repeat rounds that make G17 answerable at all.
- **[[wiki/entities/shortcut-suite.md]]** — the same substrate (MultiNLI premises, and the HANS heuristics that motivated adversarial collection) run with the opposite economics: ANLI pays ~5 minutes of annotation per item to *remove* an artefact whose identity is then unnameable, while conjoining a tautology *installs* one whose identity is known by construction for the price of a string concatenation — so the two together bracket the cost/nameability trade F6 and F3 describe.
- **[[wiki/entities/prm800k.md]]** — the same active-learning rule with the human moved out of the rejector's seat: label where a cheap scorer is confident and a sounder test disagrees. Here that is "model-wrong ∧ human-right" at ~5 minutes of authoring per item; there it is "PRM-confident ∧ final answer wrong" at the cost of one automatic check, worth ≈2.6× data efficiency — and it comes with the negative result this page's rounds structure assumes away, that retraining the selector inside the collection loop was unstable and gave no gain.
- **[[wiki/concepts/curriculum-learning.md]]** — the inverted schedule, and the larger effect: hardness selected in closed loop against the live model means the sampler's entropy *falls* onto its failures, violating both of the definition's conditions while outscoring every monotone easy-first result in the wiki — evidence that `G32`'s generator has to leave the curriculum family.
- **[[wiki/entities/asymmetric-self-play.md]]** — the same closed adversarial loop with the paid human replaced by a policy, and the case where this page's load-bearing *rejector* comes free: an example is verified by having been reached, since the proposer walks the trajectory it publishes as a goal — which is what an embodied proposer buys over a written one, and is why the scheme needs no `model-wrong ∧ human-right` filter (`G32`, OpenAI et al. 2021).
