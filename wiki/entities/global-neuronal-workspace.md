...
| Macaque vlPFC, no-report binocular rivalry, **987 units** | The same claim at population resolution and on *internally generated* switches: ~90% of strongly selective units follow the percept, the current content decodes at up to **95%** single-trial, and the classifier generalises between constant-input (rivalry) and changing-input (physical alternation) trials, so the code is for the percept and not the retinal image (Kapoor et al. 2022 — see the section below) |
...
## The single-unit test the read-out defence asked for

> **Fourth source (263rd ingest).** Kapoor, Dwarakanath, Safavi, Werner, Besserve, Panagiotaropoulos & Logothetis 2022, *Decoding internally generated transitions of conscious contents in the prefrontal cortex without subjective reports*, Nat Commun 13:1535, doi:10.1038/s41467-022-28897-2 (`raw/kapoor-2022-conscious-transitions-pfc.md`). New data: 987 units across 6 sessions in 2 macaques, Utah array over the ventrolateral prefrontal inferior convexity, no-report binocular rivalry with optokinetic nystagmus as the perceptual read-out. Written by proponents of a prefrontal role; the design, not the verdict, is what carries.

The reply above proposed single-cell-resolution recording as the way to tell "the content is absent" from "the instrument averaged it away". That experiment existed before the dispute, in the animal, and returns **content present**:

| Confound the framework is accused of riding on | How it is removed here | Result |
|---|---|---|
| **Motor report / decision about the report** | Animals were never trained to report or categorise anything; perception is inferred from an involuntary reflex (OKN) | Perceptual modulation survives |
| **Feedforward drive from an external stimulus change** | Spontaneous rivalry switches — dichoptic input constant, percept flips endogenously | Perceptual modulation survives; the strong transient seen at externally imposed switches is **absent**, so the content signal is separable from the feedforward burst |
| **A code for the retinal image rather than the percept** | Classifier trained on physical-alternation trials, tested on rivalry trials at matched phase | Generalises (`p = 0.002`). An input code would have inverted under rivalry, where the suppressed stimulus is still on the retina |
| **The oculomotor signal used to infer the percept** | Fixation-On control suppresses OKN; train on one paradigm, test on the other (103 units) | Generalises (`p = 0.002`) |

Decoding reaches **95%** on single transitions pooled across animals, 86–96% within individual sessions, with broad cross-temporal generalisation — a *static* code during a dominance period rather than a sequence of transient ones, which is what an occupied bus should look like.

**What it buys the framework, precisely.** (i) The prefrontal decoding null is removed from the ledger against it ([[wiki/empirical-tensions.md]] T272) — the corresponding measurement at unit resolution is positive, and on the same tissue the code is spatially unclustered (opposite-preference sites ~0.4 mm apart), so both halves of the read-out argument hold at once ([[wiki/concepts/population-geometry.md]]). (ii) It answers the "prefrontal activity is post-perceptual" objection with a design rather than an argument, since there is no perceptual act to be post to. (iii) It supplies content-specificity for *low-level* features (motion direction), where the earlier no-report result used faces and complex objects — so the workspace is not restricted to high-level summaries, which is the same variable as the task-dependent scope of the ignited coalition.

**What it does not buy.** No ignition is measured: no threshold, no take-off, no all-or-none transition — only the held state. And it does not touch the perturbation null above; the two now stand as the sharpest form of T269. **Prefrontal cortex carries the current content at 95% and stimulating it changes the content 0% of the time.** Both are direct measurements in the same broad region, and the only readings that survive both are that prefrontal content is a *copy* used for control rather than the substrate of the percept, or that the elicitation instrument cannot write to a code this unclustered — the second of which is the same geometry argument running in the causal direction, and is untested ([[wiki/concepts/perturbation-elicitability.md]]).

**A cost the framework does not usually pay, visible here.** Cross-session decoding falls to 58–65% against 86–96% within a session, attributed to unit turnover. If the workspace's contents are read by downstream consumers, those consumers face the same drift, so a fixed read-out cannot be what makes a broadcast usable — the stable object has to be a latent whose alignment is re-estimated, which no version of the framework specifies.
...
## The primary record of the adversarial test

> **Sixth source (265th ingest).** Ferrante et al. (COGITATE consortium) 2025, *Adversarial testing of global neuronal workspace and integrated information theories of consciousness*, Nature, doi:10.1038/s41586-025-08888-1 (`raw/ferrante-2025-gnwt-iit-adversarial-testing.md`). `n = 256` humans across fMRI (120), MEG (102) and iEEG (34), two–three laboratories per modality, preregistered pass/fail criteria, theory-neutral analysts. The section above holds the framework's **reply** to this study; this is the study. Full treatment of the rival: [[wiki/entities/integrated-information-theory.md]].

Three corrections and two additions to the second-hand account above.

| Point | What the primary record says |
|---|---|
| **Category *is* decodable from prefrontal cortex, task-independently** | The reply's read-out defence is aimed at a null that is only partial. Cross-task generalisation of faces–objects reached **~70%** in prefrontal iEEG regions of interest and **~60%** in middle and inferior frontal cortex with fMRI — significant, in the predicted window, on task-irrelevant stimuli. What failed was **identity** (absent in prefrontal cortex for every category) and **orientation** (`BF₀₁ = 5.11–8.65` for the null in iEEG; 34–55% of prefrontal voxels with substantial-to-very-strong support for the null in fMRI, against 1–9% for the alternative) |
| **So the prefrontal null is dimension-selective, and a uniform read-out argument does not explain it** | Mixed selectivity plus a coarse kernel should cancel *every* variable in the same tissue with the same instrument. Category survived at 70% and identity and orientation did not — which is a fact about **which variables prefrontal cortex carries**, not only about how legibly it carries them. The consortium's own reading: prefrontal cortex may broadcast a *subset* — abstract categories rather than low-level details ([[wiki/empirical-tensions.md]] T272) |
| **The prefrontal representation is temporally narrow** | Prefrontal cross-task decoding generalised over only **~0.2–0.4 s** against roughly the full stimulus duration posteriorly — consistent with an admission event rather than an occupancy state, which is what this framework should want |
| **The offset null is quantified, and it has a positive control inside the same analysis** | **0 of 655** prefrontal electrodes showed the predicted onset-plus-offset profile, `BF₀₁ > 3` for every one of them, favouring an intercept-only or amplitude-varying model. The same linear mixed model **did** find the predicted profile in 10 striate/extrastriate electrodes, and an exploratory unrestricted decoding analysis found exactly one prefrontal site (inferior frontal sulcus) with transients ~0.15 s after onset and offset. Cross-temporal representational similarity: transient category representation at onset, none at offset, **including in the task-relevant condition where the signal was stronger and longer** — which is the closest the study comes to the attended-offset condition the reply demanded ([[wiki/empirical-tensions.md]] T271) |
| **Adding prefrontal cortex to a posterior decoder does not improve it — `BF₀₁ = 1.94 × 10⁴`** | The framework predicted this: workspace neurons *broadcast* rather than add information. It is the strongest quantitative statement in the wiki of "router, not payload", and it sits directly beside the perturbation null (T269). A copy that adds nothing to a decoder and cannot be written into by stimulation is doing something other than constituting the content |
| **The long-range channel survives, but only on the exploratory metric** | The preregistered phase metric (pairwise phase consistency) found **no** content-selective prefrontal↔category-selective synchrony (`BF₀₁ = 2.62–5.32`). Exploratory dynamic functional connectivity — amplitude co-modulation with the evoked response removed — found **gamma-band** connectivity between prefrontal cortex and both face- and object-selective sites inside the predicted window, task-independently, plus fusiform-face-area→inferior-frontal-gyrus coupling in fMRI. The broadcast channel is therefore evidenced by *amplitude* coupling and not by *phase* coupling, which is a weaker and different claim than the framework's stated mechanism ([[wiki/concepts/inter-areal-synchrony.md]]) |

**Net.** The framework keeps onset ignition (duration-independent, task-independent), keeps a content channel to early visual cortex, and gains a hard number for the router reading. It loses the offset commit against a positive control, and its content claim narrows from "all conscious content" to "category but not identity or orientation" — at which point *"what determines which dimensions reach the bus?"* becomes a first-class architectural question, and no version of the framework specifies it.
...
- **[[wiki/concepts/representation-probing.md]]** — the instrument that carries the prefrontal claim: three cross-condition generalisation tests (input-change vs constant-input, epoch, eye-movement present vs suppressed) each remove one confound by requiring transfer rather than by subtracting a control, which is how "decodable" becomes "decodable as the percept" (Kapoor et al. 2022).
...
- **[[wiki/concepts/metacognitive-efficiency.md]]** — bounds the framework's one concrete meta-representation proposal: error awareness as a consistency check between a fast unconscious route and a slower conscious one is a *sensitivity* mechanism, and ideal sensitivity is capped by first-order performance, so the check can deliver nothing where the first-order route is near chance.
- **[[wiki/entities/rims.md]]** — a system with this framework's sparse-interaction bottleneck and **none** of its bus: only `k_A` of `k_T` modules compute per step, but communication among them is all-to-all attention (`O(k_T²)`), the mixture is continuous, and nothing is ever exclusively published — so it buys sparsity in *who computes* rather than in *who can reach whom*, and leaves `G91`'s discrete commit entirely untouched while otherwise resembling a workspace (Goyal et al. 2019).
- **[[wiki/concepts/expected-value-of-control.md]]** — the allocation rule this page's admission gate lacks: workspace activation rising during novel acquisition, effortful execution and after errors is exactly the monitoring input list of the expected-value-of-control model, which adds *how much* to allocate — an intensity chosen by maximising payoff net of cost — where the workspace account gives only whether a content is admitted.
- **[[wiki/concepts/apical-amplification.md]]** — a cell-level competitor for this framework's core function: if the neurons receiving matching feedforward and feedback input burst and therefore dominate their targets, "what is currently broadcast" falls out of a rate threshold over ordinary pyramidal cells and needs no dedicated long-axon workspace population or read-out stage (Larkum 2013).
- **[[wiki/entities/dendritic-integration-theory.md]]** — the gate underneath this framework's gate: it explains anaesthesia, the perturbational-complexity result and the 200–300 ms latency from a coupling variable inside layer-5 pyramids rather than from the bus, which reduces a hub's privilege to *density of couplable cells* and leaves the framework's admission threshold conditional on a switch it never modelled.
