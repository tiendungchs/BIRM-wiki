...
- **[[wiki/concepts/alignment-uniformity.md]]** — the one candidate monitor for the failure on this page: `(L_align, L_uniform)` was validated against a dense conv-layer depth regression as well as global probes, so unlike the scalar monitors it has been scored on the read-out class that dies here — untested on patch tokens over a long run, which is the cheap experiment.
...
- **[[wiki/entities/objectnet.md]]** — the source of this page's ObjectNet column and the reason it is worth a column: the benchmark's controls are randomised at capture over background, rotation and viewpoint, so 79.0–80.2 against a size-matched 66.4 is a purchase made by the curation mixture on nuisance directions that no augmentation list here declares (`G95`), not another reading of ImageNet accuracy.
- **[[wiki/entities/mae.md]]** — the third axis of this page's instrument-dependence result: here one frozen encoder's probes disagree across *training time*, there two encoders' rankings invert across *read-out depth*, and in DINOv2 across *probe class* — the same defect reported from three orthogonal directions, none of which any comparison table in the wiki controls for (T310).
- **[[wiki/concepts/label-free-model-selection.md]]** — the failure no label-free selector is calibrated against: this page's dense read-out decays over training while every global measurement rises, so a monitor tracking the global probe would certify a degrading run — and neither RankMe nor the rescaled-loss monitor has been computed on patch tokens across this run.
