...
- **[[wiki/entities/integrated-information-theory.md]]** — one of the two frameworks this page synthesises, and the one it declares insufficient: a high-Φ expander graph satisfies its criterion while modelling nothing, so integration needs the spatial/temporal/causal coherence condition added on top.
- **[[wiki/concepts/certification-instruments.md]]** — why this theory's integration measures certify nothing about representation: Φ, perturbational complexity and repertoire size all score *dynamics*, and an expander graph maximises them while modelling nothing — so they supply a necessary condition and none of the sufficient one the instrument inventory is missing.
