...
**(brainstorm) Symmetry is the axis, and it is a two-sided trade.** `W = Wᵀ` buys the Lyapunov function and therefore the guarantee of convergence; it also makes the network incapable of going anywhere. Every mechanism the wiki wants on top of memory — sequences, replay, simulation, search — is motion, and motion requires either an asymmetric term ([[wiki/entities/dense-sequence-memory.md]]), a slow adaptation current ([[wiki/entities/adaptive-cann.md]]), short-term depression ([[wiki/entities/stp-flickering-cann.md]]) or noise. The classical Hopfield network is best read as the *stability* half of an architecture whose other half has to be added back, and the wiki now holds five different ways of adding it. **The fifth changes nothing inside the network and everything about the composite** ([[wiki/entities/resonator-network.md]], Frady et al. 2020): keep `g(XXᵀ·)` exactly as it is, but send its output *sideways* — multiplicatively into the inputs of `F−1` sibling cleanups, each holding a different codebook — and the composite has no Lyapunov function even though every part is symmetric. That system does not settle; it wanders and then abruptly locks onto the one combination of stored patterns consistent with an input product. So the same circuit is a memory when wired to itself and a **combinatorial search** when wired to its neighbours, and the guarantee is what is traded for the search.
...
- **[[wiki/entities/resonator-network.md]]** — this page's circuit used as a search primitive rather than a store: `F` copies of `g(XXᵀ·)` over `F` codebooks, cross-coupled by Hadamard binding instead of self-feedback, which removes the symmetry and the convergence guarantee and buys the ability to factor a bound vector whose factors are all unknown.
...
- **[[wiki/concepts/fast-weight-programming.md]]** — the same outer-product write and the same crosstalk-limited capacity, with the patterns *self-invented* rather than supplied: keys and values are outputs of trained projections, the store is heteroassociative, and a delta-rule write corrects one binding in place instead of requiring the whole matrix to be relearned (Schlag et al. 2021).
- **[[wiki/entities/remerge.md]]** — the settling half of this page without the superposition: that model iterates to an attractor over separately stored traces addressed by similarity, where this one folds them into a single weight matrix and loses the exemplars.
- **[[wiki/entities/dense-associative-memory.md]]** — the direct generalisation, and the one that recovers this page exactly at `n = 2`: keep the states, the asynchronous update and the memories, replace `E = −½σᵀTσ` with `E = −Σ_μ F(ξ^μ·σ)` for sharply growing `F`, and `0.14N` becomes `α_n N^{n−1}` with nothing else changed. It also identifies this page's update with a ReLU hidden layer (`f = F′`, ReLU ⇒ `n = 2`), so the classical store and the commonest activation function are one object read at two update counts (Krotov & Hopfield 2016).
