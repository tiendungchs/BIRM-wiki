...
`Δw_ij = η(⟨s_i s_j⟩_data − ⟨s_i s_j⟩_free)`. **This rule generalises past maximum likelihood, and stays exact when it does.** Replace "clamped to data" with "nudged by a loss", `β·ℓ(s)` added to the energy, and the same difference-of-expectations is the exact gradient of the Helmholtz free-energy difference `J(θ) = A(θ,1) − A(θ,0)` for *any* nudging strength — no convexity, no unique minimum, no infinitesimal limit, and no requirement that `⟨·⟩_data` come from clamping at all (Litman 2025; [[wiki/concepts/energy-based-models.md]]). The clamped/free contrast on this page is the `T → 0`, `β = 1` corner of a two-parameter family whose other members are equally exact. When the two averages agree the model's distribution matches the data's and learning stops — the fixed point is a *moment-matching* condition, and no error signal, no target and no backward pass appears anywhere. The rule is fully local: each synapse needs only the two units it connects, in two operating conditions.
...
- **Equilibrium is assumed, never certified.** Both phases require averages "at equilibrium"; nothing here bounds the mixing time, and for a rugged landscape (the same ruggedness that stores memories) it can be exponential. The whole learning rule rests on an unmeasured quantity. *Partly narrowed:* Litman 2025 shows the rule needs only that both phases sample their Gibbs distributions — no convexity, no unique minimum, no zero temperature — so what is unpaid is specifically the **mixing time of the sampler**, not the shape of the landscape.
- **The contrast is measured on a difference that may be below the noise floor.** The estimator's usable signal scales with how far the second phase is displaced from the first: for a weak displacement the signal-to-noise ratio of `Δs` is indistinguishable from sampling noise, and training fails outright rather than degrading (Litman 2025, on the nudged-phase analogue). Nothing on this page measures the displacement that clamping produces.
...
- **[[wiki/concepts/energy-based-models.md]]** — the finite-temperature generalisation of this page's rule: replacing "clamped" with "nudged by `β·ℓ(s)`" leaves the same difference-of-expectations update but makes it the *exact* gradient of a Helmholtz free-energy difference, which retires the convexity and unique-minimum assumptions this page's derivation quietly needs (Litman 2025).
- **[[wiki/concepts/attractor-dynamics.md]]** — the stochastic-sampling version of the same energy landscape, where settling becomes inference rather than retrieval.
- **[[wiki/concepts/equilibrium-propagation.md]]** — the same contrastive shape with the second phase repaired: a weakly nudged equilibrium is a local perturbation of the free one rather than an independent clamped chain, which is why it is the exact gradient of a regularised objective where `CD₁` is the gradient of none.
- **[[wiki/entities/two-body-dense-associative-memory.md]]** — this page's visible/hidden bipartite skeleton run deterministically: currents descend a Lyapunov energy instead of sampling at rate `exp(−E/T)`, activations are gradients of per-layer Lagrangians (so softmax and divisive normalisation are admissible), and the hidden layer's job is capacity rather than expressiveness — with no write rule offered, which is the half this page supplies (Krotov & Hopfield 2021).
