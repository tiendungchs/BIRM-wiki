...
- **Task-set selection is a posterior over a set the model already holds, and building the set is the other half.** Retrieval works because the candidate task-sets are enumerable and the Chinese-restaurant prior supplies the allocate-vs-reuse call; run against a *discovered* library rather than a handful of latents, neither is available, which is `G37`.
...
- **[[wiki/entities/affordance-active-inference-model.md]]** — the continuous rival for the same phenomenon: context as a slow attractor state that sets the *rate* of a lower generator, rather than as a cluster grown by a Chinese-restaurant prior and selected by a gating action; only the second predicts that the switch rate has an anatomical address whose depletion changes reaction time with a *sign* that depends on hierarchical level.
