...
**And it is the second-moment truncation of a larger objective, which is the cleanest available account of both its strength and its collapse boundary** ([[wiki/entities/lejepa.md]], Balestriero & LeCun 2025). Substituting the degenerate univariate test `T(u) = mean(u)² + (std(u) − 1)²` into **SIGReg** — average an arbitrary normality statistic over `M` random 1-D projections of the embedding — **recovers VICReg in the many-slices limit**, because that test enforces exactly `E[Z] = 0` and `Cov(Z) = I` and the invariance term is already a squared distance between views. So the wiki's "dimension-contrastive vs distribution-matching" split is one family indexed by *how much of the target distribution the test constrains*: two moments here, all of them under an Epps–Pulley characteristic-function test. The argument against the truncation is stated as identifiability — finitely many moments do not determine a distribution (Carleman), so a family of non-Gaussian solutions satisfies this objective exactly and the encoder is free to sit anywhere in it, "a phenomenon already observed in VICReg". The counter-argument the same source supplies is stability: moment-based tests have gradient norms scaling `O(k)` and gradient variance `O(k² m_{2(k−1)})` in the moment order, so accuracy and trainable gradients trade off — and at `k = 2` this page sits at the stable, under-identified end.

**And the under-identified end is the one that measures better** (Akbar 2026, [[wiki/entities/lejepa.md]] §Part 8, `(tentative)`). This page's covariance term, computed on a random sketch `S ∈ R^{K×C}` so that it costs `O(CK)` and can be applied to internal hidden layers, is put head to head against the full characteristic-function test on CIFAR-100 and matches or beats it in 5 of 8 settings — `+1.8` on a collapsed ViT, `+6.2` on a 6-layer MLP with no batch normalisation and no residual connections, `+4.4` on ViT+Muon — losing only on already-stabilised baselines and by ≤1.1. Two consequences for this page. The identifiability objection above is real but has not been shown to *cost* anything, so the second-moment truncation is currently the better-priced rung ([[wiki/empirical-tensions.md]] T171). And the covariance term generalises past its own setting: sketched, it works as a **supervised optimisation stabiliser** with the invariance term and the second branch both absent, which locates its effect in conditioning the representation density rather than in anything about pairs of views.
...
- **[[wiki/entities/vl-jepa.md]]** — the setting this page's per-branch argument was made for, built at scale and regularised the other way: two branches with different weights, architectures *and* modalities, currently held apart by a shared-batch InfoNCE uniformity term rather than by per-branch variance and covariance.
...
- **[[wiki/entities/simsiam.md]]** — the same two statistics reached by the update rule alone in the *minimal* system (per-channel std at `1/√d`, no EMA, no term), and the third and fourth data points for the composability row: the predictor + stop-gradient graft is inert on SimCLR and mildly negative on SwAV, which is where this page's neutral result sits in the pattern (T166).
...
- **[[wiki/concepts/alignment-uniformity.md]]** — the counterparty to this page's `l2` row: the contrastive lineage argues ℓ₂ normalisation onto the sphere is *necessary* (an unnormalised softmax is sharpened by scaling alone) and that spherical caps are linearly separable where Euclidean clusters are not, against this page's measurement that not normalising is the best setting and the sphere costs 3.5 points (T304).
...
- **[[wiki/entities/hit-jepa.md]]** — a sharper version of this page's certification limit: variance and covariance terms applied per level inside a three-level JEPA fail to prevent a factor-465 informational collapse caused by changing only the *inter-level wiring* (upsampled embeddings concatenated downward instead of attention maps), with the objective, coefficients and branches untouched — so a per-branch provision certifies neither the trajectory nor a representation assembled across branches (Li et al. 2025).
- **[[wiki/entities/lejepa.md]]** — the generalisation this page is a special case of: SIGReg with a two-moment test *is* this objective, so the difference is not a rival family but how many moments of `N(0, I)` are pinned, and the extra moments buy identifiability (no non-Gaussian solution satisfies the characteristic-function test) at no extra coefficient. — and the head-to-head that follows favours this page's rung: the covariance term on a random sketch matches or beats the full characteristic-function test in 5 of 8 supervised settings, so the extra moments buy identifiability without yet buying accuracy (T171).
- **[[wiki/concepts/representational-collapse.md]]** — locus 2 split into its two ablatable halves — variance against norm collapse, covariance against informational collapse, neither substituting for the other — and the footnote that narrows certification to the target rather than the trajectory.
- **[[wiki/entities/directclr.md]]** — the failure this page's covariance term is a direct provision against, measured in the space it is applied to: a trained contrastive embedding is rank-deficient by default, and the counter-claim is that the embedding is the wrong place to care about it (T314).
