...
**Note on NM3.** The covariance-matched Gaussian null matches the data's second-order structure but not its **power spectrum**, and resting BOLD's long-range temporal autocorrelation is exactly what shrinks the effective sample size. The stronger null in the literature matches covariance, spectrum *and* run length, and at that strength it reproduces published sliding-window state inventories at `r = 0.96` ([[wiki/concepts/stationary-surrogate-null.md]]). The same source is otherwise **favourable** to this model's central design choice: once motion and drowsiness are removed, resting BOLD is adequately described by one correlation structure, which is the premise `J = −Σ⁻¹` rests on.
...
- **[[wiki/concepts/control-unity-and-diversity.md]]** — the empirical warrant for treating `J` as a fixed property of the system rather than of the scan: the same prefrontal coupling structure is recovered under a blank fixation cross and under a five-minute animation (per-subject representational similarity `r_z = 0.90`, scalar `r = 0.88`), its mean strength predicts conflict-task performance in both 4–5-year-olds and adults, and it grows across that developmental interval — so the landscape this page perturbs with tasks has a behavioural meaning and a developmental trajectory of its own (Eng et al. 2025).
- **[[wiki/concepts/attractor-dynamics.md]]** — supplies the orthogonalised regime, the `β`-as-prior-precision reading and the symmetric/antisymmetric split that separates the landscape from the traversal over it.
- **[[wiki/concepts/mean-field-reduction.md]]** — names the rung this page occupies and what was paid for it: a connectome-as-coupling-matrix model is a neural-mass/field object whose units are populations, so the *coupling between moments* discarded at that rung — a stimulus contracting spike-timing variance and driving kurtosis super-Gaussian while parameters stay fixed — is exactly what its relaxation cannot represent about the spiking networks underneath (Deco et al. 2008).
- **[[wiki/concepts/dynamic-repertoire.md]]** — the strongest standing caution on this model's input: the functional connectivity `Σ` is estimated over a long window, and structure–function correspondence is *itself* a function of that window — short windows reveal configurations the anatomy does not predict, and within-subject FC test–retest reliability is low in a way a realistic generative model reproduces — so the four attractors may be averages over a repertoire rather than states of it ([[wiki/empirical-tensions.md]] T240).
- **[[wiki/concepts/metastability.md]]** — supplies the working-point account this model's landscape lacks: sweeping global coupling gives one attractor at low gain, a multistable band at intermediate gain and a saturated "epileptiform" state at high gain, and empirical resting data are matched at the *lower border* of that band — which is where its own basin-boundary ghost attractor comes from, and a caution that its `J`, estimated from BOLD, is read through a 0.35 Hz low-pass of population rate.
- **[[wiki/concepts/node-definition-problem.md]]** — audits the two commitments this model makes before its dynamics start: a 122-unit parcellation (a granularity choice that moves content between nodes and edges) and a between-subject correlation matrix in which per-subject differences in where the units sit are reported as differences in coupling unless areal-feature-based registration removed them.
- **[[wiki/concepts/effective-connectivity.md]]** — the alternative way to obtain this model's coupling matrix, and what the descriptive route forfeits: couplings taken as the negative inverse covariance of regional time series are symmetric and unsigned by construction, so the three quantities that carried the clinical signal in generative-model fits — edge direction, edge sign, and the within-region recurrent self-weight — are structurally unavailable here.
- **[[wiki/entities/virtual-brain-twin.md]]** — the same ambition built the generative way, and the contrast worth carrying: personalised tractography weights *and* delays instead of a group correlation statistic, an explicit subject-specific source-to-sensor observation model instead of none, and two or three free parameters inverted against that individual's recordings instead of a landscape read off the data it was fitted to.
- **[[wiki/concepts/function-to-structure-inference.md]]** — the sharpest caution on this model's coupling estimate: functional connectivity estimated over a long window is contaminated by interregional distance and by two-step path sums, and disagrees with itself across two 8-minute windows even in a simulation whose coupling matrix is literally constant.
- **[[wiki/concepts/stationary-surrogate-null.md]]** — the null that both licenses and bounds this model: a single resting correlation structure survives it, which is what a fixed `J` needs, while the dynamics rows above are scored against a covariance-matched Gaussian that does not match the BOLD power spectrum and therefore under-states sampling variability (T240).
