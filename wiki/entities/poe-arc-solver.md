...
- **[[wiki/entities/varc.md]]** — the same multi-frame consolidation with a weaker aggregator and a much smaller model: 510 augmented views reduced by *majority vote* rather than by a product of likelihoods, where this page's own ablation says the product is worth ~5 points over the mean and ~8 over `max` — and VARC's pass@300 = 66.3 against pass@2 = 54.5 is exactly the headroom that aggregator choice addresses.
