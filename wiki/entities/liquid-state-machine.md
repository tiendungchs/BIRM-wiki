...
- **[[wiki/concepts/learnable-synaptic-delays.md]]** — the opposite use of the same temporal richness: the liquid gets its heterogeneous delays and time constants for free from an untrained circuit and puts all task content in the readout, while there the delays *are* what the gradient adjusts. The measured size effect suggests they substitute — trained delays helped exactly where units were scarce and gave no benefit to large networks — which is the sharpest available argument for when a reservoir is worth its unit count.
- **[[wiki/entities/sigma-pi-reservoir.md]]** — the monolithic liquid taken apart: this page's separation property comes from one richly heterogeneous spiking circuit that buffers and expands at once, where Kleyko et al. 2025 put the buffer in a bare permutation ring and the expansion in a Pi-neuron network computing an explicitly chosen set of polynomial orders — so the biological richness is being asked to supply, implicitly and unconfigurably, a feature space that can instead be specified, priced in neurons and synapses, and diagnosed by inspection when it lacks the target's features.
- **[[wiki/concepts/neuron-complexity-index.md]]** — the same bargain relocated inside the unit: if a cortical cell's 5–8 equivalent layers are biophysically fixed and only its synapses learn, each neuron is a private fixed nonlinear spatiotemporal expansion with a trained read-in — a liquid per neuron, with evolution rather than randomness choosing the expansion **(brainstorm)**.
