...
- **[[wiki/concepts/perception-reasoning-dissociation.md]]** — this page's design is that family's zero-reasoning limit: removing the rule gives an absolute perceptual floor where substituting the parse gives only a delta (+11 to +12.5 points), and the two disagree usefully — a floor this large should have been worth more than 12 points to repair, which is the evidence that a uniform verbal description is not the parse that was missing.
