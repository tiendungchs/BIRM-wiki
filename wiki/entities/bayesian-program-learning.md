...
- **[[wiki/concepts/language-of-thought.md]]** — a probabilistic-language-of-thought instance in all but name, and now named as one by the hypothesis's own statement (Goodman, Gerstenberg & Tenenbaum 2024 list character learning among the behavioural demonstrations of concept learning as program induction): a prior over structured expressions with concepts as posterior programs, which is the format claim made in a domain where the generative process is observable. That page also supplies what this one lacks — a route by which the authored relation types could be *learned*, via library learning over solved tasks (DreamCoder), which is compression-driven and therefore disputed (T155).
- **[[wiki/entities/neo-neural-theorizer.md]]** — the same library/program factorisation with the authorship removed: relation types are given here by the prior, and there nothing is given at all — symbols are VQ codes, their semantics come from a shared executor, and program length is chosen by MDL, at the price of synthetic domains and no binding.
- **[[wiki/entities/ilp-arc-synthesizer.md]]** — the same authored-library / per-instance-program split with two substitutions: the per-instance object is a set of induced Horn clauses rather than a sampled stochastic program, and execution writes onto a shared canvas where earlier objects occlude later ones, so composition is order-dependent.
- **[[wiki/entities/dreamcoder.md]]** — the same two-level program/library factorization with the second level *unfrozen*: where BPL's primitives are fixed after pre-training and its relation types are given by the prior, DreamCoder rewrites the library every cycle by compressing refactorings of its own solved programs, which turns this page's installed hierarchy into a grown one.
