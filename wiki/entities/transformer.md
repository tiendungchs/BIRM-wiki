...
| **Fixed depth ⇒ not computationally universal** | The number of *sequential* operations is the layer count, independent of input size, so at finite precision there is for any `N = 6` stack an input length it cannot process sequentially; sharing one layer's weights across all steps removes the limit, because `T` can then be set as a function of input length ([[wiki/entities/universal-transformer.md]]) |
...
- **[[wiki/entities/universal-transformer.md]]** — the same layer with its stack tied and applied `T` times, which is the minimal change that makes depth a run-time variable: at fixed `T` it is exactly this architecture with shared layers, and letting `T` grow with input length is what supplies the computational universality a constant-depth stack cannot have — plus +0.9 BLEU EN→DE at matched parameters and 0.35–0.46 sequence accuracy on length-400 copy/reverse where this model is at 0.03–0.06.
...
- **[[wiki/concepts/fast-weight-programming.md]]** — the storage side of this architecture, which its own paper never states: concatenating immutable key/value pairs gives capacity growing with `L` and no way to edit an entry, and the linearised variant that trades that for constant state is formally a 1991 fast-weight programmer — so the softmax's real purchase is unbounded, uneditable storage at quadratic read cost (Schlag et al. 2021).
- **[[wiki/entities/lru.md]]** — the cheapest available counterexample to this paper's maximum-path-length argument: `N` independent scalar complex exponentials with `2N` recurrent parameters, `O(L)`-deep information flow, no content-dependent scoring and eigenvalues drawn uniformly from a ring, matching state-of-the-art on all six Long Range Arena tasks (Path-X 94.2% where all 11 attention variants are at chance) — which removes the last reading under which a structured operator could be seen as substituting for `O(1)` hops, and leaves the content-addressed read as attention's sole unrefuted function ([[wiki/empirical-tensions.md]] T179).
- **[[wiki/entities/scan.md]]** — the compositional splits that predate this architecture: every model scored there is a 2018 recurrent seq2seq, so the transformer's numbers on `add jump` and `length` reach the wiki only through later sources ([[wiki/entities/mlc.md]], [[wiki/entities/pcfg-set.md]]).
- **[[wiki/entities/rims.md]]** — this architecture inverted on both of its defining choices: the attention parameters sit on the **module** side rather than the input side (private per-module queries against shared input keys/values), and elements do **not** interact by default — only the top-`k_A` modules that win the input competition update at all. It loses at the training resolution on sequential MNIST (90.0 vs 91.2) and wins at all three held-out resolutions (73.4/38.1 vs 51.6/22.9), which is the signature of an inductive bias rather than of capacity (Goyal et al. 2019).
- **[[wiki/entities/esbn.md]]** — the strongest alternative on withheld-entity relational tasks and an informative failure: self-attention holds up while entities are plentiful but degrades significantly under an untrained random-projection encoder (`t = −4.0`, `p = 0.001`) where the ESBN does not, showing its relational generalisation rides on how entities are encoded rather than being abstracted away from them.
- **[[wiki/entities/varc.md]]** — how little has to change for this architecture to reach the reported average human on ARC-AGI-1: a separable 2D positional code, a 64×64 canvas with 2×2 patches, and one learnable task token — no modification to attention, depth or training objective, with the score coming from the input format and the augmentation group instead.
- **[[wiki/entities/continuous-modern-hopfield-network.md]]** — the post-hoc theory of this page's operator, and it re-reads two of its design choices: `1/√d_k`, introduced here purely to stop large `d_k` saturating the softmax, is an inverse temperature selecting whether a head returns one entry, a cluster mean, or the mean of everything; and multi-head's stated motive — repairing the "reduced effective resolution due to averaging attention-weighted positions" — is that store's global-fixed-point regime named from the other side. It also makes iterating a layer licit, since the update monotonically descends an energy (Ramsauer et al. 2020).
