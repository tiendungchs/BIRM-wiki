...
- **All corruptions are 2-D image-space operations** — none changes viewpoint, occlusion, or object–context relations, which is exactly the shift class [[wiki/entities/objectnet.md]] was built for the same year, by randomising those parameters *at capture* instead of transforming a finished image.
...
- **[[wiki/entities/stylized-imagenet.md]]** — the source of this page's best single augmentation result (mCE 76.7 → **69.3**, 13 of 15 corruptions), and the same period-level story read off an unrelated instrument: shape bias falls AlexNet → ResNet-50 over exactly the model sequence whose `Relative mCE` never improves, so accuracy progress and rule-quality progress came apart twice, measured two different ways.
- **[[wiki/entities/objectnet.md]]** — the disjoint half of the nuisance space this page's limitation names: 15 corruptions that are all functions of the array, against three controls (background, rotation, viewpoint) that are not producible from an existing photograph at all — and the same 2019 verdict from the other side, a 40–45% differenced drop that tracks but never closes across the identical AlexNet → PNASNet model sequence.
- **[[wiki/entities/pgm.md]]** — the completeness contrast for this page's shift family: there the family *is* the generator's own `[relation, object, attribute]` structure and is complete by construction, here it is the 15 corruptions the authors could implement, so "general robustness" is general over an authored set.
