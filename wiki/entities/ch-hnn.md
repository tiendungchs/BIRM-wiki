...
- **[[wiki/entities/elastic-weight-consolidation.md]]** — the protection half of this page's substitutes result, at full strength and from its own source: the same mechanism that adds 0.02 points here when episode inference is accurate carries continual learning on its own in a fixed-capacity network with no gate at all, which is the cleanest statement of why the two are insurance against the same event rather than complementary defences.
