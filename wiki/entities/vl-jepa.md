...
- **[[wiki/concepts/representational-collapse.md]]** — the continuous version of the non-term entry: the target branch's own learning supplies the asymmetry, with the decoder invoked only on demand.
- **[[wiki/entities/cpc.md]]** — the origin of the InfoNCE score this page keeps: the same log-bilinear discrimination against in-batch negatives, with the target encoder being the *same network* rather than a separately trained one in another modality.
