...
- **[[wiki/entities/lejepa.md]]** — the in-domain counter-case to this page's corpus argument: a 21M ResNet trained from scratch on 11k galaxy images beats this encoder under frozen probing (78.2 vs 67.6), full finetuning (83.3 vs 78.3) and at every shot count down to 1 per class, while losing on natural-image data of similar statistics — so transfer's advantage is distributional proximity rather than corpus size (T154).
- **[[wiki/concepts/alignment-uniformity.md]]** — the middle term of this page's three-shapes unification with a derivation attached: a contrastive loss's spreading half is a von Mises–Fisher kernel entropy estimator *with* negatives (bandwidth `κ = 1/τ`), where KoLeo is the nearest-neighbour estimator with none.
- **[[wiki/concepts/representational-collapse.md]]** — locus 5 on the target side — teacher centering and Sinkhorn–Knopp equipartition in place of a predictor — plus the pricing showing that a spreading provision buys one read-out and not others.
- **[[wiki/concepts/manifold-untangling.md]]** — where this page's linear-probe protocol comes from and how the biology tightens it: cap the reader, then test generalization across the transformations themselves (position shifts, 0.33×–3× scale, changed context), which is the axis a held-out-sample split does not probe.
- **[[wiki/entities/imagenet-c.md]]** — the benchmark family behind this page's robustness column, with the differencing it needs: a curated-vs-uncurated corpus gap of 14 points is a *data-lever* purchase of corruption robustness, and it is only interpretable against the clean-error gap on the same models, which is the arithmetic that killed the architecture-side story there.
- **[[wiki/entities/mae.md]]** — the recipe risk this page took, priced on a third axis: DINOv2's design decisions were selected on a frozen `k`-NN probe, and MAE shows that read-out *depth* 0 vs 1 reverses the ordering of two encoders outright (73.5 → 81.0 from one block; MoCo v3 wins frozen and loses adapted) — so probe class, probe checkpoint and probe adaptation budget are three free parameters, each with enough leverage to steer an architecture (T310).
