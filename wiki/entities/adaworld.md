...
**A monolithic action code, of either kind, does not compose — and this is now a controlled result.** [[wiki/entities/neo-neural-theorizer.md]] re-implements AdaWorld's design as its `Cont-Mono` baseline (conditional β-VAE, one continuous vector per pair) and the Genie/LAPO design as `Disc-Mono`, on a benchmark that holds out *compositions* rather than scenes. Both score **0.000** transferability on compositional-OOD and length-OOD splits; a third variant that refines the continuous code by test-time gradient ascent (LPN-style) reaches 0.994 self-explainability with 0.000 transfer, i.e. it perfects the fit to the pair it was given and learns nothing reusable. A program over a shared executor scores 0.933 under identical conditions. Two caveats keep this from being a refutation of the page above: the domains are synthetic (10×10 GridWorld, CIFAR-10 photometric edits, integer factorisation), and the baseline is AdaWorld's *representation class*, not its 2000M-frame training run. What it does establish is that AdaWorld's own transfer test — take a code from one video, apply it in another scene — is the *easy* axis: it asks whether a code generalises across contexts, never whether two codes combine into a transformation the training set did not contain.
...
- **[[wiki/entities/v-jepa-2.md]]** — the complementary half of the same problem, with the same planner: this page induces the action *alphabet* and predicts in pixels; V-JEPA 2 is handed the alphabet (7-D end-effector deltas) and induces the *dynamics* at scale in embedding space, then runs cross-entropy-method MPC against a goal embedding rather than a goal-image cosine. Neither induces both halves, and only one of them has been driven to a real robot.
- **[[wiki/entities/gcq.md]]** — the exact complement, and neither has both halves: this page induces an edge-label alphabet from 2000M unlabeled frames over frame pairs that are never given nodes; GCQ installs a node set (the `K` bumps of a toroidal CANN) and writes the action→displacement map by hand. Both are evidence for T148 from opposite directions — a continuous code beating a VQ-8 codebook here, a discrete *metrically ordered* codebook beating its own learnable version there.
- **[[wiki/entities/neo-neural-theorizer.md]]** — the controlled negative result on this page's representation class: re-implemented as the `Cont-Mono` baseline and scored on held-out *compositions* rather than held-out scenes, a monolithic induced action code transfers at 0.000 — and test-time gradient refinement of it does not help, which localises the deficit in the format rather than in the inference.
- **[[wiki/entities/spelkenet.md]]** — the mirror image on the spatial axis: there the induced latent action conditions a *whole frame* and the output is a predicted future, here a single hand-specified flow vector at one named pixel conditions the same kind of model and the output is a *partition* — both recover a vocabulary nobody labelled, one of transitions and one of entities.
- **[[wiki/concepts/cross-embodiment-transfer.md]]** — where this design ended up in practice: latent-action induction from video is now the standard route for pretraining robot policies on action-free human data (LAPA, Moto, UniVLA, UniSkill, and LAPA inside GR00T N1), i.e. one of five families that share a level above the body and keep only a per-embodiment decoder below it (Kawaharazuka et al. 2025).
- **[[wiki/concepts/information-bottleneck.md]]** — the principle behind the `β = 2×10⁻⁴` latent-action encoder, and the reason for its one structural blind spot: the relevance variable is *the next frame*, so the bottleneck keeps whatever predicts it and has no term that could separate an agent's action from the weather — a property of the objective's second argument, not of the architecture.
