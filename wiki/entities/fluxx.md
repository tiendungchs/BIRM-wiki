...
- **[[wiki/concepts/external-verification.md]]** — the degenerate case of an arbiter: where [[wiki/entities/nomic.md]] specifies a rotating Judge with an escalation chain and an unanimity override, this game's published dispute procedure is an **email address**, so every rule conflict is resolved off-line by the publisher and nothing in the played system decides it. That is what makes the concurrent-rule-load failure a *verification* failure and not only a memory one: the players have no in-game way to check a combined effect they have lost track of.
