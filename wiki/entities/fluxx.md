...
| Self-amendment | None (predicate fixed) | **None either** — the override rule and the turn machinery are not editable. That tier is Nomic's |
...
**7 · It does *not* test self-amendment.** No card edits the recency-override rule, the turn sequence or the win check. Fluxx is a two-tier domain with a very rich middle tier, not a three-tier one — gap **G9** is untouched, and `Fluxx Blanxx` (blank cards filled in by the players) is the only gesture at it, from outside the game rather than inside it.
