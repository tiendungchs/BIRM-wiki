...
**The size argument is stated quantitatively and belongs to the wiki.** AIME ships 30 problems across its two administrations, so one item is worth 3.33 points and the binomial standard error is ≈2.6× that of a 200-item benchmark — and the instrument as *administered* is 15 items per contest ([[wiki/entities/aime.md]]), i.e. 6.67 points each and ≈3.7×, which is the figure that applies whenever a single contest is scored. Every AIME number the wiki carries is quoted to a precision its `n` does not support (see F9, added to [[wiki/concepts/certification-instruments.md]] from this source).
...
|---|---|---|---|
| Never publish the items | [[wiki/entities/frontiermath.md]] | **No** | No (G17) |
...
**The protocol has a primary source, and one step of it is unavailable here.** The `n`-gram-against-paraphrase `δ` originates with Xu et al. 2024 ([[wiki/concepts/benchmark-contamination.md]]), where it is one half of a pair: `δ` on the train split *minus* `δ` on the test split, precisely because the difference cancels the bias the paraphrasing model itself introduces. OlymMATH has no train/test split, so it reports an uncancelled `δ` — which is exactly why the limitation below is the right one and why only cross-benchmark comparison at a fixed rewriter is licensed. The magnitudes agree where the two studies overlap: Qwen-7B reads `δ_test = 14.35` on GSM8K there against Qwen2.5-7B's 17.59% here.
...
- **[[wiki/concepts/benchmark-contamination.md]]** — the primary source for this page's `δ` metric, and the step it is missing: `δ_train − δ_test` is what cancels the paraphraser's own bias, and a benchmark with no train split cannot compute it — which is the mechanism behind this page's "relative comparisons only" caveat.
- **[[wiki/empirical-tensions.md]]** — T204 (the compute-matched budget/score control that the released 582k trajectories would settle).
- **[[wiki/architectural-gaps.md]]** — G17 absorbed both of this page's instrument disputes: whether a cross-lingual score gap measures the model's reasoning or the harness's answer extraction, and this benchmark as the third position on contamination defence — auditable, measured, and admittedly perishable.
...
- **[[wiki/entities/hle-verified.md]]** — the comparability problem this page names, at three editions instead of two: raw HLE, HLE-Verified and HLE-Rolling share no anchor subset, so a score quoted against "HLE" is now ambiguous by ~8–11 accuracy points before any model difference is read.
- **[[wiki/entities/hle.md]]** — the same benchmark-as-renewable-process position adopted under a different name (HLE-Rolling, a continuously updated fork announced once frontier models approach the noise ceiling), inheriting this page's unsolved comparability problem: successive editions share no anchor subset, so a trend across refreshes means nothing.
- **[[wiki/entities/aime.md]]** — the benchmark EASY was built to be equivalent to (DeepSeek-R1: 79.8 there, 79.6 here) and the primary source for the resolution argument this page contributed second-hand; it also supplies the human denominator this page declined to collect, and the reason the comparison to "AIME 2024" is a comparison to one particular year's item set rather than to a scale.
- **[[wiki/concepts/selective-prediction.md]]** — the Pass@k / Cons@k table read as a two-point risk–coverage measurement, and this benchmark's sharpest contribution to it: the 1.5B `Pass@64 = 30.0 / Cons@64 = 0.0` result is a confidence rate that is *anti*-monotone in loss, the empirical negation of the loss-monotonicity assumption every selective classifier rests on (`T320`).
