...
- **[[wiki/concepts/fast-weight-programming.md]]** — the same replace-by-reading-first in a differentiable store: `− v̄ ⊗ φ(k)` is the algebraic form of this model's post-before-pre depression of the overwritten cue's old associations, with orthogonality of keys doing the work the refractory period does here — and both end up with `replace` only (Schlag et al. 2021).
