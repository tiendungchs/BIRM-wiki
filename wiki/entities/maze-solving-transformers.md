...
- **[[wiki/entities/tem-transformer.md]]** — the same architecture approached from the opposite end: here a generic decoder is probed and found to hold a graph it navigates unreliably, there a decoder is *constructed* so its position encodings must path-integrate — which moves the graph from something the objective may induce to something the update rule guarantees.
- **[[wiki/entities/differentiable-neural-computer.md]]** — the control condition in the other direction: the same class of graph-traversal task solved *with* an external read/write matrix, which makes the external store a scaling-and-interference argument rather than a possibility argument, since a whole maze fits in one token's residual stream here.
- **[[wiki/entities/s4.md]]** — the same connectivity question with the opposite algorithm, and the wiki's cleanest traversal-versus-diffusion contrast: an attention head placing weight on tokens at path-length 1 is an explicit in-context adjacency that is then walked, where S4 decides path-connectivity on Path-X (length 16,384, 96.35%) with a fixed global convolution and no adjacency in the model at all — so a relational query over a graph does not require a representation of the graph's edges (Gu et al. 2022).
- **[[wiki/entities/transformer.md]]** — the causal, head-level result the founding paper asserts without showing: "individual attention heads clearly learn to perform different tasks … related to the syntactic and semantic structure of the sentences" is one prose sentence backed by dropped appendix visualisations, where this page identifies an adjacency head by direct logit attribution and ablation (Vaswani et al. 2017).
- **[[wiki/concepts/certification-instruments.md]]** — instrument `I7`'s demonstration case, and the measurement that splits it from every behavioural instrument: a maze decodable at 0.83–0.99 per wall on a model that still leaves the graph on 13–16% of rollouts.
