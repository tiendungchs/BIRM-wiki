...
- **[[wiki/concepts/offline-replay.md]]** — the offline job the same tissue is doing: auditory-cortical ensemble patterns in the 400 ms before a hippocampal ripple predict that ripple's content, and which cortical ensemble reactivates is set by cortical rather than hippocampal dynamics — so the days-long rewriting of AC measured here is a candidate writer of the consolidation channel's selection variable (Rothschild et al. 2017).
