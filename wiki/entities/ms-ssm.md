...
- **[[wiki/concepts/timescale-estimator-bias.md]]** — an estimator for this architecture's own design object: the mixing weights `c_k` of an Ornstein–Uhlenbeck mixture (`Σ c_k = 1`, at fixed total variance) are exactly the variance allocation across horizons that this model's stratification result makes load-bearing, and a Bayes factor over mixtures decides how many bands a signal warrants — so a band structure can be proposed and scored against data rather than only authored (`G67`, `G114`).
