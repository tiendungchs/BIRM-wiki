...
- **[[wiki/entities/lateral-frontoparietal-network.md]]** — a candidate substrate for stage 1, and a warning about it: ventrolateral prefrontal cortex retrieves and selects semantic content for the frontopolar integrator, and its measured sensitivity is to *associative strength* — the same surface quantity MAC's content vector uses — so the anatomy matches this model exactly where the model is known to be structure-blind (Parsons & Davies 2022).
- **[[wiki/entities/remerge.md]]** — the downstream consumer this page's stage-one result undermines: a first-pass similarity sweep over a large store returns surface matches at .53 against .12 for structural ones, so what enters that model's settling loop is mostly wrong before the loop starts.
