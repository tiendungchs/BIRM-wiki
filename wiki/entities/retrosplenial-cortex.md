...
## The primary recording: three frames in one population (Alexander & Nitz 2015)

> **Provenance.** Alexander & Nitz 2015, *Retrosplenial cortex maps the conjunction of internal and external spaces*, Nat. Neurosci. 18(8):1143–1151 (`raw/alexander-2015-retrosplenial-conjunction-of-internal-and-external-spaces.md`). The study the 2023 review's route-centered frame rests on, and the wiki's first recording in which the candidate frames are **estimated in the same neurons**.

**Design.** Six rats, a `W`-shaped track (215 cm, turns at 60/110/170 cm, walls <2 cm so distal cues stay visible), run as left-right-left (`LRL`) outbound and right-left-right (`RLR`) inbound routes. The whole track is then *moved* to a second room location and back (α → β → α′). The three frames are dissociated by construction: turn direction is egocentric, position-along-route is route-centric and invariant to the move, room position is allocentric and changes with it. 228 retrosplenial neurons (133 left / 95 right hemisphere, dysgranular and granular `b`/`c`, effects distributed across subregions) plus 109 CA1 neurons in the same animals.

| Frame | Measure | Result |
|---|---|---|
| Egocentric (left vs right turn) | Significant L/R rate difference | **45.6%** (104/228); 22% with bias ratio >2; 78/104 above shuffle |
| — *purely* egocentric | No rate difference across the six instances of the preferred turn | 21/104 cells |
| Route-centric | Of the 124 turn-*insensitive* cells: odd/even positional-profile reliability `r > 0.4` | 65/124 (52.4%) |
| Route-centric, allocentrically invariant | α vs β positional-profile correlation `r > 0.4` | 27/124 (21.7%) — shuffle gives 4.32 ± 1.7% |
| Route position as *gain* on action | Of the 104 turn-sensitive cells: rate depends on **which** instance of the preferred turn | 45 cells (19.7% of all); not explained by angular or linear velocity |
| Allocentric | Head-direction tuned | 6% of all cells |
| Allocentric (profile correlation) | α/β vs α/α′ | `μ = 0.16` vs `μ = 0.34`; α/α′ > α/β for **70%** of cells; CA1 α/β `μ = 0.07` |
| Ensemble reconstruction error | α → α′ | RSC **4.51 cm**, CA1 7.82 cm |
| Ensemble reconstruction error | α → β | RSC **51.32 cm**, CA1 63.45 cm |

**The structure of the result is a nesting, not a mixture.** The same cell's turn-direction response is scaled by *where in the route* the turn occurs, and that route-position modulation is in turn scaled by *where the route sits in the room*. Egocentric action is the carrier; route position and allocentric location are gains on it. No cell had an isolated place field. The two ensemble rows say the population is simultaneously as good as CA1 at telling α from β and far better than CA1 at reconstructing route position within a location — the two codes coexist in one 228-cell population without either being recoverable as a separate sub-population.

**Comparison to the two structures it sits between:**

| | Posterior parietal cortex | Retrosplenial cortex | CA1 |
|---|---|---|---|
| Turn/action coding | Yes | Yes (45.6%) | — |
| Route-position coding | Yes | Yes | — |
| Sensitive to the route's allocentric placement | Largely **no** | **Yes** | Yes |
| Isolated place fields | No | **No** | Yes |

The region is the *product* of its two main inputs rather than a relay of either: it has parietal route/action coding **with** the hippocampal sensitivity to room position that parietal coding lacks, and it has that sensitivity **without** the place fields that carry it in the hippocampus.

**The authors' own mechanism is a basis-function network, not a converter.** Multiplicative modulation of a response in one frame by position in another is the gain-field construction ([[wiki/concepts/reference-frame-transformation.md]]); a population of such units approximates arbitrary nonlinear maps between the frames it spans, and any downstream structure can read out current action, route position, room position or any combination by choosing weights. They call the coding *category-free* — the same construct [[wiki/concepts/population-geometry.md]] calls non-linear mixed selectivity, here with the mixed variables being reference frames rather than task conditions.

**One proposed downstream use, stated as a capability rather than a result.** Retrosplenial output could detect the offset between where self-motion integration expects distal cues to be and where they are, and drive **re-anchoring of grid and place maps** through its projection to medial entorhinal layer V — including *resets* at turns, which is where place and grid patterns are known to reset on repeating routes ([[wiki/concepts/path-integration.md]], `G39`). Visual-cue responses were not tested here.
...
**(brainstorm) The measured code is a *product*, and that is a third answer to `T47` neither position offered.** `T47` asks whether concurrent structural codes are independent frames or one code read out in many places. The 2015 recording is the first in the wiki to put candidate frames in the *same neurons*, and the answer is neither: the frames are not separate populations (against A), and they are not one code with readouts elsewhere (against B) — they are **multiplicatively combined within single units**, egocentric action gated by route position gated by allocentric location. A product code has an architectural consequence the wiki has not drawn: *there is nothing to arbitrate*. Disagreement between frames is representable only if the frames have separate carriers; in a basis-function population a "conflict" is simply a different point in the joint space, with its own response. So `G43`'s arbitration rule and this population are answers to different questions — and the bank in [[wiki/concepts/reference-frame-transformation.md]] inherits the same property from the opposite direction (one parameter bump, so conflict is undefined).

**(brainstorm) Which makes the localised-converter reading worse, not better, even though this is Position A's own region.** A converter takes a code in frame `E` and returns the same content in frame `A`. What is measured here is a population spanning three frames at once whose units are their conjunctions — the raw material a downstream reader uses to *compute* a conversion, not a module that has performed one. Alexander et al.'s later definitional objection (`T383`) is visible already in their own 2015 data: every cell reported is consistent with co-representation, and the paper's positive claim is explicitly about what downstream structures *could* extract. **The transferable rule: a basis-function population is a converter only relative to a stated readout, so the converter is the weight matrix, not the region.**
...
- **The 2015 study moves the track, it does not put cues in conflict.** α and β are two consistent configurations, so the design measures *conjunctive coding* and says nothing about arbitration; the cue-conflict deficit and the conjunctive code are still evidence from two non-overlapping paradigms.
- **The source of the route code is unidentified** — track geometry as it changes across a traversal, integration of the action sequence, or both; the authors decline to choose, and visual-cue responses were never tested.
- **The three frames are dissociated behaviourally, not statistically.** Cell counts per frame come from separate significance tests on the same 228 cells, with no joint model apportioning variance across frames, so "45.6% egocentric" and "21.7% route-invariant" are overlapping thresholded categories rather than a decomposition.
- **One track shape, one lab, six rats**, and the route frame rests on `LRL`/`RLR` sequences that confound route position with turn ordinality.
...
- **[[wiki/concepts/population-geometry.md]]** — where this region's code is stated as a geometry rather than a tuning list: turn direction × route position × room location mixed multiplicatively in single cells is that page's non-linear mixed selectivity with reference frames as the mixed variables, which buys *maps between structured spaces* rather than implementable classifications — and the fork `G122` opens under that page's mixed-selectivity assumption: this region's afferents may spread evenly over its neurons (one mixed population) or be biased toward projection-defined sub-populations with sparse local recurrence (several semi-independent circuits sharing a volume), and the measurement that would decide it — input density per afferent source per projection class — has not been made in any species.
...
- **[[wiki/entities/bb-model.md]]** — this page's translation account as running code, which is what makes it falsifiable: 20 gain-field sublayers, each a copy of the allocentric boundary-vector population tuned to one heading, gated one-hot by total head-direction activity and read in both directions on one weight set — reproducing object-cued recall, the Papez-lesion recollection/familiarity dissociation, trace cells and mental navigation, and entailing the entorhinal result that `T383`'s Position B reports as null.
- **[[wiki/concepts/reference-frame-transformation.md]]** — the operation this page's two reviews argue about, stated independently of the region: a banked, externally parameterised, bidirectional conversion, whose four properties make explicit what the localised reading commits to and what Alexander et al. 2023's generalisation to arbitrary perspective shifts costs — a group with no named element set — and where this region's own recording supplies the rival implementation of the same operation, a basis-function population whose transform is chosen by the downstream weights rather than by a gate (Alexander & Nitz 2015).
- **[[wiki/entities/dorsal-visual-stream.md]]** — the route by which visual scene information arrives here, and the lesion series that brackets what this region does: `cIPL` → posterior cingulate → retrosplenial → pre-/parasubiculum and TF/TH, with heading disorientation (landmark recognised, direction not derivable, map drawable but not describable) sitting one stage past landmark agnosia and one stage before anterograde topographic disorientation — so the conversion credited here is flanked by deficits that spare it (Kravitz et al. 2011).
- **[[wiki/concepts/attractor-dynamics.md]]** — the fork this page's buffer proposal turns on: a short-term buffer holding representations while the egocentric↔allocentric conversion runs is only needed if the transform settles iteratively, so positing one is a commitment to recurrent dynamics rather than to a single feedforward rotation.
