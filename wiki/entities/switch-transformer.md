...
- **[[wiki/concepts/selective-prediction.md]]** — module-level abstention with a price on it: rerouting overflowed tokens to a second-choice expert rather than dropping them buys nothing on quality or stability, so a rejection is not free at the module level even though the risk–coverage formalism prices it at exactly zero.
- **[[wiki/entities/rims.md]]** — supplies the mechanism this model's overflow result implies and its router cannot express: rerouting a dropped token to its second-choice expert buys nothing, i.e. a wrong module is worse than none — and a comparison against an **appended null row** makes exactly that abstention the default, turning top-`k` selection from a ranking into a reserve-price test that a threshold-free argmax over `softmax(W_r x)` has no way to represent (Goyal et al. 2019).
