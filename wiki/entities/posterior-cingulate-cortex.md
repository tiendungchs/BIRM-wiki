...
- **[[wiki/entities/bb-model.md]]** — a model that places a load-bearing population, the egocentric "parietal window", in medial parietal cortex/precuneus, and so inherits this page's demarcation problem directly: the region it names is the one this page shows is not cingulate cortex at all and is the one region retrosplenial cortex does not connect to, while the transform that feeds it is assigned to retrosplenial cortex — so the model's shortest edge crosses the boundary this page establishes is real.
- **[[wiki/entities/dorsal-visual-stream.md]]** — the pathway that gives areas 23/31 a *spatial* assignment the tripartite division does not predict: as the relay before retrosplenial cortex in the parieto-medial-temporal route, posterior cingulate units encode saccade-target location in allocentric coordinates preserved across whole-body rotation, and inactivation impairs following previously learned routes — so the egocentric→allocentric conversion has begun one relay upstream of the subregion the parcellation assigns it to (Kravitz et al. 2011).
