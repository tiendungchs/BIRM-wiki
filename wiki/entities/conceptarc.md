...
The averages are the uninteresting part. **The per-concept profiles anticorrelate between modalities**, which is the discriminability this design was built for and the first time a source uses it that way on two *families* rather than two solvers: Count 0.83 image / 0.17 text, Extend To Boundary 0.50 / 0.17, Centre 0.57 / 0.33, Top and Bottom 3D 0.47 / 0.20 — against Order 0.07 / 0.27, Move To Boundary 0.17 / 0.23, Copy 0.20 / 0.23. *Extract Objects* is at the floor for both (0.23 / 0.10), matching its position as the worst concept for GPT-4 in the original table. This is the 2023 finding — "concept difficulty does not transfer across solvers" — with the solver difference reduced to a single controlled variable, the input format. One substrate caveat the page should carry: the two LTX models trail the other VDMs, and the authors attribute it to **aggressive VAE compression discarding structural information**, so a score here is partly a measurement of the video tokenizer rather than of the model.
...
- **[[wiki/concepts/abstraction.md]]** — where Barsalou's generative definition of a concept is placed as the wiki's behavioural sense of abstraction, with this benchmark as the design that operationalises it by holding a concept fixed and varying its instantiations.
- **[[wiki/entities/blindtest.md]]** — the complementary instrument at the other end of the same pipeline: this benchmark varies the *concept* with perception held easy, BlindTest varies the *perceptual clutter* with the concept removed entirely, so reporting both separates which of the two a modality gap lives in.
- **[[wiki/entities/vdm-i2i.md]]** — the clearest demonstration that this benchmark's per-concept granularity is not decoration: video-pretrained and text-pretrained solvers of the same scale produce *anticorrelated* profiles over the 16 concepts (Count 0.83 vs 0.17, Order 0.07 vs 0.27), a split that a single average accuracy (0.41 vs 0.24) would have hidden entirely.
- **[[wiki/concepts/perception-reasoning-dissociation.md]]** — the same perception-dominant attribution obtained on smaller ARC-style corpora by substituting the parse rather than by separately scoring visual errors, and it converges with this benchmark's 49.1–77.3% visual-error rates from a different protocol, different models and a 50-task rather than 480-item sample.
