...
> **State of the wiki:** **171 sources ingested** across eight waves — wave 0 (15: framings, formal ceilings, measurement theory), wave 1 (3: the latent-graph framing), wave 2 (31: the hippocampal–entorhinal literature and its models), wave 3 (13: attractors, single neurons, cortical microcircuitry), wave 4 (23: credit assignment, plasticity, consolidation), wave 5 (35: the control layer — working memory and prefrontal cortex), wave 6 (22, **complete**: normative theories — predictive coding, active inference and expected free energy, basal ganglia, neuromodulators, options, valuation), wave 7 (28, in progress: world models and self-supervised representation learning). 76 concept pages, 66 entity pages, 66 architectural gaps, 174 empirical tensions. Per-source detail lives in the git log, in the `From` column of [[wiki/architectural-gaps.md]] and in the provenance notes at the foot of that file; **this page keeps no per-ingest log** — that format did not survive 67 sources and duplicated the gap table's provenance. What follows is thematic.

---

## The Central Thesis

**A reasoning model is a two-level graph learner: a slow level that acquires the *structure* a family of environments shares, and a fast level that binds one instance of it in a handful of observations — with the two related by a factorization `p = f(g, x)` of structural position from content. The wiki's position, after 171 sources, is that every component of this is buildable except the factorization itself, that the factorization cannot be learned from data by any objective (it must be paid for in architecture), and that no instrument currently exists to certify it was obtained.**
...
The full list is [[wiki/empirical-tensions.md]] (174 rows). The load-bearing ones:
