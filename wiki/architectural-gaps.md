...
**This page is an index.** One line per row; the evidence, sources and status reasoning for each live in its own file under `wiki/gaps/`. Read the index to decide which rows matter, then open only those files. Historical lint-pass notes are in [[wiki/registry-audit.md]].
...
| G42 | No machine fast store has a capacity model, so nothing knows when it is full | part | `PARTIAL` | 17 | [[wiki/gaps/g042.md]] |
| G43 | Nothing arbitrates between concurrent reference frames | part | `OPEN` | 7 | [[wiki/gaps/g043.md]] |
...
| G49 | No store schedules its own reads, and none has a relevance-addressed erase | part | `OPEN` | 6 | [[wiki/gaps/g049.md]] |
...
| G52 | No inter-module connection in the wiki has state of its own | part | `OPEN` | 10 | [[wiki/gaps/g052.md]] |
...
| G56 | No architecture has a run-time gain register on its own connections | part | `OPEN` | 6 | [[wiki/gaps/g056.md]] |
...
| G73 | Nothing constructs a problem representation; every architecture is handed one | part | `OPEN` | 9 | [[wiki/gaps/g073.md]] |
...
| G95 | Every self-supervised system in the wiki has its invariance group *declared* by a hand-written augmentation list; nothing here discovers which directions are nuisance from the input's own statistics | part | `PARTIAL` | 5 | [[wiki/gaps/g095.md]] |
