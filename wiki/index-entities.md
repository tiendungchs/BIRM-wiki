...
| [[wiki/entities/tolman-eichenbaum-machine.md]] | A reusable path-integrating structural code plus a Hebbian relational memory binding it to this environment's sensory codes, trained only on next-observation prediction across many worlds (TEM; SMP is the pixel-input sibling) | The wiki's clearest built realisation of `p = f(g, x)` and of the meta/instance split; the model side of place, grid, splitter, lap and evidence cells; hippocampus-as-index |
