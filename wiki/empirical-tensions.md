...
**This page is an index.** One line per row; the evidence, sources and status reasoning for each live in its own file under `wiki/tensions/`. Read the index to decide which rows matter, then open only those files. Historical lint-pass notes are in [[wiki/registry-audit.md]].
