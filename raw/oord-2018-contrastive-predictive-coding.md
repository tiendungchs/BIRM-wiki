...
source: "https://ar5iv.labs.arxiv.org/html/1807.03748"
author:
published:
created: 2026-08-31
description: "While supervised learning has enabled great progress in many applications, unsupervised learning has not seen such widespread adoption, and remains an important and challenging endeavor for artificial intelligence.In …"
tags:
  - "clippings"
---
###### Abstract
...
![Refer to caption](https://ar5iv.labs.arxiv.org/html/1807.03748/assets/contrastive_repr4.png)
...
$$
\displaystyle=\sum_{x,c}p(x,c)\log\frac{p(x|c)}{p(x)}.
$$
...
As argued in the previous section we do not predict future observations $x_{t+k}$ directly with a generative model $p_{k}(x_{t+k}|c_{t})$. Instead we model a density ratio which preserves the mutual information between $x_{t+k}$ and $c_{t}$ (Equation 1) as follows (see next sub-section for further details):

$$
f_{k}(x_{t+k},c_{t})\propto\frac{p(x_{t+k}|c_{t})}{p(x_{t+k})}
$$
...
$$
\displaystyle f_{k}(x_{t+k},c_{t})=\exp\Big(z_{t+k}^{T}W_{k}c_{t}\Big),
$$
...
$$
\displaystyle\mathcal{L_{\text{N}}}
$$
 
$$
\displaystyle=-\mathop{{}\mathbb{E}}_{X}\left[\log\frac{f_{k}(x_{t+k},c_{t})}{\sum_{x_{j}\in X}f_{k}(x_{j},c_{t})}\right]
$$

Optimizing this loss will result in $f_{k}(x_{t+k},c_{t})$ estimating the density ratio in equation 2. This can be shown as follows.
...
$$
\displaystyle p(d=i|X,c_{t})
$$
 
$$
\displaystyle=\frac{p(x_{i}|c_{t})\prod_{l\neq i}p(x_{l})}{\sum^{N}_{j=1}p(x_{j}|c_{t})\prod_{l\neq j}p(x_{l})}
$$
 
$$
\displaystyle=\frac{\frac{p(x_{i}|c_{t})}{p(x_{i})}}{\sum^{N}_{j=1}\frac{p(x_{j}|c_{t})}{p(x_{j})}}.
$$

As we can see, the optimal value for $f(x_{t+k},c_{t})$ in Equation 4 is proportional to $\frac{p(x_{t+k}|c_{t})}{p(x_{t+k})}$ and this is independent of the the choice of the number of negative samples $N-1$.
...
![Refer to caption](https://ar5iv.labs.arxiv.org/html/1807.03748/assets/figures/speakers_tsne.png)
...
Additionally, Figure 3 shows a t-SNE visualization [^33] of how discriminative the embeddings are for speaker voice-characteristics. It is important to note that the window size (maximum context size for the GRU) has a big impact on the performance, and longer segments would give better results. Our model had a maximum of 20480 timesteps to process, which is slightly longer than a second.
...
![Refer to caption](https://ar5iv.labs.arxiv.org/html/1807.03748/assets/figures/rl_1.png)
...
By optimizing InfoNCE, the CPC loss we defined in Equation 4, we are maximizing the mutual information between $c_{t}$ and $z_{t+k}$ (which is bounded by the MI between $c_{t}$ and $x_{t+k}$). This can be shown as follows.

As already shown in Section 2.3, the optimal value for $f(x_{t+k},c_{t})$ is given by $\frac{p(x_{t+k}|c_{t})}{p(x_{t+k})}$. Inserting this back in to Equation 4 and splitting $X$ into the positive example and the negative examples $X_{\text{neg}}$ results in:

$$
\displaystyle\mathcal{L^{\text{opt}}_{\text{N}}}
$$
 
$$
\displaystyle=-\mathop{{}\mathbb{E}}_{X}\log\left[\frac{\frac{p(x_{t+k}|c_{t})}{p(x_{t+k})}}{\frac{p(x_{t+k}|c_{t})}{p(x_{t+k})}+\sum_{x_{j}\in X_{\text{neg}}}\frac{p(x_{j}|c_{t})}{p(x_{j})}}\right]
$$
 
$$
\displaystyle=\mathop{{}\mathbb{E}}_{X}\log\left[1+\frac{p(x_{t+k})}{p(x_{t+k}|c_{t})}\sum_{x_{j}\in X_{\text{neg}}}\frac{p(x_{j}|c_{t})}{p(x_{j})}\right]
$$
 
$$
\displaystyle\approx\mathop{{}\mathbb{E}}_{X}\log\left[1+\frac{p(x_{t+k})}{p(x_{t+k}|c_{t})}(N-1)\mathop{{}\mathbb{E}}_{x_{j}}\frac{p(x_{j}|c_{t})}{p(x_{j})}\right]
$$
 
$$
\displaystyle=\mathop{{}\mathbb{E}}_{X}\log\left[1+\frac{p(x_{t+k})}{p(x_{t+k}|c_{t})}(N-1)\right]
$$
 
$$
\displaystyle\geq\mathop{{}\mathbb{E}}_{X}\log\left[\frac{p(x_{t+k})}{p(x_{t+k}|c_{t})}N\right]
$$
 
$$
\displaystyle=-I(x_{t+k},c_{t})+\log(N),
$$

Therefore, $I(x_{t+k},c_{t})\geq\log(N)-\mathcal{L^{\text{opt}}_{\text{N}}}$. This trivially also holds for other $f$ that obtain a worse (higher) $\mathcal{L}_{\text{N}}$. Equation 8 quickly becomes more accurate as $N$ increases. At the same time $\log(N)-\mathcal{L_{\text{N}}}$ also increases, so it’s useful to use large values of $N$.

InfoNCE is also related to MINE [^54]. Without loss of generality let’s write $f(x,c)=e^{F(x,c)}$, then

$$
\displaystyle\mathop{{}\mathbb{E}}_{X}\left[\log\frac{f(x,c)}{\sum_{x_{j}\in X}f(x_{j},c)}\right]
$$
 
$$
\displaystyle=\mathop{{}\mathbb{E}}_{(x,c)}\Big[F(x,c)\Big]-\mathop{{}\mathbb{E}}_{(x,c)}\Big[\log\sum_{x_{j}\in X}e^{F(x_{j},c)}\Big]
$$
 
$$
\displaystyle=\mathop{{}\mathbb{E}}_{(x,c)}\Big[F(x,c)\Big]-\mathop{{}\mathbb{E}}_{(x,c)}\Big[\log\Big(e^{F(x,c)}+\sum_{x_{j}\in X_{\text{neg}}}e^{F(x_{j},c)}\Big)\Big]
$$
 
$$
\displaystyle\leq\mathop{{}\mathbb{E}}_{(x,c)}\Big[F(x,c)\Big]-\mathop{{}\mathbb{E}}_{c}\Big[\log\sum_{x_{j}\in X_{\text{neg}}}e^{F(x_{j},c)}\Big]
$$
 
$$
\displaystyle=\mathop{{}\mathbb{E}}_{(x,c)}\Big[F(x,c)\Big]-\mathop{{}\mathbb{E}}_{c}\Big[\log\frac{1}{N-1}\sum_{x_{j}\in X_{\text{neg}}}e^{F(x_{j},c)}+\log(N-1)\Big]
$$
