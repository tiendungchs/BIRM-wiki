...
**Entity pages** (`wiki/entities/`): one page per **model, framework, or biological system**.
...
| LINT | `wiki-lint` | User says "lint the wiki" / asks for an audit |
...
- **`L3`/`L4` material lives in page bodies, not in the registries** — realization and substrate detail is ingested as concept/entity prose and found by search when a realization is chosen; only `L0`/`L1`/`L2`/`L0-INSTR` rows may be opened.
...
- Don't read between the line. Do only what is asked. Suggest edit if you find a problem. Never try to fix it on your own.
