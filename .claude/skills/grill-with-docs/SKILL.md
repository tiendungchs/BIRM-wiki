...
description: Interview the user about a plan or design, round by round, until both share one understanding of it — resolved domain terms land in CONTEXT.md as they resolve, and hard-to-reverse decisions land as ADRs under docs/adr/. Use only when the user explicitly types /grill-with-docs; suited to a repo-based change that can be settled in one session, or to documenting a repo that has no domain docs at all. Requires the grilling and domain-modeling skills.
---
